...
export class CarController {
  ...
  createWheels(chassisPosition) {
    // Define wheel positions relative to chassis
    const wheelOffsets = [
      // Front left
      new THREE.Vector3(
    ...
    wheelOffsets.forEach((offset, index) => {
      // Calculate world position for wheel (like the example)
      ...
      // Create revolute joint (like the example)
      this.createWheelJoint(wheelBody, offset, index);
    });
  }
  
  createWheelJoint(wheelBody, localOffset, wheelIndex) {
    // Create revolute joint for wheel rotation
    const jointData = RAPIER.JointData.revolute(
      localOffset, // Anchor on chassis (local to chassis)
      { x: 0, y: 0, z: 0 }, // Anchor on wheel (center of wheel)
