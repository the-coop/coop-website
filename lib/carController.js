...
export class CarController {
  ...
  createWheels(chassisPosition) {
    ...
    wheelOffsets.forEach((offset, index) => {
      ...
      const wheelBody = this.physics.createDynamicBody(wheelWorldPos, {
        ...
        angularDamping: 0.5  // Increased damping
      });
  ...
  createWheelJoint(wheelBody, localOffsetOnChassis, wheelIndex) {
    ...
    if (wheelIndex >= 2) {
      joint.configureMotorModel(RAPIER.MotorModel.ForceBased); // Use force-based instead
      // Set limits to prevent excessive forces
      joint.setLimits(-100, 100); 
    }
  ...
  update(deltaTime) {
    ...
    // Don't apply gravity here - it's applied in the main game loop
    // This prevents double gravity application
    
    // Handle steering and driving
    if (this.isOccupied) {
      let targetSteer = 0;
      ...
      if (Math.abs(this.steerAngle) > 0.01) {
        ...
        const steerTorque = this.steerAngle * 30 * speedFactor; // Reduced from 50
