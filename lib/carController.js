...
export class CarController {
  ...
  createWheelJoint(wheelBody, localOffsetOnChassis, wheelIndex) {
    ...
    // The cylinder is rotated 90 degrees around Z axis (rotation.z = PI/2)
    // So the wheel's local Z axis is now pointing sideways (where X was)
    // We want rotation around the original X axis, which is now the Z axis
    const jointData = RAPIER.JointData.revolute(
      localOffsetOnChassis, // Local anchor on chassis
      { x: 0, y: 0, z: 0 }, // Local anchor on wheel (center)
      { x: 0, y: 0, z: 1 } // Z axis for wheel rotation (because cylinder is rotated)
    );
  ...
  update(deltaTime) {
    ...
    if (this.isOccupied) {
      ...
      // For now, skip the manual steering rotation since wheels are constrained by joints
      // The steering should be implemented with a different joint type or by modifying the joint limits
      
      // Apply motor to rear wheels (like the example)
      let targetVelocity = 0;
      if (this.keys.forward) targetVelocity = this.motorSpeed; // Positive for forward
      if (this.keys.backward) targetVelocity = -this.motorSpeed * 0.5; // Negative for backward
      ...
      if (this.keys.brake) {
        ...
        // Also stop wheel motors
        this.wheelJoints.forEach((wheelJoint) => {
          if (wheelJoint.isRearWheel && wheelJoint.joint) {
            wheelJoint.joint.configureMotorVelocity(0, this.motorForce * 2);
          }
        });
      }
    }
