...
export default async function setupNetworking(token, user) {
    ...
    socket.on("player_moved", (packet) => {
        const player = WORLD.players[packet.pid];

        console.log(player.config.username);
        console.log(player);

        const jumpThresholdSquared = 25;
        const updateTime = 0.1;
    
        if (player.handle.position.distanceToSquared(packet.p) > jumpThresholdSquared) {
            player.handle.position = new Vector3(...packet.p);
        } else {
            let gap = player.handle.position.clone().sub(packet.p);
            player.correctionVelocity = gap.multiplyScalar(1 / updateTime);
            player.correctionTime = updateTime;
        }

        player.velocity = new Vector3(...packet.v);
    });
};
