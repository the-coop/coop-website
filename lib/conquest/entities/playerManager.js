import { Texture, SpriteMaterial, Sprite, Vector3, Euler, Group, BoxGeometry, MeshBasicMaterial, Mesh } from 'three';

import Player from '~/lib/conquest/entities/player';
import { PLAYER_SIZE } from '../config';
import CameraManager from '../experience/cameraManager';
import InputManager from '../experience/inputManager';
import ComputerInput from '../experience/inputs/computer';
import Physics from '../experience/physics';

const generateCharacter = config => {
    ...
    const armGeometry = new BoxGeometry(bodyDepth / 2, bodyDepth, bodyDepth / 2);
    ...
    const legGeometry = new BoxGeometry(bodyDepth / 2, bodyDepth, bodyDepth / 2);
    ...
    const footGeometry = new BoxGeometry(bodyDepth / 2, bodyDepth / 2, bodyWidth);
    ...
    rightArm.position.set(bodyWidth * .9, bodyDepth / 2, 0);
    leftArm.position.set(-(bodyWidth * .9), bodyDepth / 2, 0);

    lowerLeftArm.position.set(0, -(bodyHeight * .5), 0);
    lowerRightArm.position.set(0, -(bodyHeight * .5), 0);

    upperLeftLeg.position.set(.225, -1.35, 0);
    lowerLeftLeg.position.set(0, -1, 0);

    upperRightLeg.position.set(-.225, -1.35, 0);
    lowerRightLeg.position.set(0, -1, 0);

    leftFoot.position.set(0, -.7, .125);
    rightFoot.position.set(0, -.7, .125);
