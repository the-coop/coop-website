...
const addKeyboardMovement = () => {
    document.addEventListener('keydown', ev => {
        ...
        if (ALL_MOVEMENT_KEYS.includes(ev.key)) {
            ...
            if (UP.includes(ev.key)) me.direction.set(0, .0251, 0);
            else if (RIGHT.includes(ev.key)) me.direction.set(.0251, 0, 0);
            else if (DOWN.includes(ev.key)) me.direction.set(0, -.0251, 0);
            else if (LEFT.includes(ev.key)) me.direction.set(-.0251, 0, 0);
...
export const groundMovementListen = () => {
    addMobileMovement();
