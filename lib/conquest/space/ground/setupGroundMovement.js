...
export const addKeyboardMovement = () => {
    document.addEventListener('keydown', ev => {
        ...
        if (ALL_MOVEMENT_KEYS.includes(ev.key)) {
            ...
            const moveDistance = .00251;
            if (UP.includes(ev.key)) me.direction.set(0, moveDistance, 0).normalize();
            else if (RIGHT.includes(ev.key)) me.direction.set(moveDistance, 0, 0).normalize();
            else if (DOWN.includes(ev.key)) me.direction.set(0, -moveDistance, 0).normalize();
            else if (LEFT.includes(ev.key)) me.direction.set(-moveDistance, 0, 0).normalize();
