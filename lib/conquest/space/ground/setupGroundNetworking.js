...
const loadOthers = async () => {
    ...
    Object.keys(initialPlayers).map(playerID => {
        ...
        if (!me || me.id !== player.id) playerRecognised(player);
    });
}

export default function setupGroundNetworking(token, uiRef) {
    ...
    socket.on("player_recognised", player => playerRecognised(player, uiRef));
