...
export default function setFocusTarget(meshTarget) {
    const { controls, camera, targetStructure } = window.CONQUEST;
    ...
    //Shrink back the old oje
    if (targetStructure) {
      targetStructure.scale.x = targetStructure.scale.x / 2;
      targetStructure.scale.y = targetStructure.scale.y / 2;
      targetStructure.scale.z = targetStructure.scale.z / 4;
      window.CONQUEST.targetStructure = null;
    }
    ...
    if (meshTarget.entity_type === ENTITY_TYPES.STRUCTURE) {
        ...
        window.CONQUEST.targetStructure = meshTarget;
