...
const isFocusType = type => !!(window.CONQUEST.VIEW.focusTarget 
    && window.CONQUEST.VIEW.focusTarget.entity_type === type);
...
const planetaryTween = (cameraRef, target, radius) => 
    window.CONQUEST.VIEW.cameraTween = new Tween(cameraRef.position)
        .to({ x: target.x, y: target.y }, 1500)
        .easing(Easing.Quadratic.Out)
        .onComplete(() => {
            window.CONQUEST.VIEW.cameraTween = null;
            setTimeout(() => zoomTween(cameraRef, radius * 2), 500);
        })
        .start();


export default function setFocusTarget(meshTarget, skipTween = false) {
    ...
    if (meshTarget.entity_type === 'STRUCTURE') {
    ...
    if (meshTarget.entity_type === 'PLANETARY') {
        // Tween the position change of the camera.
        if (!skipTween)
            planetaryTween(camera, controls.target, meshTarget.radius);

        // Unlock the camera rotation.
        controls.noRotate = false;
    }

    if (meshTarget.entity_type === 'PLAYER') {
        // Tween the position change of the camera.
    ...
    window.CONQUEST.VIEW.UI.focus = meshTarget;
}
