...
export default class ComputerInput {
    ...
    static mousemove(ev) {
        ...
        if (WORLD.me.player) {
          ...
          var quaterniona = new THREE.Quaternion(); // create one and reuse it
          var quaternionb = new THREE.Quaternion(); // create one and reuse it
          quaterniona.setFromUnitVectors(baseRotation, thrustDirection);
          quaternionb.setFromEuler(this.aimEuler);
          quaterniona.multiply(quaternionb);

          // TODO: Camera seems to be disconnected from ground direction
          // Apply an "up"?

          WORLD.me.player.aim.copy(quaterniona);
        }
    }
