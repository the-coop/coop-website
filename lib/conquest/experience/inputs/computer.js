...
export default class ComputerInput {
    ...
    static aimEuler = new Euler(0, 0, 0, 'XYZ');
    ...
    static mousemove(ev) {
        const fpsMode = WORLD.settings.view.CURRENT_CAMERA_KEY === ControlsManager.CAMERA_KEYS.FIRST_PERSON;
        const trackballMode = WORLD.settings.view.CURRENT_CAMERA_KEY === ControlsManager.CAMERA_KEYS.TRACKBALL;

        // Handle computer mousemovement for FPS mode.
        if (fpsMode && WORLD.me.player) {
            // Check if in first person mode and not dead, few other checks...
            ...
            // If the player is grounded, set camera up away from planet SOI.
            // if (WORLD.me.player.isGrounded)
            // const thrustDirection = player.handle.position.clone().normalize();

            // TODO: Camera seems to be disconnected from ground direction
            // Apply an "up"?
        
            WORLD.me.player.aim.setFromEuler(this.aimEuler);
        }

        // Handle computer mousemovement for trackball (planetary orbit) mode.
        if (trackballMode) {
            console.log('Movement debugging for trackball mode');

            this.aimEuler.y -= ev.movementX * this.aimSpeed;
            this.aimEuler.x -= ev.movementY * this.aimSpeed;
            this.aimEuler.x = Math.min(Math.max(this.aimEuler.x, -1.0472), 1.0472);

            // This doesn't work, attempt to rotate trackball camera around focus target.
            
            // WORLD.camera.rotation.setFromEuler(this.aimEuler);
        }
    }
