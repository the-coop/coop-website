...
export default class Physics {
    ...
    static applyMovement(player, playerHeight, surfaceHeight, isSelf) {
        // Caculate gravity
        ...
        let z = gravity;
        if (isSelf) {
            x = 50 * ((ControlsManager.keypad.a ? 1 : 0) - (ControlsManager.keypad.d ? 1 : 0));
            y = 50 * ((ControlsManager.keypad.w ? 1 : 0) - (ControlsManager.keypad.s ? 1 : 0));
            z = (ControlsManager.keypad.space ? (player.onGround ? -10 : 0) : z);

            if (ControlsManager.keypad.space) {
                if (!player.onGround) {
        ...
        const normalMatrix = new THREE.Matrix3().getNormalMatrix(player.handle.matrix);
        const acceleration = new THREE.Vector3(x, y, z);
        acceleration.applyMatrix3(normalMatrix);
    
        // Move the player
        player.velocity.addScaledVector(acceleration, WORLD.delta);
        player.handle.position.addScaledVector(acceleration, 0.5 * WORLD.delta * WORLD.delta);
        player.handle.position.addScaledVector(player.velocity, WORLD.delta);
        if (player.correctionTime > 0) {
            let correctionAmount = Math.min(player.correctionTime, WORLD.delta);
    ...
    static applyFriction(player, playerHeight, height) {
        ...
        let speedFactor = friction * WORLD.delta;
