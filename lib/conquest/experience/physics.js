...
export default class Physics {
    ...
    static applyMovement(player, playerHeight, surfaceHeight, isSelf) {
        ...
        if (isSelf) {
            x = PLAYER_SPEED * ((ControlsManager.keypad.a ? 1 : 0) - (ControlsManager.keypad.d ? 1 : 0));
            y = PLAYER_SPEED * ((ControlsManager.keypad.w ? 1 : 0) - (ControlsManager.keypad.s ? 1 : 0));
      ...
      if (isSelf) {
        ...
        if (player.correctionTime > 0) {
            const correctionAmount = Math.min(player.correctionTime, WORLD.delta);
