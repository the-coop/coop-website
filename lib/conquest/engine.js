...
export default function engine(instance) {
    ...
    Object.keys(WORLD.players).map(key => {
        ...
        let isPlayer = player.config.player_id === WORLD?.me?.config?.player_id;
        ...
        if (isPlayer && Controls.keypad.space) {
            if (!player.onGround) {
    ...
    if (WORLD.socket && WORLD?.me?.config?.id) {
        window.WORLD.socket.emit('player_moved', {
            id: WORLD.me.config.player_id,
            v: WORLD.me.player.velocity,
            p: WORLD.me.player.mesh.position,
            r: WORLD.me.player.mesh.quaternion,
        });
    }
