    ...
    if (focusTarget) {
        ...
        if (focusTarget.entity_type === ENTITY_TYPES.STRUCTURE) {
            ...
            if (!cameraTween) {
                ...
                // const structureFocusOffset = new THREE.Vector3(0, 1, 2.5);
                // const structureFocusPosition = structureFocusOffset.applyMatrix4(earthSphere.matrix);

                // Set the position to the calculated satelite position.
                camera.position.set(controls.target.x + 2, controls.target.y, controls.target.z);
            }
        }
    }
