...
export function createPlayerModel(radius, height) {
  ...
  // Hands (these will be hidden in first-person when hands weapon is equipped)
  const handGeometry = new THREE.SphereGeometry(radius * 0.25, 8, 8);
  ...
  // Position hands at the front of the player when in "hands weapon" mode
  const leftHand = new THREE.Mesh(handGeometry, handMaterial);
  leftHand.position.set(-radius * 0.8, -height * 0.1, -radius * 1.5); // Forward position
  leftHand.name = 'LeftHand';
  playerGroup.add(leftHand);
  
  const rightHand = new THREE.Mesh(handGeometry, handMaterial);
  rightHand.position.set(radius * 0.8, -height * 0.1, -radius * 1.5); // Forward position
