export class WebSocketManager {
  ...
  // Send dynamic object updates (for host/authority)
  sendDynamicObjectUpdate(objectId, position, rotation, velocity) {
    if (!this.connected || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
      return;
    }
    
    const message = {
      type: 'dynamic_object_update',
      object_id: objectId,
      position: {
        x: position.x,
        y: position.y,
    ...
    try {
      ...
      console.error('Failed to send dynamic object update:', error);
    }
  }
