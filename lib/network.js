export class WebSocketManager {
      ...
      case 'player_state':
        if (this.onPlayerUpdate) {
          this.onPlayerUpdate(data.player_id, {
            ...
            isOnLadder: data.is_on_ladder || false,
            weaponState: data.weapon_state || null
          });
        }
        break;
  ...
  sendPlayerState(position, rotation, velocity, isGrounded = false, isSwimming = false, isOnLadder = false, weaponState = null) {
  ...
  sendVehicleState(position, rotation, velocity, vehicleId) {
    if (!this.connected || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
      return;
    }
    ...
    try {
      ...
      console.error('Failed to send vehicle state:', error);
    }
  }
