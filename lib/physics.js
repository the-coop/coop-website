...
export class PhysicsManager {
  ...
  applyGravityToBody(body, deltaTime) {
    ...
    if (this.scene && this.scene.isPositionInWater && this.scene.isPositionInWater(position)) {
      ...
      const buoyancyStrength = this.gravity.strength * 0.2; // 20% of gravity strength
      const gravityDir = new THREE.Vector3()
        .subVectors(this.gravity.center, position)
        .normalize();
      const buoyancyForce = gravityDir.multiplyScalar(-buoyancyStrength * mass); // Upward force
