...
export class PhysicsManager {
  ...
  createOtherPlayerBody(x, y, z, playerHeight, playerRadius) {
    ...
    console.log("Other player physics body created as dynamic with conservative settings at:", x.toFixed(1), y.toFixed(1), z.toFixed(1));
  ...
  applyGravityToScene(scene, deltaTime) {
    ...
    // Apply planet-centered gravity to ALL dynamic bodies including local and other players
    scene.traverse((child) => {
      if (child.isMesh && child.userData.physicsBody) {
        ...
        if (body.bodyType() === RAPIER.RigidBodyType.Dynamic) {
          ...
          // Calculate direction to planet center (same as server)
          const toPlanet = new THREE.Vector3().subVectors(this.gravity.center, objPos);
          const distanceToPlanet = toPlanet.length();
          
          // Apply conservative gravity scaling (matching server exactly)
          let gravityMultiplier = 1.0;
          if (distanceToPlanet > 150.0) {
            gravityMultiplier = Math.min(400.0 / distanceToPlanet, 0.8); // Match server exactly
          } else {
            gravityMultiplier = 0.8; // Match server exactly
          }
          ...
          if (child.userData.playerId && child.userData.serverGrounded) {
            // If server says grounded, apply reduced gravity to prevent bouncing
            const downwardVel = objVelocity.x * objGravityDir.x + objVelocity.y * objGravityDir.y + objVelocity.z * objGravityDir.z;
            if (downwardVel > 1.0) {
              applyFullGravity = false; // Don't apply more gravity if already falling fast
            }
          }
    ...
    // Apply gravity to any rigid bodies not found in scene traversal (like the local player)
    this.world.forEachRigidBody((rigidBody) => {
      if (rigidBody.bodyType() === RAPIER.RigidBodyType.Dynamic) {
        // Check if this body is already handled by scene traversal
        ...
        // If not handled by scene, apply gravity directly (this is likely the local player)
        if (!handledByScene) {
