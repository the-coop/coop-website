...
export class PhysicsManager {
  constructor() {
    ...
    this.gravity = {
      ...
      strength: 15 // Further reduced to match server gravity strength
    };
  }
    ...
    scene.traverse((child) => {
      if (child.isMesh && child.userData.physicsBody) {
        ...
        if (body.bodyType() === RAPIER.RigidBodyType.Dynamic) {
          ...
          // Apply conservative gravity scaling (matching server)
          let gravityMultiplier = 1.0;
          if (distanceToPlanet > 150.0) {
            gravityMultiplier = Math.min(500.0 / distanceToPlanet, 1.0);
          } else {
