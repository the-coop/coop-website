...
export class PhysicsManager {
  ...
  createOtherPlayerBody(x, y, z, playerHeight, playerRadius) {
    // Create as dynamic so they can be pushed and affected by gravity
    const playerBodyDesc = RAPIER.RigidBodyDesc.dynamic()
      .setTranslation(x, y, z)
      .setLinearDamping(0.5) // Moderate damping to allow movement but prevent excessive bounce
      .setAngularDamping(1.0) // High angular damping to prevent spinning
      .setCanSleep(false) // Keep awake so gravity affects them
    ...
    .setFriction(0.8) // Higher friction for better grounding
    .setRestitution(0.0) // No bouncing
    .setDensity(1.0) // Normal density so gravity affects them properly
  ...
  applyGravityToScene(scene, deltaTime) {
    ...
    // Apply planet-centered gravity to ALL dynamic bodies INCLUDING other players
    scene.traverse((child) => {
      if (child.isMesh && child.userData.physicsBody) {
        const body = child.userData.physicsBody;
        
        // Apply gravity to all dynamic bodies (including other players)
        if (body.bodyType() === RAPIER.RigidBodyType.Dynamic) {
          ...
          // For other players, apply full gravity - server will correct if needed
          body.setLinvel({
          ...
          // Log gravity application for other players
          if (child.userData.playerId) {
            console.log(`Applied gravity to player ${child.userData.playerId}: force [${objGravityForce.x.toFixed(2)}, ${objGravityForce.y.toFixed(2)}, ${objGravityForce.z.toFixed(2)}], new vel [${(objVelocity.x + objGravityForce.x).toFixed(1)}, ${(objVelocity.y + objGravityForce.y).toFixed(1)}, ${(objVelocity.z + objGravityForce.z).toFixed(1)}]`);
          }
        }
      }
    });
  ...
  updateDynamicObjects(movingPlatformBody) {
    ...
    // Update all physics bodies EXCEPT other players (they handle their own interpolation)
    this.scene.traverse((child) => {
      if (child.isMesh && child.userData.physicsBody) {
        // Skip other players - they manage their own physics/visual sync
        if (child.userData.playerId) return;
        
        const body = child.userData.physicsBody;
        
        // For non-player objects, sync visual to physics
