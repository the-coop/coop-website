...
export default class Engine {
    ...
    static loop() {
        ...
        // This prevents all vehicles from moving simultaneously
        VehicleManager.updateCurrentVehicle(1/60);
        
        // CRITICAL FIX: Skip physics for player-controlled vehicles with more specific condition
        ...
        // CRITICAL FIX: Completely reset rendering state before each frame
        this.renderer.clear(true, true, true);
        this.renderer.clearDepth();
        this.renderer.info.reset(); // Reset renderer stats to prevent memory issues
        
        // FIX: Only log camera parent issues occasionally to prevent spam
        const now = Date.now();
        if (!this._lastCameraParentCheck || now - this._lastCameraParentCheck > 5000) {
            this._lastCameraParentCheck = now;
            
            if (this.camera.parent !== this.scene && this.camera.parent !== null) {
                const parentName = this.camera.parent.name || 
                                 this.camera.parent.userData?.name || 
                                 this.camera.parent.type || 
                                 'unknown';
                console.log(`Camera parent is: ${parentName} (logging every 5s)`);
            }
        }
        
        // Render current frame - ensure no duplicate cameras by explicitly using our camera
