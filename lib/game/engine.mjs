...
export default class Engine {
    ...
    static async setup(canvasElement) {
        ...
        // Only check loaded flag
        if (this.loaded) {
            console.log('Engine is already loaded.');
            return true;
        }

        this.loaded = false; // Start with loaded false
        try {
            // 1. Scene setup
            ...
            // Only set loaded flag, leave initialised for game start
            this.loaded = true;
    ...
    static async setGameStarted(started) {
        ...
        try {
            if (started) {
                // Create protagonist first before anything else
                ...
                // Then set game started state
                State.setGameStarted(true);
                State.setInitialised(true); // Mark as fully initialised when game starts

                // Then initialize controllers
                await ControllerManager.onGameStart();

                // Initialize FPS counter last
                ...
                State.setGameStarted(false);
                State.setInitialised(false); // Reset initialised when game stops
