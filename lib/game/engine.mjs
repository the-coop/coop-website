...
export default class Engine {
    ...
    static async setup(canvasElement) {
        ...
        try {
            // Only create new scene if needed
            if (!this.scene) {
                this.scene = new Scene();
                console.log('Creating new scene');
            }
            
            // Only create new camera if needed
            if (!this.cam) {
                this.cam = new PerspectiveCamera(
                    75,
                    canvasElement.offsetWidth / window.innerHeight,
                    0.1,
                    2000
                );
            }

            // Setup or update renderer
            if (!this.rend) {
                this.rend = new WebGLRenderer({ 
            ...
            // Update renderer size and settings
            this.rend.setPixelRatio(window.devicePixelRatio);
            this.rend.setSize(canvasElement.offsetWidth, canvasElement.offsetHeight, false);
            this.rend.setClearColor(0x000000);

            // Set up scene only if not ready
            if (!this.isSceneReady) {
                await SceneManager.setup(this.scene);
                this.setupLights();
                this.isSceneReady = true;
                console.log('Scene setup complete. isSceneReady set to true.');
            }

            // Initialize protagonist if needed
            if (!PlayerManager.getProtagonist()) {
                await PlayerManager.create(this.scene, this.cam);

                // Check protagonist assignment
                const protagonist = PlayerManager.getProtagonist();
                if (!protagonist) {
                    throw new Error('Protagonist not assigned after creation');
                }
                console.log('Protagonist successfully assigned in Engine:', protagonist);
            }
            ...
            // Initialize managers after scene setup
    ...
    static setGameStarted(started) {
        ...
        InputManager.setGameRunning(started); // Ensure to call setGameRunning
