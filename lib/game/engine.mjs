    ...
    static loop() {
        ...
        try {
            if (!playerControllingVehicle) {
                ...
                // FIXED: Check if the method exists before calling
                if (typeof Physics.updateNonVehicles === 'function') {
                    Physics.updateNonVehicles();
                } else {
                    console.warn("Physics.updateNonVehicles not found - falling back to full update");
                    Physics.update();
                }
            }
        } catch (e) {
            console.error("Error during physics update:", e);
            // On error, attempt to run standard update as fallback
            try {
                Physics.update();
            } catch (fallbackError) {
                console.error("Physics fallback update also failed:", fallbackError);
            }
        }
