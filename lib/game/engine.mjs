...
export default class Engine {
    ...
    static loop() {
        ...
        // CRITICAL FIX: VehicleManager now only tracks vehicles, movement is handled by controllers
        VehicleManager.updateVehicles(1/60);
        
        // CRITICAL FIX: Skip physics for player-controlled car!
        if (!VehicleManager.currentVehicle || VehicleManager.currentVehicle.userData.type !== 'car') {
            // Only run physics for everything else
            Physics.update();
        } else {
            // For player car, run limited physics that won't reset position
