...
export default class Physics {
    ...
    // UPDATED: Check if two objects are overlapping using appropriate collision detection
    static objectsOverlapping(objA, objB) {
        try {
            ...
            // IMPROVED: Use proper collision detection based on object types
            // Only players use sphere collision, all others use OBB
            const isPlayerA = objectA.userData?.isPlayer || false;
            const isPlayerB = objectB.userData?.isPlayer || false;
            
            // If either object is a player, use sphere-based collision
            if (isPlayerA || isPlayerB) {
                // Use simplified sphere check for player collisions
                const radiusA = isPlayerA ? 
                    (objectA.userData.collisionRadius || 1.0) : 
                    this.getObjectRadius(objectA);
                    
                const radiusB = isPlayerB ? 
                    (objectB.userData.collisionRadius || 1.0) : 
                    this.getObjectRadius(objectB);
                
                const distance = objectA.position.distanceTo(objectB.position);
                return distance < (radiusA + radiusB);
            } else {
                // For non-player objects, use OBB-OBB collision
                if (objA.obb && objB.obb) {
                    if (typeof ObjectManager.runSATTest === 'function') {
                        const result = ObjectManager.runSATTest(objA.obb, objB.obb);
                        return result && result.collides;
                    }
                }
                
                // Fallback to approximate sphere check if OBB test fails/unavailable
    ...
    // NEW: Process player-specific collisions with enhanced debug visualization
    static processPlayerCollisions(player) {
        if (!player || !player.handle || player.inVehicle) return;
        
        try {
            // Always use sphere collision for players
            const playerRadius = player.collisionRadius || 1.0;
            let collisionFound = false;
            
            // Track active collisions for this frame
            ...
            // ADDED: Debug output to check collidables
            console.log(`Checking collisions against ${collidables.length} collidable objects`);
            ...
            for (const collidable of collidables) {
                ...
                // FIXED: Manual sphere-OBB collision detection without relying on ObjectManager
                let collision = false;
                let collisionResult = null;
                
                // Get object position and verify OBB is valid
                const objectPos = collidable.object.position;
                const objectOBB = collidable.obb;
                
                if (!objectOBB || !ObjectManager.validateOBB(objectOBB)) {
                    console.warn(`Invalid OBB for ${collidable.type} object`);
                    continue;
                }
                
                // OPTIMIZATION: First do a quick sphere-sphere check for early rejection
                const centerDistance = player.position.distanceTo(objectPos);
                
                // Fast rejection test - if centers are too far apart, no collision possible
                ...
                if (centerDistance > (playerRadius + maxDimension)) {
                    continue; // Skip detailed collision check
                }
                
                // IMPORTANT: Log that we're processing a potential collision
                console.log(`Testing collision with ${collidable.type} (distance = ${centerDistance.toFixed(2)})`);
                
                // CRITICAL FIX: Use properly implemented sphere-OBB test
                const closestPoint = this.findClosestPointOnOBB(player.position, objectOBB);
                
                if (closestPoint) {
                    const distance = player.position.distanceTo(closestPoint);
                    
                    // Check if distance is within player radius
                    if (distance <= playerRadius) {
                        collision = true;
                        
                        // Create vector from closest point to player center (this is the collision normal)
                        const normal = new Vector3().subVectors(player.position, closestPoint).normalize();
                        ...
                        console.log(`Collision detected with ${collidable.type}, penetration: ${collisionResult.penetration.toFixed(3)}`);
                    }
                }
    ...
    // ADD MISSING METHOD: Handle player collision with objects - critical for collision response
    ...
    // ADD HELPER METHOD: Handle collision between player and test cubes
    static handlePlayerTestCubeCollision(player, collidable, collisionResult) {
        if (!player || !collidable || !collidable.object || !collisionResult) return;
        
        try {
            const cube = collidable.object;
            
            // Skip if cube doesn't have required data
            if (!cube.userData || !cube.userData.velocity) return;
            
            // Calculate impulse based on player velocity and mass
            const playerSpeed = player.velocity.length();
            const playerDirection = player.velocity.clone().normalize();
            const playerMass = player.mass || 75;  // Default player mass
            const cubeMass = cube.userData.mass || 30;  // Default cube mass
            
            // Calculate momentum-conserving impulse (simplified physics)
            const massRatio = playerMass / cubeMass;
            const impulseStrength = playerSpeed * massRatio * 0.8; // 80% efficient transfer
            
            // Apply impulse in the player's movement direction
            cube.userData.velocity.addScaledVector(playerDirection, impulseStrength);
            
            // Make sure the cube is not static
            cube.userData.isStatic = false;
            
            // Store the impact time
            cube.userData.lastImpactTime = Date.now();
            cube.userData.lastImpactStrength = impulseStrength;
            
            console.log(`Applied impulse of ${impulseStrength.toFixed(2)} to test cube`);
        } catch (err) {
            console.error("Error handling player-test cube collision:", err);
        }
    }

    // IMPORTANT FIX: Modify the visualizeCollisions method to properly reference THREE
