...
export default class Physics {
                        ...
                        if (collide) {
                            ...
                            if (isUpwardCollision && player.falling) {
                                ...
                                if (wasGroundedBeforeCollision) {
                                    ...
                                    // IMPROVED: For collisions with test cubes on ground, mark as standing on object
                                    // This prevents the player from slipping off objects during collision
                                    if (collide.isTestCubeCollision) {
                                        player.standingOnObject = true;
                                        console.log("Surface collision with test cube - marked as standing on object");
                                    }
                                ...
                                // Handle hitting a ceiling - only set falling if not on ground
                                // FIXED: Only enter falling state if we're not on a planet surface
                                if (!wasGroundedBeforeCollision || player._wasAirborneBeforeCollision) {
                                    player.falling = true;
    ...
    static applyCollisionResponse(object, collision) {
        ...
        if ((collision.normal.y > 0.6) || 
            ...
            if (object.userData?.type === 'player' && object.soi?.object) {
                if (object.handle) {
                    ...
                    // Store surface normal for camera alignment
                    object.surfaceNormal = planetNormal ? planetNormal.clone() : new Vector3(0, 1, 0);
        ...
        // Restore consistent height above planet after collision if needed
        if (planetNormal && object._preCollisionHeight !== undefined && 
            object.soi?.radius && object.soi?.object) {
        
            const currentHeight = object.position.clone()
                .sub(object.soi.object.position).length() - object.soi.radius;
                
            // If collision response changed our height significantly, restore it
            if (Math.abs(currentHeight - object._preCollisionHeight) > 0.3) {
                object.position.copy(object.soi.object.position)
                    .add(planetNormal.clone().multiplyScalar(object.soi.radius + object._preCollisionHeight));
            }
            
            // Clear the stored height
            delete object._preCollisionHeight;
        }
    }
