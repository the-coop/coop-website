...
export default class Physics {
    ...
    static update() {
        try {
            PlayersManager.players.forEach(player => {
                ...
                if (player.handle && player.collidable) {
                    const isRegistered = ObjectManager.validateCollidable(player.handle);
                    if (!isRegistered) {
                    ...
                    try {
                        ...
                        if (collide) {
                            ...
                            if (velDot < 0) {
                                ...
                                // IMPROVED: If moving along surface, check if we're trying to slide into the collision
                                if (player.velocity.lengthSq() > 0.01) {
                                    const slideVelocity = player.velocity.clone();
                                    const slideAlignment = slideVelocity.normalize().dot(collisionNormal);
                                    
                                    // If sliding movement is somewhat toward collision normal, reduce it
                                    if (Math.abs(slideAlignment) < 0.3) {
                                        player.velocity.multiplyScalar(0.5);
                                        console.log("Reduced sliding velocity to prevent tunneling");
                                    }
                                }
                                ...
                                // NEW: Send collision details for UI visualization
                                if (typeof window !== 'undefined' && window.updateCollisionInfo) {
                                    window.updateCollisionInfo(closestObject.type, 
                                                             player.position.distanceTo(collisionPosition), 
                                                             collisionNormal);
                                }
                            }
        ...
        try {
            ...
            if (object.userData.velocity) {
                ...
                if (velDot < 0) {
                    ...
                    // NEW: Store recent collision for STRONG anti-tunneling
                    // This makes tunneling virtually impossible as objects "remember" recent collisions
                    if (!object.userData._recentCollisions) {
                        object.userData._recentCollisions = [];
                    }
                    
                    // Store collision with timestamp and extra data
                    object.userData._recentCollisions.push({
                        normal: normal.clone(),
                    ...
                    // Limit stored collisions
                    if (object.userData._recentCollisions.length > 8) {
                        object.userData._recentCollisions.shift();
                    }
                    
                    // NEW: Check if we're trying to slide into an object
                    if (object.userData.velocity.lengthSq() > 0.01) {
                        const slideVelocity = object.userData.velocity.clone();
                        const slideAlignment = slideVelocity.normalize().dot(normal);
                        
                        // If sliding direction is somewhat toward the collision normal
                        // (trying to tunnel through), block it
                        if (Math.abs(slideAlignment) < 0.3) {
                            // Apply additional damping to sliding movement
                            object.userData.velocity.multiplyScalar(0.5);
                            console.log("Prevented potential tunneling with sliding damping");
                        }
                    }
            ...
            if (other.userData && other.userData.isDynamic && other.userData.velocity) {
                ...
                if (velDot < 0) {
                    // CRITICAL CHANGE: Completely eliminate velocity component in collision direction too
    ...
    // Add improved anti-tunneling processing
