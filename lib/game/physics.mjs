...
export default class Physics {
                    ...
                    try {
                        ...
                        if (collide) {
                            ...
                            if (penetrationDistance < 0) {
                                ...
                                // Apply the smoothed correction - POSITION ONLY
                                player.position.addScaledVector(collisionNormal, smoothedCorrection);
                            ...
                            // This prevents false landing events during surface collisions
                            ...
                            if (isUpwardCollision && player.falling) {
                                ...
                                if (wasGroundedBeforeCollision) {
                                    ...
                                    player.falling = false;
                                    // Don't modify standingOnObject flag either
                                }
                            }
    ...
    static applyCollisionResponse(object, collision) {
        ...
        try {
            ...
            const penetrationDepth = collision.penetrationDepth || 0.1; // Default if not provided
            ...
            // If object has velocity, modify it to prevent further penetration
            if (object.velocity) {
    ...
    // Add missing applyGravityToObject method - process gravity for general collidable objects
    static applyGravityToObject(object) {
        if (!object || !object.userData) return;
        
        try {
            // Skip objects without velocity or marked as static
            if (!object.userData.velocity || object.userData.isStatic) return;
            ...
            if (!planet || !planet.radius) return;
            
            // Get direction to object from planet center
            const planetCenter = planet.object.position;
            ...
            // Store surface normal in object for alignment and collision
            object.userData.surfaceNormal = surfaceNormal;
            
            // Apply gravity with standard strength for non-vehicle objects
            ...
            // Apply velocity to position with simple integration
            object.position.add(object.userData.velocity);
            
            // Check for collision with planet surface
            const heightOffset = object.userData.heightOffset || 1.0;
            const groundLevel = planet.radius + heightOffset;
            
            if (distance <= groundLevel) {
                // Position correction to stay on surface
                object.position.copy(planetCenter).addScaledVector(surfaceNormal, groundLevel);
                
                // Handle collision response
                if (object.userData.falling) {
                    // Apply bounce with damping
                    const impactSpeed = -object.userData.velocity.dot(surfaceNormal);
                    const bounceFactor = object.userData.bounceFactor || 0.2;
                    
                    if (impactSpeed > 0.1) {
                        // Convert downward velocity to small upward bounce
                        object.userData.velocity.addScaledVector(surfaceNormal, impactSpeed * bounceFactor);
                        
                        // Add damping to reduce bouncing
                        object.userData.velocity.multiplyScalar(0.8);
                    } else {
                        // For very soft landings, zero the vertical velocity
                        const verticalComponent = surfaceNormal.clone().multiplyScalar(
                ...
                object.userData.velocity.multiplyScalar(frictionFactor);
            ...
            console.error("Error in object gravity processing:", e);
        }
    }
};
