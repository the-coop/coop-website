...
export default class Physics {
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            ...
            // Skip regular physics during stabilization period for cars
            if (isInStabilizationPeriod && vehicle.userData.type === 'car') {
                // Just keep the car exactly at the right height
                const planetCenter = planet.object.position;
                const toVehicle = vehicle.position.clone().sub(planetCenter);
                const distance = toVehicle.length();
                const surfaceNormal = toVehicle.normalize();
            ...
            // CRITICAL WOBBLE FIX: Handle stationary cars with extra stability
            ...
            // Handle all the rest of the vehicle physics code for when it's on the surface
                ...
                if (playerControlled) {
                    // Moving vehicles on planet surfaces like the player handle
                    if (Math.abs(vehicle.userData.speed) > 0.01) {
                        // Get forward direction in vehicle's local space
                        const forward = new Vector3(0, 0, -1).applyQuaternion(vehicle.quaternion);
                        
                        if (vehicle.userData.type === 'car' || 
                           (vehicle.userData.type === 'airplane' && vehicle.userData.altitude <= 0)) {
                            // For cars or grounded aircraft: Project movement onto planet surface
                            const projectedForward = forward.clone().projectOnPlane(surfaceNormal).normalize();
                            
                            // CRITICAL FIX: Use surface-aligned movement similar to player handle
                            const movement = projectedForward.multiplyScalar(vehicle.userData.speed * (1/60) * 10);
                            vehicle.position.add(movement);
                            
                            // After movement, snap back to correct height above planet surface
                            vehicle.position.copy(planetCenter).addScaledVector(
                                vehicle.position.clone().sub(planetCenter).normalize(),
                                collisionDistance
                            );
                            
                            // If aircraft is gaining speed for takeoff attempt
                            if (vehicle.userData.type === 'airplane' && vehicle.userData.speed > 25) {
                                // Begin transition to flight
                                this.vehicleLiftoff(vehicle, surfaceNormal);
                            }
                        }
                        else if (vehicle.userData.type === 'airplane' && vehicle.userData.altitude > 0) {
                            // In-flight aircraft move in their forward direction
                            const movement = forward.clone().multiplyScalar(vehicle.userData.speed * (1/60) * 10);
                            vehicle.position.add(movement);
                            
                            // Apply lift physics (simplified)
                            const liftFactor = vehicle.userData.liftFactor || 0.5;
                            const speedFactor = Math.min(1.0, vehicle.userData.speed / 40);
                            
                            // Gradually reduce altitude if not maintaining enough speed
                            if (vehicle.userData.speed < 20) {
                                vehicle.userData.altitude -= (0.2 - speedFactor * 0.2) * (1/60) * 10;
                                if (vehicle.userData.altitude < 0) {
                                    vehicle.userData.altitude = 0;
                                    // Force back to ground-based movement
                                    this.vehicleLanding(vehicle, surfaceNormal);
                                }
                            }
                        }
