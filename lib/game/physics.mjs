...
export default class Physics {
    ...
    // NEW METHOD: Added to fix the car bouncing issue by ensuring proper height
    static maintainVehicleSurfaceHeight(vehicle) {
        ...
        try {
            ...
            // CRITICAL FIX: For car type, maintain exact height above planet surface
            // This prevents small bouncing movements while driving
            if (vehicle.userData.type === 'car') {
                // Get exact height we want to maintain
                const targetHeight = planet.radius + 2.0;
                
                // If we're not exactly at that height, adjust position
                if (Math.abs(distance - targetHeight) > 0.05) {
                    // Smoothly adjust height to avoid abrupt position changes
                    const correctionFactor = 0.3; // Lower = smoother but slower correction
                    const targetPosition = planetCenter.clone().addScaledVector(
                        surfaceNormal, 
                        targetHeight
                    );
                    
                    // Blend current position with target position (only height component)
                    const currentRadialDistance = vehicle.position.distanceTo(planetCenter);
                    const currentDirection = vehicle.position.clone().sub(planetCenter).normalize();
                    const newPosition = planetCenter.clone().addScaledVector(
                        currentDirection,
                        currentRadialDistance * (1 - correctionFactor) + targetHeight * correctionFactor
                    );
                    
                    // Apply the new position
                    vehicle.position.copy(newPosition);
                    
                    // Update any physics handles if they exist
                    if (vehicle.userData.physicsHandle) {
                        vehicle.userData.physicsHandle.position.copy(vehicle.position);
                    }
                }
                
                // Make sure vehicle is properly oriented to surface
                if (!vehicle.userData.falling) {
                    this.stabilizeVehicleOnSurface(vehicle, surfaceNormal);
                }
            }
        } catch (e) {
            console.error("Error maintaining vehicle height:", e);
        }
    }
