...
export default class Physics {
        ...
        try {
            ...
            if (object.userData.falling && 
                ...
                if (distance <= groundLevel + 1.0) {
                    ...
                    object.userData.hardFreezeEndTime = Date.now() + 500; // Reduced from 1000ms to 500ms
    ...
    static objectsOverlapping(objA, objB) {
        try {
            ...
            // Get radius for each object for sphere-based collision
    ...
    static applyCollisionImpulse(objectA, objectB, direction, penetration, massA, massB) {
        try {
            ...
            // If objects are already separating, skip impulse (unless it's a vehicle-vehicle collision)
            ...
            // CRITICAL FIX: Don't skip vehicle-vehicle collisions even if technically separating
            const isVehicleCollision = isVehicleA && isVehicleB;
            // NEW: Explicitly track vehicle-player collisions
            ...
            // IMPROVED: Calculate coefficient of restitution (bounciness)
            let restitution;
            
            if (isVehicleCollision) {
                // REDUCED: Vehicle-to-vehicle collisions should have MUCH less bounce
                restitution = 0.005; // DRASTICALLY REDUCED from 0.02 to 0.005 for minimal bounce
                
                // Calculate relative speed for speed-dependent effects
                const relativeSpeed = Math.abs(relativeVelocity);
                console.log(`Vehicle collision detected with relative speed: ${relativeSpeed.toFixed(2)}`);
                
                // Apply speed-dependent effects
                if (relativeSpeed > 10) {
                    // High-speed collision - add special effects if available
                    if (typeof window.gameEffects?.createCollisionEffect === 'function') {
                        const midpoint = new Vector3().addVectors(objectA.position, objectB.position).multiplyScalar(0.5);
                        window.gameEffects.createCollisionEffect(midpoint, relativeSpeed);
                    }
                    
                    // Add audio effect for high-speed collision if available
                    if (typeof window.gameAudio?.playCollisionSound === 'function') {
                        window.gameAudio.playCollisionSound(relativeSpeed);
                    }
                }
            } else if (isVehiclePlayerCollision) {
                // NEW: Special handling for vehicle-player collisions - more push, less bounce
                restitution = 0.1; // Higher than vehicle-vehicle but still low
                console.log(`Vehicle-player collision detected`);
                
                // Determine which is the vehicle and which is the player
                const vehicle = isVehicleA ? objectA : objectB;
                const player = isVehicleA ? objectB : objectA;
                const vehicleSpeed = vehicle.userData.speed || vehicle.userData.velocity.length();
                
                console.log(`Vehicle speed at collision: ${vehicleSpeed.toFixed(2)}`);
                
                // Add impact effects based on vehicle speed
                if (vehicleSpeed > 5) {
                    console.log("High-speed vehicle hit player");
                    // Player stumbling/ragdoll effect could be triggered here
                }
            } else {
            ...
            // IMPROVED: Calculate momentum-conserving impulse with proper mass consideration
            ...
            // CRITICAL FIX: Account for whether object is static/immovable
            const isStaticA = objectA.userData?.isStatic || false;
            const isStaticB = objectB.userData?.isStatic || false;
            
            // IMPROVED: For vehicle-vehicle or vehicle-player collisions, ensure NEITHER is treated as static
            if (isVehicleCollision || isVehiclePlayerCollision) {
                // For these collisions, always apply impulse to both
                velA.addScaledVector(impulse, 1/massA);
                velB.addScaledVector(impulse, -1/massB);
                
                // Log momentum transfer for debugging
                console.log(`Collision impulse: ${impulseFactor.toFixed(2)}`);
                console.log(`Object A new velocity: ${velA.length().toFixed(2)}`);
                console.log(`Object B new velocity: ${velB.length().toFixed(2)}`);
            } 
            // For other collisions, respect static property
            else {
                // Only apply impulse to non-static objects
                if (!isStaticA) {
                    velA.addScaledVector(impulse, 1/massA);
                }
                if (!isStaticB) {
            ...
            // CRITICAL FIX: For vehicle collisions, ensure momentum transfers properly
            if (isVehicleCollision || isVehiclePlayerCollision) {
                // Get speed and direction
                const speedA = velA.length();
                const speedB = velB.length();
                
                // Get object movement directions
                let directionA = new Vector3(0, 0, -1);
                let directionB = new Vector3(0, 0, -1);
                
                if (objectA.quaternion) {
                    directionA.applyQuaternion(objectA.quaternion).normalize();
                }
                
                if (objectB.quaternion) {
                    directionB.applyQuaternion(objectB.quaternion).normalize();
                }
                
                // Calculate dot product to determine if collision is head-on or from side/rear
                const collisionAngle = directionA.dot(directionB);
                
                // Apply reduced damping to allow more momentum transfer
                let dampingFactor = 0.4; // REDUCED from 0.7 to 0.4 - much less damping for better momentum transfer
                if (collisionAngle < -0.5) { // Head-on collision
                    dampingFactor = 0.5; // REDUCED from 0.8 to 0.5
                } else if (collisionAngle > 0.5) { // Rear collision
                    dampingFactor = 0.3; // REDUCED from 0.5 to 0.3 for rear impacts
                }
                
                // IMPROVED: Use Planet's surface normal to constrain momentum transfer to the surface
                let planetNormalA = null;
                let planetNormalB = null;
                
                // Get planet normals from objects
                if (objectA.userData.surfaceNormal) {
                    planetNormalA = objectA.userData.surfaceNormal;
                } else if (objectA.userData.planet && objectA.userData.planet.object) {
                    planetNormalA = objectA.position.clone().sub(objectA.userData.planet.object.position).normalize();
                }
                
                if (objectB.userData.surfaceNormal) {
                    planetNormalB = objectB.userData.surfaceNormal;
                } else if (objectB.userData.planet && objectB.userData.planet.object) {
                    planetNormalB = objectB.position.clone().sub(objectB.userData.planet.object.position).normalize();
                }
                
                // IMPROVED: If both objects are on the same planet, ensure momentum transfers along surface
                if (planetNormalA && planetNormalB) {
                    // Project vehicle velocities onto planet surface
                    const projectVelocityToSurface = (vel, normal) => {
                        // Remove any component of velocity that points away from the surface
                        const normalComponent = normal.clone().multiplyScalar(vel.dot(normal));
                        // Allow a tiny bit of vertical bounce by not fully removing the normal component
                        const reducedNormalComponent = normalComponent.multiplyScalar(0.95); // Keep just 5% of vertical component
                        return vel.clone().sub(reducedNormalComponent);
                    };
                    
                    // Project both velocities to their respective surfaces
                    const surfaceVelA = projectVelocityToSurface(velA, planetNormalA);
                    const surfaceVelB = projectVelocityToSurface(velB, planetNormalB);
                    
                    // NEW: Ensure high momentum transfer for stationary objects getting hit
                    const transferMomentumToStationary = (fromVel, toVel, fromObj, toObj, fromMass, toMass) => {
                        const fromSpeed = fromVel.length();
                        const toSpeed = toVel.length();
                        
                        if (toSpeed < 0.5 && fromSpeed > 2.0) {
                            // Calculate impact direction along surface
                            const surfaceImpactDir = direction.clone()
                                .projectOnPlane(toObj.userData.surfaceNormal || planetNormalB)
                                .normalize();
                                
                            // IMPROVED: More generous momentum transfer for realistic physics
                            // Use physics-based transfer ratio based on masses and speed
                            const baseMassRatio = fromMass / (fromMass + toMass);
                            const speedFactor = Math.min(1.0, fromSpeed / 10); // Cap effect at 10 units/sec
                            const transferRatio = Math.min(0.8, Math.max(0.5, baseMassRatio * (1 + speedFactor)));
                            
                            // Calculate transferred speed
                            const transferSpeed = fromSpeed * transferRatio;
                            
                            // Apply transferred speed along surface
                            toVel.addScaledVector(surfaceImpactDir, transferSpeed);
                            console.log(`Transferring momentum: speed ${transferSpeed.toFixed(2)}, ratio ${transferRatio.toFixed(2)}`);
                            
                            // Reduce original object's velocity proportionally
                            fromVel.multiplyScalar(1 - transferRatio * 0.6); // Reduced multiplier for conservation of momentum
                        }
                    };
                    
                    // Apply momentum transfer in both directions
                    transferMomentumToStationary(surfaceVelA, surfaceVelB, objectA, objectB, massA, massB);
                    transferMomentumToStationary(surfaceVelB, surfaceVelA, objectB, objectA, massB, massA);
                    
                    // Apply surface-constrained velocities
                    velA.copy(surfaceVelA);
                    velB.copy(surfaceVelB);
                }
                
                // Apply gentler damping to allow more movement transfer
                velA.multiplyScalar(1 - dampingFactor * 0.2); // REDUCED from 0.3 to 0.2
                velB.multiplyScalar(1 - dampingFactor * 0.2); // REDUCED from 0.3 to 0.2
                
                // CRITICAL FIX: Update speed properties for both objects
                if (typeof objectA.userData.speed !== 'undefined') {
                    objectA.userData.speed = velA.length();
                    console.log(`Object A speed after collision: ${objectA.userData.speed.toFixed(2)}`);
                }
                if (typeof objectB.userData.speed !== 'undefined') {
                    objectB.userData.speed = velB.length();
                    console.log(`Object B speed after collision: ${objectB.userData.speed.toFixed(2)}`);
                }
                
                // IMPROVED: Special player handling for vehicle collisions
                if (isVehiclePlayerCollision) {
                    const player = isPlayerA ? objectA : objectB;
                    const vehicle = isPlayerA ? objectB : objectA;
                    const playerVel = isPlayerA ? velA : velB;
                    
                    // For player safety, increase player velocity more when hit by vehicles
                    // This prevents players from getting "stuck" to vehicles
                    const vehicleSpeed = vehicle.userData.speed || 0;
                    if (vehicleSpeed > 3) {
                        // Add extra "push" for player safety
                        const pushDirection = new Vector3().subVectors(player.position, vehicle.position).normalize();
                        const pushStrength = Math.min(vehicleSpeed * 0.5, 8); // Cap at 8 units/sec
                        playerVel.addScaledVector(pushDirection, pushStrength);
                        console.log(`Added extra player push: ${pushStrength.toFixed(2)}`);
                    }
                }
                
                // Add a brief "stun" effect after significant collisions
                if (Math.abs(relativeVelocity) > 5) {
                    const stunDuration = Math.min(500, Math.abs(relativeVelocity) * 30); // Reduced for quicker recovery
                    
                    if (!isStaticA) {
                        objectA.userData.collisionStunTime = Date.now() + stunDuration;
                    }
                    if (!isStaticB) {
                        objectB.userData.collisionStunTime = Date.now() + stunDuration;
                    }
                }
                
                // CRITICAL: Ensure both objects are NOT marked as static after collision
                objectA.userData.isStatic = false;
                objectB.userData.isStatic = false;
                
                // CRITICAL: Make sure hardFreeze is not preventing movement
                if (objectA.userData.hardFreeze && !objectA.userData.isActivelyControlled) {
                    objectA.userData.hardFreeze = false;
                }
                if (objectB.userData.hardFreeze && !objectB.userData.isActivelyControlled) {
                    objectB.userData.hardFreeze = false;
                }
            }
        } catch (err) {
