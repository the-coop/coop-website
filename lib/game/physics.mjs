...
export default class Physics {
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            ...
            // CRITICAL FIX: Use stored gravity factor that was specifically set during creation
            const vehicleGravityFactor = vehicle.userData.gravityFactor || 0.1;
            const effectiveGravity = GRAVITY_CONSTANT * vehicleGravityFactor;
            
            // ENHANCED: Scale gravity even more aggressively with distance for space spawns
            // This creates a more gradual transition from space to near-surface
            const distanceFactor = Math.pow(distance / (planet.radius * 15), 2);
            ...
            if (vehicle.userData.falling) {
                ...
                vehicle.userData.velocity.multiplyScalar(1 - airResistance * 0.03);
            }
