...
export default class Physics {
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            ...
            const vehicleGravityFactor = vehicle.userData.gravityFactor || 0.4; // Further reduced from 0.5 to 0.4
            ...
            const landingBuffer = 3.0; // Further increased from 2.0 to 3.0 for earlier landing detection
            ...
            if (distance <= groundLevel + landingBuffer) {
                ...
                if (vehicle.userData.falling) {
                    ...
                    // ADDED: Track landing time for stabilization period
                    ...
                    if (vehicle.userData.needsLandingStabilization) {
                        ...
                        if (stabilizationElapsed > stabilizationDuration) {
                            ...
                            if (vehicle.userData.debug) {
                                console.log("Vehicle landing stabilization complete");
                            }
                        }
                    }
                }
    ...
    static maintainVehicleSurfaceHeight(vehicle) {
        ...
        try {
            ...
            // IMPROVED: Use more aggressive position correction to ensure proper height
            // Only adjust if significantly different from target height
            if (Math.abs(distance - groundLevel) > 0.1) { // Reduced threshold from 0.5 to 0.1
                ...
                // IMPROVED: Use stronger correction factor for more precise height maintenance
                vehicle.position.lerp(newPosition, 0.3); // Increased from 0.1 to 0.3
            ...
            if (typeof VehicleManager.alignVehicleToPlanetSurface === 'function') {
                // IMPROVED: Use stronger alignment factor for more precise orientation
                VehicleManager.alignVehicleToPlanetSurface(vehicle, surfaceNormal, 0.2); // Increased from 0.1 to 0.2
            }
