...
export default class Physics {
    ...
    static applyGravity(object) {
        ...
        try {
            ...
            // Use object-specific gravity factor or fallback
    ...
    static maintainObjectSurfaceHeight(object) {
        ...
        try {
            ...
            // CRITICAL FIX: Special handling for newly spawned vehicles on the ground
            if (object.userData.type === 'vehicle' || object.userData.type === 'car' || object.userData.type === 'airplane') {
                if (object.userData.freshlySpawned && !object.userData.falling) {
                    // For freshly spawned vehicles already on the ground, skip landing detection
                    // and just ensure they're at the right height
                    const correctionFactor = 0.5; // Faster correction for initial positioning
                    ...
                    // If significantly off from correct height, adjust position
                    if (Math.abs(distance - groundLevel) > 0.2) {
                        ...
                        // Ensure collision bounds are updated
                        if (object.collidable) {
                            ObjectManager.updateCollidableBounds(object);
                        }
                    }
                    
                    // Set proper alignment with the surface
                    if (typeof VehicleManager.alignVehicleToPlanetSurface === 'function') {
                        VehicleManager.alignVehicleToPlanetSurface(object, surfaceNormal, 0.5, false);
                    }
                    
                    // Mark as stabilized after first position sync
                    if (!object.userData.positionSynced) {
                        object.userData.positionSynced = true;
                        object.userData.landingTime = Date.now();
                    }
                    
                    // Clear freshly spawned flag after 2 seconds
                    if (!object.userData._clearFreshSpawnTimeout) {
                        object.userData._clearFreshSpawnTimeout = setTimeout(() => {
                            object.userData.freshlySpawned = false;
                            object.userData.fullyStabilized = true;
                        }, 2000);
                    }
                    
                    return; // Skip the rest of the physics processing
                }
            }
            ...
            if (object.userData.enhancedStabilization && !object.userData.falling) {
                this.processEnhancedStabilization(object, surfaceNormal);
            }
