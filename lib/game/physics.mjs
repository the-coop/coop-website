...
export default class Physics {
                ...
                try {
                    ...
                    if ((onPlanet || canLiftoff) && !hadOBBCollision && !player.standingOnObject) {
                        ...
                        if ((player.falling || player.wasOnObject) && !inVehicle && ControlManager.controller?.landing) {
                            ...
                            // FIXED: Ensure all object-related flags are properly cleared
                            player.standingOnObject = false;
                        }
                        ...
                        // NEW: Improved detection for jumping off objects
                        // This helps ensure landing is called when returning to planet surface
                            ...
                            if (upwardVelocity > 0.3) {
                                ...
                                player.standingOnObject = false; // FIXED: Properly clear this flag
                                player.falling = true;  // FIXED: Mark as falling immediately
    ...
    // NEW: Method to correct edge cases in player state
    static validatePlayerPhysicsState(player) {
        ...
        try {
            ...
            // If wasOnObject flag has been set for too long without landing
            if (player.wasOnObject) {
                if (!player._wasOnObjectTime) {
                    player._wasOnObjectTime = Date.now();
                }
                
                // If it's been more than 3 seconds, assume the flag is stale
                if (Date.now() - player._wasOnObjectTime > 3000) {
                    console.warn("Clearing stale wasOnObject flag");
