...
// REDUCED: Further reduced gravity constant from 0.35 to 0.2 for gentler physics
...
export default class Physics {
    ...
    static update() {
        try {
            ...
            // FIXED: Changed this.updateObjects() to Physics.updateObjects() for proper static method reference
            // or directly call the method since we're in the same class
            this.updateObjects();
    ...
    static updatePlayer(player) {
        ...
        try {
            ...
            if (!player.standingOnObject) {
                // REDUCED: Further reduced player gravity influence
                const playerGravityFactor = 0.85; // Reduce player gravity by 15%
                const gravity = GRAVITY_CONSTANT * playerGravityFactor / Math.pow(distance / planetRadius, 2);
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            ...
            // IMPROVED: Ensure vehicle has SOI set correctly for gravity calculations
            if (!vehicle.userData.soi) {
                // Calculate sphere of influence if not already set
                vehicle.userData.soi = this.calculateSOI(vehicle.position);
            }
            
            // Use soi for planet reference to ensure consistency with player physics
            const planet = vehicle.userData.soi || vehicle.userData.planet;
            if (!planet) return;
            ...
            // Calculate gravity with vehicle-specific adjustment using SOI
            // FURTHER REDUCED: Use even lower gravity factor for gentler falls
            const vehicleGravityFactor = vehicle.userData.gravityFactor || 0.1; // Default even lower now (0.3 -> 0.1)
            const effectiveGravity = GRAVITY_CONSTANT * vehicleGravityFactor;
            
            // ADDED: Scale gravity by distance more aggressively for vehicles
            // This creates a more gradual transition from space to near-surface
            const distanceFactor = Math.pow(distance / (planet.radius * 10), 2);
            const scaledGravity = effectiveGravity / (1 + distanceFactor);
            
            // Apply gravity to velocity with reduced strength
            vehicle.userData.velocity.addScaledVector(surfaceNormal, -scaledGravity);
            
            // INCREASED: Enhanced air resistance for falling vehicles for smoother landings
            if (vehicle.userData.falling) {
                ...
                const airResistance = Math.min(0.08 * speed * speed, 0.7); // Further increased from 0.06/0.6 to 0.08/0.7
                vehicle.userData.velocity.multiplyScalar(1 - airResistance * 0.03); // Increased from 0.025 to 0.03
            }
            ...
            if (distance <= groundLevel + landingBuffer) {
                ...
                if (vehicle.userData.falling) {
                    ...
                    // ADDED: If vehicle was freshly spawned, completely cancel all momentum for stability
                    if (vehicle.userData.freshlySpawned) {
                        // Complete momentum cancellation for freshly spawned vehicles
                        vehicle.userData.velocity.set(0, 0, 0);
                        vehicle.userData.speed = 0;
                        ...
                        // Set up hard freeze for a full second to ensure vehicle is completely stabilized
                        vehicle.userData.hardFreeze = true;
                        vehicle.userData.hardFreezeEndTime = Date.now() + 2000; // Increased to 2 seconds for better stability
                        
                        // Mark vehicle as no longer freshly spawned
                        vehicle.userData.freshlySpawned = false;
                        
                        console.log(`Freshly spawned vehicle ${vehicle.userData.name || 'unnamed'} landed - all momentum cancelled`);
                    } 
                    else {
                    ...
                    vehicle.userData.landingTime = Date.now();
                    vehicle.userData.landingImpactSpeed = approachSpeed;
                    ...
                    if (approachSpeed > 0.5) {
                        ...
                        vehicle.userData.stabilizationDuration = Math.min(3000, approachSpeed * 1000); // Up to 3 seconds based on impact
                    }
                    ...
                    if (vehicle.userData.debug) {
                        console.log(`Vehicle ${vehicle.userData.name || 'unnamed'} landed successfully with impact speed ${approachSpeed.toFixed(2)}`);
                    }
                } else {
    ...
    static processCollisions() {
        try {
            // Process any pending collisions between objects
            // This could handle object-object interactions like test cube collisions
