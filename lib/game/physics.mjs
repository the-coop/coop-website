            ...
            // Handle all the rest of the vehicle physics code for when it's on the surface
                ...
                if (playerControlled && onSurface) {
                    ...
                    if (Math.abs(vehicle.userData.speed) > 0.01) {
                        ...
                        // Create movement along the projected forward vector with better coefficient
                        ...
                        if (Math.abs(vehicle.userData.speed) > 1) {
                            console.log(`Car moving: speed=${vehicle.userData.speed.toFixed(2)}, dist=${movement.length().toFixed(2)}, height=${heightAboveSurface.toFixed(2)}`);
                        }
                        
                        // CRITICAL FIX: After movement, ensure correct height above planet
                        // This ensures the car follows the planet curvature
                        vehicle.position.copy(planetCenter).addScaledVector(
                            vehicle.position.clone().sub(planetCenter).normalize(),
                            collisionDistance
                        );
                        
                        // Apply a small banking effect on curves if steering
                        if (vehicle.userData.steeringInput && Math.abs(vehicle.userData.steeringInput) > 0.1 &&
                            Math.abs(vehicle.userData.speed) > 5) {
                            const bankAngle = vehicle.userData.steeringInput * Math.min(0.1, Math.abs(vehicle.userData.speed) / 100);
                            ...
                            vehicle.up.lerp(bankNormal, 0.1);
                        }
                    }
                    ...
                    const adaptiveStrength = speed > 5 ? 0.3 : (speed > 2 ? 0.5 : 0.8);
