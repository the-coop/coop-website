...
export default class Physics {
    ...
    static processPlayerCollisions(player) {
        ...
        try {
            // MODIFIED: Always use sphere collision for players
            const useSphericalCollision = true; // Always use sphere for players
            ...
            for (const collidable of collidables) {
                ...
                if (useSphericalCollision) {
                    ...
                    // IMPROVED: For vehicle and test cubes, use OBB collision bounds rather than simple radius
                    if (collidable.type === 'vehicle' || collidable.type === 'testCube') {
                        // Use sphere vs OBB collision
                        collision = ObjectManager.testSphereOBBCollision(
                            player.position,
                            playerRadius,
                            collidable.obb
                        );
                    } else {
                        // Use simple sphere-sphere distance check for other objects
