                        ...
                        if (collide) {
                            ...
                            if (penetrationDistance < 0) {
                                ...
                                if (typeof Engine !== 'undefined' && Engine.camera && player === PlayersManager.self) {
                                    // NEW: Save more detailed camera state before any modifications
                                    const camera = Engine.camera;
                                    
                                    // First check if the camera is already in a valid state
                                    const upVector = player.soi ? 
                                        player.position.clone().sub(player.soi.object.position).normalize() : 
                                        new Vector3(0, 1, 0);
                                    ...
                                    // Only save the camera state if it's in a reasonable orientation
                                    if (upAlignment > 0.7) {
                                        player._lastStableCameraQuaternion = camera.quaternion.clone();
                                        player._lastStableCameraRotation = camera.rotation.clone();
                                        player._lastStableCameraUp = camera.up.clone();
                                        
                                        // NEW: Store the camera's forward direction too
                                        player._lastStableCameraForward = new Vector3(0, 0, -1)
                                            .applyQuaternion(camera.quaternion)
                                            .normalize();
                                            
                                        console.log("Stored stable camera orientation before collision");
                                    } else {
                                        console.log("Camera already in poor orientation - not storing");
                                    }
                                ...
                                // IMPROVED: Position correction with temporal smoothing to reduce jittering
                                
                                // NEW: Track collision history for better temporal smoothing
                                if (!player._collisionHistory) player._collisionHistory = [];
                                
                                // Add this collision to history
                                player._collisionHistory.push({
                                    normal: collisionNormal.clone(),
                                    time: Date.now(),
                                    objectType: closestObject.type
                                });
                                ...
                                // Limit history size
                                while (player._collisionHistory.length > 5) {
                                    player._collisionHistory.shift();
                                }
                                
                                // Calculate average collision normal from recent history
                                // This prevents rapid changes in collision normal direction
                                let averageNormal = collisionNormal.clone();
                                if (player._collisionHistory.length > 1) {
                                    // Only use recent collisions (within last 100ms)
                                    const now = Date.now();
                                    const recentCollisions = player._collisionHistory.filter(
                                        c => now - c.time < 100
                                    );
                                    
                                    if (recentCollisions.length > 1) {
                                        // Create average normal
                                        averageNormal.set(0, 0, 0);
                                        recentCollisions.forEach(c => averageNormal.add(c.normal));
                                        averageNormal.divideScalar(recentCollisions.length);
                                        averageNormal.normalize();
                                        
                                        console.log(`Using averaged collision normal from ${recentCollisions.length} recent collisions`);
                                    }
                                }
                                ...
                                // IMPROVED: More gradual position correction to prevent jitter
                                // Use smaller correction factor for smoother response
                                const smoothingFactor = wasGroundedBeforeCollision ? 0.65 : 0.8;
                                ...
                                const normalDotUp = averageNormal.dot(upVector);
                                let correctionNormal = averageNormal.clone();
                            
                                // CRITICAL FIX: Don't overwrite the player's surfaceNormal during object collisions
                                // Store the original surface normal before any modifications
                                const originalSurfaceNormal = player.surfaceNormal ? 
                                    player.surfaceNormal.clone() : upVector.clone();
                                
                                // CRITICAL FIX: Always preserve standing state when colliding with test cubes at surface level
                                if (isTestCubeCollision && wasGroundedBeforeCollision) {
                                    player.falling = false;
                                    player.standingOnObject = true;
                                    console.log("Preserved grounded state during test cube collision");
                                }
                                ...
                                if (Math.abs(normalDotUp) < 0.6 && wasGroundedBeforeCollision && preCollisionHeight !== null) {
                                    ...
                                    correctionNormal = averageNormal.clone().projectOnPlane(upVector).normalize();
                                    
                                    // IMPROVED: Force this to be perfectly parallel to surface for better movement
                                    if (correctionNormal.lengthSq() > 0.1) {
                                        console.log("Using height-preserving lateral correction");
                                        
                                        // CRITICAL FIX: Object collisions on surface should NEVER cause falling
                                        player.falling = false;
                                        // NEW: Explicitly mark player as on an object during ground-level collisions
                                        player.standingOnObject = true;
                                    } else {
                                        // Fall back to original normal if projection failed
                                        correctionNormal = averageNormal.clone();
                                        
                                        // CRITICAL FIX: Even on fallback, don't allow changing falling state
                                ...
                                // NEW: Store previous position for position smoothing
                                if (!player._previousPositions) player._previousPositions = [];
                                
                                // Keep a short history of recent positions (for smoothing)
                                player._previousPositions.push({
                                    position: player.position.clone(),
                                    time: Date.now()
                                });
                                
                                // Limit history size
                                while (player._previousPositions.length > 4) {
                                    player._previousPositions.shift();
                                }
                                ...
                                // NEW: Blend with previous position for smoother transitions during rapid collisions
                                if (player._previousPositions.length > 1 && player._wasRecentCollision) {
                                    // Get very recent position (within last 32ms)
                                    const now = Date.now();
                                    const recentPositions = player._previousPositions.filter(
                                        p => now - p.time < 32
                                    );
                                    
                                    if (recentPositions.length > 0) {
                                        // Use most recent position for blending
                                        const blendFactor = 0.3; // 30% previous, 70% new target
                                        player.position.lerp(
                                            targetPosition, 
                                            1 - blendFactor // Need to invert because lerp is (current, target, factor)
                                        );
                                        console.log("Applied position smoothing during collision");
                                    } else {
                                        // No recent positions, just use target directly
                                        player.position.copy(targetPosition);
                                    }
                                } else {
                                    // Standard correction without blending
                                    player.position.copy(targetPosition);
                                }
                                
                                // Track that we had a recent collision for next frame
                                player._wasRecentCollision = true;
                                ...
                                if (player.handle) {
                                    ...
                                    // ENHANCED: For test cube collisions, be even more aggressive with camera preservation
                                    if (isTestCubeCollision && Engine.camera) {
                                        const upVector = player._planetSurfaceNormal || 
                                            player.surfaceNormal || 
                                            new Vector3(0, 1, 0);
                                            
                                        // Force camera up vector to match planet surface
                                        Engine.camera.up.copy(upVector);
                                        
                                        // Ensure player surfaceNormal is never changed by test cube collisions
                                        if (player._planetSurfaceNormal) {
                                            player.surfaceNormal = player._planetSurfaceNormal.clone();
                                        }
                                    }

                                    // NEW: Also ensure camera has correct up vector during collision
                                    if (typeof Engine !== 'undefined' && Engine.camera && player === PlayersManager.self) {
                                        // Get the most stable up vector available - prioritize planet normal
                                        const upVector = player._planetSurfaceNormal || 
                                            player.surfaceNormal || 
                                            new Vector3(0, 1, 0);
                                        ...
                                        // NEW: If we have a stored stable camera state, also ensure camera forward direction
                                        // is preserved during collisions to prevent disorientation
                                        if (player._lastStableCameraForward && player._lastStableCameraUp) {
                                            // Reconstruct camera orientation from up and forward vectors
                                            const forward = player._lastStableCameraForward;
                                            const up = upVector; // Use current up vector rather than stored one
                                            ...
                                            // Apply this corrected orientation
                                            Engine.camera.quaternion.copy(correctionQuat);
                                        }
                                    }
                                }
                                ...
                                player._lastCollisionObjectType = closestObject.type;
                                
                                // NEW: Record that this specifically was a test cube collision if applicable
                                if (isTestCubeCollision) {
                                    player._lastTestCubeCollisionTime = Date.now();
                                }
                                
                                // NEW: Improved velocity modification for smoother sliding
                                if (player.velocity) {
                                    // Get dot product of velocity with collision normal
                                    const velDot = player.velocity.dot(correctionNormal);
                                    
                                    if (velDot < 0) { // Only counter velocity moving into the collision
                                        // MODIFIED: No deflection - simply cancel out velocity component in collision direction
                                        // Calculate the component of velocity in the direction of the normal
                                        const normalComponent = correctionNormal.clone().multiplyScalar(velDot);
                                        
                                        // Remove this component from the velocity (no additional force applied)
                                        player.velocity.sub(normalComponent);
                                    }
                                }
                                
                                // CRITICAL FIX: Restore the original surface normal to preserve player orientation
                                // This maintains consistent player alignment with the planet during object collisions
                                if (originalSurfaceNormal && wasGroundedBeforeCollision) {
                                    player.surfaceNormal = originalSurfaceNormal;
                                    console.log("Preserved original surface normal during collision");
                                    
                                    // For test cubes specifically, ensure we always use the planet normal
                                    if (isTestCubeCollision && player._planetSurfaceNormal) {
                                        player.surfaceNormal = player._planetSurfaceNormal.clone();
                                        console.log("Using planet surface normal for test cube collision");
                                    }
                                }
                            }
                        }
    ...
    static applyCollisionResponse(object, collision) {
        ...
        if (object.type === 'player') {
            ...
            if (typeof Engine !== 'undefined' && Engine.camera && object === PlayersManager.self) {
                ...
                if (object._preCollisionCameraData) {
                    const preQuat = object._preCollisionCameraData.quaternion;
                    
                    // Get the current forward direction and project it onto surface plane
                    const forward = new Vector3(0, 0, -1).applyQuaternion(preQuat);
                    const projectedForward = forward.projectOnPlane(upVector).normalize();
                    
                    if (projectedForward.lengthSq() > 0.1) {
                        // Create a target point to look at
                        const targetPos = object.position.clone().add(projectedForward);
                        // Make camera look at this target while keeping up vector
                        const originalUp = camera.up.clone();
                        camera.lookAt(targetPos);
                        camera.up.copy(originalUp); // Restore up vector that lookAt changes
                    }
                }
            }
        }

        // MODIFIED: Remove velocity component toward collision without adding opposing force
        if (object.velocity) {
            ...
            if (velDot < 0) {
                ...
                // REMOVED: No velocity scaling to reduce bounce effect
                // object.velocity.multiplyScalar(0.9); - REMOVED
            }
        }

        // Keep standing if collision normal is significantly upward or aligned with planet normal
