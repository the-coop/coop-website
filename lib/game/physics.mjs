...
export default class Physics {
    ...
    static vehicleLanding(vehicle, surfaceNormal) {
        ...
        try {
            ...
            if (vehicle.userData.type === 'car') {
                // CRITICAL FIX: Force immediate, complete alignment to surface with maximum strength
                VehicleManager.alignVehicleToPlanetSurface(vehicle, surfaceNormal, 1.0);
                
                // CRITICAL FIX: Force exact position on surface with no offset
                if (vehicle.userData.planet) {
                    const planetCenter = vehicle.userData.planet.object.position;
                    const heightOffset = 1.5; // Match the collision distance offset
                    vehicle.position.copy(planetCenter).addScaledVector(surfaceNormal, 
                        vehicle.userData.planet.radius + heightOffset);
                }
