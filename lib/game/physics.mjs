...
export default class Physics {
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            ...
            // FIX: Only apply gravity/physics if vehicle is still moving or not on surface
            if (!onSurface || !isStationaryVehicle) {
                ...
                if (vehicle.userData.type === 'airplane' && vehicle.userData.altitude > 0) {
                    ...
                    // Standard gravity for ground vehicles or landed airplanes
                    const gravity = GRAVITY_CONSTANT * 1.0; // Reduced from 1.2
                    vehicle.userData.velocity.addScaledVector(surfaceNormal, -gravity);
                    
                    // Handle surface contact
                    if (onSurface) {
                        // Position precisely on surface
                        vehicle.position.copy(planetCenter).addScaledVector(surfaceNormal, collisionDistance);
                        
                        // Damp velocity for smoother landing
                        ...
                        // Apply surface alignment with appropriate damping
                ...
                // Apply velocity to position - but cap it first
                const maxVelocity = 3;
