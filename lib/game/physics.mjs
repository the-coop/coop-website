                ...
                for (let step = 0; step < numSteps && !hadOBBCollision; step++) {
                    try {
                        ...
                        if (collide) {
                            ...
                            // IMPROVED: Instead of completely blocking movement, allow sliding along surfaces
                            // Calculate how far we've moved into the collision
                            const penetrationDistance = stepVelocity.dot(collisionNormal);
                            
                            // Only move back by the penetration distance (plus a small buffer)
                            const slidingPosition = player.position.clone().addScaledVector(
                                collisionNormal, 
                                -(penetrationDistance + 0.05) // Small buffer to prevent sticking
                            );
                            
                            // Apply the corrected position that allows sliding
                            player.position.copy(slidingPosition);
                            ...
                            // CRITICAL FIX: Modify velocity for sliding rather than complete blocking
                            // Remove velocity component in the direction of the collision normal
                            const velDot = player.velocity.dot(collisionNormal);
                            
                            if (velDot < 0) {
                                // Remove velocity component in direction of collision normal
                                ...
                                console.log("Collision handled - velocity adjusted for sliding");
                            }
                            
                            // IMPROVED: Better collision classification to prevent unintended falling
                            
                            // Classify collision type based on normal direction
                            const isUpwardCollision = normalDotUp > 0.6; // Reduced from 0.7 for better detection
                            const isLateralCollision = Math.abs(normalDotUp) < 0.6; // Increased from 0.5
                            const isDownwardCollision = normalDotUp < -0.5;
                            
                            // CRITICAL FIX: Special handling for test cubes and other objects on the surface
                            const isOnSurface = !player.falling || player.standingOnObject;
                            const isTestCube = closestObject && closestObject.type === 'testCube';
                            
                            // Detect landing on objects by checking if collision is from above
                            ...
                            // IMPROVED: Handle lateral collisions with objects on the surface
                            else if (isLateralCollision) {
                                console.log("Lateral collision with object");
                                
                                // CRITICAL FIX: If player is already on the ground or object, keep them grounded
                                if (isOnSurface) {
                                    console.log("Player is on surface - maintaining grounded state during collision");
                                    player.falling = false;
                                    
                                    // Make sure any upward velocity component is removed to keep player grounded
                                    const upwardVel = player.velocity.dot(upVector);
                                    if (upwardVel > 0) {
                                        // Remove upward velocity to keep player on ground
                                        player.velocity.addScaledVector(upVector, -upwardVel);
                                    }
                                    
                                    // For test cubes and other objects on the surface, allow sliding along them
                                    if (isTestCube || closestObject.type === 'wall') {
                                        console.log("Sliding along object surface");
                                        
                                        // Don't change falling state when colliding with objects on the surface
                                        player.falling = false;
                                        
                                        // If we were standing on an object before this collision, maintain that state
                                        if (player.standingOnObject) {
                                            console.log("Maintaining standing on object state");
                                        }
                                    }
                                }
                                        
                                // If object is very close to the ground, consider it a ground object
                                if (closestObject && closestObject.object && closestObject.object.userData) {
                                    const objHeight = closestObject.object.userData.height || 0;
                                    if (objHeight < 2.0) { // Increased from 1.0 to better handle various test cubes
                                        console.log("Low object collision - treating as ground");
                                        // Keep player grounded for small objects
                                        player.falling = false;
                                    }
                                }
                            }
                            // Handle downward collision (ceiling-like)
                            else if (isDownwardCollision) {
                                console.log("Downward collision - ceiling or overhang");
                                // Always mark as falling when hitting a ceiling
                                player.falling = true;
                                
                                // Make sure we completely zero any upward velocity
                                const upwardVel = player.velocity.dot(upVector);
                                if (upwardVel > 0) {
                            ...
                            if (typeof window !== 'undefined' && window.gameNotify) {
                                if (collide.isWallCollision) {
                                    ...
                                    window.gameNotify(`Collision with ${closestObject.type} - sliding along surface`);
                                }
                            }
