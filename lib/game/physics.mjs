...
export default class Physics {
    ...
    static processPlayerCollisions(player) {
        ...
        try {
            ...
            for (const collidable of collidables) {
                ...
                // SIMPLIFIED: Use direct sphere collision test for better reliability
                ...
                // Get max dimension of the object's OBB
                ...
                // For OBB collision, find the closest point on the OBB to the player
                let collision = false;
                let collisionResult = null;
                
                // CRITICAL FIX: Direct sphere-OBB test
                const objPos = collidable.object.position.clone();
                
                // For better stability, use simple bounding sphere test first
                if (centerDistance <= playerRadius + objectRadius) {
                    // Get closest point on object to player sphere
                    let closestPoint;
                    
                    // CRITICAL FIX: Proper sphere-OBB test
                    if (collidable.obb) {
                        // Transform player position to OBB's local space
                        const localPoint = player.position.clone().sub(collidable.obb.center);
                        
                        // Simple algorithm: get closest point by clamping in local space
                        // First transform to OBB local space
                        const invRotation = new Matrix3().copy(collidable.obb.rotation).transpose();
                        localPoint.applyMatrix3(invRotation);
                        
                        // Clamp to OBB bounds
                        const clampedPoint = new Vector3(
                            Math.max(-collidable.obb.halfSize.x, Math.min(collidable.obb.halfSize.x, localPoint.x)),
                            Math.max(-collidable.obb.halfSize.y, Math.min(collidable.obb.halfSize.y, localPoint.y)),
                            Math.max(-collidable.obb.halfSize.z, Math.min(collidable.obb.halfSize.z, localPoint.z))
                        );
                        
                        // Transform back to world space
                        closestPoint = clampedPoint.clone().applyMatrix3(collidable.obb.rotation).add(collidable.obb.center);
                    } else {
                    ...
                    // If distance is within player radius, we have a collision
                    if (distance <= playerRadius) {
                        collision = true;
                        
                        // Calculate collision normal (from closest point to player center)
                        ...
                        console.log(`Collision detected with ${collidable.type || 'unknown'} object, penetration: ${collisionResult.penetration.toFixed(2)}`);
                    }
                }
    ...
    // Complete handlePlayerObjectCollision - this was cut off in the original file
    ...
    // VISUALIZE COLLISIONS METHOD needs to be completed as well
