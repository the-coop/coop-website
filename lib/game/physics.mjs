...
export default class Physics {
    ...
    static vehicleLanding(vehicle, surfaceNormal) {
        ...
        try {
            ...
            if (vehicle.userData.type === 'car') {
                // CRITICAL FIX: First, store the vehicle's forward direction on the tangent plane
                ...
                const projectedForward = originalForward.clone().projectOnPlane(toVehicle).normalize();
                
                // CRITICAL FIX: Force exact position on surface with no offset
                vehicle.position.copy(planetCenter).addScaledVector(toVehicle, 
                    vehicle.userData.planet.radius + 1.5); // Match the collision height offset
                
                // CRITICAL FIX: Two-step alignment process
                // Step 1: Align vehicle UP direction to surface normal with maximum strength
                VehicleManager.alignVehicleToPlanetSurface(vehicle, toVehicle, 1.0);
                
                // Step 2: Adjust yaw to preserve original driving direction
                if (projectedForward.lengthSq() > 0.01) {
                    // Get current forward after alignment
                    const newForward = new Vector3(0, 0, -1).applyQuaternion(vehicle.quaternion);
                    const newProjectedForward = newForward.clone().projectOnPlane(toVehicle).normalize();
                    
                    // Calculate angle to rotate back to original direction
                    const angle = Math.atan2(
                        new Vector3().crossVectors(newProjectedForward, projectedForward).dot(toVehicle),
                        newProjectedForward.dot(projectedForward)
                    );
                    
                    // Apply rotation around surface normal to preserve vehicle direction
                    vehicle.rotateOnWorldAxis(toVehicle, angle);
                }
                
                // Kill most of the velocity but keep some horizontal momentum
                const horizontalVelocity = vehicle.userData.velocity.clone().projectOnPlane(toVehicle);
                vehicle.userData.velocity.copy(horizontalVelocity.multiplyScalar(0.6));
            } 
            else if (vehicle.userData.type === 'airplane') {
