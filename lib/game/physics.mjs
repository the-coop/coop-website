                ...
                for (let step = 0; step < numSteps && !hadOBBCollision; step++) {
                    try {
                        ...
                        if (collide) {
                            ...
                            // FIXED: Improved collision classification to prevent unintended falling
                            
                            // Classify collision type based on normal direction
                            const isUpwardCollision = normalDotUp > 0.7;
                            const isLateralCollision = Math.abs(normalDotUp) < 0.5;
                            ...
                            // FIXED: Properly handle lateral collisions to prevent unintended falling
                            else if (isLateralCollision) {
                                console.log("Lateral collision with object");
                                
                                // CRITICAL FIX: Don't change falling state during lateral collisions
                                // This prevents the player from getting pushed upward off the planet
                                
                                // If player was already on ground (not falling), keep them grounded
                                if (!player.falling) {
                                ...
                                // This helps when moving against low obstacles like small steps
