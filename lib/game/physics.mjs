...
import ControlManager from './control.mjs'; // Add missing ControlManager import
import { Vector3, Box3, Object3D, Plane, Quaternion, Matrix4 } from 'three';
...
export default class Physics {
    ...
    static applyGravity(object) {
        ...
        try {
            ...
            // IMPROVED: Use object-specific gravity factor (make sure freshly spawned vehicles use normal gravity)
            // Remove special handling for freshly spawned vehicles to ensure they fall naturally
            const gravityFactor = object.userData.gravityFactor || 
                                (object.userData.type === 'player' ? 0.85 :
                                 object.userData.type === 'vehicle' ? 0.3 : 0.4);
    ...
    static maintainObjectSurfaceHeight(object) {
        ...
        try {
            ...
            if (object.userData.type === 'car') {
                heightOffset = object.userData.fixedHeightOffset || 2.5; // FIXED: Reduced from 3.0
            ...
            if (object.userData.falling) {
                ...
                // Skip all other processing to let physics handle the falling vehicle
                return;
            }
            
            // Don't prematurely ground freshly spawned vehicles that should be falling
            if (object.userData.freshlySpawned && object.userData.falling) {
                // If vehicle is falling and above the ground, don't interfere with gravity
                if (distance > groundLevel + 1.0) {
                    console.log(`Vehicle ${object.name || 'unnamed'} is falling naturally - not forcing to ground`);
                    
                    // Apply natural gravity instead of forcing to ground
                    this.applyGravity(object);
                    ...
                    return; // Skip the rest of the grounding code
                }
            }
            
            // CRITICAL FIX: Special handling for cars
            if (object.userData.type === 'car' && object === VehicleManager.currentVehicle) {
                // For player-controlled cars, use stricter height maintenance
                heightTolerance = 0.05;
                
                if (object.userData.isActivelyControlled && !object.userData.falling) {
                    // For actively driving cars, be extra precise
                    const correctionFactor = 0.2; // Faster correction 
                    ...
                    // Apply gentler correction
                    object.position.lerp(newPosition, correctionFactor);
                    
                    // Don't mess with the car's velocity if actively driving
                    return;
                }
            }
            
            // ADDED: Detect when object touches the ground and set falling=false
            if (object.userData.falling && distance <= groundLevel + 3.0) {
                // Object has landed
                ...
                // Zero out any angular velocity to prevent post-landing spin
                ...
                // For vehicles, apply specialized alignment
                if (object.userData.type === 'vehicle') {
                    this.handleVehicleLanding(object, surfaceNormal);
                }
                
                // Clear freshlySpawned flag when landing
                if (object.userData.freshlySpawned) {
                    object.userData.freshlySpawned = false;
                    console.log(`Vehicle ${object.name || 'unnamed'} has landed - no longer freshly spawned`);
                }
            }
            
            // Process hard freeze state
            if (object.userData.hardFreeze && !object.userData.falling) {
                this.processHardFreezeState(object, planetCenter, surfaceNormal, groundLevel);
