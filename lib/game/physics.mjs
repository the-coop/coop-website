...
export default class Physics {
    ...
    static maintainObjectSurfaceHeight(object) {
        ...
        // SKIP ENTIRELY if this object is controlled by CarController
        if (object.userData._controlledByCarController) {
            return;
        }
    ...
    static alignObjectToPlanetSurface(object, surfaceNormal) {
        ...
        if (object.userData.type === 'vehicle' || object.userData.type === 'car' || object.userData.type === 'airplane') {
            // CRITICAL FIX: Don't apply alignment to controlled vehicles at all from physics system
            // Let VehicleManager handle all vehicle alignment to prevent conflicts
            if (object === VehicleManager.currentVehicle && object.userData.isActivelyControlled) {
                // Skip physics-based alignment for actively controlled vehicles
                return;
            }
            
            // Use much smaller alignment factor for fully stabilized vehicles
            const alignmentFactor = object.userData.fullyStabilized ? 0.001 : 0.05; // Reduced from 0.2
            
            // CRITICAL FIX: Don't align if vehicle was just aligned by VehicleManager
            if (object.userData._lastVehicleManagerAlignTime && 
                Date.now() - object.userData._lastVehicleManagerAlignTime < 16) { // 16ms = 1 frame at 60fps
                return;
            }
