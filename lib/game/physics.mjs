...
export default class Physics {
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            ...
            // FIXED: Handle completely stationary vehicles correctly
            const isStationaryVehicle = !vehicle.userData.isOccupied && 
                (!vehicle.userData.speed || Math.abs(vehicle.userData.speed) < 0.1);
            
            // Special handling for stationary vehicles on surface to prevent wobbling
            ...
            // For all other vehicles apply normal physics
            ...
            if (vehicle.userData.type === 'airplane' && vehicle.userData.altitude > 0) {
                ...
                const movement = forward.clone().multiplyScalar(vehicle.userData.speed * 0.01); // Scale down for AI planes
            ...
            // GROUND VEHICLES (CARS, LANDED PLANES)
                ...
                // Apply gravity force
                const gravity = GRAVITY_CONSTANT * 1.0; // Reduced from 1.2 for stability
                vehicle.userData.velocity.addScaledVector(surfaceNormal, -gravity);
                
                // Handle surface contact
                if (onSurface) {
                    // Position on surface correctly
                    vehicle.position.copy(planetCenter).addScaledVector(surfaceNormal, collisionDistance);
                    
                    // Apply surface movement based on speed for AI vehicles
                    if (!vehicle.userData.isOccupied && Math.abs(vehicle.userData.speed) > 0.01) {
                        ...
                        const movement = projectedForward.multiplyScalar(vehicle.userData.speed * 0.01); // Scale down for AI
                        vehicle.position.add(movement);
                        
                        // Maintain correct height after movement
                    ...
                    // Handle landing event if just touched surface
                    if (!wasOnSurface) {
                    ...
                    // Align vehicle to surface
                    VehicleManager.alignVehicleToPlanetSurface(vehicle, surfaceNormal, 
                        isStationaryVehicle ? 0.9 : 0.3);
                    
                    vehicle.userData.onSurface = true;
                } else {
                    vehicle.userData.onSurface = false;
                    
                    // Update position based on velocity for falling vehicles
