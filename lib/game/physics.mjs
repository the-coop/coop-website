    ...
    static update() {
        try {
            PlayersManager.players.forEach(player => {
                ...
                // ENHANCED: Improve collision priority handling
                ...
                for (let step = 0; step < numSteps && !hadOBBCollision; step++) {
                    try {
                        ...
                        // ENHANCED: Check player handle collision with all objects including walls
                        // This is crucial - we use the handle for ALL collision detection
                        let collide = null;
                        if (player.handle && player.collidable) {
                        ...
                        // FIXED: Improve collision response with much stronger wall blocking
                        if (collide) {
                            hadOBBCollision = true;
                            const { collisionNormal, collisionPosition, closestObject } = collide;
                            
                            // IMPROVED: Much stronger position correction for wall collisions
                            // Use penetration value if provided or use stronger defaults for walls
                            const penetrationFactor = collide.isWallCollision ? 2.5 : 
                                                    (collide.penetration || 1.0);
                            
                            // Calculate safety offset - make this much stronger for walls
                            const safetyOffset = collisionNormal.clone().multiplyScalar(penetrationFactor);
                            
                            // For wall collisions, create a bigger safety margin
                            if (collide.isWallCollision) {
                                // CRITICAL FIX: For walls, completely revert position and add safety margin
                                player.position.copy(originalPosition);
                                player.position.add(safetyOffset);
                            } else {
                                // For non-walls, use the collision point with safety offset
                                const safePosition = collisionPosition.clone().add(safetyOffset);
                                player.position.copy(safePosition);
                            }
                            
                            // Update handle position to match player position
                            if (player.handle) {
                                player.handle.position.copy(player.position);
                            }
                            
                            // IMPROVED: More aggressive velocity reflection for solid bounces
                            const speedDamp = collide.isWallCollision ? 0.95 : 0.7; // Increased for walls (95% reduction)
                            const velDot = player.velocity.dot(collisionNormal);
                            
                            if (velDot < 0) {
                                // CRITICAL FIX: For walls, completely remove velocity in the wall's direction
                                if (collide.isWallCollision) {
                                    ...
                                    // Additional damping for wall collisions
                                    player.velocity.multiplyScalar(0.5); // Strong friction against walls
                                } else {
                                    // Standard bounce for non-wall objects
                                    player.velocity.addScaledVector(collisionNormal, -velDot * (1 + speedDamp));
                                    player.velocity.multiplyScalar(0.7); // Regular friction
                                }
                            }
                            
                            // IMPORTANT: Update collision bounds again after correction
                            if (player.handle && player.collidable) {
                                ObjectManager.updateCollidableBounds(player.handle);
                            }
                            
                            // ADDED: Show collision notification for wall
                            if (collide.isWallCollision && typeof window !== 'undefined' && window.gameNotify) {
                                window.gameNotify("Wall collision - movement blocked");
                            }
                            
                            // Log position correction for debugging
                            const delta = player.position.clone().sub(originalPosition);
                            console.log(`Collision position correction: ${delta.length().toFixed(2)} units`);
                            console.log(`New player position: ${player.position.x.toFixed(2)}, ${player.position.y.toFixed(2)}, ${player.position.z.toFixed(2)}`);
                ...
                // ENHANCED: Planet gravity and surface handling AFTER OBB collisions
                // Only snap to surface if no OBB collisions occurred
                try {
