        ...
        try {
            ...
            // Handle all the rest of the vehicle physics code for when it's on the surface
                ...
                if (playerControlled && onSurface) {
                    // IMPROVED: Better car movement along surface
                    if (Math.abs(vehicle.userData.speed) > 0.01) {
                        // Get vehicle's forward direction using negative Z
                        const forward = new Vector3(0, 0, -1).applyQuaternion(vehicle.quaternion);
                        
                        // CRITICAL FIX: Project forward onto tangent plane for correct surface movement
                        const projectedForward = forward.clone().projectOnPlane(surfaceNormal).normalize();
                        
                        // CRITICAL FIX: Much stronger movement coefficient for responsiveness
                        const movementFactor = 30; // Significantly increased
                        const movement = projectedForward.multiplyScalar(vehicle.userData.speed * (1/60) * movementFactor);
                        
                        // Apply movement directly to position
                        vehicle.position.add(movement);
                        
                        // Debug log for active movement
                        if (Math.abs(vehicle.userData.speed) > 1) {
                            console.log(`Car moving with speed=${vehicle.userData.speed.toFixed(2)}, movement distance=${movement.length().toFixed(2)}`);
                        }
                        
                        // Maintain height above planet surface
                        vehicle.position.copy(planetCenter).addScaledVector(
                            vehicle.position.clone().sub(planetCenter).normalize(),
                            collisionDistance
                        );
                        
                        // Apply banking effect for more realistic visuals
                        if (vehicle.userData.steeringInput && Math.abs(vehicle.userData.steeringInput) > 0.1 &&
                            Math.abs(vehicle.userData.speed) > 5) {
                            const bankAngle = vehicle.userData.steeringInput * Math.min(0.15, Math.abs(vehicle.userData.speed) / 80);
                            const rightVector = new Vector3().crossVectors(surfaceNormal, projectedForward).normalize();
                            const bankNormal = surfaceNormal.clone().addScaledVector(rightVector, -bankAngle).normalize();
                            vehicle.up.lerp(bankNormal, 0.15);
                        }
                    }
                    
                    // Adaptive alignment strength to maintain stability
                    const speed = Math.abs(vehicle.userData.speed);
                    const adaptiveStrength = speed > 10 ? 0.2 : (speed > 5 ? 0.3 : 0.5);
