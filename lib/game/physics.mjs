...
export default class Physics {
    ...
    static maintainObjectSurfaceHeight(object) {
        ...
        const managedByController = object.userData._controlledByCarController && 
                                  ...
                                  (Date.now() - object.userData._lastHeightManagement < 1000);
        ...
        try {
            ...
            // ADDED: Check specifically for vehicles that are stuck for a longer time
            // This prevents emergency corrections from firing too often and fighting with CarController
            const isStuckVehicle = 
                (object.userData.type === 'car' || object.userData.type === 'airplane') &&
                Math.abs(heightAboveSurface - heightOffset) > 15.0 && // Increased threshold from 5.0 to 15.0
                !object.userData.falling &&
                (!object.userData._lastHeightManagement || // Either not managed recently
                 Date.now() - object.userData._lastHeightManagement > 2000); // Or not managed for 2+ seconds
            
            if (isStuckVehicle) {
                console.log(`Vehicle detected as severely stuck: ${heightAboveSurface.toFixed(2)} units above surface (should be ${heightOffset.toFixed(2)})`);
                
                // Force a correction
                object.userData._needsSurfaceCorrection = true;
                ...
                object.position.lerp(correctedPosition, correctionFactor);
                object.updateMatrix();
                object.updateMatrixWorld(true);
                
                console.log(`Applied emergency height correction to vehicle`);
                
                // Clear the flag after a delay
                setTimeout(() => {
                    if (object && object.userData) {
                        object.userData._needsSurfaceCorrection = false;
                    }
                }, 2000);
            ...
            // FIXED: Define a reasonable height tolerance value for when to apply corrections
            const heightTolerance = 0.25; // Only correct if more than 0.25 units away from target height
    ...
    static alignObjectToPlanetSurface(object, surfaceNormal) {
        ...
        // IMPROVED: More precise check for vehicles managed by controllers
        // Only skip if the vehicle was recently aligned by a controller
        const managedByController = object.userData && (
            (object.userData._alignedByCarController && 
             object.userData._lastAlignmentTime && 
             Date.now() - object.userData._lastAlignmentTime < 500) ||
            object.userData.isActivelyControlled ||
            object.userData._controlledByAirplaneController
        );
        
        // CRITICAL FIX: Skip alignment for falling vehicles/objects to match player behavior
        if (object.userData && object.userData.falling) {
            return; // Skip alignment for falling vehicles/objects
        }
        
        if (managedByController) {
            return; // Skip alignment as it's handled elsewhere
        }
        
        // Continue with general alignment for other objects
        try {
            ...
            if (rotationAxis.lengthSq() > 0.001) {
                ...
                // IMPROVED: Explicitly set up vector to the surface normal
                // This is crucial for proper alignment and matches how the player is handled
