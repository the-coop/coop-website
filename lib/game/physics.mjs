...
export default class Physics {
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            // CRITICAL FIX: Initialize velocity properly if needed
            ...
            // FIX: Add strong stabilization for stationary vehicles to prevent spinning
            const isStationaryVehicle = !vehicle.userData.isOccupied && 
                               (!vehicle.userData.speed || Math.abs(vehicle.userData.speed) < 0.1);
            
            // FIX: Apply much stronger damping to prevent oscillation
            if (isStationaryVehicle && onSurface) {
                ...
                VehicleManager.alignVehicleToPlanetSurface(vehicle, surfaceNormal, 0.9);
            ...
            // GROUND VEHICLES (CARS, LANDED PLANES)
                ...
                // FIX: Apply stronger velocity caps to prevent high-speed oscillations
                ...
                if (onSurface) {
                    ...
                    // FIX: Apply stronger friction for parked vehicles
                    ...
                    // Cancel any movement into the planet surface
                    ...
                    // FIX: More stable surface alignment
                    VehicleManager.alignVehicleToPlanetSurface(vehicle, surfaceNormal, 
                        isStationaryVehicle ? 0.8 : 0.3);
                    
                    vehicle.userData.onSurface = true;
                } else {
                    // Vehicle is falling - in the air
                    vehicle.userData.onSurface = false;
                    
                    // Update liftoff state if needed
                    if (wasOnSurface) {
            ...
            // Update position with velocity (if not completely stationary)
            if (!isStationaryVehicle || !onSurface) {
