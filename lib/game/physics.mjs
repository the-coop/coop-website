            ...
            PlayersManager.players.forEach(player => {
                ...
                for (let step = 0; step < numSteps; step++) {
                    try {
                        ...
                        let collide = null;
                        try {
                            // FIXED: Use ALL collidable objects, not just those on player's planet
                            if (ObjectManager.collidableObjects && ObjectManager.collidableObjects.length > 0) {
                                collide = ObjectManager.checkCollisions(player, ObjectManager.collidableObjects, subStep);
                            }
                        } catch (err) {
