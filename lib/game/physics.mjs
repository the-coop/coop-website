                ...
                for (let step = 0; step < numSteps && !hadOBBCollision; step++) {
                    try {
                        ...
                        if (collide) {
                            ...
                            // This ensures the player CANNOT move through walls
                            player.position.copy(originalPosition);
                            
                            // Add a small safety offset in the collision normal direction
                            player.position.add(safetyOffset.multiplyScalar(0.2)); // Reduced multiplier for more natural feel
                            ...
                            // This is what stops movement in the collision direction
                            ...
                            // Show collision notification for wall
        ...
        try {
            ...
            if (collisions.length > 0) {
                ...
                for (const collision of collisions) {
                    const normal = collision.normal || new Vector3().subVectors(vehicle.position, collision.other.position).normalize();
