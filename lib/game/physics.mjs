    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            ...
            // Handle all the rest of the vehicle physics code for when it's on the surface
                ...
                if (playerControlled) {
                    // Check if vehicle has a non-zero speed that should be applied
                    console.log(`Vehicle speed check: ${vehicle.userData.speed?.toFixed(2) || 0}`);
                    
                    // CRITICAL FIX: Process movement regardless of surface state
                    if (Math.abs(vehicle.userData.speed) > 0.01) {
                        ...
                        if (vehicle.userData.type === 'car') {
                            // Car movement - needs projection if on surface
                            const projectedForward = forward.clone().projectOnPlane(surfaceNormal).normalize();
                            
                            // CRITICAL FIX: More direct movement calculation with higher multiplier
                            const movement = projectedForward.multiplyScalar(vehicle.userData.speed * (1/60) * 8);
                            
                            // CRITICAL FIX: Apply movement directly without any conditional
                            vehicle.position.add(movement);
                            console.log(`Car moving: speed=${vehicle.userData.speed.toFixed(2)}, distance=${movement.length().toFixed(2)}`);
                            
                            // Ensure car stays at correct height
                            vehicle.position.copy(planetCenter).addScaledVector(
                                vehicle.position.clone().sub(planetCenter).normalize(),
                                collisionDistance
                            );
                            ...
                            // CRITICAL FIX: Simplify airplane movement logic
                            // Direct movement based on current orientation
                            const movement = forward.clone().multiplyScalar(vehicle.userData.speed * (1/60) * 8);
                            
                            // CRITICAL FIX: Apply movement directly without conditionals
                            vehicle.position.add(movement);
                            console.log(`Airplane moving: speed=${vehicle.userData.speed.toFixed(2)}, distance=${movement.length().toFixed(2)}`);
                            
                            // If on ground, maintain correct height
