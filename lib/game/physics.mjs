...
export default class Physics {
    ...
    static applyCollisionResponse(object, collision) {
        ...
        if (object.type === 'player') {
            let finalNormal = collision.normal.clone();
            
            // If we have planet data, project collision normal to avoid pushing player off the surface
            if (object.soi && object.soi.object && object.soi.radius) {
                const planetNormal = object.position.clone()
                    .sub(object.soi.object.position)
                    .normalize();

                const dotUp = finalNormal.dot(planetNormal);
                // If collision pushes upward more than 0.3, project it sideways
                if (dotUp > 0.3) {
                    finalNormal = finalNormal.projectOnPlane(planetNormal).normalize();
                }
            }

            const pushbackDistance = Math.max(collision.penetrationDepth || 0.2, 0.2) * 1.2;
            object.position.addScaledVector(finalNormal, pushbackDistance);

            // Remove velocity component toward collision
            if (object.velocity) {
                const velDot = object.velocity.dot(finalNormal);
                if (velDot < 0) {
                    object.velocity.addScaledVector(finalNormal, -velDot * 1.1);
            ...
            // For non-player objects, also apply any rotation logic if desired
        }
    }
};
