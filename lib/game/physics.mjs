...
export default class Physics {
            ...
            PlayersManager.players.forEach(player => {
                ...
                for (let step = 0; step < numSteps && !hadOBBCollision; step++) {
                    try {
                        ...
                        // Handle OBB collision response with stronger correction
                        if (collide) {
                            hadOBBCollision = true;
                            const { collisionNormal, collisionPosition } = collide;
                            
                            // IMPROVED: Even stronger safety buffer for wall collisions
                            // Use penetration value if provided or use stronger defaults for walls
                            const penetrationFactor = collide.isWallCollision ? 1.5 : 
                                                    (collide.penetration || 0.5);
                            ...
                            // CRITICAL FIX: More direct position correction to prevent getting stuck in walls
                            const originalPosition = player.position.clone();
                            ...
                            const speedDamp = collide.isWallCollision ? 0.9 : 0.7; // Increased even more for walls
                            const velDot = player.velocity.dot(collisionNormal);
                            if (velDot < 0) {
                                player.velocity.addScaledVector(collisionNormal, -velDot * (1 + speedDamp));
                            }
                            
                            // Apply more friction to slow sliding along surfaces
                            player.velocity.multiplyScalar(0.7); // Increased friction from 0.8 to 0.7
                            ...
                            // ADDED: Log position correction for wall collisions
                            if (collide.isWallCollision) {
                                const delta = player.position.clone().sub(originalPosition);
                                console.log(`Wall collision position correction: ${delta.length().toFixed(2)} units`);
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        try {
            ...
            // FIXED: Replace checkAllCollisions with checkCollisionsWithObject
            // because checkAllCollisions doesn't exist
