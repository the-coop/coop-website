    ...
    static update() {
        PlayersManager.players.map(player => {
            ...
            for (let step = 0; step < numSteps; step++) {
                ...
                // CRITICAL FIX: Ensure SOI is valid and has objects property
                ...
                // FIXED: Check collisions against ALL collidable objects, not just planet.soi.objects
                // This ensures we check against walls AND vehicles
                const collide = ObjectManager.checkCollisions(player, ObjectManager.collidableObjects, subStep);
                if (collide) {
            ...
            // CRITICAL FIX: Ensure planet data exists before accessing
            if (!player.soi || !player.soi.radius || !player.soi.object) {
                console.error("Invalid planet data in player SOI");
                return; // Skip further physics processing for this player
            }
        ...
        for (const collidable of ObjectManager.collidableObjects) {
            ...
            // ENHANCED: Always update OBB bounds for dynamic objects
