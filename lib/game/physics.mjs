...
export default class Physics {
    ...
    static calculateSOI(position) {
        ...
        SceneManager.planets.map(planet => {
            ...
            // Uses Three.js distanceTo() which computes sqrt((x2-x1)^2 + (y2-y1)^2 + (z2-z1)^2)
    ...
    static update() {
        PlayersManager.players.map(player => {
            ...
            if (distance <= collisionDistance) {
                ...
                // Notify controller when transitioning from falling to landed state
                // This allows for landing effects, sound, camera adjustments etc
                if (player.falling) 
                    ControlManager.controller?.landing();
            ...
            if (distance > collisionDistance) {
                ...
                const gravity = this.GRAVITY_CONSTANT / Math.pow(distance / planetRadius, 2);
