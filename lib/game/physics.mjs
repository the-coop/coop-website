                ...
                for (let step = 0; step < numSteps && !hadOBBCollision; step++) {
                    try {
                        ...
                        if (collide) {
                            ...
                            // CRITICAL FIX: Project collision normal onto planet surface
                            // This is the core fix - always move laterally along the planet surface
                            let correctionNormal = collisionNormal.clone();
                            
                            // CRITICAL FIX: Always use surfaceAligned movement for ALL test cube collisions
                            ...
                            // For other collisions and non-grounded states, use standard collision response
                            ...
                            // CRITICAL FIX: For test cube collisions, ALWAYS maintain planet height
                            ...
                            // NEW: Modify velocity to move along surface for future frames
                            if (isTestCubeCollision && player.velocity && player._planetSurfaceNormal) {
                                // Project velocity onto planet surface plane
                                player.velocity = player.velocity.projectOnPlane(player._planetSurfaceNormal).normalize()
                                    .multiplyScalar(player.velocity.length() * 0.9); // Slight slowdown
                            }
                        }
                    } catch (stepErr) {
