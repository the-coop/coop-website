    ...
    static update() {
        try {
            PlayersManager.players.forEach(player => {
                ...
                // ADDED: First process OBB collisions without planet surface effects
                // This ensures wall collisions take priority over planet surface collisions
                ...
                for (let step = 0; step < numSteps; step++) {
                    try {
                        ...
                        // Handle OBB collision response
                        if (collide) {
                            hadOBBCollision = true;
                            const { collisionNormal, collisionPosition } = collide;
                            
                            // Add safety buffer to keep player away from colliding object
                            const safetyOffset = collisionNormal.clone().multiplyScalar(0.1);
                ...
                // This ensures OBB collisions take precedence, and planet surface is a fallback
                try {
