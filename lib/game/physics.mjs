...
export default class Physics {
    ...
    static applyVehiclePhysics(vehicle) {
        ...
        if (vehicle.userData.type === 'airplane' && vehicle.userData.altitude > 0) {
            ...
            if (onSurface) {
                ...
                // Apply stronger surface friction for parked vehicles
                const frictionFactor = vehicle.userData.speed > 0.1 ? 
                    (planet.CoF || 0.15) : 
                    (planet.CoF * 4 || 0.6); // Much stronger friction when nearly stopped
                ...
                // Completely stop the vehicle if it's moving very slowly
                if (Math.abs(vehicle.userData.speed) < 0.1) {
                    vehicle.userData.speed = 0;
                    // Apply strong damping to reduce wobbling
    ...
    static vehicleLanding(vehicle, surfaceNormal) {
        ...
        if (vehicle.userData.type === 'car') {
            ...
            // Reduce bounce by strongly damping vertical velocity
            ...
            // Set altitude to zero when landing
