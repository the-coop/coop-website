...
export default class ControlManager {
    ...
    static update() {
        ...
        if (PlayersManager.self && PlayersManager.self.handle) {
            // CRITICAL FIX: Improved key handling for vehicle exit
    ...
    static exitVehicleAndChangeController() {
        if (VehicleManager.currentVehicle) {
            ...
            if (prevController && prevController.cleanup) {
                try {
                    console.log("Cleaning up previous controller");
            ...
            // CRITICAL FIX: Carefully preserve the camera position during transition
            if (Engine.camera) {
                const worldPos = new Vector3();
                ...
                // CRITICAL FIX: Complete reset of camera rotation to prevent carry-over issues
                Engine.camera.rotation.set(0, 0, 0);
                Engine.camera.quaternion.identity();
                ...
                console.log('Camera completely reset');
            }
