...
export default class ControlManager {
    ...
    static update() {
        ...
        if (PlayersManager.self && PlayersManager.self.handle) {
            ...
            // Update current controller (skip for controllers with fixed camera)
            if (VehicleManager.currentVehicle && 
                VehicleManager.currentVehicle.userData && 
                VehicleManager.currentVehicle.userData._cameraFixed) {
                // Skip conflicting controller updates to prevent camera distortion
                // Let CarController handle camera exclusively
            }
