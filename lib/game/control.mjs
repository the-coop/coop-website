...
export default class ControlManager {
    ...
    static update() {
        ...
        if (PlayersManager.self && PlayersManager.self.handle) {
            ...
            if (this.controller && this.controller.update) {
                ...
                // Check for controller switch requests
                else if (result && result.startsWith('switch:')) {
                    const controllerName = result.split(':')[1];
                    console.log(`Controller requested switch to ${controllerName}`);
                    
                    // Handle switching between different controller types
                    switch (controllerName) {
                        case 'fps':
                            this.change(FPSController);
                            break;
                        case 'thirdperson':
                            this.change(ThirdPersonController);
                            break;
                        case 'car':
                            this.change(CarController);
                            break;
                        case 'firstpersoncar':
                            this.change(FirstPersonCarController);
                            break;
                        case 'airplane':
                            this.change(AirplaneController);
                            break;
                        case 'firstpersonairplane':
                            this.change(FirstPersonAirplaneController);
                            break;
                    }
                }
            }
    ...
    static exitVehicleAndChangeController() {
        if (VehicleManager.currentVehicle) {
            ...
            if (VehicleManager.exitVehicle()) {
                ...
                // First clean up the current controller
                const prevController = this.controller;
                this.controller = null;
    ...
    static getCurrentViewMode() {
        if (this.controller === FPSController) {
            ...
            return 'Third Person Car';
        } else if (this.controller === FirstPersonCarController) {
            return 'First Person Car';
        } else if (this.controller === AirplaneController) {
            return 'Third Person Airplane';
        } else if (this.controller === FirstPersonAirplaneController) {
            return 'First Person Airplane';
        } else {
            return 'Unknown';
        }
    }
    
    // Get the controller to switch to based on the current one
    static getViewToggleController() {
        // Check for direct controller instance comparison
        ...
        if (this.controller === CarController) return FirstPersonCarController;
        if (this.controller === FirstPersonCarController) return CarController;
        if (this.controller === AirplaneController) return FirstPersonAirplaneController;
        if (this.controller === FirstPersonAirplaneController) return AirplaneController;
