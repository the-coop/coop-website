        ...
        if (result && VehicleManager.currentVehicle) {
            ...
            if (vehicleType === 'airplane') {
                ...
                if (VehicleManager.currentVehicle) {
                    ...
                    // ADDED: Important flag for camera control
                    car.userData._lockCamera = true;
                    
                    // Force camera to update next frame
                    car.userData._needsCameraSetup = true;
                }
                
                // First set the controller reference
                this.controller = CarController;
                
                // Then call reset to initialize it
                console.log("Calling CarController.reset() to initialize vehicle control");
                CarController.reset();
                
                // Notify the player about the vehicle entry
                if (typeof window !== 'undefined' && window.gameNotify) {
                    window.gameNotify("Entered vehicle - use WASD to drive");
                }
            ...
            // Notify the player that no vehicle was found
            if (typeof window !== 'undefined' && window.gameNotify) {
                window.gameNotify("No vehicle found nearby - get closer to a vehicle");
            }
        }
        return false;
    }
