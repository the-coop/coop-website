...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        // FIXED: Increase minimum height offset further (4 instead of 3)
        const adjustedHeightOffset = Math.max(4, heightOffset);
        ...
        // Add wheel rotation helper method with gradual steering
        car.userData.rotateWheels = function(angle, targetSteeringAngle = 0) {
            // Initialize steering properties if needed
            if (this.currentSteeringAngle === undefined) {
                this.currentSteeringAngle = 0;
            }
            
            // Calculate how much to change the steering this frame (gradual turn)
            const steeringDelta = Math.min(0.03, Math.abs(targetSteeringAngle - this.currentSteeringAngle));
            
            // Update current steering angle gradually toward target
            if (this.currentSteeringAngle < targetSteeringAngle) {
                this.currentSteeringAngle = Math.min(this.currentSteeringAngle + steeringDelta, targetSteeringAngle);
            } else if (this.currentSteeringAngle > targetSteeringAngle) {
                this.currentSteeringAngle = Math.max(this.currentSteeringAngle - steeringDelta, targetSteeringAngle);
            }
            
            // FIXED: Apply the steering angle to the CORRECT AXES for each wheel
            // Since wheels are rotated with z=Math.PI/2, steering should be on x-axis
            if (this.wheels.frontLeft) {
                // First reset all rotations to default
                this.wheels.frontLeft.rotation.set(0, 0, Math.PI/2);
                // Then apply steering around the correct axis (x-axis for our wheel setup)
                this.wheels.frontLeft.rotateX(-this.currentSteeringAngle);
            }
            
            if (this.wheels.frontRight) {
                // First reset all rotations to default
                this.wheels.frontRight.rotation.set(0, 0, Math.PI/2);
                // Then apply steering around the correct axis (x-axis for our wheel setup)
                this.wheels.frontRight.rotateX(-this.currentSteeringAngle);
            }
            
            // Keep back wheels at default rotation (no steering)
        ...
        car.userData.currentSteeringAngle = 0;
        car.userData.targetSteeringAngle = 0;
        car.userData.lastTurnDirection = 0;
        car.userData.turnInertia = 0;
    ...
    static createVehicleBase(type, planet, latitude, longitude, heightOffset = 2) {
        ...
        try {
            ...
            try {
                ...
                try {
                    // FIXED: Increase minimum height for cars to 2.0 (up from 1.5)
                    const actualHeightOffset = type === 'car' ? Math.max(2.0, heightOffset) : Math.max(3, heightOffset);
