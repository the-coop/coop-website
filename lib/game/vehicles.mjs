...
export default class VehicleManager {
    ...
    static alignVehicleToPlanetSurface(vehicle, surfaceNormal, lerpFactor = 0.1, forceFullAlignment = false) {
        ...
        try {
            ...
            if (isActiveCar) {
                ...
                // IMPROVED: Use much smaller lerp factor for active cars to prevent sudden rotations
    ...
    static resetWheelsBaseOrientation(vehicle) {
        ...
        try {
            ...
            // Align front wheels with steering input if available
            const steeringInput = vehicle.userData.steeringInput || 0;
            const maxSteerAngle = 0.4; // About 23 degrees
            ...
            Object.entries(wheels).forEach(([wheelName, wheel]) => {
                ...
                if (wheelName.includes('front')) {
                    wheel.rotation.set(
                        ...
                        steeringInput * maxSteerAngle, // Apply steering to y-rotation
                        Math.PI/2            // Correct perpendicular orientation
                    );
                } else {
                    // For rear wheels just keep roll and perpendicular orientation
                    wheel.rotation.set(
                        currentRoll,  // Keep current roll rotation
                ...
                // ADDED: If planet normal is available, use it to better align wheels with surface
                if (surfaceNormal) {
                    // Ensure wheel cylindrical axis is perpendicular to forward direction and aligned to surface
                    const worldQuaternion = new Quaternion();
                    if (wheel.parent) {
                        wheel.parent.getWorldQuaternion(worldQuaternion);
                        
                        // Get car's forward and right directions
                        const carForward = new Vector3(0, 0, -1).applyQuaternion(worldQuaternion);
                        const carRight = new Vector3(1, 0, 0).applyQuaternion(worldQuaternion);
                        
                        // Project car right onto planet surface to get wheel axle direction
                        const wheelAxle = carRight.projectOnPlane(surfaceNormal).normalize();
                        
                        // Additional fine-tuning for wheel orientation if needed
                        if (wheelName.includes('front') && Math.abs(steeringInput) > 0.01) {
                            // For front wheels with steering, calculate a slight adjustment
                            const steeringDirection = new Vector3()
                                .addScaledVector(carForward, Math.cos(steeringInput * maxSteerAngle))
                                .addScaledVector(carRight, Math.sin(steeringInput * maxSteerAngle))
                                .normalize();
                                
                            // Project steering direction onto surface plane
                            const surfaceSteeringDir = steeringDirection.projectOnPlane(surfaceNormal).normalize();
                            
                            // Calculate wheel's local rotation to match steering
                            // This is a more advanced adjustment - only enable if needed
                            // wheel.quaternion adjustment would go here if needed
                        }
                    }
                }
            });
    ...
    static tryEnterNearbyVehicle() {
        ...
        if (closestVehicle) {
            ...
            // ADDED: Make sure the vehicle is aligned to surface and grounded
            if (this.currentVehicle.userData.planet && this.currentVehicle.userData.planet.object) {
                ...
                // Apply strong initial alignment
