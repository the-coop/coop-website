...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        ...
        body.position.y = 1.0; // Position body correctly relative to wheels
        car.add(body);
        ...
        // Position wheels at the bottom (y = -0.5) so they touch the ground
        // Front left wheel
        ...
        const physicsHandle = new Mesh(
            new BoxGeometry(6, 3, 10), // Adjust physics box to account for wheels at bottom
            new MeshBasicMaterial({ color: 0xFF0000, wireframe: true, visible: false })
        );
        physicsHandle.position.set(0, 0.5, 0); // Center the physics handle to match the new wheel positions
        ...
        if (this.currentVehicle && this.currentVehicle.userData.isOccupied) {
            ...
            if (vehicle.userData.type === 'car') {
                ...
                if (planet && planet.object) {
                    ...
                    const heightOffset = 0.75; // Reduced car height from 1.5 to 0.75
    ...
    static snapVehicleToGround(vehicle) {
        ...
        try {
            ...
            const heightOffset = vehicle.userData.type === 'car' ? 0.75 : 2; // Reduced car height from 1.5 to 0.75
