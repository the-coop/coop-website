...
export default class VehicleManager {
    ...
    static updateVehicles(deltaTime = 1/60) {
        ...
        // Debug the input state at the beginning of the update
        if (this.input && (Math.abs(this.input.movement.x) > 0.01 || Math.abs(this.input.movement.z) > 0.01)) {
            console.log(`VehicleManager.updateVehicles - Input state: x=${this.input.movement.x.toFixed(2)}, z=${this.input.movement.z.toFixed(2)}`);
        }
        ...
        for (const vehicle of this.vehicles) {
            ...
            if (vehicle.userData.isOccupied && vehicle === this.currentVehicle) {
                // CRITICAL FIX: Double check we have valid input before processing
                if (!this.input) {
                    console.warn("VehicleManager.input is null during updateVehicles");
                    this.input = {
    ...
    static handleVehicleInput(vehicle, deltaTime) {
        // CRITICAL FIX: Make sure we have a valid input object and LOG THE ISSUE if not
        if (!this.input) {
            console.error("VehicleManager.input is null in handleVehicleInput");
            this.input = {
                movement: new Vector3(),
                rotation: new Vector3(),
                action: false,
        ...
        // CRITICAL FIX: Clear debug noise - only log significant input
        const hasSignificantInput = this.input && this.input.movement && 
                                   (Math.abs(this.input.movement.x) > 0.01 || 
                                    Math.abs(this.input.movement.z) > 0.01);
        
        if (hasSignificantInput) {
            console.log(`Vehicle input handling: movement=${JSON.stringify({
                x: this.input.movement.x.toFixed(2),
                y: this.input.movement.y.toFixed(2),
                z: this.input.movement.z.toFixed(2)
            })}`);
        }
        
        if (vehicle.userData.type === 'car') {
            // CRITICAL FIX: Add direct debug of raw input values
            console.log(`Raw car input values: x=${this.input.movement.x}, z=${this.input.movement.z}`);
            
            // FIXED: More direct input handling with clear acceleration and steering
            let accelerationInput = 0;
            let steeringInput = 0;
            
            if (this.input && this.input.movement) {
                // Get forward/backward input (negative Z is forward)
                accelerationInput = -this.input.movement.z; 
                
                // Get steering input with some minimal filtering
                steeringInput = this.input.movement.x;
            }
            
            // Store steering input for visual effects
            vehicle.userData.steeringInput = steeringInput;
            
            // Log all car inputs during vehicle movement
            console.log(`Car inputs processed: accel=${accelerationInput.toFixed(2)}, steer=${steeringInput.toFixed(2)}`);
            
            // CRITICAL FIX: Apply acceleration with MUCH stronger values for immediate feedback
            if (Math.abs(accelerationInput) > 0.01) {
                // Dramatically increase acceleration for testing
                const accelerationStrength = 80; // Doubled from 40 to 80 for more obvious movement
                const acceleration = vehicle.userData.acceleration * accelerationInput * deltaTime * accelerationStrength;
                vehicle.userData.speed += acceleration;
                console.log(`Car speed adjusted: ${vehicle.userData.speed.toFixed(2)}`);
            } else {
                // Apply constant drag when no input
                vehicle.userData.speed *= 0.98; // Less aggressive slow down
                
                // Zero out very small values to prevent drift
                if (Math.abs(vehicle.userData.speed) < 0.05) {
            ...
            // Only apply steering when car is actually moving
            if (Math.abs(steeringInput) > 0.01 && Math.abs(vehicle.userData.speed) > 0.5) {
                const steeringStrength = 15; // Increased from 10 for more responsive steering
                const speed = Math.abs(vehicle.userData.speed);
                
                // Scale steering by speed - more responsive at medium speeds
                // This creates more natural steering feel
                const speedFactor = Math.min(1.0, speed / 10); // Max effect at speed 10
                
                // Reverse steering direction when going backwards
                const steeringDirection = Math.sign(vehicle.userData.speed);
                
                // Calculate final steering based on all factors
                const steeringValue = -vehicle.userData.handling * steeringInput * 
                                     steeringDirection * steeringStrength * speedFactor;
                
                // Apply steering to vehicle
                vehicle.rotateY(steeringValue);
                console.log(`Applied steering: ${steeringValue.toFixed(4)}`);
            }
            
            // CRITICAL FIX: Update velocity vector in world space for physics system
            if (Math.abs(vehicle.userData.speed) > 0.1) {
                const forwardDir = new Vector3(0, 0, -1).applyQuaternion(vehicle.quaternion);
                vehicle.userData.velocity = forwardDir.multiplyScalar(vehicle.userData.speed);
                console.log(`Updated velocity: ${vehicle.userData.velocity.x.toFixed(2)}, ${vehicle.userData.velocity.y.toFixed(2)}, ${vehicle.userData.velocity.z.toFixed(2)}`);
            } else {
