...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        ...
        car.userData.acceleration = 0.25; // FURTHER REDUCED from 0.3 to 0.25 
        car.userData.handling = 0.03;
        car.userData.friction = 0.99; 
        car.userData.isOccupied = false;
        car.userData.maxSpeedReverse = 15;
        
        // EXTREMELY REDUCED acceleration properties for near-imperceptible initial movement
        car.userData.accelerationCurve = {
            startingPower: 0.005,  // DRASTICALLY REDUCED from 0.02 to 0.005 - almost no initial acceleration
            maxPower: 0.25,        // REDUCED from 0.35 to 0.25 - much lower peak power
            powerBuildSpeed: 0.015 // REDUCED from 0.04 to 0.015 - extremely slow power build-up
        };
        car.userData.currentAccelPower = car.userData.accelerationCurve.startingPower;
        car.userData.inertiaFactor = 0;
        car.userData.lastSpeed = 0; // Track previous speed for debugging
            ...
            if (this.input) {
                ...
                if (vehicle.userData.type === 'car') {
                    ...
                    if (Math.abs(vehicle.userData.input.movement.z) > inputThreshold) {
                        ...
                        // FIXED: Make acceleration power build up MUCH more gradually
                        ...
                        vehicle.userData.currentAccelPower = Math.min(
                            ...
                            vehicle.userData.currentAccelPower + 
                                (accelCurve.powerBuildSpeed * Math.abs(inputPower) * safeDeltaTime * 0.3) // REDUCED from 0.5 to 0.3
                        );
                        
                        // Calculate acceleration with power curve
                        const acceleration = vehicle.userData.acceleration || 0.25; // Use lower base acceleration
                        
                        // IMPROVED: Much more progressive speed factor for extremely gradual start
                        let speedFactor = 1.0;
                        if (Math.abs(vehicle.userData.speed) < 10.0) {  // INCREASED range from 8.0 to 10.0
                            // More progressive curve for low speeds - even slower acceleration at low speeds
                            speedFactor = 0.05 + (Math.abs(vehicle.userData.speed) / 10.0) * 0.95; // REDUCED initial factor from 0.1 to 0.05
                        }
                        
                        // Apply final effective acceleration
                        const effectiveAccel = acceleration * vehicle.userData.currentAccelPower * speedFactor;
                        
                        // FURTHER REDUCED acceleration application
                        vehicle.userData.speed += inputPower * effectiveAccel * 0.15; // REDUCED from 0.2 to 0.15
