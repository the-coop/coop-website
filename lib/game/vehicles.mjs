...
export default class VehicleManager {
    ...
    static handleVehicleInput(vehicle, deltaTime) {
        ...
        if (hasInput) {
            console.log(`Vehicle receiving input: x=${this.input.movement.x.toFixed(2)}, z=${this.input.movement.z.toFixed(2)}`);
        }
        ...
        if (vehicle.userData.type === 'car') {
            ...
            if (this.input && this.input.movement) {
                ...
                if (Math.abs(accelerationInput) > 0.01 || Math.abs(steeringInput) > 0.01) {
                    console.log(`Car input values: acceleration=${accelerationInput.toFixed(2)}, steering=${steeringInput.toFixed(2)}`);
                }
            }
            
            // Store steering input for physics effects
            vehicle.userData.steeringInput = steeringInput;
            
            // Apply acceleration with higher responsiveness
            if (Math.abs(accelerationInput) > 0.01) {
                // CRITICAL FIX: Much higher acceleration factor for responsive controls
                const accelerationFactor = 25;
                vehicle.userData.speed += vehicle.userData.acceleration * accelerationInput * deltaTime * accelerationFactor;
                console.log(`Car speed updated: ${vehicle.userData.speed.toFixed(2)}`);
            } else {
                // Apply drag when no input
                vehicle.userData.speed *= 0.95;
            }
            
            // Apply steering with higher responsiveness
            if (Math.abs(steeringInput) > 0.01 && Math.abs(vehicle.userData.speed) > 0.01) {
                // More responsive steering with higher multiplier
                const steeringFactor = 12; // Increased significantly
                const steeringAmount = -vehicle.userData.handling * steeringInput * Math.sign(vehicle.userData.speed) * steeringFactor;
                vehicle.rotateY(steeringAmount);
                console.log(`Applied steering: ${steeringAmount.toFixed(4)}`);
            }
            ...
            // CRITICAL FIX: Calculate velocity in world space from car's local forward direction
            if (Math.abs(vehicle.userData.speed) > 0.1) {
                const forward = new Vector3(0, 0, -1).applyQuaternion(vehicle.quaternion);
                vehicle.userData.velocity = forward.multiplyScalar(vehicle.userData.speed);
            } else {
