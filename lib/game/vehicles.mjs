...
export default class VehicleManager {
    ...
    static alignVehicleToPlanetSurface(vehicle, surfaceNormal, slerpFactor = 0.2) {
        ...
        try {
            ...
            // CRITICAL CAR WOBBLE FIX: Improve car handling
            if (vehicle.userData.type === 'car') {
                // Create a perfect basis with the surface normal as up vector
                const forward = new Vector3(0, 0, 1);
                
                // Store current forward direction for restoration
                ...
                // CRITICAL FIX: More robust handling for edge cases
                // If we're at a pole or problematic area, use original orientation as a guide
                if (vehicle.userData.initialQuaternion && vehicle.userData.falling) {
                    const originalForward = new Vector3(0, 0, -1)
                        .applyQuaternion(vehicle.userData.initialQuaternion)
                        .projectOnPlane(up)
                        .normalize();
                        
                    if (originalForward.lengthSq() > 0.01) {
                        // Use original orientation as a reference when first landing
                        vehicle.userData._tempForward = originalForward.clone();
                    }
                }
                
                // Use stored forward direction if available
                const projectedCurrentForward = currentForward.clone().projectOnPlane(up).normalize();
                if (vehicle.userData._tempForward && vehicle.userData.falling) {
                    projectedCurrentForward.copy(vehicle.userData._tempForward);
                }
