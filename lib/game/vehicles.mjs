...
export default class VehicleManager {
    ...
    static createVehicleWithPosition(type, planet, position, suppressAlignment = false) {
        try {
            ...
            console.log(`Creating ${type} at absolute position ${position.toArray()}`);
            ...
            vehicle.userData = {
                ...
                isOccupied: false,
                planet: planet,
                soi: planet, // ADDED: Set SOI to match planet for consistency with player physics
            ...
            // Calculate surface normal for orientation only if we're not suppressing alignment
            if (!suppressAlignment) {
                ...
                // When alignment is suppressed, still store a reference to the surface normal
                // but don't apply any rotation - the vehicle will keep its default orientation
                const surfaceNormal = vehicle.position.clone()
                    .sub(planet.object.position)
                    .normalize();
                vehicle.userData.surfaceNormal = surfaceNormal;
