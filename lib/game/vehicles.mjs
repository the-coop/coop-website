...
export default class VehicleManager {
    ...
    static exitVehicle() {
        ...
        try {
            // Store vehicle reference and position before clearing
            const exitedVehicle = this.currentVehicle;
            const vehiclePos = new Vector3();
            exitedVehicle.getWorldPosition(vehiclePos);
            
            // Calculate exit position with large offset to prevent re-entry
            ...
            // SIMPLIFIED: Force camera detachment with no transition
            ...
            // Update player state
            if (PlayersManager.self) {
                // First restore the original handle, then position ONLY the handle
                if (PlayersManager.self._originalHandle) {
                    ...
                    // Clear vehicle relationship in player handle
                    if (PlayersManager.self.handle && PlayersManager.self.handle.userData) {
