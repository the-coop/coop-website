...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        ...
        // FIXED: Position wheels lower so they visually touch the ground
        const wheelFL = new Mesh(wheelGeometry, wheelMaterial);
        wheelFL.position.set(-3, -0.8, 3); // Lowered from -0.5 to -0.8 to touch ground
        ...
        wheelFR.position.set(3, -0.8, 3); // Lowered from -0.5 to -0.8
        ...
        wheelRL.position.set(-3, -0.8, -3); // Lowered from -0.5 to -0.8
        ...
        wheelRR.position.set(3, -0.8, -3); // Lowered from -0.5 to -0.8
    ...
    // Add a dedicated method for handling camera transitions
    static setupVehicleCamera(vehicle) {
        if (!Engine.camera || !vehicle) return;
        
        console.log("Setting up vehicle camera immediately");
        
        try {
            // Get camera's current world position (for debugging only)
            const startPos = new Vector3();
            if (Engine.camera.parent) {
                Engine.camera.getWorldPosition(startPos);
            } else {
                startPos.copy(Engine.camera.position);
            }
            
            // First completely detach camera from its current parent
            if (Engine.camera.parent) {
                Engine.camera.parent.remove(Engine.camera);
            }
            
            // Don't add to scene as intermediate step - go directly to vehicle
            // This eliminates the "in-between" state that causes glitches
            vehicle.add(Engine.camera);
            
            // Position and orient camera within vehicle
            ...
            // Force a render to update camera immediately
            Engine.renderer.render(Engine.scene, Engine.camera);
            
            console.log(`Camera moved from ${startPos.toFixed(1)} to vehicle position`);
        } catch (e) {
            console.error("Error setting up vehicle camera:", e);
        }
    }
    ...
    static updateCurrentVehicle(deltaTime = 1/60) {
        ...
        if (this.currentVehicle && this.currentVehicle.userData.isOccupied) {
            ...
            // FIXED: Move camera initialization directly to enterVehicle
            // Removed camera setup code from here to prevent double-handling
            // The camera is now set up immediately when entering the vehicle
            
            // IMPROVED: Enhanced height stabilization for occupied vehicles
            if (vehicle.userData.type === 'car' && vehicle.userData.planet) {
                ...
                // IMPROVED: Ensure consistent height with more aggressive correction
    ...
    static enterVehicle(vehicle) {
        ...
        if (vehicle.userData.type === 'car') {
            // FIXED: Set up camera IMMEDIATELY during vehicle entry
            // instead of waiting for update loop
