...
export default class VehicleManager {
    ...
    static handleVehicleInput(vehicle, deltaTime) {
        ...
        if (!this.input && !Engine.keyStates) {
            console.log("No input available for vehicle");
        ...
        // CRITICAL FIX: Detailed debug logging of input processing
        console.log(`Vehicle input debug:`, {
            hasValidInput: !!this.input,
            hasKeyStates: !!Engine.keyStates,
            keys: Engine.keyStates ? Object.keys(Engine.keyStates).filter(k => Engine.keyStates[k]).join(',') : 'none',
            controllerInput: this.input ? `z:${this.input.movement.z.toFixed(2)}` : 'none',
            currentSpeed: vehicle.userData.speed.toFixed(2)
        });
        
        if (vehicle.userData.type === 'car') {
            // CRITICAL FIX: Always prioritize keyboard input for more responsive controls
            let accelerationInput = 0;
            let steeringInput = 0;
            
            // Get direct keyboard input first - most reliable method
            if (Engine.keyStates) {
                if (Engine.keyStates['KeyW'] || Engine.keyStates['ArrowUp']) accelerationInput = 1;
                if (Engine.keyStates['KeyS'] || Engine.keyStates['ArrowDown']) accelerationInput = -1;
                if (Engine.keyStates['KeyA'] || Engine.keyStates['ArrowLeft']) steeringInput = -1;
                if (Engine.keyStates['KeyD'] || Engine.keyStates['ArrowRight']) steeringInput = 1;
                
                // Log when keys are pressed for easier debugging
                if (accelerationInput !== 0 || steeringInput !== 0) {
                    console.log(`Keyboard input: accel=${accelerationInput}, steering=${steeringInput}`);
                }
            }
            
            // Fall back to controller input if available and no keyboard input
            if (Math.abs(accelerationInput) < 0.1 && this.input && this.input.movement) {
                accelerationInput = -this.input.movement.z;
            }
            
            if (Math.abs(steeringInput) < 0.1 && this.input && this.input.movement && Math.abs(this.input.movement.x) > 0.01) {
                steeringInput = this.input.movement.x;
            }
            
            // Apply acceleration with increased responsiveness
            if (Math.abs(accelerationInput) > 0.01) {
                // CRITICAL FIX: Much higher acceleration multiplier for responsive driving
                vehicle.userData.speed += vehicle.userData.acceleration * accelerationInput * deltaTime * 8;
                console.log(`Applied acceleration: ${accelerationInput.toFixed(2)}, new speed: ${vehicle.userData.speed.toFixed(2)}`);
            } else {
                // Apply stronger drag when no input for better stopping
                vehicle.userData.speed *= 0.95;
            }
            
            // Apply steering with improved responsiveness
            if (Math.abs(steeringInput) > 0.01 && Math.abs(vehicle.userData.speed) > 0.01) {
                // CRITICAL FIX: More responsive steering with higher multiplier
                const steeringAmount = -vehicle.userData.handling * steeringInput * Math.sign(vehicle.userData.speed) * 3;
                vehicle.rotateY(steeringAmount);
                console.log(`Applied steering: ${steeringAmount.toFixed(3)}`);
            }
            
            // CRITICAL FIX: Apply speed limits
            ...
            // CRITICAL FIX: Similar improvements for airplane controls
            // Throttle control
            let throttleInput = 0;
            
            // Get input from controller if available
            if (this.input && this.input.movement) {
                throttleInput = -this.input.movement.z;
            }
            
            // Fallback to keyboard
            if (Math.abs(throttleInput) < 0.1 && Engine.keyStates) {
                if (Engine.keyStates['ArrowUp'] || Engine.keyStates['KeyW']) throttleInput = 1;
                if (Engine.keyStates['ArrowDown'] || Engine.keyStates['KeyS']) throttleInput = -1;
            }
            
            // Apply throttle
            if (Math.abs(throttleInput) > 0.01) {
                console.log(`Airplane throttle: ${throttleInput.toFixed(2)}`);
                vehicle.userData.speed += vehicle.userData.acceleration * throttleInput * deltaTime * 2;
            } else {
                // Apply air drag
            ...
            // CRITICAL ADDITION: Apply lift/drag aerodynamics
            if (Math.abs(vehicle.userData.speed) > 0) {
                // Automatic drag effect (air resistance increases with speed squared)
                const dragFactor = vehicle.userData.dragFactor || 0.02;
                const drag = dragFactor * Math.pow(Math.abs(vehicle.userData.speed), 2) * deltaTime;
                
                // Apply drag proportional to speed squared (realistic aerodynamics)
                vehicle.userData.speed *= (1 - Math.min(0.1, drag));
            }
            
            // Enforce max speed limit
            const maxSpeed = vehicle.userData.maxSpeed || 80;
            vehicle.userData.speed = Math.max(-maxSpeed * 0.3, Math.min(maxSpeed, vehicle.userData.speed));
            
            // Update altitude info UI if available
            if (vehicle.userData.isOccupied && Math.random() < 0.05) { // Only update occasionally to avoid spam
                if (typeof window !== 'undefined' && window.gameNotify && vehicle.userData.altitude > 5) {
                    window.gameNotify(`Altitude: ${Math.floor(vehicle.userData.altitude)}m, Speed: ${Math.floor(vehicle.userData.speed)} km/h`);
                }
            }
        }
    }
        ...
        try {
            ...
            if (vehicle.userData.type === 'car') {
                ...
                if (vehicle.userData._stabilizeUntil && Date.now() < vehicle.userData._stabilizeUntil) {
                    // Direct look-at with no interpolation during stabilization
                    vehicle.lookAt(lookTarget);
                } else {
                    // Use a blend between current orientation and the target one
                    // Create a temporary object to get the target quaternion
                    const tempObj = vehicle.clone();
                    tempObj.up.copy(up);
