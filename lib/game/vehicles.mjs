...
export default class VehicleManager {
    ...
    static alignVehicleToPlanetSurface(vehicle, surfaceNormal, lerpFactor = 0.1, forceFullAlignment = false) {
        ...
        try {
            ...
            // IMPROVED: Explicitly set the vehicle's up vector to surface normal
            // This ensures proper alignment like the player handle uses
            vehicle.up = surfaceNormal.clone();
            ...
            if (isActiveCar) {
                ...
                // IMPROVED: For a moving car, use a more aggressive alignment factor
                if (vehicle.userData.speed && Math.abs(vehicle.userData.speed) > 1.0) {
                    lerpFactor = Math.min(Math.max(lerpFactor, 0.08), 0.15);
                } else {
                    // Use much smaller lerp factor for smoother steering when not moving much
            ...
            // REMOVED: Special case for just-exited vehicles to snap immediately to surface
            // Let physics handle alignment based on falling state instead
            ...
            // Mark vehicle as aligned by VehicleManager to prevent redundant alignment
            vehicle.userData._alignedByVehicleManager = true;
            vehicle.userData._lastAlignmentTime = Date.now();
        } catch (err) {
    ...
    static resetWheelsBaseOrientation(vehicle) {
        ...
        try {
            ...
            // Update skip condition to remove special handling for just-exited vehicles
            const skipActivelyControlled = vehicle === this.currentVehicle && 
                (vehicle.userData?.isActivelyControlled || vehicle.userData?._controlledByCarController);
                
            if (skipActivelyControlled) {
                return;
            }
    ...
    static exitVehicle() {
        ...
        try {
            ...
            // Update vehicle state - ADDED: Allow physics to determine if vehicle should fall
            const wasAirborne = this.currentVehicle.userData.falling;
            this.currentVehicle.userData.isOccupied = false;
            this.currentVehicle.userData.player = null;
            
            // ADDED: Make vehicle affected by physics from now on
            this.currentVehicle.userData._physicsControlled = true;
