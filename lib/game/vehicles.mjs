    ...
    static createCarOnSurface(planet, latitude, longitude, heightOffset = 3) {
        try {
            ...
            if (collidable && collidable.obb) {
                ...
                // CRITICAL FIX: Add debug reference to help identify this collision object
                collidable.debugName = `Car-${vehicle.id || Math.floor(Math.random() * 1000)}`;
            }
    ...
    static setupAirplaneComponents(vehicle) {
        ...
        try {
            ...
            if (collidable && collidable.obb) {
                ...
                // Add debug reference
                collidable.debugName = `Airplane-${vehicle.id || Math.floor(Math.random() * 1000)}`;
            }
