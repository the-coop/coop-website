...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        ...
        car.userData.acceleration = 0.4; // REDUCED from 0.5 - base acceleration lower
        car.userData.handling = 0.03;
        car.userData.friction = 0.99; 
        car.userData.isOccupied = false;
        car.userData.maxSpeedReverse = 15;
        
        // FURTHER REDUCED acceleration properties for much more gradual start
        car.userData.accelerationCurve = {
            startingPower: 0.05,  // REDUCED from 0.1 to 0.05 - extremely gentle start
            maxPower: 0.4,        // REDUCED from 0.6 to 0.4 - lower peak acceleration
            powerBuildSpeed: 0.08 // REDUCED from 0.15 to 0.08 - much slower ramp-up
        };
            ...
            if (this.input) {
                ...
                if (vehicle.userData.type === 'car') {
                    ...
                    if (Math.abs(vehicle.userData.input.movement.z) > inputThreshold) {
                        ...
                        // Build up acceleration power more gradually with much slower rate
                        vehicle.userData.currentAccelPower = Math.min(
                            accelCurve.maxPower,
                            vehicle.userData.currentAccelPower + 
                                (accelCurve.powerBuildSpeed * Math.abs(inputPower) * safeDeltaTime * 0.8) // REDUCED by 20%
                        );
                        
                        // Calculate acceleration with power curve
                        const acceleration = vehicle.userData.acceleration || 0.4; // Use lower base acceleration
                        
                        // IMPROVED: Much more progressive speed factor for smoother start
                        let speedFactor = 1.0;
                        if (Math.abs(vehicle.userData.speed) < 5.0) {
                            // More progressive curve for low speeds - slower acceleration at very low speed
                            speedFactor = 0.2 + (Math.abs(vehicle.userData.speed) / 5.0) * 0.8;
                        }
                        
                        // Apply final effective acceleration
                        const effectiveAccel = acceleration * vehicle.userData.currentAccelPower * speedFactor;
                        
                        // REDUCED acceleration application by 40%
                        vehicle.userData.speed += inputPower * effectiveAccel * 0.3; // Reduced from 0.5 to 0.3
