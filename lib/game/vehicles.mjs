...
export default class VehicleManager {
    ...
    static handleVehicleInput(vehicle, deltaTime) {
        ...
        // CRITICAL FIX: Apply speed to vehicle velocity
        if (Math.abs(vehicle.userData.speed) > 0.01) {
            ...
            // Make velocity changes more significant for faster response
            const velocityChange = forwardDir.clone().multiplyScalar(vehicle.userData.speed * deltaTime * 2);
            vehicle.userData.velocity.add(velocityChange);
            
            // Log velocity for debugging
            console.log(`Vehicle velocity magnitude: ${vehicle.userData.velocity.length().toFixed(2)}`);
        }
    }
    ...
    static updatePlayerControlledVehiclePhysics(vehicle, deltaTime) {
        ...
        // CRITICAL FIX: Apply movement differently based on vehicle type
        if (vehicle.userData.type === 'car') {
            // Handle car surface movement
            if (onSurface) {
                // Force car to stay on surface
                vehicle.position.copy(planetCenter).addScaledVector(surfaceNormal, collisionDistance);
                
                // FIXED: More direct movement implementation for cars
                if (Math.abs(vehicle.userData.speed) > 0.01) {
                    ...
                    // CRITICAL FIX: Directly move car position with speed
                    const movement = projectedForward.multiplyScalar(vehicle.userData.speed * deltaTime * 3); // Increased multiplier
                    vehicle.position.add(movement);
                    
                    // Log movement for debugging
                    console.log(`Car movement: ${movement.length().toFixed(2)} units`);
            ...
            // Handle airplane physics
            if (vehicle.userData.altitude > 0) {
                // FIXED: More direct movement implementation for airplanes
                if (Math.abs(vehicle.userData.speed) > 0.01) {
                    // Get airplane's forward direction
                    const forward = new Vector3(0, 0, -1).applyQuaternion(vehicle.quaternion);
                    
                    // CRITICAL FIX: Directly move airplane position based on speed
                    const movement = forward.clone().multiplyScalar(vehicle.userData.speed * deltaTime * 3); // Increased multiplier
                    vehicle.position.add(movement);
                ...
                // Handle taxiing on ground like car movement
                vehicle.position.copy(planetCenter).addScaledVector(surfaceNormal, collisionDistance);
                ...
                if (Math.abs(vehicle.userData.speed) > 0.01) {
                    ...
                    // CRITICAL FIX: Directly move airplane position during taxiing
                    const movement = projectedForward.multiplyScalar(vehicle.userData.speed * deltaTime * 3);
                    vehicle.position.add(movement);
                    
                    // Re-snap to surface after movement
                    const newToVehicle = vehicle.position.clone().sub(planetCenter);
                    newToVehicle.normalize();
                    vehicle.position.copy(planetCenter).addScaledVector(newToVehicle, collisionDistance);
                }
