...
export default class VehicleManager {
    ...
    static setupVehicleInteractions() {
        try {
            ...
            // Set up key listener for vehicle entry/exit if not already done
            if (!this._interactionListenersSet) {
                if (typeof window !== 'undefined') {
                    window.addEventListener('keydown', (e) => {
                        // E key for Enter/Exit vehicle
                        if (e.key === 'e' || e.key === 'E') {
                            if (PlayersManager.self) {
                                if (PlayersManager.self.inVehicle) {
                                    this.exitVehicle();
                                } else {
                    ...
                    this._interactionListenersSet = true;
                    console.log("Vehicle interaction key listeners initialized");
                }
            }
