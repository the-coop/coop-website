...
export default class VehicleManager {
    ...
    static exitVehicle() {
        ...
        // Get the vehicle's world position for positioning the player
        ...
        // First get the camera position in world space before removing from vehicle
        ...
        // Determine exit direction based on vehicle type
        const exitOffset = new Vector3();
        if (this.currentVehicle.userData.type === 'airplane') {
            // Exit to the side of the airplane
            exitOffset.set(5, 0, 0);
        } else {
            // Default exit direction (back and up)
            exitOffset.set(0, 3, 5);
        }
        
        // Apply vehicle rotation to the exit offset
        exitOffset.applyQuaternion(vehicleQuaternion);
        
        // Set camera to exit position
        Engine.camera.position.copy(vehiclePosition).add(exitOffset);
        
        // Update the player's position if we have a player
        if (PlayersManager.self) {
            PlayersManager.self.position.copy(Engine.camera.position);
            PlayersManager.self.handle.position.copy(Engine.camera.position);
        }
