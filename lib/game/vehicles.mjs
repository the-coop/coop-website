...
export default class VehicleManager {
    ...
    static alignVehicleToPlanetSurface(vehicle, surfaceNormal, lerpFactor = 0.1, forceFullAlignment = false) {
        ...
        try {
            ...
            // Handle different phases with gentler corrections
                ...
                // ADDED: If we're in hard freeze, use extremely strong alignment
                if (vehicle.userData.hardFreeze) {
                    finalLerpFactor = 0.98; // Nearly instant alignment during hard freeze
                }
                else if (timeSinceLanding < 500) {
                    ...
                    // ADDED: Check for fully stabilized state
                    if (vehicle.userData.fullyStabilized) {
                        // Near-zero alignment for fully stabilized vehicles to prevent jitter
                        finalLerpFactor = 0.001; // Almost no correction for stable vehicles
                    } else {
            ...
            if (vehicle.userData.angularVelocity) {
                // ADDED: Use more aggressive damping during hard freeze
                const dampingFactor = vehicle.userData.hardFreeze ? 0.99 : 0.7;
    ...
    // ADDED: Enhanced wheel reset method with improved stability
    static resetWheelsBaseOrientation(vehicle) {
        ...
        try {
            ...
            if (!surfaceNormal && vehicle.userData.planet && vehicle.userData.planet.object) {
    ...
    static tryEnterNearbyVehicle() {
        ...
        // ADDED: First, clean up any incorrectly marked vehicles
        this.repairVehicleStates();
        ...
        if (closestVehicle) {
            ...
            // ADDED: Ensure vehicle has the correct type based on name before entering
            this.verifyVehicleType(closestVehicle);
    ...
    // ADDED: New method to verify and fix vehicle types
    static verifyVehicleType(vehicle) {
        if (!vehicle || !vehicle.userData) return;
        
        // If the type is missing or just "vehicle", attempt to determine the correct type
        if (!vehicle.userData.type || vehicle.userData.type === 'vehicle') {
            console.log(`Vehicle has invalid type: "${vehicle.userData.type}" - attempting to fix`);
            
            // Detect type from vehicle name
            if (vehicle.name) {
                if (vehicle.name.toLowerCase().includes('car')) {
                    vehicle.userData.type = 'car';
                    console.log(`Fixed vehicle type to "car" based on name: ${vehicle.name}`);
                } else if (vehicle.name.toLowerCase().includes('airplane')) {
                    vehicle.userData.type = 'airplane';
                    console.log(`Fixed vehicle type to "airplane" based on name: ${vehicle.name}`);
                }
            }
            
            // If still not determined, try to detect from components
            if (!vehicle.userData.type || vehicle.userData.type === 'vehicle') {
                // Check for unique components
                if (vehicle.userData.wheels) {
                    vehicle.userData.type = 'car';
                    console.log('Fixed vehicle type to "car" based on components');
                } else if (vehicle.userData.propeller) {
                    vehicle.userData.type = 'airplane';
                    console.log('Fixed vehicle type to "airplane" based on components');
                } else {
                    // Last resort - make it a car by default
                    vehicle.userData.type = 'car';
                    console.log('Set default vehicle type to "car"');
                }
            }
        }
        
        return vehicle.userData.type;
    }
    
    // ADDED: Enhanced repair method to fix vehicle states and types
    static repairVehicleStates() {
        try {
            let repairedCount = 0;
            let typeFixCount = 0;
            
            // Check for vehicles incorrectly marked as occupied
            for (const vehicle of this.vehicles) {
                if (!vehicle || !vehicle.userData) continue;
                
                // Check if vehicle is marked as occupied but not actually active
                if (vehicle.userData.isOccupied && vehicle !== this.currentVehicle) {
                    // Fix incorrectly marked vehicle
                    console.log(`Fixing vehicle ${vehicle.name || 'unnamed'} incorrectly marked as occupied`);
                ...
                // Verify and fix vehicle type if needed
                if (!vehicle.userData.type || vehicle.userData.type === 'vehicle') {
                    this.verifyVehicleType(vehicle);
                    typeFixCount++;
                }
            }
            
            if (repairedCount > 0 || typeFixCount > 0) {
                console.log(`Repaired ${repairedCount} vehicles with incorrect occupation state, fixed ${typeFixCount} vehicle types`);
            }
            
            return repairedCount + typeFixCount;
        } catch (err) {
            console.error("Error repairing vehicle states:", err);
            return 0;
        }
    }
