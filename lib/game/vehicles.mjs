import { BoxGeometry, CylinderGeometry, Mesh, MeshBasicMaterial, Vector3, Quaternion, Matrix4 } from 'three';
import SceneManager from './scene.mjs';
import Engine from './engine.mjs';
import PlayersManager from './players.mjs';
import ObjectManager from './object.mjs'; // Ensure ObjectManager is properly imported
...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        ...
        const adjustedHeightOffset = 3.5; // Reduced from 5.5 back to 3.5 - enough to keep it above ground
        ...
        body.position.y = -0.5; // FIXED: Moved body down from 0.0 to -0.5 so it's closer to the wheels
        car.add(body);
        ...
        wheelFL.position.set(-1.7, -1.4, 1.8); // Moved outward (-1.7) and lower (-1.4)
        ...
        wheelFR.position.set(1.7, -1.4, 1.8); // Moved outward (1.7) and lower (-1.4)
        ...
        wheelRL.position.set(-1.7, -1.4, -1.8); // Moved outward (-1.7) and lower (-1.4)
        ...
        wheelRR.position.set(1.7, -1.4, -1.8); // Moved outward (1.7) and lower (-1.4)
        ...
        // Create physics handle with proper dimensions
        const physicsHandle = new Mesh(
            new BoxGeometry(carWidth, carHeight, carLength),
            new MeshBasicMaterial({ color: 0xFF0000, wireframe: true, visible: false })
        );
        physicsHandle.position.set(0, -0.4, 0); // FIXED: Moved down from -0.1 to -0.4 to match body
        car.add(physicsHandle);
        car.userData.physicsHandle = physicsHandle;
        
        // UNIFIED: Use standardized object registration system
        const carDimensions = { width: carWidth, height: carHeight, depth: carLength };
        car.collidable = ObjectManager.registerGameObject(car, 'vehicle', carDimensions, false);
        
        // Store direct reference to OBB for convenience
        car.userData.obb = car.collidable?.obb;
        ...
        car.userData.acceleration = 0.2; // FURTHER REDUCED from 0.25 to 0.2
        car.userData.handling = 0.03;
        car.userData.friction = 0.99; 
        car.userData.isOccupied = false;
        car.userData.maxSpeedReverse = 15;
        
        // ULTRA-LOW acceleration properties for incredibly gradual start
        car.userData.accelerationCurve = {
            startingPower: 0.001,   // DRASTICALLY REDUCED from 0.005 to 0.001 - almost imperceptible initial acceleration
            maxPower: 0.2,          // REDUCED from 0.25 to 0.2 - lower peak power
            powerBuildSpeed: 0.008  // REDUCED from 0.015 to 0.008 - extremely slow power build-up
        };
        car.userData.currentAccelPower = car.userData.accelerationCurve.startingPower;
        car.userData.inertiaFactor = 0;
        ...
        // NEW: Add turning physics parameters
        car.userData.turnDrag = 0.08;           // How much turning slows the car down
        car.userData.minTurnSpeed = 1.0;        // Minimum speed needed for turning to be effective
        car.userData.optimalTurnSpeed = 15.0;   // Speed at which turning is most effective
        car.userData.turnAccelerationPenalty = 0.7; // Acceleration effectiveness during turns (0-1)
        
        // FIXED: Use a more appropriate fixed radius offset
        car.userData.fixedHeightOffset = 3.5; // Reduced from 5.5 to 3.5
        car.userData.collisionHeightOffset = 1.5; // Add separate collision height offset
        ...
        car.userData.rotateWheels = function(targetAngle) {
            ...
            // Apply the current wheel angle to the wheels - INVERTED to fix visual direction
            // When we turn right, wheels should visually turn right
        ...
        console.log(`Created car "${car.userData.name}" on ${planet.name} at ${latitude}°, ${longitude}° with enhanced OBB collision`);
        return car;
    }
    ...
    static createAirplane(planet, latitude, longitude, heightOffset = 5) {
        ...
        // ADDED: Basic airplane shape
        if (airplane) {
            // Create airplane body
            const bodyGeometry = new BoxGeometry(4, 1, 8); 
            const bodyMaterial = new MeshBasicMaterial({ color: 0x3366AA });
            const body = new Mesh(bodyGeometry, bodyMaterial);
            body.position.y = 0;
            airplane.add(body);
            
            // Create wings
            const wingGeometry = new BoxGeometry(12, 0.5, 3);
            const wingMaterial = new MeshBasicMaterial({ color: 0x2255AA });
            const wings = new Mesh(wingGeometry, wingMaterial);
            wings.position.y = 0.2;
            airplane.add(wings);
            
            // Create tail fin
            const tailFinGeometry = new BoxGeometry(0.5, 2, 2);
            const tailFinMaterial = new MeshBasicMaterial({ color: 0x2255AA });
            const tailFin = new Mesh(tailFinGeometry, tailFinMaterial);
            tailFin.position.set(0, 0.8, -3.5);
            airplane.add(tailFin);
            
            // Create horizontal stabilizers
            const stabilizerGeometry = new BoxGeometry(6, 0.4, 1.5);
            const stabilizerMaterial = new MeshBasicMaterial({ color: 0x2255AA });
            const stabilizer = new Mesh(stabilizerGeometry, stabilizerMaterial);
            stabilizer.position.set(0, 0, -3.5);
            airplane.add(stabilizer);
            
            // FIXED: Create proper collision box using accurate dimensions
            const airplaneWidth = 12;  // Wing span
            const airplaneHeight = 2.5; // Overall height
            const airplaneLength = 8;  // Length from nose to tail
            
            // Create physics handle
            const physicsHandle = new Mesh(
                new BoxGeometry(airplaneWidth, airplaneHeight, airplaneLength),
                new MeshBasicMaterial({ color: 0x3366AA, wireframe: true, visible: false })
            );
            airplane.add(physicsHandle);
            airplane.userData.physicsHandle = physicsHandle;
            
            // Register with collision system
            const airplaneDimensions = { 
                width: airplaneWidth, 
                height: airplaneHeight, 
                depth: airplaneLength 
            };
            airplane.collidable = ObjectManager.registerGameObject(
                airplane, 'vehicle', airplaneDimensions, false
            );
            
            // Set airplane physics properties
            airplane.userData.maxSpeed = 80;
            airplane.userData.acceleration = 0.3;
            airplane.userData.handling = 0.04;
            airplane.userData.friction = 0.98;
            airplane.userData.liftCoefficient = 0.05;
            airplane.userData.maxAltitude = 500;
            
            // FIXED: Always set these properties explicitly
            airplane.userData.fixedHeightOffset = 50;  // Initial height above planet
            airplane.userData.falling = true;         // Start in falling state (physics will stabilize)
            airplane.userData.onSurface = false;      // Not on surface
        }
        
        console.log(`Created airplane at lat=${latitude}°, lon=${longitude}°, height=${heightOffset}`);
        return airplane;
    }
    ...
    static createVehicleBase(type, planet, latitude, longitude, heightOffset = 2) {
        ...
        try {
            ...
            try {
                ...
                try {
                    // FIXED: Explicitly increase vehicle height for airplanes
                    let actualHeightOffset = heightOffset;
                    if (type === 'airplane') {
                        // Make sure airplanes have a minimum height
                        actualHeightOffset = Math.max(heightOffset, 50);
                        console.log(`Using increased height offset ${actualHeightOffset} for airplane`); 
                    } else if (type === 'car') {
                        actualHeightOffset = Math.max(heightOffset, 3.5);
                    }
    ...
    static setupVehicleCamera(vehicle) {
        ...
        try {
            ...
            if (vehicle.userData.type === 'car') {
                // IMPROVED: Much lower camera with less tilt for more natural driving perspective
                Engine.camera.position.set(0, 1.5, -5); // Significantly lower (was 2.5)
                Engine.camera.rotation.set(0.08, Math.PI, 0); // Less tilt (was 0.1)
            } 
    ...
    static updateCurrentVehicle(deltaTime = 1/60) {
        ...
        if (this.currentVehicle && this.currentVehicle.userData.isOccupied) {
            ...
            if (this.input) {
                ...
                if (vehicle.userData.type === 'car') {
                    // Store last speed for comparison
                    vehicle.userData.lastSpeed = vehicle.userData.speed;
                    
                    // Get turning input early to factor into acceleration
                    const turnInputAmount = this.input.movement.x;
                    const turnInputMagnitude = Math.abs(turnInputAmount);
                    
                    // Calculate turn effect - will be used both for steering and speed loss
                    const rawSteeringAngle = turnInputAmount * 0.5; // Base steering angle calculation
                    
                    // Calculate how much the car is actually turning based on input and current wheel angle
                    const actualTurning = Math.abs(vehicle.userData.currentWheelAngle);
                    
                    if (Math.abs(vehicle.userData.input.movement.z) > 0.05) {
                        // FIXED: More granular threshold and smoother acceleration
                        const inputThreshold = 0.05; // Threshold for detecting movement input
                        
                        if (Math.abs(vehicle.userData.input.movement.z) > inputThreshold) {
                            // IMPROVED: Use acceleration curve for more realistic starting and acceleration
                            const accelCurve = vehicle.userData.accelerationCurve || {
                                startingPower: 0.001,
                                maxPower: 0.2,
                                powerBuildSpeed: 0.008
                            };
                            
                            // Calculate input power - larger for forward, smaller for reverse
                            const inputPower = vehicle.userData.input.movement.z > 0 ? 
                                vehicle.userData.input.movement.z : 
                                vehicle.userData.input.movement.z * 0.6;
                            
                            // Ensure we always have a small deltaTime to prevent jumps
                            const safeDeltaTime = Math.min(0.05, deltaTime || 1/60);
                            
                            // Build up acceleration power more gradually with even slower rate
                            vehicle.userData.currentAccelPower = Math.min(
                                accelCurve.maxPower,
                                vehicle.userData.currentAccelPower + 
                                    (accelCurve.powerBuildSpeed * Math.abs(inputPower) * safeDeltaTime * 0.3)
                            );
                            
                            // Calculate acceleration with power curve
                            const acceleration = vehicle.userData.acceleration || 0.2;
                            
                            // IMPROVED: Much more progressive speed factor for extremely gradual start
                            let speedFactor = 1.0;
                            if (Math.abs(vehicle.userData.speed) < 10.0) {
                                // Use an exponential curve for even smoother initial acceleration
                                // This will make starting nearly imperceptible but gradually increase
                                const normalizedSpeed = Math.abs(vehicle.userData.speed) / 10.0;
                                speedFactor = 0.03 + Math.pow(normalizedSpeed, 1.5) * 0.97; // REDUCED initial factor from 0.05 to 0.03
                            }
                            
                            // NEW: Apply turning penalty to acceleration - harder to accelerate while turning
                            const turnPenalty = vehicle.userData.turnAccelerationPenalty || 0.7; 
                            const accelerationEfficiency = 1.0 - (actualTurning * (1.0 - turnPenalty));
                            
                            // Apply final effective acceleration with turn penalty
                            const effectiveAccel = acceleration * vehicle.userData.currentAccelPower * 
                                                  speedFactor * accelerationEfficiency;
                            
                            // REDUCED acceleration application
                            vehicle.userData.speed += inputPower * effectiveAccel * 0.15;
                            
                            // Build up inertia only when actually accelerating
                            vehicle.userData.inertiaFactor = Math.min(
                                1.0, 
                                vehicle.userData.inertiaFactor + (0.005 * Math.abs(inputPower))
                            );
                            ...
                            // DEBUG: Log acceleration data if speed changed dramatically
                            if (Math.abs(vehicle.userData.speed - vehicle.userData.lastSpeed) > 2.0) {
                                console.log(`Car speed jumped from ${vehicle.userData.lastSpeed.toFixed(2)} to ${vehicle.userData.speed.toFixed(2)}`);
                                console.log(`Power: ${vehicle.userData.currentAccelPower.toFixed(2)}, Input: ${inputPower.toFixed(2)}`);
                            }
                        } else {
                            // IMPROVED: Deceleration affected by current speed, inertia, AND turning
                            // Higher speeds and more inertia result in slower deceleration
                            const speedFactor = Math.abs(vehicle.userData.speed) / vehicle.userData.maxSpeed;
                            const inertiaFactor = vehicle.userData.inertiaFactor || 0;
                            
                            // Calculate dynamic friction - higher values = less friction at high speeds
                            // Add stronger speed loss when turning without acceleration
                            const baseFriction = 0.99 + (0.008 * speedFactor * inertiaFactor);
                            
                            // ENHANCED: Much stronger speed loss when turning without accelerating
                            // This simulates realistic turning friction when not applying power
                            const turningFriction = Math.max(0, actualTurning * 0.15); // Increased from 0.08 to 0.15
                            
                            // Friction is stronger when turning (lower value = more friction)
                            const frictionFactor = Math.min(0.999, Math.max(0.97, baseFriction - turningFriction));
                            
                            // Apply the calculated friction
                            vehicle.userData.speed *= frictionFactor;
                            
                            // Gradually reduce inertia over time when not accelerating, but slower for cars than players
                            vehicle.userData.inertiaFactor = Math.max(
                                0, 
                                vehicle.userData.inertiaFactor - 0.003
                            );
                            
                            // Only stop completely at very low speed
                            if (Math.abs(vehicle.userData.speed) < 0.03) {
                                vehicle.userData.speed = 0;
                                vehicle.userData.inertiaFactor = 0;
                            }
                        }
                    }
                    
                    // NEW: Apply speed loss from turning - more turning = more speed loss
                    if (actualTurning > 0.01 && Math.abs(vehicle.userData.speed) > 0.5) {
                        // ENHANCED: Increased turning drag effect for more realistic physics
                        const turnDrag = (vehicle.userData.turnDrag || 0.12) * actualTurning; // Increased from 0.08 to 0.12
                        vehicle.userData.speed *= (1.0 - turnDrag * deltaTime);
                    }
                    
                    // NEW: Check for collisions with all objects including players, vehicles, and walls
                    if (Math.abs(vehicle.userData.speed) > 0.5) {
                        // UNIFIED: Use the standardized collision check method
                        ...
                        if (collisions.length > 0) {
                            ...
                            for (const collision of collisions) {
                                ...
                                const impactSpeed = Math.abs(vehicle.userData.speed);
                                const impactForce = Math.min(impactSpeed * 0.5, 5.0); // Cap the maximum impact force
                                
                                // Apply collision response - reduce speed based on impact force
                                vehicle.userData.speed *= Math.max(0.2, 1.0 - impactForce * 0.2);
                                
                                // Apply directional bounce effect based on collision normal
                                const velocityAlignment = vehicle.userData.velocity ? 
                                    -vehicle.userData.velocity.dot(collisionNormal) : 0;
                                
                                if (velocityAlignment > 0 && vehicle.userData.velocity) {
                                    // We're moving toward the object, bounce back
                                    vehicle.userData.velocity.addScaledVector(collisionNormal, velocityAlignment * 1.5);
                                }
                                
                                // Notify about significant collisions
                                if (impactSpeed > 5.0 && typeof window !== 'undefined' && window.gameNotify) {
                                    window.gameNotify(`Collision with ${collision.other.userData?.type || 'object'}`);
                                }
                                
                                // Apply impulse to the other object if it's dynamic
                                const otherObject = collision.other;
                                if (otherObject && otherObject.userData && 
                                    otherObject.userData.isDynamic && otherObject.userData.velocity) {
                                    
                                    // Calculate impulse based on relative mass
                                    const vehicleMass = vehicle.userData.mass || 1000;
                                    const otherMass = otherObject.userData.mass || 100;
                                    const massRatio = vehicleMass / (otherMass + vehicleMass);
                                    
                                    // Apply impulse to other object
                                    const impulse = collisionNormal.clone().multiplyScalar(-impactForce * massRatio * 2);
                                    otherObject.userData.velocity.add(impulse);
                                    
                                    // Special case for player collisions
                                    if (otherObject.userData.isPlayer && impactSpeed > 3.0) {
                                        // Stronger impulse for players to create more dramatic effect
                                        otherObject.userData.velocity.addScaledVector(collisionNormal, impactSpeed * 0.8);
                                    }
                                }
                            }
                        }
                    ...
                    // IMPROVED: More realistic turning based on speed and steering angle
                    if (Math.abs(vehicle.userData.speed) > (vehicle.userData.minTurnSpeed || 1.0)) {
                        const handling = vehicle.userData.handling || 0.03;
                        
                        // Calculate speed-dependent turn effectiveness
                        // - At low speeds, turning is less effective
                        // - At medium speeds, turning is most effective
                        // - At high speeds, turning effectiveness gradually decreases
                        const optimalSpeed = vehicle.userData.optimalTurnSpeed || 15.0;
                        const speedRatio = Math.abs(vehicle.userData.speed) / optimalSpeed;
                        
                        // Turn effectiveness follows a curve: starts low, peaks around optimal speed, then decreases
                        const turnEffectiveness = Math.min(1.0, speedRatio * (2.0 - speedRatio));
                        
                        const turnAmount = turnInputAmount * handling * turnEffectiveness;
                        const turnDirection = vehicle.userData.speed < 0 ? -1 : 1;
                        
                        // Apply turn to vehicle orientation
            ...
            if (vehicle.userData.type === 'car' && vehicle.userData.planet) {
                ...
                const targetHeight = vehicle.userData.planet.radius + 
                    (vehicle.userData.fixedHeightOffset || 3.25); // Updated from 1.94 to 3.25
    ...
    // ENHANCED: Align vehicle to planet surface with better OBB handling
    static alignVehicleToPlanetSurface(vehicle, surfaceNormal, slerpFactor = 0.2) {
        if (!vehicle || !surfaceNormal) return;
        
        try {
            // Create the desired rotation - 'vehicle up' should align with surface normal
            const upVector = new Vector3(0, 1, 0);
            
            // Handle case where normal and up are almost parallel
            if (Math.abs(upVector.dot(surfaceNormal)) > 0.99999) {
                // No rotation needed in this case
                return;
            }
            
            // Create the rotation quaternion to align vehicle with planet surface
            const alignmentQuaternion = new Quaternion();
            
            // Use lookAt mechanism for alignment, creating a world-to-local matrix
            const worldToLocal = new Matrix4();
            worldToLocal.lookAt(
                new Vector3(0, 0, 0),  // origin
                surfaceNormal,         // direction to look at (surface normal)
                new Vector3(0, 0, 1)   // vehicle "forward" direction
            );
            
            // Convert matrix to quaternion
            const alignQuat = new Quaternion().setFromRotationMatrix(worldToLocal);
            
            // Smoothly interpolate between current rotation and target rotation
            vehicle.quaternion.slerp(alignQuat, slerpFactor);
            
            // Force matrix update
            vehicle.updateMatrix();
            vehicle.updateMatrixWorld(true);
            
            // Update collision bounds (with safety check)
            if (vehicle.collidable && typeof ObjectManager.updateCollidableBounds === 'function') {
                ObjectManager.updateCollidableBounds(vehicle);
            } else if (!vehicle.collidable) {
                // This vehicle doesn't have collision data, let's set it up
                if (typeof ObjectManager.registerGameObject === 'function') {
                    const dimensions = {
                    ...
                    vehicle.collidable = ObjectManager.registerGameObject(vehicle, 'vehicle', dimensions, false);
                    console.log(`Created new vehicle collision data for ${vehicle.userData.name || 'unnamed'}`);
                } else {
                    console.warn("ObjectManager.registerGameObject is not available, can't set up vehicle collision");
                }
            } else {
                console.warn("ObjectManager.updateCollidableBounds is not available - skipping collision update");
            }
        } catch (err) {
    ...
    static enterVehicle(vehicle) {
        ...
        // ADDED: More aggressive type checking to ensure vehicle is valid
        if (!vehicle.userData || !vehicle.userData.type) {
            console.error("Vehicle has no valid type information:", vehicle);
            return false;
        }
        ...
        // ENHANCED: Add extra validation and logging before setting current vehicle
        console.log(`Vehicle data before entry: type=${vehicle.userData.type}, isOccupied=${vehicle.userData.isOccupied}`);
        ...
        for (const otherVehicle of this.vehicles) {
            ...
            if (otherVehicle.userData && otherVehicle.userData.isOccupied) {
                console.warn(`Found another vehicle marked as occupied: ${otherVehicle.userData.name}`);
        ...
        // ENHANCED: Clear the current vehicle before setting it again
        this.currentVehicle = null;
        ...
        // ENHANCED: Add additional validation after setting current vehicle
        console.log(`VehicleManager status after entry:
            - Current vehicle set: ${this.currentVehicle === vehicle}
            - Vehicle marked occupied: ${vehicle.userData.isOccupied}
            - Vehicle type: ${vehicle.userData.type}`);
