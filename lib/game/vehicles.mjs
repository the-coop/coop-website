...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        ...
        car.userData.friction = 0.90; // Reduced from 0.98 to prevent oscillation
    ...
    static handleVehicleInput(vehicle, deltaTime) {
        // CRITICAL FIX: Check if we have input to process
        if (!this.input && !Engine.keyStates) {
            console.log("No input available for vehicle");
            // Apply stronger drag when no input controller is available
        ...
        // CRITICAL FIX: Always log input state for better debugging
        ...
        if (vehicle.userData.type === 'car') {
            ...
            // CRITICAL FIX: Better input detection and movement processing
            // Get input from controller if available
            if (this.input && this.input.movement) {
                accelerationInput = -this.input.movement.z;
            }
            
            // CRITICAL FIX: Always check keyboard input as fallback
            if (Math.abs(accelerationInput) < 0.1 && Engine.keyStates) {
                if (Engine.keyStates['ArrowUp'] || Engine.keyStates['KeyW']) accelerationInput = 1;
                if (Engine.keyStates['ArrowDown'] || Engine.keyStates['KeyS']) accelerationInput = -1;
            }
            ...
            if (Math.abs(accelerationInput) > 0.01) {
                ...
                // CRITICAL FIX: More responsive acceleration with higher multiplier
                vehicle.userData.speed += vehicle.userData.acceleration * accelerationInput * deltaTime * 2;
            ...
            // Fallback to keyboard
                ...
                if (Engine.keyStates['ArrowLeft'] || Engine.keyStates['KeyA']) steeringInput = -1;
                if (Engine.keyStates['ArrowRight'] || Engine.keyStates['KeyD']) steeringInput = 1;
            }
            
            // Apply steering rotation only if we have input and sufficient speed
            if (Math.abs(steeringInput) > 0.01 && Math.abs(vehicle.userData.speed) > 0.1) {
                console.log(`Car steering input: ${steeringInput.toFixed(2)}`);
                // CRITICAL FIX: Make steering more responsive
                vehicle.rotateY(-vehicle.userData.handling * steeringInput * Math.sign(vehicle.userData.speed) * 1.5);
            }
        }
