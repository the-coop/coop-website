...
export default class VehicleManager {
    ...
    static handleVehicleInput(vehicle, deltaTime) {
        ...
        // Debug logging for input values only when significant input detected
        ...
        if (vehicle.userData.type === 'car') {
            ...
            if (this.input && this.input.movement) {
                ...
                // Apply some minimal filtering for smoother steering
                if (Math.abs(steeringInput) < 0.05) steeringInput = 0;
                if (Math.abs(accelerationInput) < 0.05) accelerationInput = 0;
            }
            
            // Store steering input for visual effects
            vehicle.userData.steeringInput = steeringInput;
            
            // DEBUG: Log input values when they exist
            ...
            // Apply acceleration using a more direct approach
            if (Math.abs(accelerationInput) > 0.01) {
                const accelerationStrength = 20; // Good responsive value
            ...
            // Update velocity vector in world space for physics system
