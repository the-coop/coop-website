    ...
    static updateVehiclePhysics(vehicle, deltaTime) {
        ...
        if (vehicle.userData.type === 'airplane' && vehicle.userData.altitude > 0) {
            ...
            if (Math.abs(vehicle.userData.speed) > 0.01) {
                ...
                // Calculate up vector (points away from planet center)
                ...
                // This keeps the car's forward direction on the planet surface
                const projectedForward = vehicleForward.clone().projectOnPlane(up).normalize();
                ...
                // Recalculate forward to ensure it's perpendicular to up and right
                ...
                // Smoothly rotate vehicle to align with planet surface
                const targetQuat = new Quaternion().setFromRotationMatrix(rotMatrix);
                vehicle.quaternion.slerp(targetQuat, 0.3); // Smoother rotation (0.3 instead of 0.1)
            }
        }
    }
}