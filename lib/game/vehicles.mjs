...
export default class VehicleManager {
    ...
    static createVehicleBase(type, planet, latitude, longitude, heightOffset = 2) {
        ...
        // IMPROVED: Better initial falling velocity based on height
        ...
        // CRITICAL FIX: More reliable falling state initialization
        // Cars that are close to the surface should not start in falling state
        const isSignificantHeight = type === 'car' ? heightOffset > 3 : heightOffset > 2;
        vehicle.userData.falling = isSignificantHeight;
        vehicle.userData.onSurface = !isSignificantHeight;
        
        // Initialize vehicle landing check tracker
        vehicle.userData._checkedForLanding = !isSignificantHeight;
        
        // Log the initial state for debugging
        console.log(`Created ${type} with falling=${vehicle.userData.falling}, height=${heightOffset}`);
    ...
    static handleVehicleInput(vehicle, deltaTime) {
        // CRITICAL FIX: Make sure we have a valid input object
        if (!this.input) {
            console.warn("VehicleManager.input is null, creating default");
            this.input = {
        ...
        console.log(`Vehicle input handling: movement=${JSON.stringify(this.input?.movement)}`);
        ...
        if (vehicle.userData.type === 'car') {
            ...
            // DEBUG: Log input values always for diagnostics
            console.log(`Car inputs: accel=${accelerationInput.toFixed(2)}, steer=${steeringInput.toFixed(2)}`);
            
            // CRITICAL FIX: Apply acceleration using a more direct approach with stronger values
            if (Math.abs(accelerationInput) > 0.01) {
                const accelerationStrength = 30; // Increased for more responsive feel
            ...
            if (Math.abs(steeringInput) > 0.01 && Math.abs(vehicle.userData.speed) > 0.5) {
                const steeringStrength = 10; // Higher values = more responsive steering
