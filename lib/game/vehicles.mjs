...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        ...
        // Create wheels - FIXED: Better wheel setup with cleaner rotation axes
        const wheelGeometry = new CylinderGeometry(1.5, 1.5, 1, 16);
        const wheelMaterial = new MeshBasicMaterial({ color: 0x333333 });
        
        // FIXED: Initialize all wheels with base rotation only (Z axis)
        // This puts the cylinder flat (sideways) as a wheel should be
        // Front left wheel
    ...
    // ENHANCED FIX: Only update the current vehicle, not all vehicles
