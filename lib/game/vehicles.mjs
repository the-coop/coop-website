    ...
    static updateCurrentVehicle(deltaTime = 1/60) {
        ...
        if (this.currentVehicle && this.currentVehicle.userData.isOccupied) {
            ...
            if (this.input) {
                ...
                // CRITICAL FIX: Copy the input but invert the Z axis for correct movement direction
                // This way W/Space will move forward and S will move backward
                vehicle.userData.input.movement.x = this.input.movement.x;
                vehicle.userData.input.movement.y = this.input.movement.y;
                vehicle.userData.input.movement.z = -this.input.movement.z; // Invert Z axis
                ...
                if (vehicle.userData.type === 'car') {
                    // Process forward/backward movement - now with inverted Z
                    if (Math.abs(vehicle.userData.input.movement.z) > 0.05) {
                        ...
                        // Limit max speed based on direction - now Z is already inverted
