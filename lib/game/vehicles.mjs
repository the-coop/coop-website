                ...
                if (vehicle.userData.type === 'car') {
                    ...
                    if (Math.abs(vehicle.userData.input.movement.z) > 0.05) {
                        ...
                        if (Math.abs(vehicle.userData.input.movement.z) > inputThreshold) {
                            ...
                            // IMPROVED: Deceleration affected by current speed and inertia
                            ...
                            // Cars have higher friction values than players due to rolling wheels vs walking feet
                            const dynamicFriction = 0.99 + (0.008 * speedFactor * inertiaFactor); // Increased from 0.985
                            const frictionFactor = Math.min(0.999, Math.max(0.97, dynamicFriction)); // Higher max (0.998->0.999)
                            ...
                            vehicle.userData.inertiaFactor = Math.max(
                                0, 
                                vehicle.userData.inertiaFactor - 0.003  // Reduced from 0.005 for more persistent momentum
                            );
                            
                            // Only stop completely at very low speed
                            if (Math.abs(vehicle.userData.speed) < 0.03) { // Reduced from 0.05 for more gradual stopping
                                vehicle.userData.speed = 0;
                    ...
                    if (actualTurning > 0.01 && Math.abs(vehicle.userData.speed) > 0.5) {
                        const turnDrag = (vehicle.userData.turnDrag || 0.08) * actualTurning;
