...
export default class VehicleManager {
    ...
    static setupCarComponents(vehicle) {
        ...
        try {
            ...
            // Position wheels - raised Y position to prevent clipping
            const wheelFL = new Mesh(wheelGeometry, wheelMaterial);
            wheelFL.position.set(-1.7, -1.1, 1.8); // Raised from -1.4 to -1.1        
            ...
            wheelFR.position.set(1.7, -1.1, 1.8); // Raised from -1.4 to -1.1      
            ...
            wheelRL.position.set(-1.7, -1.1, -1.8); // Raised from -1.4 to -1.1       
            ...
            wheelRR.position.set(1.7, -1.1, -1.8); // Raised from -1.4 to -1.1       
            ...
            windshield.position.set(0, 0.5, 0.7); // Raised from 0.2 to 0.5
    ...
    static exitVehicle() {
        ...
        try {
            // ENHANCED: Preserve surface normal from vehicle to player
            if (this.currentVehicle.userData && this.currentVehicle.userData.surfaceNormal) {
                const surfaceNormal = this.currentVehicle.userData.surfaceNormal.clone();
                
                // Pass this to player for consistent orientation
                if (PlayersManager.self) {
                    PlayersManager.self.surfaceNormal = surfaceNormal;
                    
                    // Also store in player handle
                    if (PlayersManager.self.handle && PlayersManager.self.handle.userData) {
                        PlayersManager.self.handle.userData.surfaceNormal = surfaceNormal;
                    }
                    
                    // Store globally for reference
                    window.lastSurfaceNormal = surfaceNormal;
                }
            }
