...
export default class VehicleManager {
    ...
    static alignVehicleToPlanetSurface(vehicle, surfaceNormal, lerpFactor = 0.1, forceFullAlignment = false) {
        ...
        try {
            ...
            // IMPROVED: Adjust alignment behavior based on vehicle state and special scenarios
            ...
            // Handle different phases
                ...
                // More gentle correction when falling to prevent wild rotation
                finalLerpFactor = Math.min(0.02, lerpFactor); // Very gentle correction while falling (reduced from 0.03)
            } else if (vehicle.userData.landingTime) {
                const timeSinceLanding = Date.now() - vehicle.userData.landingTime;
                
                if (timeSinceLanding < 500) {
                    // Very aggressive correction during the first half second after landing
                    finalLerpFactor = Math.max(0.7, lerpFactor); // Increased from 0.5 (70% alignment per frame)
                } else if (timeSinceLanding < 2000) {
                    // Still strong correction for 2 seconds after landing
                    finalLerpFactor = Math.max(0.4, lerpFactor); // Medium-strong correction
                } else {
                    // Normal alignment for stable grounded state
                    finalLerpFactor = lerpFactor;
                }
            }
            
            // IMPROVED: Extra correction for badly aligned vehicles
            const upDot = new Vector3(0, 1, 0).applyQuaternion(vehicle.quaternion).dot(surfaceNormal);
            const badAlignment = Math.abs(upDot) < 0.7; // Less strict threshold (changed from 0.5 to 0.7)
            
            if (badAlignment && !vehicle.userData.falling) {
                // More aggressive correction for badly aligned vehicles (likely rolling)
                finalLerpFactor = Math.max(0.4, finalLerpFactor * 2);
                console.log(`Applied aggressive alignment correction to poorly aligned vehicle: ${vehicle.userData.name || 'unnamed'}`);
            }
    ...
    static updateCar(car, deltaTime) {
        ...
        try {
            ...
            // IMPROVED: More aggressive surface height maintenance with better alignment
            if (window.Physics && typeof window.Physics.maintainVehicleSurfaceHeight === 'function') {
                ...
                // ADDED: Additionally, ensure vehicle is properly oriented to surface
                if (car.userData.planet && !car.userData.falling) {
                    ...
                    if (planet && planet.object) {
                        ...
                        // Check if the alignment is poor and needs correction
                        const upVector = new Vector3(0, 1, 0).applyQuaternion(car.quaternion);
                        const alignmentDot = upVector.dot(toVehicle);
                        
                        if (Math.abs(alignmentDot) < 0.9) { // If alignment is poor
                            // Force alignment with moderate factor
                            this.alignVehicleToPlanetSurface(car, toVehicle, 0.2);
                        }
                    }
                }
            }
