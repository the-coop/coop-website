...
export default class VehicleManager {
    ...
    static handleVehicleInput(vehicle, deltaTime) {
        ...
        console.log(`Vehicle input debug:`, {
            hasInput: !!this.input,
            carSpeed: vehicle.userData.speed
        });
        
        if (vehicle.userData.type === 'car') {
            // CRITICAL FIX: Directly check key states to avoid input system issues
            ...
            // Get input from controller if available and keyboard input is not active
            ...
            // Output acceleration for debugging
            if (Math.abs(accelerationInput) > 0.01) {
                console.log(`Car acceleration input: ${accelerationInput.toFixed(2)}`);
                // CRITICAL FIX: Much higher multiplier for more responsive acceleration
                vehicle.userData.speed += vehicle.userData.acceleration * accelerationInput * deltaTime * 4;
                console.log(`New speed: ${vehicle.userData.speed.toFixed(2)}`);
            } else {
                // Apply drag when no input
                vehicle.userData.speed *= 0.98;
            }
            
            // Apply steering rotation only if we have input and any speed
            if (Math.abs(steeringInput) > 0.01 && Math.abs(vehicle.userData.speed) > 0.01) {
                ...
                // CRITICAL FIX: Stronger more responsive steering
                vehicle.rotateY(-vehicle.userData.handling * steeringInput * Math.sign(vehicle.userData.speed) * 2);
            }
            
            // CRITICAL ADDITION: Apply different speed limits for forward/reverse
            const maxForwardSpeed = vehicle.userData.maxSpeed || 40;
            const maxReverseSpeed = vehicle.userData.maxSpeedReverse || 15;
            
            // Clamp speed within appropriate limits based on direction
            if (vehicle.userData.speed > 0) {
