...
export default class VehicleManager {
    ...
    static createCar(planet, latitude, longitude, heightOffset = 3) {
        try {
            ...
            // Use a more moderate height offset for visual positioning
            const adjustedHeightOffset = heightOffset || 20; // Default to 20 units if not specified
            ...
            car.userData.falling = true;     // SIMPLIFIED: Only use falling flag
            ...
            car.userData.gravityFactor = 0.7; // ADDED: Use 70% of normal gravity
            
            // CRITICAL FIX: Add anti-spin damping
            car.userData.rotationDamping = 0.95; // Damping factor for rotational movement
    ...
    static createAirplane(planet, latitude, longitude, heightOffset = 10) {
        try {
            ...
            const adjustedHeightOffset = heightOffset || 30; // Default to 30 units if not specified 
            ...
            airplane.userData.gravityFactor = 0.5; // ADDED: Use 50% of normal gravity
            
            // CRITICAL FIX: Add anti-spin damping
            airplane.userData.rotationDamping = 0.97; // Higher damping for airplanes
    ...
    static alignVehicleToPlanetSurface(vehicle, surfaceNormal, lerpFactor = 0.1) {
        ...
        try {
            ...
            // CRITICAL FIX: For falling vehicles, use a more aggressive alignment factor
            // This prevents them from spinning wildly during descent
            const finalLerpFactor = vehicle.userData.falling ? 
                                  Math.min(0.05, lerpFactor) : // Very gentle correction while falling
                                  lerpFactor;                  // Normal alignment when on surface
            
            // CRITICAL FIX: If vehicle was previously spinning badly (large angular difference),
            // use a higher correction factor to stabilize more quickly
            const upDot = new Vector3(0, 1, 0).applyQuaternion(vehicle.quaternion).dot(surfaceNormal);
            const badAlignment = Math.abs(upDot) < 0.5;
            
            if (badAlignment) {
                // More aggressive correction for badly aligned vehicles (likely spinning)
                vehicle.quaternion.slerp(alignmentQuaternion, Math.min(0.2, lerpFactor * 2));
                console.log(`Applied aggressive alignment correction to spinning vehicle: ${vehicle.userData.name || 'unnamed'}`);
            } else {
                // Normal smooth alignment
            ...
            if (vehicle.userData.angularVelocity) {
                vehicle.userData.angularVelocity.multiplyScalar(0.9); // Damping factor
            }
        } catch (err) {
