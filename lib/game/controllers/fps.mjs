...
export default class FPSController {
    ...
    static isSetup = false; // Setup flag

    static async setup() {
        if (this.isSetup) {
            console.log('FPSController is already set up.');
            return true;
        }

        if (!State.isGameStarted) {
            console.error('FPS setup failed: game not started');
            return false;
        }

        // Check for protagonist in State
        const protagonist = State.protagonist;
        if (!protagonist) {
            console.error('FPS setup failed: no protagonist in State');
            return false;
        }

        if (!protagonist.mesh || !protagonist.cameraPivot) {
            console.error('FPS setup failed: protagonist missing required components', {
                hasMesh: !!protagonist.mesh,
                hasCameraPivot: !!protagonist.cameraPivot
            });
            return false;
        }
        ...
        try {
            ...
            this.isSetup = true;
            console.log('FPSController setup complete with protagonist.');
            return true;
        } catch (err) {
            console.error('FPSController setup failed:', err);
            this.cleanup();
            return false;
        }
    }
    ...
    static cleanup() {
        ...
        this.isSetup = false;
        console.log('FPSController has been cleaned up.');
    }
    ...
    static handlePointerLockChange() {
        // Handle pointer lock changes specific to FPSController
        const isLocked = !!document.pointerLockElement;
        console.log(`FPSController pointer lock changed: ${isLocked}`);
        // Additional logic...
    }
}
