...
export default class ControllerManager {
    // Remove static property initializations from here
    ...
    static async setup() {
        if (this.ready && State.isGameStarted) {
            return true;
        }
        ...
        try {
            ...
            // Initialize with current state control mode (defaults to fps)
            await this.switchMode(State.controlMode);

            // Wait for protagonist to be available
            ...
            if (!PlayerManager.protagonist) {
                throw new Error('Protagonist not available for controller setup');
            }
            ...
            this.ready = true; // Update ready flag instead of isInitialized
            this.loaded = true;

            State.setControllersInitialized(true); // Add this line

            console.log('ControllerManager setup complete with FPS controller');
            return true;
        } catch (err) {
    ...
    static async switchMode(mode) {
        ...
        try {
            ...
            State.setControlMode(newMode); // Update state instead of local property
    ...
    static getCurrentMode() {
        return State.controlMode; // Get from state instead of local property
    }
