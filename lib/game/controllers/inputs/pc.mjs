...
import FirstPersonCarController from '../FirstPersonCarController.mjs';
import FirstPersonAirplaneController from '../FirstPersonAirplaneController.mjs';
...
export default class PC {
    ...
    static update() {
        ...
        // Handle view switching (O key) for all controller types
        if (this.active['KeyO'] && !this.viewSwitchPressed) {
            this.viewSwitchPressed = true;
            
            // Switch camera view based on current controller
            if (ControlManager.controller === FPSController) {
                console.log('Switching to ThirdPersonController');
                ControlManager.change(ThirdPersonController);
            } else if (ControlManager.controller === ThirdPersonController) {
                console.log('Switching to FPSController');
                ControlManager.change(FPSController);
            } else if (ControlManager.controller === CarController) {
                console.log('Switching to FirstPersonCarController');
                ControlManager.change(FirstPersonCarController);
            } else if (ControlManager.controller === FirstPersonCarController) {
                console.log('Switching to CarController');
                ControlManager.change(CarController);
            } else if (ControlManager.controller === AirplaneController) {
                console.log('Switching to FirstPersonAirplaneController');
                ControlManager.change(FirstPersonAirplaneController);
            } else if (ControlManager.controller === FirstPersonAirplaneController) {
                console.log('Switching to AirplaneController');
                ControlManager.change(AirplaneController);
            }
