...
export default class PC {
    ...
    static update() {
        ...
        // Handle view switching (O key) for all controllers
        if (this.active['KeyO'] && !this.viewSwitchPressed) {
            this.viewSwitchPressed = true;
            
            // Get the controller we should switch to based on current one
            const nextController = ControlManager.getViewToggleController();
            
            if (nextController) {
                // Switch to the paired controller
                ...
                // Log error if no paired controller found
                console.error(`No paired controller found for ${ControlManager.controller?.constructor?.name}`);
            }
        ...
        if (this.active['KeyE'] && !this.interactPressed) {
            ...
            // Set the exit flag if the controller input supports it
