...
export default class PC {
    ...
    static updateCommonInputs() {        
        ...
        // CRITICAL FIX: Clear movement input BEFORE applying new values
        ControlManager.controller.input.movement.set(0, 0, 0);
        
        // Process inputs that are common to all controller types
        if (this.active['KeyW'] || this.active['ArrowUp']) ControlManager.controller.input.movement.z -= 1;
        if (this.active['KeyS'] || this.active['ArrowDown']) ControlManager.controller.input.movement.z += 1;
        if (this.active['KeyA'] || this.active['ArrowLeft']) ControlManager.controller.input.movement.x -= 1;
        if (this.active['KeyD'] || this.active['ArrowRight']) ControlManager.controller.input.movement.x += 1;
        
        // CRITICAL FIX: Add Space as an alternative for forward movement in vehicles
        if (this.active['Space'] && (ControlManager.controller === CarController || 
                                     ControlManager.controller === AirplaneController)) {
            ControlManager.controller.input.movement.z -= 1; // Same as W - move forward
        }
        
        // Debug input for vehicles
        if ((ControlManager.controller === CarController || ControlManager.controller === AirplaneController) &&
            (Math.abs(ControlManager.controller.input.movement.x) > 0.01 || 
             Math.abs(ControlManager.controller.input.movement.z) > 0.01)) {
            console.log(`PC input detected: x=${ControlManager.controller.input.movement.x}, z=${ControlManager.controller.input.movement.z}`);
        }

        // Handle additional airplane specific inputs
        if (ControlManager.controller === AirplaneController) {
            // Handle pitch control
            if (this.active['KeyI'] || this.active['ArrowUp']) ControlManager.controller.input.movement.y -= 1; // Pitch up
            if (this.active['KeyK'] || this.active['ArrowDown']) ControlManager.controller.input.movement.y += 1; // Pitch down
            
            // Debug airplane input
            if (Math.abs(ControlManager.controller.input.movement.x) > 0.01 || 
                Math.abs(ControlManager.controller.input.movement.z) > 0.01 ||
                Math.abs(ControlManager.controller.input.movement.y) > 0.01) {
                console.log(`✈️ Airplane input detected: x=${ControlManager.controller.input.movement.x}, y=${ControlManager.controller.input.movement.y}, z=${ControlManager.controller.input.movement.z}`);
            }
        }
    }
