...
export default class FPSController {
            ...
            if (this.input.movement.lengthSq() > 0) {
                ...
                if (PlayersManager.self._movementBlocked) {
                    ...
                    if (dotWithNormal < 0) {
                        ...
                        // Project movement onto the surface defined by both the collision normal and ground normal
                        // This ensures we slide along the intersection of these two surfaces
                        ...
                        if (slideDir.lengthSq() > 0.01) {
                            ...
                            worldSpaceMovement.copy(slideDir).multiplyScalar(slideComponent * 0.7); // Reduce speed by 30%
                            
                            console.log("FPS movement projected along collision surface");
                        } else {
                ...
                if (PlayersManager.self.activeCollisions && PlayersManager.self.activeCollisions.length > 0) {
                    ...
                    // Process only the most significant collision to avoid competing adjustments
                    if (recentCollisions.length > 0) {
                        // Find the collision with the most head-on impact
                        let strongestCollision = recentCollisions[0];
                        let strongestImpact = worldSpaceMovement.dot(strongestCollision.normal);
                        
                        recentCollisions.forEach(collision => {
                            const impact = worldSpaceMovement.dot(collision.normal);
                        ...
                        // Only handle the strongest collision to prevent jitter from competing adjustments
                        if (strongestImpact < 0) {
                            const surfaceNormal = PlayersManager.self.surfaceNormal;
                            
                            // FIXED: Better projection algorithm to stop camera clipping through objects
                            // First get perpendicular component of movement to both normals
                            const projectedMovement = new Vector3();
                            
                            // Create a direction that's perpendicular to both the collision normal and surface normal
                            const slideDirection = new Vector3().crossVectors(
                                surfaceNormal, strongestCollision.normal
                            ).normalize();
                            
                            if (slideDirection.lengthSq() > 0.01) {
                                // Project movement onto this slide direction for proper sliding along object boundaries
                                const slideComponent = worldSpaceMovement.dot(slideDirection);
                                projectedMovement.copy(slideDirection).multiplyScalar(slideComponent);
                                
                                // Scale down the slide speed to prevent clipping through corners
                                projectedMovement.multiplyScalar(0.8);
                                
                                worldSpaceMovement.copy(projectedMovement);
                                movementBlocked = true;
                            } else {
                                // Fallback: if slide direction is invalid, just stop movement completely
                                // This prevents clipping through in corner cases
                                worldSpaceMovement.set(0, 0, 0);
                                movementBlocked = true;
                            }
                            
                            // If movement is now very small, just cancel it entirely
                            if (worldSpaceMovement.lengthSq() < 0.0005) {
                                worldSpaceMovement.set(0, 0, 0);
                            }
                        }
                    }
    ...
    static landing(normal) {
        ...
        try {
            ...
            // NEW: Don't process redundant landing events too close together
            const now = Date.now();
            if (player._lastLandingTime && now - player._lastLandingTime < 200) {
                console.log("Skipping redundant landing event - too soon after last one");
                return;
            }
