...
export default class FPSController {
    ...
    static update(player, camera) {
        ...
        try {
            ...
            // FIXED: Improved state check to prevent surface collisions from causing falling
            // Choose aiming mode based on a more accurate ground state check
            if ((PlayersManager.self.falling && !PlayersManager.self.standingOnObject) && 
                 // Additional check: don't enter falling mode during ground-level collisions
                !PlayersManager.self.currentlyColliding) {
                this.falling();
                
                // NEW: Debug log when using falling controls
                if (PlayersManager.self.currentlyColliding) {
                    console.log("Using falling controls despite collision - check physics");
                }
            } else {
                // If we're standing on object OR on planet surface OR colliding at ground level, use grounded
                this.grounded();
                
                // NEW: Better detection and correction of mistaken falling state
                if (PlayersManager.self.falling) {
                    // If any of these conditions are true, player shouldn't be falling
                    const shouldNotBeFalling = 
                        PlayersManager.self.standingOnObject ||
                        PlayersManager.self.currentlyColliding ||
                        (PlayersManager.self.soi && PlayersManager.self._lastGroundDistance < 1.0);
                    
                    if (shouldNotBeFalling) {
                        console.log("Correcting erroneous falling state during collision");
        ...
        // NEW: Anti-inversion safety check - always run this
    ...
    static enforceCameraOrientation(camera, player) {
        ...
        try {
            // Get the current up vector (planet normal or global up)
            ...
            // Check for camera inversion: camera up should be roughly aligned with world up
            const upAlignment = cameraUp.dot(upVector);
            
            // ENHANCED: More aggressive inversion detection for test cube collisions
            if (upAlignment < 0.5) { // Changed from 0.3 to 0.5 for earlier intervention
                console.log("Detected possible camera inversion - fixing orientation");
                
                // Force camera up to match the correct up vector
                camera.up.copy(upVector);
                
                // ENHANCED: More aggressive correction for test cube collisions
                if (upAlignment < 0.0) { // Any negative alignment means severe inversion
                    // NEW: Check if this occurred right after a collision with a test cube
                    const isTestCubeCollision = player._lastCollisionObject?.userData?.type === 'testCube';
                    const isRecentCollision = player._lastCollisionTime && 
                        (Date.now() - player._lastCollisionTime < 500);
                        
                    if (isTestCubeCollision && isRecentCollision) {
                        console.log("Severe camera inversion after test cube collision - applying emergency fix");
                        
                        // EMERGENCY FIX: Complete camera orientation reset
                        // 1. First force the correct up vector
                        camera.up.copy(upVector);
                        
                        // 2. Completely reset rotation to match player handle
                        if (player.handle) {
                        ...
                        // Start from identity quaternion
                        const correctionQuat = new Quaternion();
                        
                        // Find the right and forward vectors for proper orientation
                        const forward = cameraForward.projectOnPlane(upVector).normalize();
                        if (forward.lengthSq() < 0.1) {
                            // If projection failed, use a default forward
                            forward.set(1, 0, 0).projectOnPlane(upVector).normalize();
                        }
                        
                        const right = new Vector3().crossVectors(upVector, forward).normalize();
                        
                        // Build a corrected orientation matrix
                        // Create rotation matrix from orthogonal vectors
                        const rotMatrix = new Matrix4().makeBasis(
                            right,
                            upVector,
                            forward.clone().negate() // negate for camera space
                        );
                        
                        // Extract quaternion from rotation matrix
                        correctionQuat.setFromRotationMatrix(rotMatrix);
                        
                        // Apply the correction with blending to avoid jarring changes
                        camera.quaternion.slerp(correctionQuat, 0.8);
                        
                        // Also reset the pitch to sensible value
                        this.surfacePitch = 0;
                        
                        console.log("Applied emergency camera orientation correction");
                    }
                }
            }
            
            // NEW: Additional check specifically for upside-down camera
            const worldUp = new Vector3(0, 1, 0);
            const worldUpAlignment = camera.up.dot(worldUp);
            
            if (worldUpAlignment < -0.7) {
                console.log("Camera is completely upside down in world space - emergency fix");
                camera.up.set(0, 1, 0);
                
                // Create a corrected forward direction
                const forward = new Vector3(0, 0, -1);
                
                // Look at a point in front of player
                const target = player.position.clone().add(forward);
            ...
            // NEW: Check if camera has undergone a large, sudden change in orientation
            // This can help detect when the camera has been incorrectly modified
            if (player._lastGoodCameraOrientation) {
                const lastQuat = player._lastGoodCameraOrientation.quaternion;
                const currentQuat = camera.quaternion.clone();
                
                // Calculate angle between last good orientation and current
                const angle = lastQuat.angleTo(currentQuat);
                
                // If the change is too large and sudden (more than 45 degrees)
                if (angle > Math.PI/4 && !player._orientationFixInProgress) {
                    console.log("Detected sudden large camera orientation change - may indicate camera break");
                    
                    // Check if this happened right after a collision
                    const isRecentCollision = player._lastCollisionTime && 
                        (Date.now() - player._lastCollisionTime < 300);
                        
                    if (isRecentCollision) {
                        console.log("Large camera change after collision - initiating recovery");
                        
                        // Set recovery flag
                        player._orientationFixInProgress = true;
                        player._orientationFixStartTime = Date.now();
                        
                        // Smoothly restore previous good orientation
                        camera.quaternion.slerp(lastQuat, 0.7);
                        
                        // Ensure up vector is correct
                        camera.up.copy(upVector);
                        
                        // Clear fix flag after a short time
                        setTimeout(() => {
                            player._orientationFixInProgress = false;
                        }, 500);
                    }
                }
            }
            
            // Store the last known good camera orientation whenever we're not in a collision
            // and not currently fixing orientation
            if (!player._collisionInProgress && !player._cameraFrozen && 
                !player._orientationFixInProgress && camera.up.dot(upVector) > 0.9) {
