...
export default class FPSController {
    ...
    static falling() {
        ...
        if (typeof PlayersManager === 'undefined' || !PlayersManager || !PlayersManager.self)
            return;
        ...
        try {
            ...
            if (this.input.movement.lengthSq() > 0) {
                ...
                const cameraDirection = new Vector3(0, 0, -1).applyEuler(Engine.camera.rotation);
                const cameraRight = new Vector3(1, 0, 0).applyEuler(Engine.camera.rotation);
                
                const moveDirection = new Vector3();
                moveDirection.addScaledVector(cameraDirection, this.input.movement.z);
                moveDirection.addScaledVector(cameraRight, this.input.movement.x);
                moveDirection.y = 0; // No vertical movement from input
                moveDirection.normalize();
                
                // Apply movement to player position
                player.position.addScaledVector(moveDirection, movementSpeed);
                
                // Update handle position to match player
                if (player.handle) {
    ...
    static landing(up) {
        ...
        try {
            ...
            if (Engine.camera) {
                ...
                ObjectManager.alignObjectToSurface(Engine.camera, up, alignOptions);
