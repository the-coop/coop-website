...
export default class FPSController {
                ...
                if (PlayersManager.self.activeCollisions && PlayersManager.self.activeCollisions.length > 0) {
                    ...
                    for (const collision of recentCollisions) {
                        ...
                        if (movementIntoCollision) {
                            ...
                            // Remove this component from movement (projection onto collision plane)
                            worldSpaceMovement.sub(blockedComponent);
                            
                            // Apply some friction along the surface
                            worldSpaceMovement.multiplyScalar(0.8);
                            
                            console.log("FPSController: Blocked movement component going into collision");
                            movementBlocked = true;
                            
                            // If movement is now very small, just cancel it entirely
                            if (worldSpaceMovement.lengthSq() < 0.001) {
                            ...
                            if (testCubes.length > 0) {
                                ...
                                for (const testCube of testCubes) {
                                    ...
                                    if (worldSpaceMovement.dot(toTestCube) > 0) {
                                        // Scale back movement or completely block it
                                        worldSpaceMovement.multiplyScalar(0.2);
                                        console.log("Movement limited due to nearby test cube");
                                        break;
                                    }
                                }
                            }
    ...
    // Add a debug flag to aid in landing troubleshooting
    ...
    static landing(normal) {
        ...
        try {
            ...
            // If landing on a highly angled surface, realign camera
            if (Math.abs(alignmentDot) < 0.95) {
                ...
                if (rotationAxis.lengthSq() > 0.001) {  // Valid rotation axis
                    ...
                    // Apply the rotation to the camera - use a stronger slerp factor if coming from an object
                    if (fromObject) {
                        // More aggressive camera correction when landing from objects
                        Engine.camera.quaternion.slerp(alignQuat, 0.8);
                    } else {
                        // Standard correction for regular landings
                        Engine.camera.quaternion.slerp(alignQuat, 0.5);
                    }
                }
            }
