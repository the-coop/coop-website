        ...
        try {
            ...
            if (this.input.movement.lengthSq() > 0) {
                ...
                if (PlayersManager.self.activeCollisions && PlayersManager.self.activeCollisions.length > 0) {
                    ...
                    // Also check if player is currently colliding (from physics system)
                    if (PlayersManager.self.currentlyColliding) {
                        // Apply additional movement damping when colliding
                        worldSpaceMovement.multiplyScalar(0.9);
                        
                        if (typeof window !== 'undefined' && window.updateCollisionInfo) {
                            window.updateCollisionInfo('Active Collision', null, null, true);
                        }
                    }
                }
