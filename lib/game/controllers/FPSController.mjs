...
export default class FPSController {
    ...
    static landing(up) {
        ...
        try {
            ...
            const alignmentDot = camUp.dot(up);
            ...
            if (Math.abs(alignmentDot) < 0.95 || significantFall) {
                ...
                const rotationAxis = new Vector3().crossVectors(camUp, up).normalize();
    ...
    static update(player, camera) {
        ...
        if (player && camera && player._planetSurfaceNormal) {
            camera.up.copy(player._planetSurfaceNormal);
        }
    ...
    static enforceCameraOrientation(camera, player) {
        ...
        try {
            // CRITICAL FIX: Always use planet surface normal as reference
            // This ensures camera orientation stays consistent with planet geometry
