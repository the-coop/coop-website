...
export default class CarController {
    ...
    static performDuplicateControlCheck() {
        ...
        for (const vehicle of VehicleManager.vehicles) {
            ...
            if (vehicle.userData.isOccupied || vehicle.userData.hasPlayerInside || 
                ...
                if (vehicle !== VehicleManager.currentVehicle) {
                    console.warn(`Found vehicle incorrectly marked as player-controlled: ${vehicle.userData.name || 'unnamed'}`);
        ...
        if (playerControlledCount > 1) {
            ...
            for (const vehicle of problematicVehicles) {
                console.log(`Fixing control state for: ${vehicle.userData.name || 'unnamed'}`);
                ...
                // ADDED: Clear any collision states that might cause false positives
                if (vehicle.userData.currentlyColliding) {
                    vehicle.userData.currentlyColliding = false;
                }
                if (vehicle.userData.collidingWith) {
                    vehicle.userData.collidingWith = [];
                }
    ...
    static handleCarMovement(car, deltaTime) {
        ...
        // ADDED: Calculate if car is on surface without using collision system
        const heightOffset = car.userData.fixedHeightOffset || 3.0;
        const onSurface = Math.abs(distanceToPlanet - (planet.radius + heightOffset)) < 0.2;
        car.userData.onSurface = onSurface;
        
        // CRITICAL FIX: Clear any collision state with planets
        if (car.userData.collidingWith) {
            car.userData.collidingWith = car.userData.collidingWith.filter(obj => 
                !obj.userData || !obj.userData.isPlanet
            );
        }
        
        // CRITICAL FIX: Don't count planet contact as a collision
        if (car.userData.currentlyColliding && car.userData._lastCollisionWith && 
            car.userData._lastCollisionWith.userData && car.userData._lastCollisionWith.userData.isPlanet) {
            car.userData.currentlyColliding = false;
            car.userData._lastCollisionWith = null;
        }
