...
export default class CarController {
    ...
    static reset() {
        ...
        try {
            // Stabilize the car to prevent rotation glitches
            this._stabilizeCarOrientation(car);
            
            // Complete camera reset - critical for proper initialization
            ...
            // SIMPLIFIED: Direct camera placement - avoid complex transitions
            // ------------------------------------------
            ...
            // 4. Calculate look target with minimal offset for better view 
            const lookAtPos = car.position.clone();
            this._lastLookAtPos = lookAtPos.clone();
            
            // 5. Orient camera with direct lookAt
            Engine.camera.up.copy(surfaceNormal);
            Engine.camera.lookAt(lookAtPos);
            
            // Ensure player handle is set to vehicle while in car
            if (PlayersManager.self && !PlayersManager.self._originalHandle) {
                PlayersManager.self._originalHandle = PlayersManager.self.handle;
                PlayersManager.self.handle = car;
            }
            
            console.log('Camera positioned directly behind car');
