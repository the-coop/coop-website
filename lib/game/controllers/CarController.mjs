...
export default class CarController {
    ...
    // Setup camera behind car with improved approach
    static setupCamera() {
        const car = VehicleManager.currentVehicle;
        ...
        try {
            ...
            console.log("Setting up car camera with improved orientation handling");
            
            // Set camera up vector based on planet surface
            Engine.camera.up.copy(surfaceNormal);
            
            // Attach camera to car to inherit car movement
            Engine.scene.remove(Engine.camera);
            car.add(Engine.camera);
            
            // Position camera relative to car with better 3rd person view
            Engine.camera.position.set(0, this.cameraHeight, this.cameraDistance);
            
            // IMPROVED: Use lookAt instead of direct rotation setting
            // Look at a point ahead of the car for better forward view
            const lookTarget = new Vector3(0, 0, -this.cameraLookOffset);
            Engine.camera.lookAt(lookTarget);
            
            // Initialize camera state for tracking camera movement
            ...
            // Set flag on car to indicate camera is attached
            car.userData._cameraAttached = true;
            car.userData._cameraAttachTime = Date.now();
            
            console.log("Camera configured with improved 3rd person view");
            return true;
        } catch (e) {
    ...
    static update() {
        ...
        try {
            ...
            // NEW: Update camera with rotation input
    ...
    // NEW: Update camera position and rotation
    static updateCamera(car) {
        if (!car || !Engine.camera || Engine.camera.parent !== car) return;
        
        try {
            // Process camera rotation input similar to FPS controller
            ...
            const xPos = horizontalDistance * Math.sin(this.cameraRotation.yaw);
            const zPos = horizontalDistance * Math.cos(this.cameraRotation.yaw);
            
            // Calculate target camera position
            const targetPosition = new Vector3(
                xPos,
                this.cameraHeight + verticalOffset,
                zPos
            );
    ...
    static turnCarOnSurface(car, steeringInput) {
        ...
        if (car.userData.falling) {
            // ENHANCED: When falling/airborne, allow free rotation like player
            ...
            // ORIGINAL: When on ground, use surface-aligned turning
            // Calculate turn amount based on speed and steering
            const speed = Math.abs(car.userData.speed);
            const turnFactor = 0.02 * Math.min(1, speed / 8);
            const turnAmount = steeringInput * turnFactor;
    ...
    // Clean up when exiting car - improved camera handling
    static cleanup() {
        const car = VehicleManager.currentVehicle;
        if (!car) return;
        ...
        try {
            ...
            // REMOVED: Force immediate alignment to planet surface on exit
            // This allows the vehicle to maintain its current orientation when exited
            // The physics system will naturally align it when needed based on falling state
    ...
    static alignCarToSurface(car, forcedLerpFactor = null) {
        ...
        try {
            ...
            // UPDATED: Use centralized alignment method
            ObjectManager.alignObjectToSurface(car, surfaceNormal, {
