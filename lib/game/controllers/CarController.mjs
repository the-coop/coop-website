...
export default class CarController {
    ...
    static update() {
        ...
        try {
            ...
            // FIXED: Apply stronger surface alignment to keep car properly oriented
    ...
    static alignCarToSurface(car, forcedLerpFactor = null) {
        ...
        try {
            ...
            if (window.ObjectManager && typeof window.ObjectManager.alignObjectToSurface === 'function') {
                ...
                // Fallback direct alignment if ObjectManager not available
                this.directAlignToSurface(car, surfaceNormal, alignmentFactor);
            }
        } catch (e) {
    ...
    // NEW: Add direct alignment method as fallback
    static directAlignToSurface(car, surfaceNormal, lerpFactor) {
        ...
        try {
            ...
            console.error("Error in direct alignment:", err);
        }
    }

    // NEW: Add missing method to check wheel surface contact
    static checkWheelSurfaceContact(car) {
        if (!car || !car.userData || !this.vehicleState.wheelPositions) return;
        ...
        try {
            ...
            // Store these contact points for use in physics
            this.vehicleState.surfaceContactPoints = [];
            
            // Create ground contact points under each wheel position
            // This helps with detecting if the car is properly on the ground
            for (const wheelPos of this.vehicleState.wheelPositions) {
                if (!wheelPos.wheel) continue;
                
                // Get wheel position in world space
                const wheelWorldPosition = new Vector3();
                wheelPos.wheel.getWorldPosition(wheelWorldPosition);
                
                // Create a contact point directly below this wheel
                const contactPoint = wheelWorldPosition.clone().addScaledVector(surfaceNormal, -1);
                this.vehicleState.surfaceContactPoints.push(contactPoint);
                
                // Calculate distance to planet surface
                const distanceToPlanet = wheelWorldPosition.distanceTo(planetCenter);
                const heightAboveSurface = distanceToPlanet - planetRadius;
                
                // Store the lowest wheel height
                if (this.vehicleState.lowestWheelHeight === undefined || 
                    heightAboveSurface < this.vehicleState.lowestWheelHeight) {
                    this.vehicleState.lowestWheelHeight = heightAboveSurface;
                }
            }
            
            // Determine if vehicle is stable (wheels on ground)
            const targetHeight = car.userData.fixedHeightOffset || 3.0;
            const heightDifference = Math.abs(this.vehicleState.lowestWheelHeight - targetHeight);
            
            // Consider vehicle stable if height is close to target
            this.vehicleState.isStable = heightDifference < 1.0;
            
            // Reset air time if stable
            if (this.vehicleState.isStable) {
                this.vehicleState.airTime = 0;
                car.userData.falling = false;
            } else if (car.userData.falling) {
                this.vehicleState.airTime += 1/60; // Increment air time
            }
            
        } catch (err) {
            console.error("Error checking wheel contact:", err);
        }
    }
}
