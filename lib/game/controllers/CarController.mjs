...
export default class CarController {
    ...
    // SIMPLIFIED: Camera configuration (now relative to car's local space)
    static fixedCameraOffset = {
        x: 0,              // Centered on car's x-axis
        y: 5,              // Height above car
        z: 15,             // Distance behind car
        lookAheadZ: -10    // Look ahead point in front of car
    };

    // REMOVED: No longer need separate camera state since camera moves with car
    ...
    static reset() {
        console.log('Initializing Car Controller with attached camera');
        ...
        try {
            ...
            // SIMPLIFIED: Directly attach camera to car like in AirplaneController
            if (Engine.camera) {
                ...
                // Add camera directly to car
                car.add(Engine.camera);
                
                // Position camera behind and above car in local space
                Engine.camera.position.set(0, this.fixedCameraOffset.y, this.fixedCameraOffset.z);
                ...
                Engine.camera.lookAt(new Vector3(0, 0, this.fixedCameraOffset.lookAheadZ));
                
                console.log("Camera attached to car with fixed positioning");
            } else {
                console.error('No camera available for CarController');
            }
            
            console.log('Car Controller initialized with attached camera');
        } catch (e) {
    ...
    static update() {
        ...
        try {
            ...
            // REMOVED: No need to update camera position, it's attached to car
    ...
    static cleanup() {
        ...
        try {
            ...
            // SIMPLIFIED: Detach camera from car and add to scene
