    ...
    static handleCarMovementOnPlanet(car) {
        ...
        try {
            ...
            if (Math.abs(accelerationInput) > 0.1) {
                // IMPROVED: Use trueForward for acceleration to follow planet curvature
                // This ensures car always moves along the surface
                const accelerationForce = 0.03;
                
                // Get forward alignment factor to adjust speed
                const forwardAlignment = Math.max(0.3, trueForward.dot(carForward));
                
                // Add acceleration to velocity using the TRUE forward direction
            ...
            if (car.userData.velocity && car.userData.velocity.lengthSq() > 0.0001) {
                ...
                // CRITICAL FIX: Project velocity onto the planet surface tangent plane
                // This prevents the car from flying off the surface
                ...
                // FIXED: Direct height correction to prevent floating
            ...
            // FIXED: Apply more aggressive surface alignment after position change
            // This ensures the car bottom stays aligned with the planet surface
