...
export default class CarController {
    ...
    static moveCarOnSurface(car) {
        ...
        try {
            ...
            // Maintain height above planet - more gentle adjustment
            const planetCenter = planet.object.position;
            ...
            // Only correct if height difference is significant
            if (Math.abs(currentDistance - targetDistance) > 0.2) {
                const correctionFactor = 0.2; // Gentler correction
                const newPosition = planetCenter.clone().add(
                    toVehicle.normalize().multiplyScalar(
                        currentDistance + (targetDistance - currentDistance) * correctionFactor
                    )
                );
            ...
            // Re-align car to the new surface normal - more gently
            this.alignCarToSurface(car);
    ...
    // Align car orientation to planet surface - more gently
    static alignCarToSurface(car) {
        ...
        try {
            ...
            // Apply gentler alignment - reduced from 0.2 to 0.1
