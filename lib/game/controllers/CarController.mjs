...
export default class CarController {
    ...
    static maxSteeringAngle = 0.8; // INCREASED from 0.6 to 0.8 for tighter turning
    static steeringSmoothFactor = 0.7; // DECREASED from 0.75 to 0.7 for more responsive steering
    ...
    static turnRadius = 6; // DECREASED from 8 to 6 for tighter turns
    static maxSteeringRate = 0.05; // INCREASED from 0.03 to 0.05 for faster turning
    ...
    static reset() {
        ...
        if (car.userData && car.userData.planet) {
            ...
            // Force immediate alignment to surface - using standard alignment instead of snap
    ...
    static handleCarMovementOnPlanet(car) {
        ...
        try {
            ...
            if (Math.abs(steeringInput) > 0.05) { // DECREASED threshold from 0.1 to 0.05 for more responsive steering
                ...
                const turnFactor = Math.min(1.0, currentSpeed / 2.0); // DECREASED from 3.0 to 2.0 for even tighter turning
            ...
            if (Math.abs(accelerationInput) > 0.1) {
                // REDUCED: Lower acceleration force to make car slower
                const accelerationForce = 0.015; // REDUCED from 0.03 to 0.015
    ...
    // FIXED: Update wheel orientation to correctly represent steering
    static updateWheelOrientation(car, steeringAngle) {
        ...
        try {
            Object.entries(car.userData.wheels).forEach(([wheelName, wheel]) => {
                ...
                // FIXED: Complete reset of wheel rotation to ensure proper orientation
                if (wheelName.includes('front')) {
                    // Set steering for front wheels
                    // IMPORTANT: Using complete rotation reset to avoid axis conflicts
    ...
    // SIMPLIFIED: Minimal cleanup on exit - just reset steering
    static cleanupOnExit(car) {
        if (!car || !car.userData) return;
        
        console.log("Minimal cleanup of car steering on exit");
        
        try {
            // Reset controller's steering angle
            this.steeringAngle = 0;
            
            // Reset car's stored steering angle
            if (car.userData) {
            ...
            // Mark controller as no longer managing this vehicle
            car.userData._controlledByCarController = false;
            car.userData._heightManagedByController = false;
        } catch (err) {
            console.error("Error resetting car steering on exit:", err);
        }
    }
}
