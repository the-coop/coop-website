...
export default class CarController {
    ...
    static handleCarMovement(car, deltaTime) {
        ...
        // FIXED: Use direct input.x without negation so A = left, D = right
        const steeringInput = this.input.movement.x; // Remove the negative sign to fix inverted steering
        ...
        // IMPROVED: Realistic car steering with proper turning radius and pivot point
        ...
        // Smoothly interpolate wheel angle for natural steering feel
        const steerSpeed = 5.0 * deltaTime; 
        ...
        if (Math.abs(car.userData.speed) > 0.5) {
            // IMPROVED: Calculate proper turning radius based on wheel geometry
            // Car dimensions (approximated based on typical sedan proportions)
            const wheelBase = 6; // Distance between front and rear axles
            ...
            // As wheel angle approaches zero, turning radius approaches infinity
            const steeringAngle = car.userData.wheelSteerAngle;
            const minTurningRadius = wheelBase / Math.tan(Math.abs(steeringAngle) + 0.001); // Add small constant to prevent division by zero
            
            // Add understeer effect at higher speeds (larger turning radius)
            const understeerFactor = 1 + (Math.abs(car.userData.speed) / maxForwardSpeed) * 0.5;
            ...
            // Direction of rotation depends on forward/reverse
            const reverseFactor = car.userData.speed < 0 ? -1 : 1;
            
            // Calculate actual turn amount this frame
            const turnAmount = angularVelocity * reverseFactor * deltaTime;
            
            // Debug info about turning
            if (Math.abs(steeringAngle) > 0.01) {
                console.log(`🚗 Turn: radius=${effectiveTurningRadius.toFixed(1)}, angle=${(turnAmount * 180/Math.PI).toFixed(2)}°`);
            }
