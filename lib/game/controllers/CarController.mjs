...
export default class CarController {
    ...
    static forceAlignCarToSurface(car, surfaceNormal) {
        ...
        try {
            ...
            if (projectedForward.lengthSq() > 0.001) {
                ...
                // Recalculate proper forward vector to ensure orthogonality
                const trueForward = new Vector3().crossVectors(rightVector, surfaceNormal).normalize();
                
                // Store for later use
                car.userData._carForward = trueForward.clone();
        ...
        try {
            ...
            if (Math.abs(steeringInput) > 0.1) {
                ...
                if (currentSpeed > 0.5) {
                    ...
                    const turnAngle = effectiveTurnRate * (car.userData.speed > 0 ? 1 : -1);
                    
                    // CRITICAL FIX: Use the surface normal for rotation to stay aligned to planet
                    const steerQuat = new Quaternion().setFromAxisAngle(surfaceNormal, turnAngle);
                    
                    // FIXED: Apply rotation correctly to prevent unintended spinning
                    // Instead of premultiplying (which can cause unexpected rotations),
                    // we'll properly realign the car after the turn
            ...
            if (car.userData.velocity && car.userData.velocity.lengthSq() > 0.0001) {
                ...
                // IMPROVED: More direct height correction
                // Ensure car stays at correct height above planet surface
                const planetCenter = planet.object.position;
                ...
                // IMPROVED: Snap directly to the correct height (no lerping)
                // This keeps the car firmly on the ground like the FPS controller
            ...
            // CRITICAL FIX: More aggressive surface alignment after movement
            // Apply alignment AFTER position changes to ensure car stays on surface
            this.alignCarToSurface(car, surfaceNormal);
    ...
    // IMPROVED: More aggressive surface alignment for car
    static alignCarToSurface(car, surfaceNormal) {
        if (!car || !surfaceNormal) return;
        ...
        try {
            ...
            // FIXED: Use stronger alignment strength to keep car firmly on ground
            // Create rotation quaternion with strong alignment
            const strength = Math.min(1.0, this.alignmentStrength * 5.0); // Increased from 3.0 to 5.0
            const deltaAngle = angle * strength;
    ...
    // NEW: Dedicated method for updating wheel orientation
