...
export default class AirplaneController {
    ...
    static update() {
        ...
        // Check for E key press and explicitly return 'exit' to trigger vehicle exit
        if (Engine.keyStates && (Engine.keyStates['e'] || Engine.keyStates['E'])) {
            console.log('E key detected in AirplaneController.update(), requesting exit');
            // Don't clear the key state here - let ControlManager handle it
            return 'exit';
        }
