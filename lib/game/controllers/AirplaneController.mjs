...
export default class AirplaneController {
    ...
    static update() {
        ...
        if (airplane.userData.planet) {
            ...
            // IMPROVED: Update camera up vector based on airplane state
            if (!airplane.userData.falling && airplane.userData.altitude <= 1) {
                // On ground - use surface normal directly, like FPS controller's grounded state
                Engine.camera.up.copy(toVehicle);
            } else {
                // In flight - blend between airplane's up vector and planet's surface normal
                // This provides better visual orientation while allowing free rotation
                const airplaneUp = new Vector3(0, 1, 0).applyQuaternion(airplane.quaternion);
                const blendFactor = Math.min(0.3, 1 / (airplane.userData.altitude || 1));
                const upVector = airplaneUp.clone().lerp(toVehicle, blendFactor);
    ...
    static handleAirplaneMovement(airplane, deltaTime) {
        ...
        if (airplane.userData.planet) {
            ...
            // CRITICAL IMPROVEMENT: Update falling state based on altitude
            // This ensures alignment behaves correctly in Physics system
            const minFlyingHeight = 5; // Set minimum height to be considered flying
            if (altitude > minFlyingHeight) {
                airplane.userData.falling = true; // In the air = falling state (no surface alignment)
                airplane.userData.onSurface = false;
            } else if (altitude <= 1) {
                airplane.userData.falling = false; // On ground = not falling (apply surface alignment)
        ...
        // IMPROVED: Different rotation handling based on flying vs grounded state
        if (airplane.userData.falling || altitude > 1) {
            // IN AIR: Use world-aligned rotations like FPS controller's falling state
            ...
            // ON GROUND: More restricted movement aligned to surface
            // Create quaternion for limited steering when on ground
            if (Math.abs(rollInput) > 0.01) {
                // Limited turning on ground using surface normal as rotation axis
                const turnRate = 1.0 * controlEffectiveness;
                const turnRotation = new Quaternion().setFromAxisAngle(
                    surfaceNormal,
                    -rollInput * turnRate * deltaTime
                );
                airplane.quaternion.premultiply(turnRotation);
            }
            
            // When on ground, make sure we're aligned to surface
            if (surfaceNormal) {
                // Explicitly set up vector to surface normal
                airplane.up.copy(surfaceNormal);
                
                // Apply surface alignment using VehicleManager's method
                VehicleManager.alignVehicleToPlanetSurface(airplane, surfaceNormal, 0.2);
            }
        }
        ...
        if (airplane.userData.planet && (!canGenerateLift || altitude < 1)) {
            ...
            if (onGround) {
                ...
                // IMPROVED: Apply surface alignment only when on ground
                // This is like FPS controller's grounded state
                VehicleManager.alignVehicleToPlanetSurface(airplane, surfaceNormal, 0.3, true);
            } else {
                airplane.userData.onSurface = false;
            }
        }
