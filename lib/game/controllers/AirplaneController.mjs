...
export default class AirplaneController {
    ...
    static update() {
        ...
        // CRITICAL FIX: Pass movement inputs to VehicleManager and log for debugging
        ...
        if (Math.abs(this.input.movement.x) > 0.01 || Math.abs(this.input.movement.z) > 0.01 || Math.abs(this.input.movement.y) > 0.01) {
            console.log(`Airplane input: x=${this.input.movement.x.toFixed(2)}, y=${this.input.movement.y.toFixed(2)}, z=${this.input.movement.z.toFixed(2)}`);
        }
