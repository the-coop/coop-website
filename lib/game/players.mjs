...
export default class PlayersManager {
    ...
    static spawn(self = true, customPosition = null) {
        const player = {
            ...
            position: customPosition || new Vector3(5100, 50, 0),  // Added 50 units of height by default
        ...
        if (self) {
            ...
            // ENHANCED: Notify player they're falling with gravity
            if (typeof window !== 'undefined' && window.gameNotify) {
                window.gameNotify("You're spawning above the planet - gravity will pull you down");
            }
        }
    ...
    static initializePlayerCollider(player, forceRecreation = false) {
        ...
        try {
            ...
            if (forceRecreation || !player.collidable) {
                ...
                // FIXED: Adjust player collider dimensions to match camera position and prevent clipping
                const playerDimensions = {
                    width: 1.4,    // Same width for proper side collisions 
                    height: 2.8,   // Increased from 1.8 to 2.8 to match higher camera position
                    depth: 1.4     // Same depth for proper collisions from front/back
                };
