...
export default class PlayersManager {
    ...
    static initializePlayerCollider(player, forceRecreation = false, dimensions = null) {
        ...
        try {
            ...
            // ADDED: Set player to use sphere collision by default
            player.handle.userData.useSphericalCollision = true;
            
            // ADDED: Set player mass for momentum-based collision response
            player.handle.userData.mass = Physics.DEFAULT_MASSES.player;
            
            // ENHANCED: Store mass in player object directly too
            ...
            if (forceRecreation || !player.collidable) {
                ...
                // Get dimensions based on collision shape preference
                const useSphericalCollision = player.handle.userData.useSphericalCollision;
                let playerDimensions;
                
                if (dimensions) {
                    // Use provided dimensions if specified
                    playerDimensions = dimensions;
                } else if (useSphericalCollision) {
                    // Use sphere dimensions
                    const radius = player.handle.userData.collisionRadius || 1.0;
                    playerDimensions = {
                        width: radius * 2,
                        height: radius * 2,
                        depth: radius * 2
                ...
                // ADDED: Mark this as a sphere collider for the collision system
                if (player.collidable && useSphericalCollision) {
                    player.collidable.useSphereCollision = true;
                    player.collidable.sphereRadius = playerDimensions.width / 2;
                }
                ...
                if (player.collidable) {
                    ...
                    console.log(`Player collider created successfully as ${useSphericalCollision ? 'sphere' : 'box'}`);
                ...
                if (!updated) {
                    ...
                    return this.initializePlayerCollider(player, true, dimensions);
                }
            }
