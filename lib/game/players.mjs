...
export default class PlayersManager {
    ...
    static spawn(self = true, customPosition = null) {
        console.log(`Spawning ${self ? 'local' : 'remote'} player at position:`, customPosition || new Vector3(5100, 2000, 0));
        ...
        console.log("Player initial SOI:", player.soi ? player.soi.name : "None");
        ...
        if (self) {
            ...
            // Make player reference globally available for debug
            if (typeof window !== 'undefined') {
                window.currentPlayer = player;
                console.log("Local player reference set and available as window.currentPlayer");
            }
        ...
        const collider = this.initializePlayerCollider(player, true); // true = force recreation
        console.log("Player collision initialized:", collider ? "SUCCESS" : "FAILED");
        
        this.players.push(player);
        
        // FIXED: Use the directly imported ObjectManager instead of window.ObjectManager
        ObjectManager.validatePlayerCollision(player);
        console.log("Player registered with global ObjectManager");
