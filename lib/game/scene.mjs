...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        if (this.planets[0]) {
            ...
            // Create cars at randomly distributed heights - ADJUSTED to be less extreme
            const createDramaticCars = (count = 4) => {
                for (let i = 0; i < count; i++) {
                    ...
                    // FIXED: More reasonable heights between 200-500 units
                    const height = 200 + Math.random() * 300;
                    
                    // Create car with this height
                    VehicleManager.createCar(alpha, latitude, longitude, height);
                }
                console.log(`Created ${count} cars on ${alpha.name} at better heights`);
            };
