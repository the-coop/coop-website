...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        // DRAMATICALLY INCREASED: Even more extreme spawn heights
        // Alpha planet is at [0, -4000, 0]
        const alphaVehiclePositions = [
            // ULTRA-HIGH ALTITUDE - matching player spawn heights for dramatic entry
            new Vector3(-800, 15000, 600),    // 19000 units above planet center
            new Vector3(800, 15500, -600),    // 19500 units above planet center
            new Vector3(600, 16000, 800),     // 20000 units above planet center 
            new Vector3(-600, 16500, -800),   // 20500 units above planet center
            new Vector3(-300, 17000, 900),    // 21000 units above planet center
            new Vector3(900, 17500, 300),     // 21500 units above planet center
            new Vector3(400, 18000, -900),    // 22000 units above planet center
            new Vector3(-900, 18500, -400)    // 22500 units above planet center
        ];
        ...
        alphaVehiclePositions.forEach((position, index) => {
            try {
                ...
                // CRITICAL FIX: Use position directly with forced suppressAlignment=true
                // This ensures the vehicle maintains its exact position in world space
                ...
                if (car) {
                    ...
                    // CRITICAL FIX: Set extremely low gravity factor for distant vehicles
                    car.userData.gravityFactor = 0.01; // Even lower gravity for more dramatic entry
                    ...
                    // ADDED: Force matrix update to lock in position
                    car.updateMatrix();
        ...
        // Create airplanes with absolute positions at even higher altitudes
        try {
            if (typeof VehicleManager.createVehicleWithPosition === 'function') {
                const planePositions = [
                    // EVEN HIGHER positions for airplanes
                    new Vector3(-700, 19000, -700),  // 23000 units above planet center 
                    new Vector3(700, 19500, 700),    // 23500 units above planet center
                    new Vector3(0, 20000, -1000)     // 24000 units above planet center
                ];
                ...
                planePositions.forEach((position, index) => {
                    ...
                    if (airplane) {
                        ...
                        // CRITICAL FIX: Set extremely low gravity factor for distant vehicles
                        airplane.userData.gravityFactor = 0.02; // Even lower gravity than cars
                        
                        // ADDED: Force matrix update to lock in position
                        airplane.updateMatrix();
        ...
        if (this.planets.length > 1) {
            ...
            if (beta && beta.object) {
                ...
                const betaVehiclePositions = [
                    new Vector3(9000, 2000, 800),   // 4000 units from planet center on X axis
                    new Vector3(1000, 2200, 600),   // 4000 units from planet center on X axis
                    new Vector3(9500, 2400, -800),  // 4500 units from planet center on X axis
                    new Vector3(500, 2600, -600)    // 4500 units from planet center on X axis
                ];
                ...
                betaVehiclePositions.forEach((position, index) => {
                    try {
                        ...
                        if (car) {
                            ...
                            // CRITICAL FIX: Set extremely low gravity factor for distant vehicles
                            car.userData.gravityFactor = 0.03; // Very low gravity
                ...
                if (typeof VehicleManager.createVehicleWithPosition === 'function') {
                    const betaPlanePositions = [
                        new Vector3(8600, 2800, -900),   // Higher above planet
                        new Vector3(1400, 3000, 900)     // Higher above planet
                    ];
