...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        if (this.planets[0]) {
            ...
            // Create cars at randomly distributed extreme heights for more varied falls
            const createDramaticCars = (count = 4) => {
                for (let i = 0; i < count; i++) {
                    ...
                    // Random heights between 800-2000 units
                    const height = 800 + Math.random() * 1200;
                    
                    // Create car with this height
                    VehicleManager.createCar(alpha, latitude, longitude, height);
                }
                console.log(`Created ${count} cars on ${alpha.name} at dramatic heights`);
            };
            
            createDramaticCars(6); // Create 6 cars for more dramatic effect
            
            // Create airplanes on Alpha planet at extreme elevations
            VehicleManager.createAirplane(alpha, 30, 120, 2000);
            VehicleManager.createAirplane(alpha, -30, -120, 1800);
        }
        ...
        if (this.planets[1]) {
            ...
            // Similar random distribution for Beta planet
            const createDramaticCars = (count = 4) => {
                for (let i = 0; i < count; i++) {
                ...
                console.log(`Created ${count} cars on ${beta.name} at dramatic heights`);
            };
            
            createDramaticCars(4); // Create 4 cars on beta planet
            
            // Create airplanes
            VehicleManager.createAirplane(beta, -15, 90, 1700);
            VehicleManager.createAirplane(beta, 15, -90, 1900);
        }
