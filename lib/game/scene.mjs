...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        const carPositions = [
            { lat: 0, lon: 0, height: 1500 },           // MASSIVELY INCREASED: From 150 to 1500 for dramatic falls
            { lat: 30, lon: 45, height: 1600 },         // MASSIVELY INCREASED: From 160 to 1600
            { lat: -30, lon: 90, height: 1700 },        // MASSIVELY INCREASED: From 170 to 1700
            { lat: 45, lon: -45, height: 1800 },        // MASSIVELY INCREASED: From 180 to 1800
            { lat: -45, lon: -90, height: 1900 },       // MASSIVELY INCREASED: From 190 to 1900
            { lat: 15, lon: 180, height: 2000 },        // MASSIVELY INCREASED: From 200 to 2000
            { lat: 60, lon: 120, height: 2100 },        // MASSIVELY INCREASED: From 210 to 2100
            { lat: -60, lon: -120, height: 2200 }       // MASSIVELY INCREASED: From 220 to 2200
        ];
        ...
        carPositions.forEach(({lat, lon, height}, index) => {
            try {
                ...
                if (car) {
                    ...
                    // IMPROVED: Initialize with proper alignment to planet surface
                    car.userData.falling = true;
                    car.userData.velocity = new Vector3(0, 0, 0);
                    
                    // ADDED: Ensure cars are properly oriented to surface normal
                    const toSurface = car.position.clone().sub(alpha.object.position).normalize();
                    VehicleManager.alignVehicleToPlanetSurface(car, toSurface, 1.0); // Full alignment
                }
            } catch (e) {
        ...
        try {
            ...
            if (typeof VehicleManager.createAirplane === 'function') {
                const planePositions = [
                    { lat: 10, lon: 20, height: 3000 },    // MASSIVELY INCREASED: From 300 to 3000 for dramatic falls
                    { lat: -20, lon: -30, height: 3200 },  // MASSIVELY INCREASED: From 320 to 3200
                    { lat: 45, lon: 140, height: 3400 }    // MASSIVELY INCREASED: From 340 to 3400
                ];
                ...
                planePositions.forEach(({lat, lon, height}, index) => {
                    ...
                    if (airplane) {
                        ...
                        console.log(`Created airplane ${airplane.name} at lat=${lat}, lon=${lon}, absolute height=${alpha.radius + height}`);
                        
                        // IMPROVED: Initialize with proper orientation
                        airplane.userData.falling = true;
                        airplane.userData.velocity = new Vector3(0, 0, 0);
                        
                        // ADDED: Ensure airplanes are properly oriented to surface normal
                        const toSurface = airplane.position.clone().sub(alpha.object.position).normalize();
                        VehicleManager.alignVehicleToPlanetSurface(airplane, toSurface, 1.0); // Full alignment
                    }
                });
            } else {
            ...
            if (beta && beta.object) {
                ...
                for (let i = 0; i < 4; i++) {
                    try {
                        ...
                        const height = 500 + (i * 100); // MASSIVELY INCREASED: From 50 to 500 base height + increasing
                ...
                if (typeof VehicleManager.createAirplane === 'function') {
                    for (let i = 0; i < 2; i++) {
                        try {
                            ...
                            const height = 1000 + (i * 150); // MASSIVELY INCREASED: From 100 to 1000 base height
