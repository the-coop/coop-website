...
export default class SceneManager {
    ...
    static setupVehicles() {
        console.log("Setting up vehicles on planets - using player spawn position with small offsets");
        ...
        // UPDATED: Use positions close to player spawn position (5000, 220, 200) with small offsets
        const vehiclePositions = [
            // Positions around the player spawn point for better accessibility
            { position: new Vector3(5000, 400, 300), type: 'car' },    // 10 units behind player
            { position: new Vector3(5010, 300, 500), type: 'car' },    // 10 units to the right
            { position: new Vector3(4990, 400, 500), type: 'car' },    // 10 units to the left
            { position: new Vector3(5500, 425, 300), type: 'car' },    // 10 units in front
            { position: new Vector3(5015, 350, 400), type: 'car' },    // Diagonal offset
            { position: new Vector3(4985, 300, 225), type: 'car' },    // Diagonal offset
            { position: new Vector3(4985, 403, 300), type: 'car' },    // Diagonal offset
            { position: new Vector3(5015, 220, 500), type: 'car' },    // Diagonal offset
            
            // Airplanes slightly higher
            { position: new Vector3(5020, 240, 220), type: 'airplane' }, // Higher and offset from player
            { position: new Vector3(4980, 240, 180), type: 'airplane' }, // Higher and offset from player
            { position: new Vector3(5000, 250, 230), type: 'airplane' }  // Higher and behind player
        ];
        
        // Create vehicles at calculated positions
        vehiclePositions.forEach((data, index) => {
            try {
                ...
                const groundDistance = planet.radius + (type === 'car' ? 2.8 : 5.0); // Different heights for car vs airplane
                const isOnGround = Math.abs(distanceToPlanet - groundDistance) < 0.5;
                ...
                console.log(`Vehicle is ${isOnGround ? 'ON ground' : 'ABOVE ground'}`);
                
                // CRITICAL FIX: Calculate the proper surface normal at spawn position
                const toPosition = position.clone().sub(planetCenter);
                const surfaceNormal = toPosition.normalize();
                
                // IMPROVED: If position is not exactly on ground, project it there
                let finalPosition;
                if (!isOnGround) {
                    // Find the correct position on the planet surface at the right height
                    finalPosition = planetCenter.clone().addScaledVector(
                        surfaceNormal, 
                        groundDistance
                    );
                    console.log(`Adjusting position to correct height: ${finalPosition.toArray()}`);
                } else {
                    finalPosition = position.clone();
                }
                
                // CRITICAL FIX: Use position directly with correct alignment setting
                const car = VehicleManager.createVehicleWithPosition(type, planet, finalPosition, false);
                
                if (car) {
                    car.name = `${type}-${index}`;
                    console.log(`Created ${type} ${car.name} at position ${car.position.toArray()}`);
                    
                    // Set basic properties
                    car.userData.type = type;
                    car.userData.falling = true; // Vehicles start on the ground
                    ...
                    car.userData.soi = planet;
                    car.userData.planet = planet;
                    
                    // Set normal gravity factor for ground-spawned vehicles
                    car.userData.gravityFactor = 0.2;
                    
                    // Store the surface normal for physics handling
                    car.userData.surfaceNormal = surfaceNormal.clone();
                    
                    // ADDED: Set proper height offset for physics system
                    car.userData.fixedHeightOffset = type === 'car' ? 2.8 : 5.0;
                    
                    // Force full alignment to planet surface
                    if (typeof VehicleManager.alignVehicleToPlanetSurface === 'function') {
                        VehicleManager.alignVehicleToPlanetSurface(car, surfaceNormal, 1.0, true);
                    }
                    
                    // ADDED: Force matrix update to lock in position and orientation
                    car.updateMatrix();
                ...
                console.error(`Error creating vehicle ${index}:`, e);
            }
        });
    }
