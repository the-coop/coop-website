...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        if (this.planets[0]) {
            ...
            // Create cars at more varied locations with increased height
            VehicleManager.createCar(alpha, 0, 0, 50);        // At equator intersection - high up
            VehicleManager.createCar(alpha, 45, 45, 100);     // Northeast - higher
            VehicleManager.createCar(alpha, -45, -45, 75);    // Southwest - medium height
            VehicleManager.createCar(alpha, 90, 0, 150);      // North pole - highest
            console.log(`Created 4 cars on ${alpha.name} at elevated positions`);
            
            // Create airplanes on Alpha planet at high elevation
            VehicleManager.createAirplane(alpha, 30, 120, 200); // Very high airplane
            VehicleManager.createAirplane(alpha, -30, -120, 100); // Another airplane at different height
        }
        
        // Create vehicles on the second planet (Beta) with better spacing
        if (this.planets[1]) {
            const beta = this.planets[1];
            // Create cars at varied heights
            VehicleManager.createCar(beta, 30, 30, 80);       // Northeast - high
            VehicleManager.createCar(beta, -30, -30, 120);    // Southwest - higher
            VehicleManager.createCar(beta, 0, 90, 40);        // East - medium height
            console.log(`Created 3 cars on ${beta.name} at elevated positions`);
            
            // Create airplanes on Beta planet with different coordinates and heights
            VehicleManager.createAirplane(beta, -15, 90, 150);   // Southeast - high up
            VehicleManager.createAirplane(beta, 15, -90, 180);   // Northwest - even higher
        }
        ...
        console.log(`Created a total of ${VehicleManager.vehicles.length} vehicles: ${carCount} cars and ${airplaneCount} airplanes at various altitudes`);
    }
};
