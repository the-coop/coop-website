...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        // Create vehicles on the first planet (Alpha) with MUCH higher spawning points
        if (this.planets[0]) {
            const alpha = this.planets[0];
            // Create cars at extremely high positions
            VehicleManager.createCar(alpha, 0, 0, 800);         // VERY high above equator
            VehicleManager.createCar(alpha, 45, 45, 1000);      // Even higher
            VehicleManager.createCar(alpha, -45, -45, 1200);    // Super high
            VehicleManager.createCar(alpha, 90, 0, 1500);       // Extreme height
            console.log(`Created 4 cars on ${alpha.name} at dramatic heights - watch them fall!`);
            
            // Create airplanes on Alpha planet at extreme elevations
            VehicleManager.createAirplane(alpha, 30, 120, 2000); // Stratospheric airplane
            VehicleManager.createAirplane(alpha, -30, -120, 1800); // Almost in orbit
        }
        
        // Create vehicles on the second planet (Beta) with MUCH higher spawning points
        if (this.planets[1]) {
            const beta = this.planets[1];
            // Create cars at extreme heights
            VehicleManager.createCar(beta, 30, 30, 900);        // Very high
            VehicleManager.createCar(beta, -30, -30, 1100);     // Higher still
            VehicleManager.createCar(beta, 0, 90, 1300);        // Extremely high
            console.log(`Created 3 cars on ${beta.name} at dramatic heights - they'll fall a long way!`);
            
            // Create airplanes on Beta planet with extreme elevations
            VehicleManager.createAirplane(beta, -15, 90, 1700);  // Stratospheric
            VehicleManager.createAirplane(beta, 15, -90, 1900);  // Near orbital
        }
