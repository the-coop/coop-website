...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        // ENHANCED: Use even more extreme heights for more dramatic falling
        const vehiclePositions = [
            // Positions at various heights above player spawn position for dramatic gravity effects
            { position: new Vector3(5000, 500, 300), type: 'car' },    // Higher above surface
            { position: new Vector3(5010, 600, 500), type: 'car' },    // Even higher
            { position: new Vector3(4990, 700, 500), type: 'car' },    // Even higher
            { position: new Vector3(5500, 800, 300), type: 'car' },    // Significantly higher
            { position: new Vector3(5015, 900, 400), type: 'car' },    // Dramatically higher
            { position: new Vector3(4985, 1000, 225), type: 'car' },   // Very high
            { position: new Vector3(4985, 1100, 300), type: 'car' },   // Extra high
            { position: new Vector3(5015, 1200, 500), type: 'car' },   // Super high
            
            // Airplanes at even higher altitude
            { position: new Vector3(5020, 1500, 220), type: 'airplane' }, // Very high
            { position: new Vector3(4980, 2000, 180), type: 'airplane' }, // Higher
            { position: new Vector3(5000, 2500, 230), type: 'airplane' }  // Highest
        ];
        ...
        vehiclePositions.forEach((data, index) => {
            try {
                ...
                if (vehicle) {
                    // FIXED: Store ID in userData instead of trying to set read-only id property
                    vehicle.userData.id = `${type}-${index}-${Date.now() % 10000}`;
                    vehicle.name = `${type}-${index}`;
                    ...
                    if (vehicle.collidable) {
                        ...
                        if (typeof ObjectManager.checkObjectCollisions === 'function') {
                            setTimeout(() => {
                                const collisions = ObjectManager.checkObjectCollisions(vehicle);
                                console.log(`Vehicle ${vehicle.name} initial collisions: ${collisions.length}`);
                            }, 100);
                        }
                    } else {
                        ...
                        const monitorInterval = setInterval(() => {
                            ...
                            if (Date.now() % 2000 < 100) {
                                ...
                                if (vehicle.userData.falling && currentHeight < 10) {
                                    ...
                                    console.log(`Vehicle position: [${vehicle.position.x.toFixed(1)}, ${vehicle.position.y.toFixed(1)}, ${vehicle.position.z.toFixed(1)}]`);
                                    console.log(`Planet center: [${planetCenter.x.toFixed(1)}, ${planetCenter.y.toFixed(1)}, ${planetCenter.z.toFixed(1)}]`);
