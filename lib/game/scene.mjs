...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        // UPDATED: Use positions significantly above surface to demonstrate gravity
        const vehiclePositions = [
            // Positions at various heights above player spawn position for dramatic gravity effects
            { position: new Vector3(5000, 350, 300), type: 'car' },    // Higher above surface
            { position: new Vector3(5010, 400, 500), type: 'car' },    // Even higher
            { position: new Vector3(4990, 450, 500), type: 'car' },    // Even higher
            { position: new Vector3(5500, 500, 300), type: 'car' },    // Significantly higher
            { position: new Vector3(5015, 550, 400), type: 'car' },    // Dramatically higher
            { position: new Vector3(4985, 350, 225), type: 'car' },    // Moderately high
            { position: new Vector3(4985, 450, 300), type: 'car' },    // High
            { position: new Vector3(5015, 300, 500), type: 'car' },    // Medium height
            
            // Airplanes at even higher altitude
            { position: new Vector3(5020, 600, 220), type: 'airplane' }, // Very high
            { position: new Vector3(4980, 700, 180), type: 'airplane' }, // Higher
            { position: new Vector3(5000, 800, 230), type: 'airplane' }  // Highest
        ];
        ...
        vehiclePositions.forEach((data, index) => {
            try {
                ...
                if (vehicle) {
                    ...
                    // Set proper height offset for physics system
                    vehicle.userData.fixedHeightOffset = type === 'car' ? 2.8 : 5.0;
                    
                    // CRITICAL FIX: Do NOT force alignment to planet surface initially
                    // This allows gravity to naturally affect the vehicle
