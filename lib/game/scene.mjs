...
export default class SceneManager {
    ...
    static setupVehicles() {
        ...
        const carPositions = [
            { lat: 0, lon: 0, height: 1500 },
            { lat: 30, lon: 45, height: 1600 },
            { lat: -30, lon: 90, height: 1700 },
            { lat: 45, lon: -45, height: 1800 },
            { lat: -45, lon: -90, height: 1900 },
            { lat: 15, lon: 180, height: 2000 },
            { lat: 60, lon: 120, height: 2100 },
            { lat: -60, lon: -120, height: 2200 }
        ];
        ...
        carPositions.forEach(({lat, lon, height}, index) => {
            try {
                ...
                if (car) {
                    ...
                    // ADDED: Explicitly ensure type is set correctly
                    car.userData.type = 'car';
                    
                    // Initialize with proper alignment to planet surface
        ...
        try {
            ...
            if (typeof VehicleManager.createAirplane === 'function') {
                const planePositions = [
                    { lat: 10, lon: 20, height: 3000 },
                    { lat: -20, lon: -30, height: 3200 },
                    { lat: 45, lon: 140, height: 3400 }
                ];
