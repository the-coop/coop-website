    ...
    static checkCollisions(player, objects, timeStep) {
        ...
        try {
            ...
            try {
                ...
                // ADDED: Keep track of distances for all potential collisions
                ...
                for (let step = 0; step < numSteps; step++) {
                    ...
                    // FIXED: Safe iteration over objects
                    for (let i = 0; i < objects.length; i++) {
                        ...
                        // Get actual distance to object - IMPROVED to filter out distant objects
                        const distToObject = stepPosition.distanceTo(item.object.position);
                        const objectSize = item.aabb.getSize(new Vector3()).length();
                        
                        // Skip objects that are too far away - must be closer than combined size + safety margin
                        const combinedSize = objectSize + 2.0; // Player size + safety margin
                        if (distToObject > combinedSize * 2) {
                            // Object is too far away to collide
                            continue;
                        }
                        
                        // FIXED: Add more specific type filtering
                        if (item.type !== 'wall' && item.type !== 'vehicle' && item.type !== 'generic') {
                            continue;
                        }
                        
                        // Skip collision check if player is in this vehicle
                        if (item.type === 'vehicle' && player.inVehicle && 
                            window.VehicleManager && item.object === window.VehicleManager.currentVehicle) {
                            continue;
                        }
                        
                        // FIXED: Don't update bounds on every check - too expensive
                        // Only update if we suspect the object has moved
                        if (item.object.userData && item.object.userData.hasMovedSinceLastCheck) {
                            this.updateCollidableBounds(item.object);
                            item.object.userData.hasMovedSinceLastCheck = false;
                        }
                        
                        // CRITICAL FIX: Safety check before intersection test
                        if (!worldPlayerOBB || !item.obb) {
                            console.warn(`Missing OBB for intersection test: playerOBB=${!!worldPlayerOBB}, objectOBB=${!!item.obb}`);
                            continue;
                        }
                        
                        // SAFE INTERSECTION TEST: Wrap in try-catch to prevent crashes
                        ...
                        try {
                            ...
                            console.log(`OBB data:`, {
                                playerCenter: worldPlayerOBB.center?.toArray() || 'invalid',
                                playerSize: worldPlayerOBB.halfSize?.toArray() || 'invalid',
                                objectCenter: item.obb?.center?.toArray() || 'invalid',
                                objectSize: item.obb?.halfSize?.toArray() || 'invalid'
                            });
                            continue; // Skip this object if intersection test fails
                        }
                        ...
                        if (intersects) {
                            try {
                                ...
                                // IMPROVED: Store all potential collisions to sort by distance
                                if (time <= timeStep) {
                ...
                // ADDED: Sort potential collisions by time and verify they're not too far
                if (potentialCollisions.length > 0) {
                    potentialCollisions.sort((a, b) => a.time - b.time);
                    
                    // Take the closest collision that passes distance verification
                    for (const collision of potentialCollisions) {
                        // Skip collisions with objects that are suspiciously far
                        if (collision.distance > 20) {
                            console.warn(`Skipping suspicious collision with ${collision.object.type} at distance ${collision.distance.toFixed(2)}`);
                            continue;
                        }
                        ...
                        // DEBUG: Log collision with wall or vehicle
                        if (typeof window !== 'undefined' && window.gameNotify && 
                            closestObject.type !== player._lastCollidedWith) {
                            window.gameNotify(`Collision with ${closestObject.type} (dist: ${collision.distance.toFixed(1)})`);
