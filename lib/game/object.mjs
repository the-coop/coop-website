            ...
            try {
                ...
                for (let step = 0; step < numSteps; step++) {
                    ...
                    for (let i = 0; i < objects.length; i++) {
                        ...
                        // IMPROVED: Get actual distance to object for more accurate filtering
                        const distToObject = stepPosition.distanceTo(item.object.position);
                        
                        // Get the actual size of the object's bounding box for better distance checking
                        const objectSize = item.aabb.getSize(new Vector3()).length() * 0.5;
                        
                        // Add player size to total size threshold
                        const playerSize = 1.0; // Approximate player radius
                        
                        // Skip objects that are too far away based on actual sizes + buffer
                        // This ensures we only check objects that could reasonably collide
                        const buffer = 2.0; // Additional buffer for safety
                        const distanceThreshold = objectSize + playerSize + buffer;
                        
                        if (distToObject > distanceThreshold) {
                            // Object is too far away to possibly collide
                            continue;
                        }
