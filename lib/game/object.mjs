        ...
        try {
            ...
            try {
                ...
                for (let step = 0; step < numSteps; step++) {
                    ...
                    // Position the player OBB at this step - CRITICAL FIX: define stepMatrix outside loops
                    // so it's available in all contexts where it might be needed
                    ...
                    // First pass: Only process wall objects (higher priority)
                    for (let i = 0; i < objects.length; i++) {
                        ...
                        // Only process walls in first pass
                        ...
                        try {
                            // FIXED: Use the current step's worldPlayerOBB that was already defined
                            // No need to create a new one here - prevents reference errors
                ...
                // Second pass: Process non-wall objects with the SAME stepMatrix
                for (let step = 0; step < numSteps; step++) {
                    // Calculate position at this substep
                    const stepPosition = player.position.clone().addScaledVector(
                        player.velocity, 
                    ...
                    for (let i = 0; i < objects.length; i++) {
                        ...
                        // Skip walls - already processed in first pass
                        if (item.type === 'wall') continue;
                        ...
                        let intersects = false;
                        try {
                            // CRITICAL FIX: Use the already defined worldPlayerOBB variable
                            // No need to create a new OBB here - prevents reference errors
                            
                            // Try main OBB-OBB intersection check
                            intersects = worldPlayerOBB.intersectsOBB(item.obb);
                            
                            // Additional tests for more reliable detection
                            // 2. Check if at least one corner of the player OBB is inside the wall OBB
                            if (!intersects && isWall) {
                                // This is a more lenient test for walls
                                const corners = this.getOBBCorners(worldPlayerOBB);
                            ...
                            if (isWall && intersects) {
                                ...
                                if (this._debugEnabled) {
                                    console.log(`COLLISION DETAILS:
                                      ...
                                      - Position: [${item.object.position.x.toFixed(1)}, ${item.object.position.y.toFixed(1)}, ${item.object.position.z.toFixed(1)}]
                                      - Distance: ${distToObject.toFixed(2)} units
                                      - OBB Center: [${item.obb.center.x.toFixed(1)}, ${item.obb.center.y.toFixed(1)}, ${item.obb.center.z.toFixed(2)}]
                                      - OBB Size: [${(item.obb.halfSize.x*2).toFixed(1)}, ${(item.obb.halfSize.y*2).toFixed(1)}, ${(item.obb.halfSize.z*2).toFixed(1)}]
                                      - Collision method: ${intersects._method || "standard OBB"}
                                    `);
                                }
