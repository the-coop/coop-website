...
export default class ObjectManager {
                ...
                for (const other of potentialColliders) {
                    ...
                    try {
                        ...
                        if (!intersects && other.type === 'wall') {
                            ...
                            // ADDED: Also check if any wall corners are inside the vehicle
                            if (!intersects) {
                            ...
                            // ADDED: Edge-to-edge check for wall collisions
                            if (!intersects) {
    ...
    // FIXED: OBB intersection test method
    static checkOBBIntersection(obb1, obb2) {
        if (!obb1 || !obb2) return false;
        
        try {
            // Validate OBBs before intersection check
    ...
    // ENHANCED: Debug toggle to show all active collisions
    static toggleDebug(enabled = null) {
        if (enabled === null) {
            this._debugEnabled = !this._debugEnabled; // Toggle current state
        } else {
            this._debugEnabled = enabled;
        }
        
        console.log(`Collision debug mode ${this._debugEnabled ? 'ENABLED' : 'DISABLED'}`);
        
        // When enabling debug, report current collidable state
        if (this._debugEnabled) {
            console.log(`Active collidables: ${this.collidableObjects.length}`);
            
            // Count by type
            const typeCounts = {};
            this.collidableObjects.forEach(c => {
                if (!c || !c.type) return;
                typeCounts[c.type] = (typeCounts[c.type] || 0) + 1;
            });
            
            // Log counts by type
            console.log("Collidable objects by type:");
    ...
    // ADDED: Helper method to track what's colliding with what (for debugging)
    static _collisionTracker = {
        lastCollisions: {},
        reportCollision: function(object1Type, object2Type, position, time) {
            const key = `${object1Type}-${object2Type}`;
            const now = Date.now();
            
            // Only report every 500ms for the same collision pair
            if (!this.lastCollisions[key] || now - this.lastCollisions[key].time > 500) {
                this.lastCollisions[key] = {
                    time: now,
                    position: position,
                    count: 1
                };
                
                return true; // Report this collision
            } else {
                // Update existing collision data
                this.lastCollisions[key].count++;
                this.lastCollisions[key].position = position;
                
                // Only report if count is a power of 2 (1, 2, 4, 8, 16...)
                // This prevents log flooding while still showing frequency
                return (this.lastCollisions[key].count & (this.lastCollisions[key].count - 1)) === 0;
            }
        }
    };
