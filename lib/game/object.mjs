...
export default class ObjectManager {
    ...
    static runSATTest(obb1, obb2) {
        ...
        try {
            // Better debug output with object info
            ...
            const isVehicleA = obb1.userData?.isVehicle || obb1.userData?.vehicleReference;
            const isVehicleB = obb2.userData?.isVehicle || obb2.userData?.vehicleReference;
            const isVehicleCollision = isVehicleA && isVehicleB;
            
            // Use simple distance check for early rejection
            ...
            console.log(`Distance between objects: ${centerDistance.toFixed(2)}, combined size: ${combinedSizes.toFixed(2)}`);
            
            // If centers are too far apart, no collision (early rejection)
            if (centerDistance > combinedSizes * 2.0) {
                return { collides: false };
            }
            
            // IMPORTANT: Flag if objects are very close for more generous collision detection
            const isVeryClose = centerDistance < combinedSizes * 0.7;
            
            // Find closest points between OBBs
            const contactPoint = this.findContactPoint(obb1, obb2);
            if (!contactPoint) {
                console.log("No contact point found in SAT test");
                return { collides: false };
            }
            ...
            // Log distance from contact point
            console.log(`Contact point distance: ${result.distance.toFixed(4)}`);
            
            // MORE LENIENT collision thresholds - increased for better detection
            const collisionThreshold = isVehicleCollision ? 
                (isVeryClose ? 0.5 : 0.3) : // More lenient threshold for vehicles
                (isVeryClose ? 0.3 : 0.2);  // More lenient standard threshold for other objects
            ...
            if (result.distance < collisionThreshold) {
                ...
                // ADDED: Store additional debug info for normals visualization
                if (this._debugEnabled && normal) {
                    normal._debugInfo = {
                        startPoint: point.clone(),
                        length: this._debugSettings?.normalLength || 5.0, // Increased for better visibility
                        color: isVehicleCollision ? 0xff0000 : 0x00ffff
                    };
                }
    ...
    // ENHANCED: Test collision between a sphere and specific face of an OBB
    static testSphereOBBFaceCollision(sphereCenter, sphereRadius, obb) {
        ...
        try {
            ...
            if (distance <= sphereRadius) {
                ...
                // FIXED: Calculate proper normal - must point away from OBB
                const correctedNormal = new Vector3().subVectors(
                    sphereCenter, 
                    closestPoint
                ).normalize();
                
                // If normal length is too small, use face normal
                if (correctedNormal.lengthSq() < 0.001) {
                    correctedNormal.copy(face.normal);
                }
    ...
    // CRITICAL FIX: findClosestPointOnOBB to make it work consistently for all collision tests
    static findClosestPointOnOBB(point, obb) {
        if (!point || !obb) return null;
        if (!this.validateOBB(obb)) {
            console.warn("Invalid OBB in findClosestPointOnOBB");
    ...
    // ENHANCED: Check collision between two objects, using appropriate collision shape
    static checkCollision(object1, object2) {
        ...
        const isPlayer1 = collidable1.object.userData?.isPlayer || false;
        const isPlayer2 = collidable2.object.userData?.isPlayer || false;
        
        // Case 1: Both are players (sphere vs sphere)
        if (isPlayer1 && isPlayer2) {
            const radius1 = collidable1.object.userData.collisionRadius || 1.0;
            const radius2 = collidable2.object.userData.collisionRadius || 1.0;
        ...
        // Case 2: First is player, second is other object (sphere vs OBB)
            ...
            return this.testSphereOBBCollision(
                object1.position,
                object1.userData.collisionRadius || 1.0,
                collidable2.obb
            );
        }
        // Case 3: Second is player, first is other object (OBB vs sphere)
            ...
            return this.testSphereOBBCollision(
                object2.position,
                object2.userData.collisionRadius || 1.0,
                collidable1.obb
            );
        }
