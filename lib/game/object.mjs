        ...
        try {
            ...
            if (result.distance < collisionThreshold) {
                ...
                if (isPlayerCollision) {
                    ...
                    // If player is on a planet surface, ensure we don't push perpendicular to the surface
                    if (playerOBB.userData?.playerReference && 
                        ...
                        // If collision would push us too much into or away from the planet,
                        // adjust it to be more parallel to the surface
                        if (Math.abs(normalDotPlanet) > 0.5) { // Significant vertical component
                            // Project normal onto surface plane but keep more of its original direction
                            // to ensure better collision blocking
                            const projectedNormal = normal.clone()
                                .addScaledVector(planetNormal, -normalDotPlanet * 0.7); // Reduce vertical by 70%
                                
                            // Only use projected normal if it's still valid
                            if (projectedNormal.lengthSq() > 0.3) {
                                normal.copy(projectedNormal.normalize());
                                console.log("Adjusted collision normal to be more parallel to planet surface");
                            }
                        }
                    }
                } else {
