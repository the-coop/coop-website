...
export default class ObjectManager {
    ...
    static checkCollisions(player, objects, timeStep) {
        ...
        for (let step = 0; step < numSteps; step++) {
            ...
            // FIXED: Check ALL objects, not just those on the same planet
            // Also filter by what we want to collide with (not objects on far away planets)
            objects.forEach((item) => {
                // Skip invalid objects
                if (!item.object || !item.aabb) return;
                
                // FIXED: Ensure we're checking vehicles too (check by type, not just by array source)
                if (item.type !== 'wall' && item.type !== 'vehicle' && item.type !== 'generic') return;
                
                // Skip collision check if player is in this vehicle
                if (item.type === 'vehicle' && player.inVehicle && 
                    item.object === window.VehicleManager?.currentVehicle) {
                    return;
                }
                
                // Make sure object's OBB is up to date
