...
export default class ObjectManager {
    ...
    // FIX: Update createWall method to properly handle latitude/longitude values
    static createWall(planet, latitude, longitude, scale = 1.0) {
        ...
        try {
            // FIXED: Ensure latitude and longitude are numbers
            const numLatitude = Number(latitude);
            const numLongitude = Number(longitude);
            
            if (isNaN(numLatitude) || isNaN(numLongitude)) {
                console.error(`Invalid latitude or longitude: lat=${latitude}, lon=${longitude}`);
                return null;
            }
            ...
            wall.name = `Wall-${numLatitude.toFixed(0)}-${numLongitude.toFixed(0)}`;
            ...
            SceneManager.positionObjectOnPlanet(wall, planet, numLatitude, numLongitude, wallHeight / 2);
            ...
            console.log(`Created wall at lat=${numLatitude}, lon=${numLongitude} with dimensions ${wallWidth}x${wallHeight}x${wallDepth}`);
    ...
    // NEW: Add the missing createAirplane method
    static createAirplane(planet, latitude, longitude, scale = 1.0) {
        if (!planet || !planet.object) {
            console.error("Invalid planet for airplane creation");
            return null;
        }
        
        try {
            // Default airplane dimensions - can be scaled
            const airplaneWidth = 10 * scale;
            const airplaneHeight = 2 * scale;
            const airplaneDepth = 20 * scale;
            
            // Create airplane geometry
            const geometry = new BoxGeometry(airplaneWidth, airplaneHeight, airplaneDepth);
            const material = new MeshBasicMaterial({ 
                color: 0x00ff00,
                wireframe: false
            });
            
            const airplane = new Mesh(geometry, material);
            airplane.name = `Airplane-${latitude.toFixed(0)}-${longitude.toFixed(0)}`;
            
            // Store airplane properties in userData
            airplane.userData = {
                type: 'airplane',
                airplaneWidth: airplaneWidth,
                airplaneHeight: airplaneHeight,
                airplaneDepth: airplaneDepth,
                airplaneData: {
                    latitude: latitude,
                    longitude: longitude,
                    scale: scale
            ...
            // Position on planet surface
            SceneManager.positionObjectOnPlanet(airplane, planet, latitude, longitude, airplaneHeight / 2);
            
            // Add to scene
            Engine.scene.add(airplane);
            ...
            this.registerGameObject(airplane, 'airplane', dimensions, true); // true = static object
            
            console.log(`Created airplane at lat=${latitude}, lon=${longitude} with dimensions ${airplaneWidth}x${airplaneHeight}x${airplaneDepth}`);
            
            // Store reference in planet's objects array if it exists
            if (Array.isArray(planet.objects)) {
                planet.objects.push(airplane);
            }
