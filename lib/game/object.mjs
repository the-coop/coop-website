...
export default class ObjectManager {
            ...
            if (isPlayerCollision) {
                ...
                if (playerOBB.userData?.playerReference && playerOBB.userData.playerReference.soi) {
                    ...
                    if (player.soi && !player.falling) {
                        ...
                        // FIXED: Better handling of collision normals to prevent surface clipping
                        // Use different approach based on whether the object is above or at the player level
                        ...
                        // Create a better sliding normal based on object position relative to player
                        if (heightDiff > 0.5) {
                            // Object is significantly higher than player - use proper climbing normal
                            // Remove significant upward component to prevent climbing too steep slopes
                            const upComponent = toObject.dot(planetNormal);
                            // If upward angle is too steep, block movement entirely
                            if (upComponent > 0.7) {
                                // Very steep - block with horizontal normal
                                const horizontalDir = toObject.clone().projectOnPlane(planetNormal).normalize();
                                if (horizontalDir.lengthSq() > 0.01) {
                                    // Update pointB to block horizontal movement toward obstacle
                                    pointB.copy(pointA).addScaledVector(horizontalDir, minDist);
                                }
                            } else {
                                // Moderate slope - allow some climbing with adjusted normal
                                const climbNormal = toObject.clone().projectOnPlane(planetNormal).normalize();
                                if (climbNormal.lengthSq() > 0.01) {
                                    pointB.copy(pointA).addScaledVector(climbNormal, minDist);
                                }
                            }
                        } else {
                            // Object is at same level or below player - use standard sliding behavior
                            // Project contact vector onto planet surface plane for clean sliding
                            ...
                            if (surfaceAlignedNormal.lengthSq() > 0.01) {
                                pointB.copy(pointA).addScaledVector(surfaceAlignedNormal, minDist);
                            }
                        }
                    }
                }
    ...
    static runSATTest(obb1, obb2) {
        ...
        try {
            // Initialize a result variable to fix the ReferenceError
            const result = { 
                distance: Infinity,
                pointA: null,
                pointB: null
            ...
            // First find closest points between OBBs
            const contactPoint = this.findContactPoint(obb1, obb2);
            if (!contactPoint) {
                return { collides: false };
            }
            
            // Update our result with contact point data
            ...
            if (result.distance < collisionThreshold) {
                ...
                if (isPlayerCollision) {
                    ...
                    // FIXED: Further enhance normal calculation to prevent movement through objects
                    // Get a direct vector from centers - this is more robust for blocking movement
                    const centerToCenter = new Vector3().subVectors(
                        otherOBB.center,
                        playerOBB.center
                    ).normalize();
                    
                    // Blend between normal calculation methods based on dot product
                    // This ensures better blocking while still allowing sliding when appropriate
                    const standardNormal = new Vector3().subVectors(result.pointA, result.pointB).normalize();
                    
                    // If the center-to-center and standard normal are very different,
                    // prefer the center-to-center for better blocking
                    const dotBetweenNormals = standardNormal.dot(centerToCenter);
                    
                    // IMPROVED: Now use the center-to-center direction more aggressively
                    // This ensures solid blocking of movement
                    if (dotBetweenNormals < 0.7) { // If normals differ by more than ~45 degrees
                        // Use center-to-center with stronger weight for better blocking
                        normal = centerToCenter;
                        console.log("Using center-to-center normal for stronger collision blocking");
                    } else {
                        // Standard normal is good enough
                        normal = standardNormal;
                    }
                    
                    // FIXED: If player is on a planet surface, ensure we don't cause player to fall
                    // by forcing the collision normal to be perpendicular to the surface normal
                    if (playerOBB.userData?.playerReference && 
                        ...
                        // Check how much the collision normal faces into the planet
                        const normalDotPlanet = normal.dot(planetNormal);
                        
                        // IMPROVED: Better surface alignment for object collisions
                        // Always project collision normal onto planet surface for grounded collisions
                        // This ensures player slides along surface rather than being pushed off
                        const projectedNormal = normal.clone()
                            .projectOnPlane(planetNormal)
                            .normalize();
                            
                        // Only use projected normal if it's still valid
                        if (projectedNormal.lengthSq() > 0.3) {
                            normal.copy(projectedNormal);
                            console.log("Adjusted collision normal to be parallel to planet surface for grounded collision");
                            
                            // CRITICAL: Attach data to normal to indicate this is a surface-projected collision
                            normal.isSurfaceAligned = true;
                        }
                    }
                } else {
