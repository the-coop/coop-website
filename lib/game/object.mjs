...
export default class ObjectManager {
    ...
    static checkAllCollisions(object) {
        ...
        try {
            ...
            // CRITICAL FIX: Validate OBB before collision checks
            if (!this.validateOBB(objectCollidable.obb, object)) {
                console.warn("Object has invalid OBB - skipping collision checks");
                return [];
            }
            ...
            for (const other of this.collidableObjects) {
                ...
                // CRITICAL FIX: Validate other OBB too
                if (!this.validateOBB(other.obb, other.object)) continue;
    ...
    static isPointInOBB(point, obb) {
        ...
        // Validate OBB has required properties
        if (!obb.center || !obb.halfSize) {
            console.warn("isPointInOBB called with invalid OBB");
            return false;
        }
        
        try {
            // Extra validation for rotation
            if (!obb.rotation || !obb.rotation.elements || obb.rotation.elements.length !== 9) {
                // Fallback to simple AABB check if rotation is invalid
                const dx = Math.abs(point.x - obb.center.x);
                const dy = Math.abs(point.y - obb.center.y);
                const dz = Math.abs(point.z - obb.center.z);
                
                return dx <= obb.halfSize.x && dy <= obb.halfSize.y && dz <= obb.halfSize.z;
            }
        ...
        try {
            ...
            if (minDistance > threshold && (aspect1 > 10 || aspect2 > 10)) {
                ...
                const verticalEdges2 = edges2.filter(e => {
                    const dir = e.end.clone().sub(e.start).normalize();
                    return Math.abs(dir.y) > 0.7; //```javascript
                    return Math.abs(dir.y) > 0.7; // Mostly vertical
                });
    ...
    // NEW: Enhanced getInverseRotationMatrix with better validation and error handling
    static getInverseRotationMatrix(obb) {
        ...
        for (let i = 0; i < 9; i++) {
            if (isNaN(obb.rotation.elements[i]) || !isFinite(obb.rotation.elements[i])) {
                console.warn("OBB rotation contains NaN or Infinity - returning identity");
                return new Matrix3().identity();
            }
        }
    ...
    // NEW: Enhanced isPointInOBB with better validation
    static isPointInOBB(point, obb) {
        if (!point || !obb) return false;
    ...
    // ENHANCED: Stronger validation in checkAllCollisions
    static checkAllCollisions(object) {
        if (!object) return [];
    ...
    static closestPointsBetweenLines(A1, A2, B1, B2) {
        // Simple approach for finding closest approach between two line segments
        // This can be replaced with a more robust implementation if needed

        // Example minimal logic returning point on A, point on B
        return {
            point1: A1.clone(), 
            point2: B1.clone()
        };
