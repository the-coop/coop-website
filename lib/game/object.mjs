...
export default class ObjectManager {
    ...
    // ENHANCED: Find closest point on OBB to a given point with reduced logging
    static findClosestPointOnOBB(point, obb) {
        ...
        try {
            // Distance-based logging filter - only log for nearby objects to reduce spam
            const debugLogging = false; // Set to true only for specific debugging sessions
            const debugDistance = 100; // Only log if distance is less than this
            const shouldLog = debugLogging && 
                            point.distanceTo(obb.center) < debugDistance;
            
            // Check if we need to do verbose logging for this calculation
            if (shouldLog) {
            ...
            // Only log if we're within debug distance
            if (shouldLog) {
    ...
    static alignObjectToSurface = function(object, surfaceNormal, options = {}) {
    ...
    // NEW: Add missing validatePlayerCollision method needed by PlayersManager
    static validatePlayerCollision(player) {
        if (!player || !player.handle) return false;
        
        try {
            // Check if player is in collidable objects
            const isRegistered = this.collidableObjects.some(c => 
                c && c.object === player.handle);
                
            if (!isRegistered) {
                console.warn("Player not found in ObjectManager - reinitializing collision");
                
                // Try to find PlayersManager to reinitialize the collider
                ...
                // If we can't access PlayersManager, try to register the player directly
                if (player.handle && !player.collidable) {
                    const dimensions = {
                        width: player.collisionRadius ? player.collisionRadius * 2 : 2.4,
                        height: player.collisionRadius ? player.collisionRadius * 2 : 2.4,
                        depth: player.collisionRadius ? player.collisionRadius * 2 : 2.4
                    };
                    
                    player.collidable = this.registerGameObject(player.handle, 'player', dimensions, false);
                    console.log("Direct player collision registration");
            ...
            console.error("Error validating player collision:", err);
            return false;
        }
    }
