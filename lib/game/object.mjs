...
export default class ObjectManager {
    ...
    // Get the half size of the bounding box
    const Size = new Vector3();
    boundingBox.getSize(Size);

    // Compute the direction vector from the center
    const direction = new Vector3()
        .subVectors(position, center);

    // Determine the major axis (normal direction)
    const absDirection = new Vector3(
    ...
    if (absDirection.x > absDirection
        .y && absDirection.x >
        absDirection.z) {
        normal.set(Math.sign(direction
            .x), 0, 0);
    } else if (absDirection.y >
        absDirection.x && absDirection
            .y > absDirection.z) {
        normal.set(0, Math.sign(
            direction.y), 0);
    } else {
        normal.set(0, 0, Math.sign(
            direction.z));
    }
