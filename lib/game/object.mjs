        ...
        try {
            ...
            if (result.distance < collisionThreshold) {
                ...
                if (isPlayerCollision) {
                    ...
                    // IMPROVED: Choose best normal based on context
                    if (player && !player.falling && planetNormal) {
                        // Player is on planet surface - ensure collision normal is surface-aligned
                        
                        // First get the unmodified standard normal for reference
                        const unmodifiedNormal = standardNormal.clone();
                        
                        // Project the standard normal onto the surface plane
                        ...
                        if (projectedNormal.lengthSq() > 0.3) {
                            // NEW: Store original normal for reference
                            const originalNormal = standardNormal.clone();
                            
                            // Check if this is a lateral or vertical collision
                            const dotWithUp = originalNormal.dot(planetNormal);
                            const isLateralCollision = Math.abs(dotWithUp) < 0.5;
                            
                            if (isLateralCollision) {
                                // For lateral collisions, use pure surface-projected normal
                                normal = projectedNormal;
                                console.log("Using pure surface-projected normal for lateral collision");
                            } else {
                                // For more vertical collisions, blend with the original normal
                                // This preserves some vertical component while still emphasizing surface movement
                                normal = projectedNormal.clone()
                                    .multiplyScalar(0.85) // 85% projected
                                    .add(originalNormal.multiplyScalar(0.15)); // 15% original
                                normal.normalize();
                                console.log("Using blended surface-projected normal");
                            }
                            ...
                            // Fallback to center-to-center if projection failed
                            normal = centerToCenter;
                            console.log("Using center-to-center normal (projection failed)");
                        }
                    } else {
                        // Player is falling or not on planet - use regular collision logic
                        const dotBetweenNormals = standardNormal.dot(centerToCenter);
                        
                        if (dotBetweenNormals < 0.7) {
                            normal = centerToCenter;
                            console.log("Using center-to-center normal for non-grounded collision");
                        } else {
                            normal = standardNormal;
                            console.log("Using standard point-based normal");
                        }
                    }
                } else {
