...
export default class ObjectManager {
    ...
    // ADDED: Missing OBB intersection test method
    /**
    ...
    // ADDED: Implementation of the missing checkAllCollisions method
    /**
     * Checks for collisions between the specified object and all other objects
     * that match the specified types.
     * 
     * @param {Object3D} object - The object to check collisions for
     * @param {Array} types - Array of object types to check against (e.g. ['wall', 'vehicle'])
     * @returns {Array} Array of collision objects with contact information
     */
    static checkAllCollisions(object, types = null) {
        ...
        try {
            ...
            // Filter for specified types if provided
            let collidableSet = this.collidableObjects;
            if (types && Array.isArray(types) && types.length > 0) {
                collidableSet = collidableSet.filter(c => 
                    c && c.active && types.includes(c.type) && c.object !== object
                );
            } else {
                collidableSet = collidableSet.filter(c => 
                    c && c.active && c.object !== object
                );
            }
            
            // Use the existing method to check for collisions
            return this.checkCollisionsWithObject(object);
        } catch (err) {
