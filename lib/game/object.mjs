...
export default class ObjectManager {
    ...
    static runSATTest(obb1, obb2) {
        ...
        try {
            ...
            // IMPROVED: Detect vehicle-vehicle collision for special handling
            const isVehicleA = obb1.userData?.isVehicle || obb1.object?.userData?.isVehicle || 
                              obb1.object?.userData?.type === 'car' || obb1.object?.userData?.type === 'airplane';
            const isVehicleB = obb2.userData?.isVehicle || obb2.object?.userData?.isVehicle || 
                              obb2.object?.userData?.type === 'car' || obb2.object?.userData?.type === 'airplane';
            const isVehicleCollision = isVehicleA && isVehicleB;
            
            // Add early intersection test with enlarged boundaries for tunneling prevention
            ...
            // IMPROVED: For vehicle collisions, use a larger margin to detect impacts sooner
            const distanceMultiplier = isVehicleCollision ? 2.0 : 1.5;
            
            // If centers are too far apart, no chance of collision (early exit optimization)
            if (centerDistance > combinedSizes * distanceMultiplier) {
                return { collides: false };
            }
            ...
            // IMPROVED: For vehicle-vehicle collisions, use a more generous threshold
            const collisionThreshold = isVehicleCollision ? 
                (isVeryClose ? 0.3 : 0.2) : // More lenient threshold for vehicles
                (isVeryClose ? 0.2 : 0.1);  // Standard threshold for other objects
            
            if (result.distance < collisionThreshold) {
                // This is code to determine collision normals and points
                let normal;

                // Calculate standard point-based normal
                const standardNormal = new Vector3().subVectors(result.pointA, result.pointB).normalize();
                
                // For vehicle collisions, prefer center-to-center direction for better physical response
                if (isVehicleCollision) {
                    // Use center-to-center for primary direction
                    const centerToCenter = new Vector3().subVectors(
                        obb2.center,
                        obb1.center
                    ).normalize();
                    
                    // For glancing/side impacts, blend with standard normal
                    const dotProduct = centerToCenter.dot(standardNormal);
                    // Use more of center-to-center vector for clear impacts, blend for glancing hits
                    if (dotProduct > 0.7) {
                        // Clear impact: mostly use center-to-center
                        normal = centerToCenter;
                    } else {
                        // Glancing hit: blend the vectors
                        normal = standardNormal.clone().multiplyScalar(0.4)
                            .add(centerToCenter.multiplyScalar(0.6))
                            .normalize();
                    }
                    
                    // Add vehicle collision flag
                    normal.isVehicleCollision = true;
                } else {
                    // For non-vehicle collisions, use standard normal calculation
                ...
                return {
                    ...
                    isVehicleCollision: isVehicleCollision
                };
            }
    ...
    // NEW: Test collision between a sphere and an OBB
    ...
    // NEW: Find which face of an OBB is closest to a point
    static findClosestFaceToPoint(obb, point) {
        if (!obb || !point || !this.validateOBB(obb)) {
            return { normal: new Vector3(0, 1, 0), distance: Infinity };
        }
        ...
        try {
            ...
            // Calculate distances to each face plane
            const distances = [
                { axis: 'x', sign: 1, dist: obb.halfSize.x - localPoint.x },
                { axis: 'x', sign: -1, dist: obb.halfSize.x + localPoint.x },
                { axis: 'y', sign: 1, dist: obb.halfSize.y - localPoint.y },
                { axis: 'y', sign: -1, dist: obb.halfSize.y + localPoint.y },
                { axis: 'z', sign: 1, dist: obb.halfSize.z - localPoint.z },
                { axis: 'z', sign: -1, dist: obb.halfSize.z + localPoint.z }
            ];
            
            // Find closest face
            let closestFace = distances[0];
            for (let i = 1; i < distances.length; i++) {
                if (distances[i].dist < closestFace.dist) {
                    closestFace = distances[i];
                }
            }
            
            // Create normal vector in local space
            const localNormal = new Vector3();
            switch(closestFace.axis) {
                case 'x': localNormal.set(closestFace.sign, 0, 0); break;
                case 'y': localNormal.set(0, closestFace.sign, 0); break;
                case 'z': localNormal.set(0, 0, closestFace.sign); break;
            }
            
            // Transform normal to world space
            const worldNormal = localNormal.clone().applyMatrix3(obb.rotation);
            
            // Calculate face center in world space
            const localFaceCenter = localNormal.clone().multiply(obb.halfSize);
            const worldFaceCenter = localFaceCenter.clone().applyMatrix3(obb.rotation).add(obb.center);
            
            return {
                normal: worldNormal,
                center: worldFaceCenter,
                distance: closestFace.dist,
                axis: closestFace.axis,
                sign: closestFace.sign
            };
        } catch (err) {
            console.error("Error finding closest face:", err);
            return { normal: new Vector3(0, 1, 0), distance: Infinity };
        }
    }
    
    // NEW: Test collision between a sphere and specific face of an OBB
    static testSphereOBBFaceCollision(sphereCenter, sphereRadius, obb) {
        ...
        try {
            ...
            if (distance <= sphereRadius) {
                const face = this.findClosestFaceToPoint(obb, sphereCenter);
                return {
                    collides: true,
                    distance: distance,
