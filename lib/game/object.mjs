...
export default class ObjectManager {
            ...
            try {
                ...
                for (let step = 0; step < numSteps; step++) {
                    ...
                    for (let i = 0; i < objects.length; i++) {
                        ...
                        // FIXED: Calculate the actual size of the object's OBB for better distance checks
                        // We need to consider the full size of the object, not just a fixed buffer
                        const objectSize = item.aabb.getSize(new Vector3());
                        const objectMaxDimension = Math.max(objectSize.x, objectSize.y, objectSize.z);
                        const playerSize = 1.5; // Slightly larger player collision size
                        
                        // FIXED: Use actual object dimensions for thresholds rather than fixed values
                        // For walls, use the full wall dimension + extra buffer to ensure we catch the edges
                        const buffer = isWall ? objectMaxDimension : 2.0;
    ...
    // Unified collision check that works for all entity types
    ...
    // Helper for OBB intersection check with better error handling
    ...
    static createWall(planet, objectData) {
        ...
        try {
            ...
            // Register the wall as a STATIC collidable object with the proper bounding box
            const aabb = new Box3();
            aabb.setFromObject(wall);
            
            // Make sure the AABB is slightly expanded to prevent tunneling
            const expansionAmount = 0.1; // Add a small buffer around the wall
            aabb.min.addScalar(-expansionAmount);
            aabb.max.addScalar(expansionAmount);
            ...
            console.log(`Wall OBB center: ${collidable.obb.center.x.toFixed(1)}, ${collidable.obb.center.y.toFixed(1)}, ${collidable.obb.center.z.toFixed(1)}`);
            console.log(`Wall OBB halfSize: ${collidable.obb.halfSize.x.toFixed(1)}, ${collidable.obb.halfSize.y.toFixed(1)}, ${collidable.obb.halfSize.z.toFixed(1)}`);
