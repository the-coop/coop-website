...
export class WeaponSystem {
  ...
  updateWeaponVisibility() {
    ...
    // ALWAYS show viewmodel in first-person mode when we have a current weapon
    ...
    if (this.weaponViewModel) {
      ...
      console.log('Weapon view model children:', this.weaponViewModel.children.length);
    ...
    if (isThirdPerson) {
      ...
      // In first-person, always hide the player model hands (they're shown in viewmodel)
      this.setPlayerHandsVisible(false);
    }
    
    // External weapon model visibility
    // Show for non-hands weapons when player is visible to others
    if (this.externalWeaponModel) {
      this.externalWeaponModel.visible = this.currentWeapon && this.currentWeapon.type !== 'hands';
    }
  ...
  clear() {
    ...
    // Reset inventory to starting state - only hands
    this.inventory = [
      { type: 'hands', ammo: Infinity }, // Slot 0: Always hands
      null, // Slot 1: Empty
      null, // Slot 2: Empty
