...
export class WeaponSystem {
  ...
  createWeaponModels() {
    ...
    const flashlightLight = new THREE.SpotLight(0xffffaa, 2, 30, Math.PI / 6, 0.5, 1);
    flashlightLight.position.set(-0.05, -0.03, -0.41);
    flashlightLight.target.position.set(-0.05, -0.03, -10);
    shotgunGroup.add(flashlightLight);
    shotgunGroup.add(flashlightLight.target);
    
    // Store reference to the light so we can toggle it
  ...
  createMuzzleFlash() {
    ...
    const muzzlePos = this.getWeaponWorldPosition();
  ...
  updateViewModel() {
    ...
    this.thirdPersonWeapon.position.set(0.2, 0.3, -0.2); // Adjust for player mesh
    this.thirdPersonWeapon.rotation.set(0, Math.PI, 0);
    this.thirdPersonWeapon.scale.setScalar(0.8); // Slightly smaller for third-person
  ...
  fireProjectile(weaponType) {
    ...
    const startPos = this.getWeaponWorldPosition();
    ...
    if (hit) {
      ...
      this.createTracer(startPos, hitPoint);
      ...
      this.createTracer(startPos, endPoint);
    }
  }
  ...
  update(deltaTime) {
    ...
    // Update reload
    if (this.isReloading) {
      const reloadProgress = (Date.now() - this.reloadStartTime) / (this.currentWeapon.reloadTime * 1000);
    ...
    // Update pickups - FIX: Use correct property name
    this.weaponPickups.forEach((pickup, id) => {
      // Rotate pickup
      ...
      if (distance < 2.0) {
        // Pick up weapon
        this.pickupWeapon(id); // Pass the pickup ID, not weaponType
      }
    });
  }
