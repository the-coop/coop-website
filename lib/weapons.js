...
export class WeaponSystem {
  ...
  createWeaponModels() {
    ...
    // Center the hands - one on each side
    leftHand.position.set(-0.15, 0, 0);  // Left hand on left side
    rightHand.position.set(0.15, 0, 0);  // Right hand on right side
  ...
  createMuzzleFlash() {
    // Create a more visible muzzle flash
    ...
    // Create additional flash for more visibility
    const flash2 = new THREE.Mesh(flashGeometry, flashMaterial.clone());
    flash2.rotation.z = Math.PI / 4;
    this.muzzleFlash.add(flash2);
    
    // Add glow sphere for volumetric effect
  ...
  showMuzzleFlash() {
    ...
    // Randomize the main flash rotation
    this.muzzleFlash.rotation.z = Math.random() * Math.PI * 2;
    
    // Also randomize child elements if they exist
    this.muzzleFlash.children.forEach((child, index) => {
      if (index === 0) { // Second flash plane
