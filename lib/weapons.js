...
export class WeaponSystem {
  constructor(scene, physics, player) {
    ...
    // Weapon definitions - remove specific weapon assignments to inventory
    this.weaponTypes = {
      hands: {
        name: 'Hands',
        model: null,
        ...
        range: 2,
        ammo: Infinity,
        holdOffset: new THREE.Vector3(0, -0.2, -0.4),
    ...
    // Player inventory - start with only hands, other slots empty and agnostic
    this.inventory = [
      { type: 'hands', ammo: Infinity }, // Slot 0: Always hands
      null, // Slot 1: Can hold any weapon
      null, // Slot 2: Can hold any weapon
      null, // Slot 3: Can hold any weapon
      null  // Slot 4: Can hold any weapon
    ];
  ...
  loadWeaponModels() {
    // Load ALL weapon models upfront - no lazy loading
    console.log('Loading all weapon models...');
    
    try {
      this.weaponTypes.hands.model = createHandsModel();
      console.log('Hands model loaded:', this.weaponTypes.hands.model);
  ...
  updateViewModel() {
    ...
    // Create third-person weapon model - only for non-hands weapons
    if (this.currentWeapon.type !== 'hands') {
    ...
    // Add muzzle flash if weapon can shoot (not for hands)
    if (weaponType.muzzleOffset && weaponType.damage > 0 && this.currentWeapon.type !== 'hands') {
  ...
  createExternalWeaponModel(weaponDef) {
    ...
    // Don't create external model for hands - player model hands will be shown instead
    if (weaponDef.name === 'Hands' || this.currentWeapon.type === 'hands') {
      console.log('Not creating external weapon model for hands');
      return;
    }
  ...
  updateWeaponVisibility() {
    ...
    if (isInVehicle) {
      ...
      this.setPlayerHandsVisible(false);
      console.log('In vehicle - hiding all weapons and hands');
      return;
    }
    
    // First-person view model visibility
    // Show viewmodel in first-person mode when we have a current weapon (including hands)
    ...
    if (this.weaponViewModel) {
      ...
      this.weaponViewModel.traverse((child) => {
        if (child.isMesh) {
          ...
          // Log mesh details for hands
          if (this.currentWeapon.type === 'hands') {
            console.log('Hands mesh:', child.name || 'unnamed', 'visible:', child.visible);
          }
        }
      });
    }
    ...
    // Only show in third-person mode and only for non-hands weapons
    this.thirdPersonWeaponGroup.visible = isThirdPerson && this.currentWeapon && this.currentWeapon.type !== 'hands';
    
    // Handle hands visibility in third-person
    if (isThirdPerson) {
      if (this.currentWeapon && this.currentWeapon.type === 'hands') {
        // Show player's built-in hands when hands weapon is selected
        this.setPlayerHandsVisible(true);
      } else {
        // Hide player's built-in hands when other weapons are selected
        this.setPlayerHandsVisible(false);
      }
    } else {
      // In first-person, always hide the player model hands
      // The hands weapon viewmodel will be shown instead
      this.setPlayerHandsVisible(false);
    }
    
    // External weapon model visibility - FIXED LOGIC
    // Only show external weapon model when in third-person mode for non-hands weapons
    // This model is meant for OTHER players to see what weapon you're holding
    // In multiplayer, this would always be visible to others, but for local player
    // it should only be visible when they're in third-person mode
    if (this.externalWeaponModel) {
      const shouldShowExternal = isThirdPerson && 
                                 this.currentWeapon && 
                                 this.currentWeapon.type !== 'hands';
  ...
  setPlayerHandsVisible(visible) {
    if (!this.player.mesh) return;
    
    // Find and toggle visibility of hands in the player model
    this.player.mesh.traverse((child) => {
      if (child.name && (child.name.includes('Hand') || child.name.includes('Finger') || child.name.includes('Thumb'))) {
        child.visible = visible;
      }
    });
    
    console.log(`Player hands visibility set to: ${visible}`);
  }
  ...
  pickupWeapon(pickupId) {
    ...
    // Check for empty slots first (skip slot 0 which is always hands)
    ...
    // If no empty slot, replace current weapon (only if not hands)
    ...
    // If still no slot available, inform player
    if (slotIndex === -1) {
      console.log('No available weapon slots - inventory full');
      return false;
    }
  ...
  updateAllTargetIndicators(allTargets, lockData) {
    ...
    allTargets.forEach(target => {
      ...
      // Update color based on lock cone status
      ...
      if (isLockedTarget && lockData.isLocked) {
        // Locked target - red
        indicator.diamond.material.color.setHex(0xff0000);
        indicator.diamond.material.opacity = 0.9;
        indicator.diamond.visible = true;
        indicator.box.visible = true;
        indicator.box.material.color.setHex(0xff0000);
        
        // Animate locked indicator
        const pulse = Math.sin(Date.now() * 0.005) * 0.1 + 0.9;
        indicator.group.scale.setScalar(scale * pulse);
        indicator.diamond.rotation.z = Date.now() * 0.001;
      } else if (target.inLockCone) {
        // In lock cone - orange
        indicator.diamond.material.color.setHex(0xff8800);
        indicator.diamond.material.opacity = 0.7;
        indicator.diamond.visible = true;
        indicator.box.visible = true;
        indicator.box.material.color.setHex(0xff8800);
        
        // Show lock progress if this is the current target
        if (isLockedTarget && lockData.lockProgress > 0 && lockData.lockProgress < 1) {
          ...
          if (!indicator.progressRing) {
            ...
            indicator.progressRing = new THREE.Mesh(progressGeometry, progressMaterial);
            indicator.progressRing.renderOrder = 998;
            indicator.group.add(indicator.progressRing);
          }
          
          const angle = lockData.lockProgress * Math.PI * 2;
          indicator.progressRing.visible = true;
          indicator.progressRing.geometry.dispose();
          indicator.progressRing.geometry = new THREE.RingGeometry(2.5, 2.7, 32, 1, 0, angle);
        } else if (indicator.progressRing) {
          indicator.progressRing.visible = false;
        }
      } else {
        // Not in lock cone - white
        indicator.diamond.material.color.setHex(0xffffff);
        indicator.diamond.material.opacity = 0.5;
        indicator.diamond.visible = true;
  ...
  clear() {
    ...
    // Reset inventory to starting state - hands in slot 0
    this.inventory = [
      { type: 'hands', ammo: Infinity }, // Slot 0: Always hands
      null, // Slot 1: Empty
      null, // Slot 2: Empty
    ...
    // Update view model to show hands
    this.updateViewModel();
