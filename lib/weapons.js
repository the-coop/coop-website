...
export class WeaponSystem {
  constructor(scene, physics, player) {
    ...
    this.weaponTypes = {
      hands: {
        ...
        range: 2,
        ammo: Infinity,
        holdOffset: new THREE.Vector3(0, -0.2, -0.4),  // Changed from (0.3, -0.2, -0.4) to center hands
  ...
  updateViewModel() {
    ...
    if (!weaponType || !weaponType.model) {
      console.log(`Invalid weapon type: ${this.currentWeapon.type}`);
      this.updateWeaponVisibility();
      return;
    }
    
    console.log(`Creating view model for: ${this.currentWeapon.type}`);
    
    // Clone the weapon model - this includes hands when hands are selected
    ...
    // No separate hands model - hands ARE a weapon type
  ...
  updateWeaponVisibility() {
    ...
    if (isInVehicle) {
      ...
      // Hide player hands when in vehicle
    ...
    // Show viewmodel only in first-person mode AND only if we have a current weapon
  ...
  pickupWeapon(pickupId) {
    ...
    console.log('ViewModelGroup children:', this.viewModelGroup.children.length);
