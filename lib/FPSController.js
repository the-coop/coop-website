...
export class FPSController {
  ...
  handleAllMovement(deltaTime, frameCount) {
    ...
    try {
      ...
      // Apply gravity to scene FIRST - but skip local player if we have recent server update
      const recentServerUpdate = this.lastServerUpdate && 
                                 (Date.now() - this.lastServerUpdate.timestamp) < 100;
      
      if (!recentServerUpdate) {
        // Only apply gravity if we don't have recent server authority
        this.physics.applyGravityToScene(this.scene.scene, deltaTime);
      } else {
        // Apply gravity only to other objects, not local player
        this.physics.applyGravityToOtherObjects(this.scene.scene, deltaTime, this.playerBody);
      }
