...
export class FPSController {
  ...
  createPlayer(x, y, z) {
    ...
    if (this.playerBody) {
      console.log("Player already exists, not creating again");
      return;
    }
  ...
  checkGrounded(frameCount) {
    ...
    try {
      ...
      // CRITICAL: Trust server state more aggressively to prevent conflicts
      if (this.lastServerUpdate && (Date.now() - this.lastServerUpdate.timestamp) < 500) {
        const serverGrounded = this.lastServerUpdate.isGrounded;
        
        // If server says grounded and we have ANY indication of ground, trust server
        if (serverGrounded && (hasGroundCollisions || hasRayHits || recentGroundContact || lowDownwardVelocity)) {
          this.isGrounded = true;
          // Reset ground contact time to prevent false airborne transitions
          this.lastGroundContact = currentTime;
        } 
        // Only become airborne if server agrees AND we have no ground contact
        else if (!serverGrounded && !hasGroundCollisions && !hasRayHits && !lowDownwardVelocity) {
          this.isGrounded = false;
        }
        // In conflict cases, trust server
        else if (this.isGrounded !== serverGrounded) {
          this.isGrounded = serverGrounded;
          if (serverGrounded) {
            this.lastGroundContact = currentTime;
          }
        }
      ...
      // Only transition to airborne if we're REALLY sure
      if (this.wasGrounded && !this.isGrounded) {
        // Double-check with server before transitioning
        if (this.lastServerUpdate && this.lastServerUpdate.isGrounded && 
            (Date.now() - this.lastServerUpdate.timestamp) < 200) {
          // Server still thinks we're grounded, don't transition
          this.isGrounded = true;
          this.lastGroundContact = currentTime;
        } else {
          // OK to transition to airborne
  ...
  getPlayerState() {
    // Return null if player doesn't exist yet
  ...
  reconcileWithServer(serverData, worldOriginOffset) {
    ...
    if (ackIndex >= 0) {
      ...
      const shouldReconcile = this.predictionError > 0.1 || groundingConflict; // Increased threshold
      ...
      if (shouldReconcile) {
        ...
        // Update ground state based on server
        if (serverData.isGrounded) {
          // Server says grounded - ensure we maintain ground contact
          this.lastGroundContact = Date.now();
          // Don't clear collisions - they might be valid
        } else {
          // Server says airborne - clear ground state
          this.groundCollisions.clear();
          this.lastGroundContact = 0;
        }
        
        // Don't re-apply pending inputs during grounding conflicts
        // This prevents the client from immediately disagreeing with server again
        if (!groundingConflict && this.pendingInputs.length > 0) {
          // Only re-apply very recent inputs with reduced strength
          const recentInputs = this.pendingInputs.slice(-2); // Only last 2 inputs
          recentInputs.forEach(input => {
            this.applyInputLocally(input.input, input.deltaTime * 0.3); // Very reduced strength
          });
        }
      }
    }
