...
export class FPSController {
  ...
  checkGrounded(frameCount) {
    ...
    try {
      ...
      if (this.isGrounded !== this.wasGrounded) {
        if (this.isGrounded) {
          console.log("Player became grounded - Collisions:", this.groundCollisions.size, "Ray hits:", hasRayHits);
        } else {
          console.log("Player became airborne - Collisions:", this.groundCollisions.size, "Ray hits:", hasRayHits);
        }
      }
      
      if (frameCount % 60 === 0) {
        console.log("Ground check - Collisions:", this.groundCollisions.size, 
                    "Ray hits:", hasRayHits, 
  ...
  handleAllMovement(deltaTime, frameCount) {
    ...
    try {
      ...
      // Apply same gravity scaling as server
      let gravityMultiplier = 1.0;
      if (distanceToPlanet > 50.0) {
        gravityMultiplier = Math.min(300.0 / distanceToPlanet, 2.0);
      } else {
        gravityMultiplier = 2.0;
      }
      
      const effectiveGravity = this.gravityStrength * gravityMultiplier;
      
      // Apply gravity to entire scene FIRST - this handles ALL dynamic bodies including the local player
      ...
      // Apply final velocity to local player
      ...
      if (frameCount % 60 === 0 && (moveLength > 0 || !this.isGrounded)) {
        const rollState = this.keys.rollLeft ? "L" : (this.keys.rollRight ? "R" : "-");
        console.log("Movement - Planet distance:", distanceToPlanet.toFixed(1),
                    "Gravity multiplier:", gravityMultiplier.toFixed(2),
                    "Effective gravity:", effectiveGravity.toFixed(1),
                    "Final vel:", newVelX.toFixed(2), newVelY.toFixed(2), newVelZ.toFixed(2),
                    "Grounded:", this.isGrounded, "Roll:", rollState);
      }
    } catch (e) {
  ...
  reconcileWithServer(serverData, worldOriginOffset) {
    ...
    if (ackIndex >= 0) {
      ...
      // Reconcile if error is too large
      if (this.predictionError > 0.1) {
        console.log('Reconciling position - error:', this.predictionError);
        ...
        this.pendingInputs.forEach(input => {
          this.applyInputLocally(input.input, input.deltaTime);
        });
      }
    }
  }
