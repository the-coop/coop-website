...
export class PlaneController {
  ...
  enterPlane(player) {
    ...
    this.scene.camera.position.set(0, 4, 12);
    // Look down slightly at the plane
  ...
  updateCamera() {
    ...
    if (this.scene.camera.parent === this.mesh) {
      ...
      const extraDist = speedFactor * 8; // Add up to 8 more units
      
      this.scene.camera.position.z = baseDist + extraDist;
      
      // Also adjust height slightly based on speed
      this.scene.camera.position.y = 4 + speedFactor * 2;
    }
  }
