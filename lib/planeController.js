...
export class PlaneController {
  ...
  handleControls(deltaTime, upDir) {
    ...
    if (this.controls.throttleUp) {
      this.enginePower = Math.min(this.enginePower + deltaTime * 1.5, 1.0); // Increased from 1.0
    } else if (this.controls.throttleDown) {
      this.enginePower = Math.max(this.enginePower - deltaTime * 1.5, 0); // Increased from 1.0
    }
    ...
    if (speed > 1 || !this.isGrounded) { // Reduced from 2
      // Pitch control with W/S - faster response
      if (this.controls.pitchUp) {
        this.pitch = Math.min(this.pitch + deltaTime * 3, this.maxPitch); // Increased from 2
