...
export class PlaneController {
  ...
  create(position) {
    ...
    // Create fuselage (more streamlined)
  ...
  createWeaponHardpoints() {
    // Add gun (internal M61 Vulcan cannon)
    ...
    for (let i = 0; i < 3; i++) {
      for (let side of [-1, 1]) {
        const pylon = new THREE.Mesh(pylonGeometry, gunMaterial);
        const xPos = side * (1.5 + i * 0.8);
        pylon.position.set(xPos, -0.4, 0);
        this.mesh.add(pylon);
        
        const missile = new THREE.Mesh(missileGeometry, missileMaterial);
        missile.rotation.x = Math.PI / 2;
        missile.position.set(xPos, -0.7, 0);
        missile.visible = true;
        missile.userData.missileIndex = this.missilePositions.length;
        this.mesh.add(missile);
        
        this.missilePositions.push({
          mesh: missile,
          position: new THREE.Vector3(xPos, -0.7, 0),
          used: false
        });
      }
    }
