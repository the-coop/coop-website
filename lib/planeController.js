...
export class PlaneController {
  ...
  create(position) {
    // Create plane mesh group
    this.mesh = new THREE.Group();
    
    // Fuselage
    const fuselageGeometry = new THREE.CylinderGeometry(0.5, 0.5, 6, 8);
    ...
    const stabilizerGeometry = new THREE.BoxGeometry(3, 0.1, 0.8);
    const stabilizer = new THREE.Mesh(stabilizerGeometry, wingMaterial);
    stabilizer.position.set(0, 0, -2.5);
  ...
  createLandingGear() {
    ...
    noseGear.add(noseWheelMesh);
    
    noseGear.position.set(0, -0.5, 2);
    ...
    for (let side of ['left', 'right']) {
      ...
      mainGear.add(strutMesh);
      
      const wheel = new THREE.CylinderGeometry(0.2, 0.2, 0.1);
      const wheelMesh = new THREE.Mesh(wheel, gearMaterial);
      wheelMesh.rotation.z = Math.PI / 2;
      wheelMesh.position.y = -0.5;
      mainGear.add(wheelMesh);
      
      const xPos = side === 'left' ? -1.5 : 1.5;
      mainGear.position.set(xPos, -0.5, 0);
      mainGear.userData.baseY = -0.5;
      this.landingGear[side] = mainGear;
  ...
  createAircraftLights() {
    ...
    this.lights.navigationRed.position.set(-5, 0, 0.5);
    this.mesh.add(this.lights.navigationRed);
    
    const navGreenGeometry = new THREE.SphereGeometry(0.05, 8, 6);
    ...
    this.lights.navigationGreen.position.set(5, 0, 0.5);
    this.mesh.add(this.lights.navigationGreen);
    
    // White strobe on tail
    const strobeGeometry = new THREE.SphereGeometry(0.04, 8, 6);
    ...
    this.lights.strobeWhite.position.set(0, 2, -2.5);
    this.mesh.add(this.lights.strobeWhite);
    
    // Landing lights
    const landingLightGeometry = new THREE.SphereGeometry(0.08, 8, 6);
    const landingLightMaterial = new THREE.MeshBasicMaterial({
      color: 0xffffff,
      emissive: 0xffffff,
      emissiveIntensity: 1.0
    });
    this.lights.landingLights = new THREE.Mesh(landingLightGeometry, landingLightMaterial);
    this.lights.landingLights.position.set(0, -0.5, 2.5);
    this.mesh.add(this.lights.landingLights);
  }
  ...
  toggleLights() {
    ...
    if (this.lights.landingLights) {
      this.lights.landingLights.visible = this.lightsEnabled;
    }
  }
  ...
  updateAircraftLights(deltaTime) {
    ...
    // White strobe - flash pattern
    if (this.lights.strobeWhite) {
      const strobeTime = this.lightAnimationTime % 1.0;
      this.lights.strobeWhite.material.emissiveIntensity = strobeTime < 0.1 ? 2.0 : 0.1;
    }
    
    // Landing lights - on when gear down
    if (this.lights.landingLights) {
      this.lights.landingLights.material.emissiveIntensity = this.landingGearExtended ? 1.0 : 0.1;
    }
  }
  ...
  enterPlane(player) {
    ...
    this.scene.camera.position.set(0, 2, -8);
  ...
  applyAerodynamics(deltaTime) {
    ...
    // Check for stall
    this.isStalling = Math.abs(angleOfAttack) > this.stallAngle;
    ...
    if (airspeed > this.minSpeed) {
      ...
      if (!this.isStalling) {
        ...
        // Reduce lift at very high angles (approaching stall)
