...
export class PlaneController {
  constructor(scene, physics, position) {
    ...
    // Plane properties
    this.maxThrust = 50.0;
    this.maxPitch = 0.5; // Reduced from 0.8
    this.maxRoll = 0.6; // Reduced from 1.0
    this.maxYawRate = 0.8; // Reduced from 1.5
    this.enginePower = 0;
    this.lift = 0;
  ...
  createPhysics(position) {
    ...
    this.body = this.physics.createDynamicBody(position, {
      ...
      angularDamping: 2.0  // Increased from 1.0
    });
  ...
  checkGrounded() {
    ...
    // Check if we're on the ground using gravity-based down
    this.isGrounded = this.physics.isPositionGrounded(
      new THREE.Vector3(
        position.x + downDirection.x * 0.5,
        position.y + downDirection.y * 0.5,
        position.z + downDirection.z * 0.5
      ),
      downDirection,
      1.0
    );
    
    // Also consider grounded if moving very slowly and close to ground
    if (velocity.length() < 2 && position.y < 2) {
      this.isGrounded = true;
    }
  }
