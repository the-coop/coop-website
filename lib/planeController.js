...
export class PlaneController {
  ...
  enterPlane(player) {
    ...
    // Position camera behind the plane (negative Z is behind when forward is +Z)
    this.scene.camera.position.set(0, 12, -40);  // Changed from positive to negative Z
  ...
  update(deltaTime) {
    ...
    if (this.isOccupied && this.keys) {
      // Fix throttle controls - use Shift for up, Ctrl for down
      this.controls.throttleUp = this.keys.run;        // Shift key
      this.controls.throttleDown = this.keys.crouch;   // Changed from rollLeft to crouch (Ctrl)
      ...
      // Fix yaw controls - Q/E should be yaw, not throttle
    ...
    if (speed > 2 || !this.isGrounded) { // Reduced from 5 to 2
      // Control effectiveness based on speed
      const controlEffectiveness = Math.min(1, speed / 15); // Reduced from 20
  ...
  handleControls(deltaTime, upDir) {
    // Fix throttle controls to use correct inputs
    if (this.controls.throttleUp) {
      this.enginePower = Math.min(this.enginePower + deltaTime * 1.0, 1.0); // Increased rate
    } else if (this.controls.throttleDown) {
      this.enginePower = Math.max(this.enginePower - deltaTime * 1.0, 0); // Increased rate
    }
    ...
    // Reduce minimum speed requirement for ground movement
    if (speed > 2 || !this.isGrounded) { // Reduced from 5 to 2
      ...
      // Yaw control with Q/E - improved for ground steering
      if (this.controls.yawLeft) {
        const yawRate = this.isGrounded ? 1.0 : 1.5; // Improved ground yaw
  ...
  updateCamera() {
    ...
    if (this.scene.camera.parent === this.mesh) {
      ...
      this.scene.camera.position.y = 12 + speedFactor * 3;
  ...
  getFlightData() {
    ...
    // Fix stall warning - don't show when grounded or landing gear extended
    ...
    return {
      ...
      stallWarning: isStalling, // Fixed stall logic
