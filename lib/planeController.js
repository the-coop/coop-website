...
export class PlaneController {
  ...
  create(position) {
    ...
    // Add bomb pylons under fuselage
    this.bombPylons = [];
    const bombPositions = [
      { x: -1, y: -0.8, z: 0 },
      { x: 1, y: -0.8, z: 0 },
    ...
    bombPositions.forEach((pos, i) => {
      if (i < this.maxBombs) {
        const pylonGeometry = new THREE.BoxGeometry(0.1, 0.2, 0.3);
  ...
  fireMissile() {
    ...
    const missile = {
      ...
      age: 0,
      maxAge: 2.0, // Changed from 15 to 2 seconds
      thrust: 50,
      guidanceActive: true,
      hasExploded: false
    };
  ...
  getFlightData() {
    ...
    return {
      ...
      missiles: this.currentMissiles, // Add missile count
      bombs: this.currentBombs       // Add bomb count
    };
  }
