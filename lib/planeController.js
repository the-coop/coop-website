...
export class PlaneController {
  constructor(scene, physics, position) {
    ...
    // Add landing gear physics references
    this.landingGearBodies = {
      front: null,
      left: null,
      right: null
    };
    this.landingGearJoints = {
      front: null,
      left: null,
      right: null
    };
    
    // Add exclusion group for wheels
    this.wheelExclusionGroup = 2; // Different from car's group
  }
  ...
  create(position) {
    ...
    // Create fuselage - FIX: Remove rotation, orient correctly
    ...
    fuselage.rotation.z = Math.PI / 2;  // Rotate around Z to point forward
    fuselage.position.x = -1;  // Adjust position since cone points along X after rotation
    this.mesh.add(fuselage);
    
    // Add cockpit canopy - adjust position
    ...
    canopy.scale.set(1.5, 0.6, 1);
    canopy.position.set(1, 0.4, 0);
    this.mesh.add(canopy);
    
    // Add swept wings - no rotation needed
    const wingGeometry = new THREE.BoxGeometry(2, 0.2, 7);
    const wings = new THREE.Mesh(wingGeometry, fuselageMaterial);
    wings.position.set(0, -0.2, 0);
    this.mesh.add(wings);
    
    // Add vertical stabilizer - no rotation needed
    const tailGeometry = new THREE.BoxGeometry(1.5, 2.5, 0.2);
    const tail = new THREE.Mesh(tailGeometry, fuselageMaterial);
    tail.position.set(-3, 0.8, 0);
    tail.rotation.y = -0.2; // Slight angle
    this.mesh.add(tail);
    
    // Add horizontal stabilizers
    const stabilizerGeometry = new THREE.BoxGeometry(1, 0.1, 3);
    const stabilizer = new THREE.Mesh(stabilizerGeometry, fuselageMaterial);
    stabilizer.position.set(-3, 0, 0);
    this.mesh.add(stabilizer);
    
    // Add engine intake
    const intakeGeometry = new THREE.BoxGeometry(2, 0.8, 1.2);
    ...
    exhaust.rotation.z = Math.PI / 2;  // Rotate to point backward
    exhaust.position.set(-4, 0, 0);
    ...
    // Position and add to scene - HIGHER spawn position
    const spawnHeight = position.y + 3; // Add 3 units to spawn height
    this.mesh.position.set(position.x, spawnHeight, position.z);
    this.mesh.castShadow = true;
    this.mesh.receiveShadow = true;
    this.scene.scene.add(this.mesh);
    
    // Create physics with higher position
    this.createPhysics(new THREE.Vector3(position.x, spawnHeight, position.z));
    ...
    console.log('F-16 created at', position, 'with height', spawnHeight);
  }
  ...
  createWeaponHardpoints() {
    ...
    gun.rotation.z = Math.PI / 2;  // Point forward
    gun.position.set(2.5, -0.3, -0.5);
    this.mesh.add(gun);
    ...
    for (let i = 0; i < 3; i++) {
      for (let side of [-1, 1]) {
        ...
        pylon.position.set(0 - i * 0.5, -0.5, side * (1 + i * 0.8));
        this.mesh.add(pylon);
        
        const missile = new THREE.Mesh(missileGeometry, missileMaterial);
        missile.rotation.z = Math.PI / 2;  // Point forward
        missile.position.set(0 - i * 0.5, -0.8, side * (1 + i * 0.8));
  ...
  createPhysics(position) {
    // Create main body for fuselage with better initial settings
    this.body = this.physics.createDynamicBody(position, {
      linearDamping: 0.3,
      angularDamping: 2.0,
      mass: 5000.0 // 5 tons for F-16
    });
    
    // Set collision groups for main body
    this.body.setCollisionGroups(0x00010001); // Group 1, collides with all
    ...
    const fuselageCollider = this.physics.createBoxCollider(
      new THREE.Vector3(4, 0.6, 0.8),  // Adjusted for rotated orientation
      {
        density: 0.3,
        friction: 0.5,
        restitution: 0.2
    ...
    const tailColliderDesc = this.physics.createBoxCollider(
      new THREE.Vector3(0.75, 1.25, 0.1),
      {
        density: 0.1,
        friction: 0.5,
        restitution: 0.2
    ...
    tailColliderDesc.setTranslation(-3, 0.4, 0);
    ...
    // Create landing gear physics bodies
    this.createLandingGearPhysics(position);
  }
  
  createLandingGearPhysics(planePosition) {
    // Front gear
    const frontGearPos = planePosition.clone().add(new THREE.Vector3(2, -0.8, 0));
    this.landingGearBodies.front = this.physics.createDynamicBody(frontGearPos, {
      linearDamping: 0.5,
      angularDamping: 0.5,
      mass: 50.0
    });
    
    const frontWheelCollider = this.physics.createCylinderCollider(0.2, 0.1, {
      friction: 0.8,
      restitution: 0.2,
      density: 1.0
    });
    this.physics.world.createCollider(frontWheelCollider, this.landingGearBodies.front);
    
    // Set collision groups - wheels don't collide with main body
    this.landingGearBodies.front.setCollisionGroups((this.wheelExclusionGroup << 16) | 0xffff);
    
    // Create revolute joint for front gear
    const frontAnchor1 = { x: 2, y: -0.8, z: 0 };
    const frontAnchor2 = { x: 0, y: 0, z: 0 };
    const frontAxis = { x: 0, y: 0, z: 1 }; // Allow rotation around Z axis
    
    this.landingGearJoints.front = this.physics.world.createImpulseJoint(
      RAPIER.JointData.revolute(frontAnchor1, frontAnchor2, frontAxis),
      this.body,
      this.landingGearBodies.front,
      true
    );
    
    // Rear gears
    const gearPositions = {
      left: new THREE.Vector3(-0.5, -0.8, -1.5),
      right: new THREE.Vector3(-0.5, -0.8, 1.5)
    };
    
    for (let side of ['left', 'right']) {
      const gearPos = planePosition.clone().add(gearPositions[side]);
      this.landingGearBodies[side] = this.physics.createDynamicBody(gearPos, {
        linearDamping: 0.5,
        angularDamping: 0.5,
        mass: 100.0
      });
      
      const wheelCollider = this.physics.createCylinderCollider(0.25, 0.15, {
        friction: 0.8,
        restitution: 0.2,
        density: 1.0
      });
      this.physics.world.createCollider(wheelCollider, this.landingGearBodies[side]);
      
      // Set collision groups
      this.landingGearBodies[side].setCollisionGroups((this.wheelExclusionGroup << 16) | 0xffff);
      
      // Create revolute joint
      const anchor1 = { 
        x: gearPositions[side].x, 
        y: gearPositions[side].y, 
        z: gearPositions[side].z 
      };
      const anchor2 = { x: 0, y: 0, z: 0 };
      const axis = { x: 0, y: 0, z: 1 };
      
      this.landingGearJoints[side] = this.physics.world.createImpulseJoint(
        RAPIER.JointData.revolute(anchor1, anchor2, axis),
        this.body,
        this.landingGearBodies[side],
        true
      );
    }
    
    // Set main body to not collide with wheels
    this.body.setCollisionGroups(0x00010001 | (this.wheelExclusionGroup << 16));
  }
  ...
  enterPlane(player) {
    ...
    this.entryTime = Date.now(); // Track entry time
    ...
    // Position camera further behind the plane
    this.scene.camera.position.set(0, 10, 35);  // Increased from (0, 8, 25) to (0, 10, 35)
    ...
    this.scene.camera.rotation.x = -0.2;  // Increased from -0.15
  ...
  updateCamera() {
    ...
    if (this.scene.camera.parent === this.mesh) {
      ...
      const baseDist = 35;  // Increased from 25
      const speedFactor = Math.min(speed / 50, 1); // Normalize to 0-1
      const extraDist = speedFactor * 15; // Increased from 10
      
      this.scene.camera.position.z = baseDist + extraDist;
      
      // Also adjust height slightly based on speed
      this.scene.camera.position.y = 10 + speedFactor * 3;  // Increased from 8 + speedFactor * 2
  ...
  destroy() {
    // Remove landing gear physics
    if (this.landingGearJoints.front) this.physics.world.removeImpulseJoint(this.landingGearJoints.front, true);
    if (this.landingGearJoints.left) this.physics.world.removeImpulseJoint(this.landingGearJoints.left, true);
    if (this.landingGearJoints.right) this.physics.world.removeImpulseJoint(this.landingGearJoints.right, true);
    
    if (this.landingGearBodies.front) this.physics.world.removeRigidBody(this.landingGearBodies.front);
    if (this.landingGearBodies.left) this.physics.world.removeRigidBody(this.landingGearBodies.left);
    if (this.landingGearBodies.right) this.physics.world.removeRigidBody(this.landingGearBodies.right);
  ...
  updateLandingGear(deltaTime) {
    ...
    if (this.landingGearTransition !== targetTransition) {
      ...
      // Enable/disable wheel physics based on gear state
      if (this.landingGearTransition < 0.5) {
        // Gear retracted - disable wheel bodies
        if (this.landingGearBodies.front) this.landingGearBodies.front.setEnabled(false);
        if (this.landingGearBodies.left) this.landingGearBodies.left.setEnabled(false);
        if (this.landingGearBodies.right) this.landingGearBodies.right.setEnabled(false);
      } else {
        // Gear extended - enable wheel bodies
        if (this.landingGearBodies.front) this.landingGearBodies.front.setEnabled(true);
        if (this.landingGearBodies.left) this.landingGearBodies.left.setEnabled(true);
        if (this.landingGearBodies.right) this.landingGearBodies.right.setEnabled(true);
      }
    }
    
    // Update wheel positions to match visual landing gear
    if (this.landingGearExtended && this.body) {
      const bodyPos = this.body.translation();
      const bodyRot = this.body.rotation();
      
      // Update wheel body positions to follow plane
      if (this.landingGearBodies.front) {
        const frontOffset = new THREE.Vector3(2, -0.8 + (1 - this.landingGearTransition) * 0.8, 0);
        frontOffset.applyQuaternion(new THREE.Quaternion(bodyRot.x, bodyRot.y, bodyRot.z, bodyRot.w));
        this.landingGearBodies.front.setTranslation({
          x: bodyPos.x + frontOffset.x,
          y: bodyPos.y + frontOffset.y,
          z: bodyPos.z + frontOffset.z
        });
      }
      
      // Similar for rear wheels
      const offsets = {
        left: new THREE.Vector3(-0.5, -0.8 + (1 - this.landingGearTransition) * 0.8, -1.5),
        right: new THREE.Vector3(-0.5, -0.8 + (1 - this.landingGearTransition) * 0.8, 1.5)
      };
      
      for (let side of ['left', 'right']) {
        if (this.landingGearBodies[side]) {
          const offset = offsets[side].clone();
          offset.applyQuaternion(new THREE.Quaternion(bodyRot.x, bodyRot.y, bodyRot.z, bodyRot.w));
          this.landingGearBodies[side].setTranslation({
