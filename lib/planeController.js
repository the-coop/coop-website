...
export class PlaneController {
  ...
  fireMissile() {
    ...
    afterburnerGroup.position.z = -0.4; // Position at missile rear
    ...
    const innerFlameGeometry = new THREE.ConeGeometry(0.04, 0.3, 6);
    ...
    // Engine light
    const engineLight = new THREE.PointLight(0xff4400, 0, 15);
    ...
    const launchVelocity = forward.multiplyScalar(30).add(planeVel); // Start slower, motor will boost
    ...
    const updateMissile = () => {
      ...
      if (!missile.motorIgnited && missile.age >= 1.0) {
        ...
        missile.thrust = 80; // Reduced from 120 - more realistic thrust
        ...
        const ignitionImpulse = missileForward.multiplyScalar(15); // Reduced from 50 - gentler kick
      ...
      if (missile.guidanceActive && missile.motorIgnited) {
        ...
        const correction = targetDir.clone()
          .sub(missileForward)
          .multiplyScalar(0.05); // Gentle guidance
      ...
      if (missile.age >= missile.maxAge && !missile.hasExploded) {
        ...
        console.log('Plane missile exploded!');
        return;
      }
