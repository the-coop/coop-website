...
export class TPController {
  constructor(scene, physics) {
    ...
    // Input state - ensure yaw controls are included
    this.keys = {
      forward: false,
      backward: false,
      left: false,
    ...
    this.vehicleExitCooldown = null;
    
    // Weapon system placeholder (TP doesn't have weapons yet)
  ...
  tryEnterVehicle() {
    // Same implementation as FPSController
    ...
    this.scene.vehicles.forEach((vehicle) => {
      ...
      if (distance < nearestDistance && !vehicle.isOccupied) {
        nearestDistance = distance;
        nearestVehicle = vehicle;
      }
    });
    
    this.scene.dynamicObjects.forEach((obj) => {
      if (obj.type === 'vehicle' && obj.controller) {
        const distance = playerPos.distanceTo(obj.controller.getPosition());
        if (distance < nearestDistance && !obj.controller.isOccupied) {
          nearestDistance = distance;
          nearestVehicle = obj.controller;
        }
      }
    });
    
    if (nearestVehicle) {
      if (nearestVehicle.enterCar) {
        return this.enterCar(nearestVehicle);
      } else if (nearestVehicle.enterPlane) {
        return this.enterPlane(nearestVehicle);
      } else if (nearestVehicle.enterHelicopter) {
        return this.enterHelicopter(nearestVehicle);
      }
    }
  ...
  checkNearbyVehicles() {
    if (!this.scene || !this.scene.vehicles) return null;
    
    const playerPos = this.getPosition();
    let nearestVehicle = null;
    let nearestDistance = 5.0; // Max interaction distance
    
    this.scene.vehicles.forEach((vehicle, id) => {
      if (!vehicle.getPosition) return;
  ...
  enterCar(car) {
    // Similar to FPSController implementation
    if (!car || !car.chassisMesh) return false;
    ...
    console.log('TP player entered car');
    return true;
  }
  
  enterPlane(plane) {
    if (!plane || !plane.mesh) return false;
    ...
    this.isInVehicle = true;
    this.currentVehicle = plane;
    plane.isOccupied = true;
    
    console.log('TP player entered plane');
    return true;
  }
  
  enterHelicopter(helicopter) {
    if (!helicopter || !helicopter.mesh) return false;
    ...
    this.isInVehicle = true;
    this.currentVehicle = helicopter;
    helicopter.isOccupied = true;
    
    console.log('TP player entered helicopter');
    return true;
  }
  
  exitVehicle(exitPosition) {
    console.log('TP player exiting vehicle at position:', exitPosition);
    ...
    // Re-enable and position physics body
    if (this.body) {
    ...
    this.vehicleExitCooldown = Date.now() + 1000;
    
    console.log('TP player exited vehicle');
  }
  
  tryPickupWeapon() {
    // Placeholder - TP controller doesn't have weapons yet
    console.log('Weapon pickup not implemented for third person controller');
  }
  ...
  getNetworkState() {
    ...
    return {
      ...
      isSwimming: this.isSwimming || false
    };
  }
  ...
  dispose() {
    ...
    console.log('Third Person Controller disposed');
  }
  
  // Add destroy as alias for dispose
  destroy() {
    this.dispose();
  }
}
