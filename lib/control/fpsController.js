  ...
  checkGrounded() {
    ...
    if (this.scene.vehicles) {
      for (const [id, vehicle] of this.scene.vehicles) {
        if (vehicle.constructor.name === 'SpaceshipController' && vehicle.isPlayerInside(playerPos)) {
          this.isInSpaceship = true;
          this.currentSpaceship = vehicle;
          
          // Use spaceship's floor normal as our "up" direction
    ...
    // If not in spaceship anymore, clear the state
    if (this.currentSpaceship && !this.currentSpaceship.isPlayerInside(playerPos)) {
      this.isInSpaceship = false;
      this.currentSpaceship = null;
