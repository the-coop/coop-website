...
export class FPSController {
  ...
  checkGrounded() {
    ...
    // FIXED: Consider slope movement when checking velocity
    let lowDownwardVelocity = false;
    if (hasRayHits && this.lastGroundNormal) {
      // If we're on a slope, check velocity relative to the slope normal
      const velocityAlongNormal = velocity.dot(this.lastGroundNormal);
      // Allow higher "downward" velocity when moving along slopes
      lowDownwardVelocity = velocityAlongNormal > -5.0 && velocityAlongNormal < 10.0;
    } else {
      // Standard check for flat surfaces
      lowDownwardVelocity = downwardVelocity < 2.0 && downwardVelocity > -0.5;
    }
    
    const recentGroundContact = (currentTime - this.physics.lastGroundContact) < 100; // Reduced from 200ms
    
    // Determine grounding with priority system
    if (hasRayHits) {
      // Ray hits are most reliable
      this.isGrounded = true;
    } else if (hasGroundCollisions && lowDownwardVelocity) {
      // Collision detection as backup
      this.isGrounded = true;
    } else if (recentGroundContact && Math.abs(velocity.dot(gravityDir)) < 0.5) {
      // Very recent contact and nearly stationary
      this.isGrounded = true;
    } else {
      // Not grounded
      this.isGrounded = false;
    ...
    // Update last ground contact time
    if (this.isGrounded) {
  ...
  handleMovement(deltaTime) {
    ...
    // Build initial movement direction (normalized)
    const moveDir = new THREE.Vector3();
    moveDir.addScaledVector(forward, moveForward / speed);
    moveDir.addScaledVector(right, moveRight / speed);
    
    // IMPORTANT: Use the actual detected ground normal, not gravity-based projection
    ...
    if (this.isGrounded) {
      ...
      if (moveDir.length() > 0.1 && effectiveGroundNormal) {
        const anticipatedNormal = this.getAnticipatedTerrainNormal(moveDir.clone().multiplyScalar(speed));
        // Blend current and anticipated normals
        effectiveGroundNormal.lerp(anticipatedNormal, 0.5).normalize();
      }
    }
    
    // Project movement onto the detected surface (not gravity plane)
    if (this.isGrounded && effectiveGroundNormal) {
      // FIXED: Project forward and right vectors onto surface properly
      // This ensures movement follows the slope both up and down
      const gravityUp = gravityDir.clone().multiplyScalar(-1);
      
      // For each direction vector, remove the component along the surface normal
      // then normalize to get the surface-aligned direction
      forward = forward.clone().sub(effectiveGroundNormal.clone().multiplyScalar(forward.dot(effectiveGroundNormal))).normalize();
      right = right.clone().sub(effectiveGroundNormal.clone().multiplyScalar(right.dot(effectiveGroundNormal))).normalize();
      ...
      if (steepSlope && moveForward < 0) {
        // When moving down a steep slope, add a small component along the slope
        // This helps the player "slide" down rather than get stuck
        ...
        forward.add(slideComponent);
      }
    }
    
    // Rebuild movement with speed using surface-projected vectors
    moveDir.set(0, 0, 0);
    ...
    if (this.isGrounded) {
      ...
      // CRITICAL: Apply movement force properly for slopes
      if (effectiveGroundNormal && moveDir.length() > 0.01) {
        // Check if movement would take us away from the surface
        const movementAlongNormal = moveDir.dot(effectiveGroundNormal);
        
        // Only remove the normal component if it's trying to lift us off the surface
        if (movementAlongNormal > 0.1) {
          moveDir.sub(effectiveGroundNormal.clone().multiplyScalar(movementAlongNormal));
        }
        
        // For downward slopes, ensure we maintain contact
        const gravityUp = gravityDir.clone().multiplyScalar(-1);
        const slopeAngle = Math.acos(effectiveGroundNormal.dot(gravityUp));
      ...
      if (moveLength === 0) {
        ...
        if (effectiveGroundNormal) {
          ...
          // FIXED: Only dampen velocity moving away from surface, not along it
