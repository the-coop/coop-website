...
export class FPSController {
  ...
  checkGrounded() {
    ...
    if (this.isGrounded && (this.centerFootHit || this.leftFootHit || this.rightFootHit)) {
      ...
      // Extract surface normal from ray hit - FIXED VERSION
      const extractNormal = (hit) => {
        if (!hit) return null;
        
        // Get the collider from the hit
        const collider = this.physics.world.getCollider(hit.colliderHandle);
        if (!collider) return null;
        
        // Calculate the hit point
        const hitPoint = this.rayDir.clone().multiplyScalar(hit.toi);
        
        // Get the normal at the hit point
        // Rapier returns the normal in the hit object directly
        if (hit.normal) {
          return new THREE.Vector3(hit.normal.x, hit.normal.y, hit.normal.z);
        }
        
        // Fallback: calculate normal from hit point (assumes hit point is on surface)
        // This is less accurate but better than nothing
        const worldHitPoint = hit === this.centerFootHit ? this.centerFootPos.clone() :
                             hit === this.leftFootHit ? this.leftFootPos.clone() :
                             this.rightFootPos.clone();
        worldHitPoint.add(hitPoint);
        
        // For now, return gravity-opposite as fallback
        return gravityDir.clone().multiplyScalar(-1);
      };
      
      // Try to get normal from center hit first (most reliable)
      if (this.centerFootHit) {
        surfaceNormal = extractNormal(this.centerFootHit);
      }
      
      // If no center hit, average the foot hits
      ...
      if (surfaceNormal) {
        ...
        // Debug log the normal and calculated slope
        ...
        // Only log significant slopes
        if (angleDegrees > 1.0) {
          console.log('Surface normal:', surfaceNormal, 'Slope angle:', angleDegrees.toFixed(1), '°');
        }
      } else {
  ...
  handleMovement(deltaTime) {
    ...
    if (this.isGrounded && effectiveGroundNormal) {
      // Project forward and right vectors onto the actual detected surface
      forward = forward.clone().projectOnPlane(effectiveGroundNormal).normalize();
      right = right.clone().projectOnPlane(effectiveGroundNormal).normalize();
    }
    ...
    if (this.isGrounded) {
      ...
      // CRITICAL: Apply movement force ONLY along the surface
      // This prevents floating or moving into the air on slopes
      if (effectiveGroundNormal && moveDir.length() > 0.01) {
        // Remove any component that would move us away from the surface
        const movementAlongNormal = moveDir.dot(effectiveGroundNormal);
        if (movementAlongNormal > 0) {
          // Movement is trying to go away from surface - remove that component
          moveDir.sub(effectiveGroundNormal.clone().multiplyScalar(movementAlongNormal));
        }
        
        // Add a small downward bias to keep us on the surface
        const gravityUp = gravityDir.clone().multiplyScalar(-1);
        const slopeAngle = Math.acos(effectiveGroundNormal.dot(gravityUp));
        
        // On slopes, add a component that pulls us toward the surface
        if (slopeAngle > 0.1) { // More than ~5 degrees
          const pullStrength = Math.min(slopeAngle * 2, 1.0); // Stronger pull on steeper slopes
          const pullTowardSurface = effectiveGroundNormal.clone().multiplyScalar(-pullStrength * speed * 0.1);
          moveDir.add(pullTowardSurface);
        }
      }
      ...
      if (moveLength === 0) {
        ...
        if (effectiveGroundNormal) {
          ...
          // If velocity is moving away from surface, dampen it heavily
