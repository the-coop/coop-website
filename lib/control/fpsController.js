...
export class FPSController {
      ...
      for (const [id, vehicle] of this.scene.vehicles) {
        if (vehicle.constructor.name === 'SpaceshipController') {
          ...
          if (vehicle.isPlayerInside(playerPos) && (vehicle.doorsOpen || this.isInSpaceship)) {
            console.log('Player inside spaceship - using floor as ground');
            this.isInSpaceship = true;
            this.currentSpaceship = vehicle;
            
            // Get the floor normal from the spaceship
            this.localGravityNormal = vehicle.getInteriorFloorNormal();
            
            // Force grounded when standing on spaceship floor
            const localY = playerPos.clone().sub(vehicle.getPosition());
            const shipQuat = new THREE.Quaternion(
              vehicle.body.rotation().x,
              vehicle.body.rotation().y,
              vehicle.body.rotation().z,
              vehicle.body.rotation().w
            );
            const inverseQuat = shipQuat.clone().invert();
            localY.applyQuaternion(inverseQuat);
            
            // Check if player is near the floor (Y = -2 in local space)
            if (Math.abs(localY.y - (-2)) < 0.5) {
  ...
  handleMovement(deltaTime) {
    ...
    if (this.isInSpaceship && this.localGravityNormal) {
      ...
      gravityDir = this.localGravityNormal.clone().multiplyScalar(-1);
      gravityStrength = this.physics.gravity.strength * 1.5; // Increase gravity strength inside spaceship
      
      // Inherit spaceship velocity
    ...
    // Apply inherited velocity from spaceship
    if (this.isInSpaceship && this.inheritedVelocity.length() > 0.1) {
      // Blend current velocity with inherited velocity
      const blendFactor = 0.95; // How much to inherit
      newVelX = newVelX * (1 - blendFactor) + this.inheritedVelocity.x * blendFactor;
      newVelY = newVelY * (1 - blendFactor) + this.inheritedVelocity.y * blendFactor;
      newVelZ = newVelZ * (1 - blendFactor) + this.inheritedVelocity.z * blendFactor;
    }
