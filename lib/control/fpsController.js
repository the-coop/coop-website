...
export class FPSController {
  ...
  checkGrounded() {
    ...
    if (insideSpaceship && spaceshipVehicle) {
      ...
      // Get the floor normal from the spaceship
      this.localGravityNormal = spaceshipVehicle.getInteriorFloorNormal();
      ...
      const localPos = playerPos.clone().sub(shipPos);
      const inverseQuat = shipQuat.clone().invert();
      localPos.applyQuaternion(inverseQuat);
      
      // Check if player is near the floor (Y = -2 in local space)
      const floorY = -2.4; // Interior floor position
      const distanceToFloor = Math.abs(localPos.y - floorY);
      
      // Consider grounded if within reasonable distance of floor
      if (distanceToFloor < 0.8) {
        this.isGrounded = true;
        this.lastGroundNormal.copy(this.localGravityNormal);
        
        // Cast a ray in ship's local down direction to ensure we're on floor
        const localDown = new THREE.Vector3(0, -1, 0);
        const worldDown = localDown.clone().applyQuaternion(shipQuat);
        
        const rayStart = playerPos.clone();
        const hit = this.physics.castRay(rayStart, worldDown, 1.0, this.colliderHandle);
        
        if (hit) {
          // Update center foot hit for ground adjustment
          this.centerFootHit = hit;
        }
    ...
    if (this.isInSpaceship && this.localGravityNormal) {
      // Use spaceship floor as gravity direction
      gravityDir = this.localGravityNormal.clone().multiplyScalar(-1);
    } else {
      // Use world gravity
  ...
  handleMovement(deltaTime) {
    ...
    if (this.isInSpaceship && this.localGravityNormal) {
      // Inside spaceship - use local gravity
      gravityDir = this.localGravityNormal.clone().multiplyScalar(-1);
      gravityStrength = this.physics.gravity.strength * 2.0; // Stronger gravity to keep player on floor
      
      // Calculate inherited velocity from spaceship motion
      if (this.currentSpaceship && this.currentSpaceship.body) {
    ...
    if (this.isGrounded && (this.centerFootHit || this.leftFootHit || this.rightFootHit)) {
      ...
      // Only adjust to ground if not in spaceship (spaceship floor is flat)
    ...
    if (this.isInSpaceship && this.inheritedVelocity.length() > 0.1) {
      ...
      relativeVel.multiplyScalar(0.5);
    ...
    if (this.isGrounded) {
      ...
      if (this.isInSpaceship && effectiveGroundNormal) {
        // Add extra force to stick to the floor
        const stickForce = effectiveGroundNormal.clone().multiplyScalar(-5.0);
      ...
      if (this.isInSpaceship) {
        ...
        if (!isMoving) {
          ...
          newVelX = this.inheritedVelocity.x + relVelX * 0.8;
          newVelY = this.inheritedVelocity.y + relVelY * 0.9;
          newVelZ = this.inheritedVelocity.z + relVelZ * 0.8;
        } else {
          // Less damping when moving
          newVelX = this.inheritedVelocity.x + relVelX * 0.95;
          newVelY = this.inheritedVelocity.y + relVelY * 0.98;
          newVelZ = this.inheritedVelocity.z + relVelZ * 0.95;
        }
      } else {
