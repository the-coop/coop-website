...
import { Ballistics } from '../engine/ballistics.js';

export class HelicopterController {
  constructor(scene, physics, spawnPosition) {
    this.scene = scene;
    ...
    // Add disassembly state
    this.isDisassembled = false;
    this.disassembledParts = [];
    this.wasPressingDisassemble = false;
    
    // Physics bodies for parts
    this.partBodies = new Map(); // Store all part bodies
    this.joints = []; // Store all joints for easy removal
    
    // Add countermeasure system
    this.countermeasures = {
      chaff: {
        count: 30,
        maxCount: 30,
        lastDeployTime: 0,
        cooldown: 500, // 0.5 seconds between uses
        duration: 3000, // Chaff cloud lasts 3 seconds
        activeClouds: []
      },
      flares: {
        count: 20,
        maxCount: 20,
        lastDeployTime: 0,
        cooldown: 1000, // 1 second between uses
        activeFlares: []
      }
    };
  ...
  create(position) {
    ...
    // Create physics with joints
    this.createPhysicsWithJoints(position);
  ...
  createPhysicsWithJoints(position) {
    // Create main fuselage body (this will be the primary body)
    ...
    // Store main body
    this.partBodies.set('fuselage', {
      body: this.body,
      collider: this.collider,
      mesh: this.mesh.userData.fuselage || this.mesh
    });
    
    // Create tail boom as separate body
    // The tail mesh is already offset in the model, so we need to account for that
    const tailPosition = new THREE.Vector3(position.x, position.y, position.z - 5.5);
    const tailBody = this.physics.createDynamicBody(tailPosition, {
      linearDamping: 0.5,
      angularDamping: 0.5
    });
    
    const tailCollider = this.physics.createBoxCollider(
    ...
    this.physics.world.createCollider(tailCollider, tailBody);
    
    // IMPORTANT: Store the tail mesh reference
    const tailMesh = this.mesh.userData.tail;
    
    // Since the tail mesh is already positioned relative to the helicopter group,
    // we need to reset its local position when it becomes a separate physics body
    if (tailMesh) {
      tailMesh.position.set(0, 0, 0); // Reset local position since physics will handle it
    }
    
    this.partBodies.set('tailBoom', {
      body: tailBody,
      collider: tailCollider,
      mesh: tailMesh  // Make sure we have the actual tail mesh
    });
    
    // Create joint connecting fuselage to tail
    const tailJoint = this.createFixedJoint(
      this.body,
      tailBody,
      new THREE.Vector3(0, 0, -2.5), // anchor on fuselage
      new THREE.Vector3(0, 0, 3)      // anchor on tail
    );
    this.joints.push(tailJoint);
    
    // Create main rotor as separate body
    const rotorPosition = new THREE.Vector3(position.x, position.y + 1.5, position.z);
    const rotorBody = this.physics.createDynamicBody(rotorPosition, {
      linearDamping: 0.1,
      angularDamping: 0.1
    });
    
    const rotorCollider = this.physics.createBoxCollider(
      new THREE.Vector3(4, 0.1, 0.2),
      {
        density: 0.1,
        friction: 0.3,
        restitution: 0.2
    ...
    this.physics.world.createCollider(rotorCollider, rotorBody);
    
    this.partBodies.set('mainRotor', {
      body: rotorBody,
      collider: rotorCollider,
      mesh: this.mainRotor,
      initialSpin: new THREE.Vector3(0, 20, 0) // Store initial spin for disassembly
    });
    
    // Create revolute joint for main rotor (allows rotation)
    const rotorJoint = this.createRevoluteJoint(
      this.body,
      rotorBody,
      new THREE.Vector3(0, 1.5, 0),  // anchor on fuselage
      new THREE.Vector3(0, 0, 0),    // anchor on rotor
      new THREE.Vector3(0, 1, 0)     // rotation axis (up)
    );
    this.joints.push(rotorJoint);
    
    // Similar setup for tail rotor
    const tailRotorPosition = new THREE.Vector3(position.x + 0.3, position.y + 0.5, position.z - 6);
    const tailRotorBody = this.physics.createDynamicBody(tailRotorPosition, {
      linearDamping: 0.1,
      angularDamping: 0.1
    });
    
    const tailRotorCollider = this.physics.createBoxCollider(
      new THREE.Vector3(0.75, 0.02, 0.1),
      {
        density: 0.05,
        friction: 0.3,
        restitution: 0.2
    ...
    this.physics.world.createCollider(tailRotorCollider, tailRotorBody);
    
    this.partBodies.set('tailRotor', {
      body: tailRotorBody,
      collider: tailRotorCollider,
      mesh: this.tailRotor,
      initialSpin: new THREE.Vector3(40, 0, 0)
    });
    
    // Joint for tail rotor
    const tailRotorJoint = this.createRevoluteJoint(
      tailBody,
      tailRotorBody,
      new THREE.Vector3(0.3, 0.5, -0.5), // anchor on tail boom
      new THREE.Vector3(0, 0, 0),        // anchor on rotor
      new THREE.Vector3(1, 0, 0)         // rotation axis (sideways)
    );
    this.joints.push(tailRotorJoint);
    
    // Add more parts as needed (landing gear, weapons, etc.)
  }
  
  createFixedJoint(body1, body2, anchor1, anchor2) {
    // Get RAPIER from physics manager
    const RAPIER = this.physics.RAPIER;
    if (!RAPIER) {
      console.error('RAPIER not available from physics manager');
      return null;
    }
    
    // Create a fixed joint (no relative movement allowed)
    const params = RAPIER.JointData.fixed(
      anchor1,
      { x: 0, y: 0, z: 0, w: 1 }, // rotation1
      anchor2,
      { x: 0, y: 0, z: 0, w: 1 }  // rotation2
    );
    
    return this.physics.world.createImpulseJoint(params, body1, body2, true);
  }
  
  createRevoluteJoint(body1, body2, anchor1, anchor2, axis) {
    // Get RAPIER from physics manager
    const RAPIER = this.physics.RAPIER;
    if (!RAPIER) {
      console.error('RAPIER not available from physics manager');
    ...
    // Create a revolute joint (allows rotation around an axis)
  ...
  deployFlares() {
    ...
    // Deploy 2 flares in the original pattern - just left and right
    const flareCount = 2;
  ...
  deployChaff() {
    const now = Date.now();
    const chaff = this.countermeasures.chaff;
    
    // Check cooldown and availability
    if (now - chaff.lastDeployTime < chaff.cooldown || chaff.count <= 0) {
      return false;
    }
    
    // Deploy chaff
    chaff.count--;
    chaff.lastDeployTime = now;
    
    // Create chaff cloud effect
    const chaffCloud = this.createChaffCloud();
    chaff.activeClouds.push({
      mesh: chaffCloud,
      deployTime: now,
    ...
    if (this.scene.networkManager) {
      this.scene.networkManager.send({
        type: 'countermeasure_deploy',
        vehicle_id: this.id,
        countermeasure_type: 'chaff',
        position: this.getPosition(),
        velocity: this.getVelocity()
      });
    }
    
    console.log(`Deployed chaff! Remaining: ${chaff.count}/${chaff.maxCount}`);
    return true;
  }
  ...
  createChaffCloud() {
    ...
    // Create metallic chaff particles
    ...
    // Add to scene at helicopter position
    group.position.copy(this.getPosition());
  ...
  createFlare(index) {
    ...
    flareGroup.add(light);
    
    // Position relative to helicopter
    const pos = this.getPosition().clone();
    const spread = 2;
    pos.x += (Math.random() - 0.5) * spread;
    pos.y -= 1; // Below helicopter
    pos.z += (Math.random() - 0.5) * spread;
    flareGroup.position.copy(pos);
    
    // Initial velocity (inherit helicopter velocity + ejection)
    const vel = this.getVelocity().clone();
    vel.y -= 5; // Eject downward
    vel.x += (Math.random() - 0.5) * 10;
    vel.z += (Math.random() - 0.5) * 10;
    
    this.scene.scene.add(flareGroup);
    
    // Create physics body for flare
    const flareBody = this.physics.createDynamicBody(pos, {
      linearDamping: 0.5,
      angularDamping: 0.5
    });
    ...
    const colliderDesc = this.physics.createBallCollider(0.3, {
      density: 0.1,
      friction: 0.3,
      restitution: 0.1
    });
    
    const collider = this.physics.world.createCollider(colliderDesc, flareBody);
    ...
    return {
      ...
      heatSignature: 2.0 // Higher than helicopter's 1.0
    };
  }
  
  updateCountermeasures(deltaTime) {
    const now = Date.now();
    
    // Update chaff clouds
    const chaff = this.countermeasures.chaff;
    for (let i = chaff.activeClouds.length - 1; i >= 0; i--) {
      const cloud = chaff.activeClouds[i];
      const age = now - cloud.deployTime;
      
      if (age > chaff.duration) {
        // Remove expired chaff
        this.scene.scene.remove(cloud.mesh);
        cloud.mesh.traverse(child => {
          if (child.geometry) child.geometry.dispose();
          if (child.material) child.material.dispose();
        });
        chaff.activeClouds.splice(i, 1);
      } else {
        // Animate chaff dispersion
        const opacity = 1 - (age / chaff.duration);
        cloud.mesh.children[0].material.opacity = opacity * 0.8;
        
        // Expand and drift
        const positions = cloud.mesh.children[0].geometry.attributes.position;
        const velocities = cloud.mesh.children[0].userData.velocities;
        
        for (let j = 0; j < velocities.length; j++) {
          positions.array[j * 3] += velocities[j].x * deltaTime;
    ...
    // Update flares
    const flares = this.countermeasures.flares;
    for (let i = flares.activeFlares.length - 1; i >= 0; i--) {
      const flare = flares.activeFlares[i];
      const age = now - flare.deployTime;
      
      if (age > flare.lifetime) {
        // Remove expired flare
        this.scene.scene.remove(flare.group);
        flare.group.traverse(child => {
          if (child.geometry) child.geometry.dispose();
          if (child.material) child.material.dispose();
        });
        
        if (flare.body && this.physics.world.bodies.contains(flare.body.handle)) {
          if (flare.collider) {
            this.physics.world.removeCollider(flare.collider, true);
          }
          this.physics.world.removeRigidBody(flare.body);
        }
        
        flares.activeFlares.splice(i, 1);
      } else {
        // Update flare position from physics
        if (flare.body) {
          const pos = flare.body.translation();
          flare.group.position.set(pos.x, pos.y, pos.z);
        }
        
        // Fade out over time
        const lifeRatio = age / flare.lifetime;
        const brightness = 1 - lifeRatio;
        
        flare.group.children[0].material.opacity = brightness;
        flare.group.children[1].material.opacity = brightness * 0.4;
        flare.group.children[2].intensity = brightness * 2;
        
        // Reduce heat signature over time
        flare.heatSignature = 2.0 * brightness;
      }
    }
  }
  
  // Check if chaff is disrupting lock-ons
  isChaffActive() {
    const now = Date.now();
    for (const cloud of this.countermeasures.chaff.activeClouds) {
      const age = now - cloud.deployTime;
      if (age < this.countermeasures.chaff.duration) {
        // Check if chaff is still near helicopter
        const distance = cloud.position.distanceTo(this.getPosition());
        if (distance < 20) { // 20 unit radius of effect
          return true;
        }
      }
    }
  ...
  // Get active flares for missile decoy
  getActiveFlares() {
    return this.countermeasures.flares.activeFlares.filter(flare => {
      const age = Date.now() - flare.deployTime;
      return age < flare.lifetime && flare.heatSignature > 0.5;
    });
  }
  
  update(deltaTime) {
    // Update all part meshes from their physics bodies
    if (!this.isDisassembled) {
      // Normal update - parts move together via joints
      this.partBodies.forEach((part, name) => {
        if (part.mesh && part.body && name !== 'fuselage') {
          const pos = part.body.translation();
          // Convert world position to local position relative to helicopter
          const localPos = this.mesh.worldToLocal(new THREE.Vector3(pos.x, pos.y, pos.z));
          part.mesh.position.copy(localPos);
          
          const rot = part.body.rotation();
          const worldQuat = new THREE.Quaternion(rot.x, rot.y, rot.z, rot.w);
          // Convert world rotation to local rotation relative to helicopter
          const helicopterWorldQuat = this.mesh.quaternion.clone();
          const localQuat = helicopterWorldQuat.clone().invert().multiply(worldQuat);
          part.mesh.quaternion.copy(localQuat);
        }
      });
    } else {
      // Disassembled update - parts fall independently
      this.partBodies.forEach((part, name) => {
        if (part.body) {
          // Apply gravity to each part
          this.physics.applyGravityToBody(part.body, deltaTime);
    ...
    if (this.isOccupied && this.keys) {
      ...
      // Disassembly (P key)
      if (this.keys.changePerspective && !this.wasPressingDisassemble) {
        this.disassemble();
      }
      this.wasPressingDisassemble = this.keys.changePerspective;
  ...
  destroy() {
    ...
    // Clean up all part bodies
    this.partBodies.forEach((part, name) => {
      if (part.collider && this.physics.world) {
        this.physics.world.removeCollider(part.collider, true);
      }
      if (part.body && this.physics.world && name !== 'fuselage') {
  ...
  // Add method to handle network countermeasure updates
  handleNetworkCountermeasure(data) {
    if (data.countermeasure_type === 'chaff') {
      const chaffCloud = this.createChaffCloud();
      chaffCloud.position.copy(data.position);
      this.countermeasures.chaff.activeClouds.push({
        mesh: chaffCloud,
        deployTime: Date.now(),
        position: data.position.clone()
      });
    } else if (data.countermeasure_type === 'flares') {
      // Create visual flares for network player
      for (let i = 0; i < 3; i++) {
        const flare = this.createFlare(i);
        flare.group.position.copy(data.position);
        flare.group.position.x += (Math.random() - 0.5) * 2;
        flare.group.position.z += (Math.random() - 0.5) * 2;
        this.countermeasures.flares.activeFlares.push(flare);
      }
    }
  }
}
