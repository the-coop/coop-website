...
export class HelicopterController {
  ...
  disassemble() {
    ...
    console.log('Disassembling helicopter by removing joints...');
    
    // Get current velocities for momentum transfer
    const velocity = this.getVelocity();
    const angularVelocity = this.body.angvel();
    
    // Remove all joints - this will cause parts to separate
    ...
    // Apply forces to parts for explosive effect
    this.partBodies.forEach((part, name) => {
      if (part.body && name !== 'fuselage') {
        // Add some random explosive force
        ...
        part.body.setLinvel({
          x: velocity.x + randomForce.x,
          y: velocity.y + randomForce.y,
          z: velocity.z + randomForce.z
        });
        
        // Apply initial spin if specified
        if (part.initialSpin) {
          part.body.setAngvel({
            x: part.initialSpin.x + (Math.random() - 0.5) * 5,
            y: part.initialSpin.y + (Math.random() - 0.5) * 5,
            z: part.initialSpin.z + (Math.random() - 0.5) * 5
          });
        } else {
          part.body.setAngvel({
            x: angularVelocity.x + (Math.random() - 0.5) * 10,
            y: angularVelocity.y + (Math.random() - 0.5) * 10,
            z: angularVelocity.z + (Math.random() - 0.5) * 10
          });
        }
      }
    });
    
    // Hide all non-physics parts that should disappear
    const userData = this.mesh.userData;
    
    // Hide cockpits
    if (userData.pilotCockpit) userData.pilotCockpit.visible = false;
    if (userData.gunnerCockpit) userData.gunnerCockpit.visible = false;
    
    // Hide weapons
    if (this.gunMount) this.gunMount.visible = false;
    if (this.rocketPods) {
      this.rocketPods.forEach(pod => {
        if (pod) pod.visible = false;
      });
    }
    ...
    if (this.lights) {
      Object.values(this.lights).forEach(light => {
        if (light) {
          light.visible = false;
          // Also disable the point lights
          if (light.userData && light.userData.pointLight) {
            light.userData.pointLight.visible = false;
          }
          // Disable spot light for search light
          if (light.userData && light.userData.spotLight) {
            light.userData.spotLight.visible = false;
          }
        }
      });
    }
    ...
    // Hide stabilizer
    if (userData.stabilizer) userData.stabilizer.visible = false;
    
    // Hide turret
    if (userData.turret) userData.turret.visible = false;
    
    // Hide any weapon pylons parent group
    const weaponPylonsGroup = this.mesh.children.find(child => 
      child.children && child.children.some(subChild => 
        subChild === this.gunMount || this.rocketPods?.includes(subChild)
      )
    );
    if (weaponPylonsGroup) weaponPylonsGroup.visible = false;
    
    // Mark as disassembled
    this.isDisassembled = true;
    
    // Exit the helicopter for the player
    ...
    if (this.pyrotechnics) {
      ...
      this.pyrotechnics.createExplosion(explosionPos, {
        size: 15,
        duration: 0.5,
        color: 0xff6600
      });
