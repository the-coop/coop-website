...
export class SpaceshipController {
  ...
  createPhysicsBody(position) {
    ...
    this.body = this.physics.createDynamicBody(position, {
      linearDamping: 0.1,  // Further reduced from 0.2
      angularDamping: 0.2  // Further reduced from 0.3
    });
    ...
    // Interior floor (for walking on inside) - reduced density
    const interiorFloorDesc = this.physics.createBoxCollider(
      new THREE.Vector3(4, 0.1, 6),
      { density: 0.3, friction: 0.8, restitution: 0.1 }  // Reduced density from 1.0
    );
  ...
  handleAutoPilot(deltaTime) {
    ...
    if (distance < 50 || this.autoPilotTargetTimer > 15000) {  // Increased time and distance
      this.setNewAutoPilotTarget();
    ...
    // Apply control torques with reduced strength
    const torqueStrength = this.rotationalThrust * mass * 0.5;  // Reduced multiplier
    
    // Yaw (turning left/right) - smoother turning
    if (Math.abs(cross.y) > 0.05) {
      const yawTorque = Math.sign(cross.y) * Math.min(Math.abs(cross.y), 0.5) * torqueStrength;
      this.body.addTorque({ 
        x: 0, 
        y: yawTorque, 
        z: 0 
    ...
    // Pitch (up/down) - more gradual pitch changes
    ...
    const desiredPitch = Math.atan2(targetHeight, horizontalDistance);
    const currentPitch = Math.asin(-forward.y);
    const pitchError = desiredPitch - currentPitch;
    
    if (Math.abs(pitchError) > 0.05) {
      const pitchTorque = Math.sign(pitchError) * Math.min(Math.abs(pitchError), 0.3) * torqueStrength;
      this.body.addTorque({ 
        x: pitchTorque, 
        y: 0, 
        z: 0 
    ...
    // Simplified lift force - just enough to maintain altitude
    const gravityCompensation = this.physics.gravity.strength * mass * 1.2;  // Reduced from 2.5
    const altitudeError = this.autoPilotTarget.y - position.y;
    const altitudeCorrection = Math.max(-0.5, Math.min(0.5, altitudeError * 0.05));  // Gentler correction
    
    // Base lift to hover + gentle altitude correction
    const liftMagnitude = gravityCompensation + (altitudeCorrection * mass * 100);  // Reduced from 500
    ...
    // Apply forward thrust - much gentler
    const thrustMagnitude = this.thrust * mass * 0.3;  // Reduced multiplier
    
    if (dot > 0.7) {
      // Well aligned - moderate thrust
      const thrustForce = forward.multiplyScalar(thrustMagnitude * 0.8);
      this.body.addForce({
        x: thrustForce.x,
        y: thrustForce.y,
        z: thrustForce.z
      ...
      // Partially aligned - minimal thrust
      const thrustForce = forward.multiplyScalar(thrustMagnitude * 0.3);
      this.body.addForce({
        x: thrustForce.x,
        y: thrustForce.y,
        z: thrustForce.z
    ...
    // No thrust if poorly aligned - just turn
    
    // Apply stronger angular damping for stability
    const angVel = this.body.angvel();
    const dampingTorque = {
      x: -angVel.x * mass * 15.0,  // Increased from 10.0
      y: -angVel.y * mass * 15.0,
      z: -angVel.z * mass * 15.0
    };
    this.body.addTorque(dampingTorque);
    
    // Apply stronger linear velocity damping
    const vel = this.body.linvel();
    const linearDamping = {
      x: -vel.x * 0.8,  // Increased from 0.5
      y: -vel.y * 0.3,  // Keep vertical damping lower
      z: -vel.z * 0.8
    };
    this.body.addForce({
    ...
    // Remove random movements for more stable flight
    
    // Speed limiting with lower max speeds
    const velocity = this.body.linvel();
    ...
    // Also limit vertical speed
    const maxVerticalSpeed = this.maxSpeed * 0.3;  // Even lower vertical speed limit
