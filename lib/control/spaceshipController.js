...
export class SpaceshipController {
  constructor(scene, physics, position) {
    ...
    this.maxSpeed = 30;  // Reduced from 100 for calmer flight
    
    // Add missing physics properties
    this.thrust = 500;  // Reduced from 2000
    this.rotationalThrust = 50;  // Reduced from 100
    this.engineOn = false;  // Add engine state
  ...
  createPhysicsBody(position) {
    ...
    // Interior floor (for walking on inside) - reduced friction for spaceship feel
    const interiorFloorDesc = this.physics.createBoxCollider(
      new THREE.Vector3(4, 0.1, 6),
      { density: 0.3, friction: 0.2, restitution: 0.1 }  // Reduced friction from 0.8 to 0.2
    );
  ...
  activateAutoPilot() {
    ...
    this.engineOn = true;  // Turn on engines!
  ...
  setNewAutoPilotTarget() {
    // Create a patrol path around the platform edges
    // Platform is 200x200 in sandbox mode
    const platformSize = 150; // Reduced from 180 - stay closer to center
    
    // Choose a corner or edge position
    const positions = [
      // Corners
      { x: platformSize, z: platformSize },
      { x: platformSize, z: -platformSize },
      { x: -platformSize, z: -platformSize },
      { x: -platformSize, z: platformSize },
      // Mid-edges
    ...
    // Pick a random position from the patrol points
    const targetPos = positions[Math.floor(Math.random() * positions.length)];
    
    // Set height between 50-70 units above platform (reduced range)
    ...
    console.log('New patrol target:', this.autoPilotTarget);
  }
  ...
  handleAutoPilot(deltaTime) {
    ...
    if (distance < 40 || this.autoPilotTargetTimer > 20000) {  // Reduced distance threshold for smoother turns
      this.setNewAutoPilotTarget();
    ...
    // Get mass for force calculations
    const mass = this.body.mass();
    
    // Calculate direction to target
    toTarget.normalize();
    
    // Get current rotation and axes
    const rotation = this.body.rotation();
    const quaternion = new THREE.Quaternion(rotation.x, rotation.y, rotation.z, rotation.w);
    ...
    // Apply control torques with much reduced strength for stability
    const torqueStrength = this.rotationalThrust * mass * 0.15;  // Reduced from 0.5 to 0.15
    
    // Yaw (turning left/right) - much smoother turning
    if (Math.abs(cross.y) > 0.1) {  // Increased threshold to reduce jitter
      const yawTorque = Math.sign(cross.y) * Math.min(Math.abs(cross.y), 0.2) * torqueStrength;  // Reduced max torque
      this.body.addTorque({ 
        x: 0, 
        y: yawTorque, 
        z: 0 
    ...
    // Pitch (up/down) - much more gradual pitch changes
    const targetHeight = this.autoPilotTarget.y - position.y;
    const horizontalDistance = Math.sqrt(toTarget.x * toTarget.x + toTarget.z * toTarget.z);
    const desiredPitch = Math.atan2(targetHeight, Math.max(horizontalDistance, 10)); // Prevent extreme pitches
    const currentPitch = Math.asin(Math.max(-0.9, Math.min(0.9, -forward.y))); // Clamp to prevent NaN
    const pitchError = desiredPitch - currentPitch;
    
    if (Math.abs(pitchError) > 0.1) {  // Increased threshold
      const pitchTorque = Math.sign(pitchError) * Math.min(Math.abs(pitchError), 0.1) * torqueStrength;  // Much smaller max torque
      this.body.addTorque({ 
        x: pitchTorque, 
        y: 0, 
        z: 0 
    ...
    // Simplified lift force - gentler altitude control
    const gravityCompensation = this.physics.gravity.strength * mass * 1.1;  // Reduced from 1.2
    const altitudeError = this.autoPilotTarget.y - position.y;
    const altitudeCorrection = Math.max(-0.3, Math.min(0.3, altitudeError * 0.02));  // Much gentler correction
    
    // Base lift to hover + gentle altitude correction
    const liftMagnitude = gravityCompensation + (altitudeCorrection * mass * 50);  // Reduced from 100
    
    // Apply lift in world up direction
    const worldUp = new THREE.Vector3(0, 1, 0);
    const liftForce = worldUp.multiplyScalar(liftMagnitude);
    ...
    // Apply forward thrust - much gentler and more stable
    const thrustMagnitude = this.thrust * mass * 0.15;  // Reduced from 0.3
    
    if (dot > 0.8) {  // Only thrust when well aligned
      const thrustForce = forward.multiplyScalar(thrustMagnitude * 0.6);  // Reduced from 0.8
      this.body.addForce({
        x: thrustForce.x,
        y: thrustForce.y,
        z: thrustForce.z
      });
    } else if (dot > 0.5) {  // Increased threshold
      const thrustForce = forward.multiplyScalar(thrustMagnitude * 0.2);  // Reduced from 0.3
      this.body.addForce({
        x: thrustForce.x,
        y: thrustForce.y,
        z: thrustForce.z
    ...
    // Apply much stronger angular damping for stability
    const angVel = this.body.angvel();
    const dampingTorque = {
      x: -angVel.x * mass * 25.0,  // Increased from 15.0
      y: -angVel.y * mass * 25.0,
      z: -angVel.z * mass * 25.0
    };
    this.body.addTorque(dampingTorque);
    
    // Apply stronger linear velocity damping for smoother movement
    const vel = this.body.linvel();
    const linearDamping = {
      x: -vel.x * 1.2,  // Increased from 0.8
      y: -vel.y * 0.5,  // Increased from 0.3
      z: -vel.z * 1.2   // Increased from 0.8
    };
    this.body.addForce({
      x: linearDamping.x * mass,
      y: linearDamping.y * mass,
      z: linearDamping.z * mass
    });
    
    // Speed limiting with lower max speeds for stability
    const velocity = this.body.linvel();
    const horizontalVel = new THREE.Vector2(velocity.x, velocity.z);
    const horizontalSpeed = horizontalVel.length();
    
    // Lower horizontal speed limit for more stable flight
    const maxHorizontalSpeed = this.maxSpeed * 0.6;  // Reduced from full maxSpeed
    ...
    // Lower vertical speed limit
    const maxVerticalSpeed = this.maxSpeed * 0.2;  // Reduced from 0.3
    if (Math.abs(velocity.y) > maxVerticalSpeed) {
      this.body.setLinvel({
        x: velocity.x,
        y: Math.sign(velocity.y) * maxVerticalSpeed,
        z: velocity.z
      });
    }
  }
  ...
  exitVehicle() {
    ...
    this.autoPilotTimer = 0;
    this.autoPilotActive = false;
    this.engineOn = false;  // Turn off engines
    
    // Hide engine glows when exiting
    if (this.engineGlows) {
      this.engineGlows.forEach(glow => glow.visible = false);
    }
