...
export class CarController {
  ...
  create(position) {
    ...
    VehicleHelpers.setupInteraction(this.chassisMesh, this, 'car');
  }
  ...
  enterCar(player) {
    const cameraPosition = new THREE.Vector3(0, 4, 12);
    const cameraRotation = new THREE.Vector3(-0.2, 0, 0);
    return VehicleHelpers.enterVehicle(this, player, cameraPosition, cameraRotation);
  }
  
  exitCar() {
    const result = VehicleHelpers.exitVehicle(this, 5);
    
    if (result) {
      // Reset car-specific controls
      if (this.keys) {
  ...
  getVelocity() {
    return VehicleHelpers.getVelocity({ body: this.chassisBody });
  }
  ...
  updateFromServer(state) {
    VehicleHelpers.updateFromServer({ body: this.chassisBody, mesh: this.chassisMesh, isOccupied: this.isOccupied }, state);
