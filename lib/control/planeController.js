...
import { Pyrotechnics } from '../engine/pyrotechnics.js';
import { VehicleHelpers, AircraftHelpers, WeaponHelpers } from '../engine/vehicles.js';
...
export class PlaneController {
  constructor(scene, physics, position) {
    ...
    this.liftCoefficient = 8.0;  // Massively increased from 3.5 - much more lift
    ...
    // Wing properties - MUCH LARGER FOR MORE LIFT
    this.wingArea = 50.0;        // Massively increased from 25.0 - huge wings
    this.aspectRatio = 8.0;      // Wing aspect ratio
    
    // Control sensitivity - REDUCED FOR STABILITY
    this.pitchSensitivity = 1.2;  // Reduced from 1.5
    this.rollSensitivity = 1.2;   // Reduced from 2.0 - much less sensitive
    this.yawSensitivity = 0.4;    // Reduced from 0.8 - much less sensitive
    
    // Stability augmentation
    this.rollStability = 0.3;     // Auto-level tendency
    this.yawStability = 0.5;      // Weathervane effect
    ...
    // Add lock-on system (same as helicopter)
    this.lockOnSystem = {
      target: null,
      targetVehicle: null,
      targetDistance: 0,
      lockProgress: 0,
      maxLockRange: 500, // Longer range for plane
      minLockTime: 2.0, // Slightly longer lock time
      lockCone: Math.PI / 8, // 22.5 degree cone (narrower)
      isLocked: false,
      lastLockTime: 0,
      lockBreakTime: 5,
      crosshair: null,
  ...
  enterPlane(player) {
    ...
    if (result && this.scene.weaponSystem) {
      if (!this.scene.weaponSystem.lockOnIndicator) {
        console.log('Creating lock-on indicators for plane');
        this.scene.weaponSystem.createLockOnIndicator();
      }
      console.log('Plane lock-on system initialized with weapon system');
    }
  ...
  fireMissile() {
    ...
    const launchVelocity = forward.multiplyScalar(20).add(planeVel); // Reduced from 25
  ...
  update(deltaTime) {
    ...
    // Update lock-on system (add this before other updates)
  ...
  // Add the same updateLockOnSystem and updateLockOnHUD methods from helicopter
  updateLockOnSystem(deltaTime) {
    ...
    if (this.scene.vehicles && this.scene.vehicles instanceof Map) {
      this.scene.vehicles.forEach((controller, id) => {
        if (controller && controller !== this && controller.body && controller.getPosition) {
          potentialTargets.push({
            type: controller.constructor.name.toLowerCase().replace('controller', ''),
      ...
      // Fallback to checking known vehicle properties
      if (this.scene.planeController && this.scene.planeController !== this && 
          this.scene.planeController.body && this.scene.planeController.mesh) {
        potentialTargets.push({
          type: 'plane',
          controller: this.scene.planeController,
          position: this.scene.planeController.getPosition(),
          id: 'plane1'
        });
      }
      
      if (this.scene.helicopterController && this.scene.helicopterController.body && 
          this.scene.helicopterController.mesh) {
        potentialTargets.push({
          type: 'helicopter',
          controller: this.scene.helicopterController,
          position: this.scene.helicopterController.getPosition(),
          id: 'helicopter1'
        });
      }
      
      if (this.scene.carController && this.scene.carController.body && this.scene.carController.mesh) {
        potentialTargets.push({
          type: 'car',
          controller: this.scene.carController,
          position: this.scene.carController.getPosition(),
          id: 'car1'
        });
      }
    }
    ...
    potentialTargets.forEach(target => {
      ...
      // Check if in lock cone
      toTarget.normalize();
      const angle = forward.dot(toTarget);
      const inLockCone = angle > Math.cos(this.lockOnSystem.lockCone);
      
      // Add to all targets list with cone status
      allTargets.push({
        ...target,
        distance: distance,
        inLockCone: inLockCone,
        angle: angle
      });
      
      // Check if this could be our lock target
      if (inLockCone && distance < this.lockOnSystem.maxLockRange && distance < closestDistance) {
        // Check line of sight
    ...
    // Update lock state for closest target in cone
    if (closestTarget) {
      if (this.lockOnSystem.targetVehicle === closestTarget.controller) {
        ...
        if (this.lockOnSystem.lockProgress >= 1.0 && !this.lockOnSystem.isLocked) {
        ...
        this.lockOnSystem.targetVehicle = closestTarget.controller;
        this.lockOnSystem.targetDistance = closestDistance;
        this.lockOnSystem.lockProgress = 0;
        this.lockOnSystem.isLocked = false;
        console.log(`New target acquired: ${closestTarget.type} at ${Math.round(closestDistance)}m`);
      }
      
      // Update target distance continuously
      this.lockOnSystem.targetDistance = closestDistance;
    } else {
      // No target in cone, decay lock
      if (this.lockOnSystem.lockProgress > 0) {
        this.lockOnSystem.lockProgress = Math.max(
          this.lockOnSystem.lockProgress - deltaTime * 2,
          0
        );
  ...
  fireFlare() {
    ...
    // Get flare position (wingtip)
    const wingtipOffset = 4.5;
    const flarePosition = new THREE.Vector3(wingtipOffset, 0, -1);
    flarePosition.applyQuaternion(this.mesh.quaternion);
    flarePosition.add(this.mesh.position);
    
    // Calculate lateral ejection velocity
    const planeVelocity = this.getVelocity();
    const ejectionDirection = new THREE.Vector3(-1, 0, -1);
    ejectionDirection.applyQuaternion(this.mesh.quaternion);
    
    const flareVelocity = planeVelocity.clone().add(ejectionDirection.multiplyScalar(10));
    
    // Create flare using pyrotechnics system
    this.pyrotechnics.createFlare(flarePosition, flareVelocity, {
      color: 0xffcc00,
      duration: 5.0,
      brightness: 5
    });
    ...
    console.log(`Flare deployed! ${this.currentFlares} remaining`);
  }
  
  createGunshotFlash(position) {
    // Create gunshot flash at the given position
    ...
    this.scene.scene.add(flashMesh);
    
    // Animate flash scale and opacity
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        this.scene.scene.remove(flashMesh);
  ...
  fireCannon() {
    ...
    // Fire cannon from centerline
    const muzzlePos = new THREE.Vector3();
    this.mesh.getWorldPosition(muzzlePos);
    muzzlePos.z -= 1.5; // Move forward from the plane's position
    
    // Get forward direction
    const forward = new THREE.Vector3(0, 0, 1);
    forward.applyQuaternion(this.mesh.quaternion);
    
    // Create cannon shell using shared helper
    const shell = WeaponHelpers.createCannonShell({
      scene: this.scene.scene,
      physics: this.physics,
      pyrotechnics: this.pyrotechnics,
      position: muzzlePos,
      velocity: forward.multiplyScalar(50), // Adjust speed
      rotation: this.body.rotation(),
      vehicleType: 'plane'
    });
    
    // Create gunshot flash effect
    this.createGunshotFlash(muzzlePos);
    
    this.weaponCooldown = 0.5; // 0.5 seconds between cannon fires
  ...
  applyAerodynamics(deltaTime) {
    ...
    if (airspeed > 1) {
      // Get velocity relative to plane's forward direction
      const relativeVelocity = airspeedVector.clone().normalize();
      
      // Project velocity onto the plane's vertical plane (formed by forward and up vectors)
      // This gives us the component of velocity in the pitch plane
      const velocityInPitchPlane = relativeVelocity.clone()
        .sub(right.clone().multiplyScalar(relativeVelocity.dot(right)));
      
      if (velocityInPitchPlane.length() > 0.1) {
        velocityInPitchPlane.normalize();
        
        // Angle of attack is the angle between forward direction and velocity in pitch plane
        const dot = forward.dot(velocityInPitchPlane);
        const clampedDot = Math.max(-1, Math.min(1, dot));
        angleOfAttack = Math.acos(clampedDot);
        
        // Determine sign - positive when nose is above velocity vector
        const cross = new THREE.Vector3().crossVectors(forward, velocityInPitchPlane);
        if (cross.dot(right) < 0) {
      ...
      // Lift direction is perpendicular to velocity in the plane of wings
      // For most efficient lift, it should be perpendicular to airflow
      const liftCross = new THREE.Vector3().crossVectors(relativeVelocity, right);
      if (liftCross.length() > 0.1) {
        liftDirection = liftCross.normalize();
      } else {
        // Fallback to plane's up direction
        liftDirection = up.clone();
      }
    }
    ...
    // Calculate lift with proper physics - MASSIVELY INCREASED
    if (airspeed > this.minSpeed) {
      // Lift coefficient varies with angle of attack
      let Cl = 0;
      
      if (!this.isStalling) {
        // More realistic lift curve
        // Peak lift at around 12-15 degrees, then drops off
        const optimalAngle = 12; // degrees
        
        if (Math.abs(angleOfAttack) <= this.criticalAngle) {
          // Below critical angle - lift increases with AoA
          // Use a more realistic curve that peaks at optimal angle
          const normalizedAngle = angleOfAttack / optimalAngle;
          
          // Parabolic curve that peaks at optimalAngle
          if (Math.abs(angleOfAttack) <= optimalAngle) {
            Cl = this.liftCoefficient * (1 - Math.pow((normalizedAngle - 1), 2));
          } else {
            // Past optimal, lift decreases
            const beyondOptimal = (Math.abs(angleOfAttack) - optimalAngle) / (this.criticalAngle - optimalAngle);
            Cl = this.liftCoefficient * (1 - beyondOptimal * 0.3);
          }
          
          // Ensure minimum lift at zero AoA (from wing camber)
          const minLift = 0.5; // Increased from 0.4 - 50% of max lift at zero AoA
          Cl = Math.max(Cl, this.liftCoefficient * minLift);
          
          // Account for negative AoA
          if (angleOfAttack < 0) {
            Cl *= 0.7; // Less efficient at negative angles
          }
        } else {
          // Approaching stall - rapid loss of lift
          const stallFactor = 1 - ((Math.abs(angleOfAttack) - this.criticalAngle) / 
                                  (this.stallAngle - this.criticalAngle));
          Cl = this.liftCoefficient * 0.5 * Math.max(0.2, stallFactor);
        }
      } else {
        // Stalled - drastically reduced lift
        Cl = this.liftCoefficient * 0.2;
      }
      
      // Lift equation: L = 0.5 * ρ * V² * S * Cl
      // MASSIVELY INCREASED SCALING TO MATCH GRAVITY STRENGTH
      const liftMagnitude = 0.5 * airspeed * airspeed * this.wingArea * Cl * 0.5; // Increased from 0.1
      ...
      // Add debug logging to see lift values
      if (airspeed > 30) {
        console.log(`Airspeed: ${airspeed.toFixed(1)}, AoA: ${angleOfAttack.toFixed(1)}°, Cl: ${Cl.toFixed(2)}, Lift: ${liftMagnitude.toFixed(1)}, Gravity: ${(gravityStrength * mass).toFixed(1)}`);
      }
    ...
    if (airspeed > 0.1) {
      ...
      const dragMagnitude = 0.5 * dragCoeff * airspeed * airspeed * this.wingArea * 0.5; // Increased scaling to match lift
    ...
    if (airspeed > controlSpeed) {
      ...
      // Get angular velocity for stability calculations
      ...
      // Roll control with auto-leveling
      let rollCommand = this.controlSurfaces.aileron;
      
      // Add auto-leveling when controls are neutral
      if (Math.abs(rollCommand) < 0.1 && airspeed > this.takeoffSpeed) {
        // Calculate bank angle (roll relative to gravity)
        const bankAngle = Math.asin(Math.max(-1, Math.min(1, right.dot(upDir))));
        
        // Apply restoring force proportional to bank angle
        rollCommand = -bankAngle * this.rollStability;
      }
      
      if (rollCommand !== 0) {
        const rollTorque = forward.clone().multiplyScalar(
          rollCommand * this.rollSensitivity * 
          controlEffectiveness * controlForce
        );
        this.body.addTorque({
          x: rollTorque.x,
      ...
      // Yaw control with weathervane stability
      let yawCommand = this.controlSurfaces.rudder;
      
      // Add weathervane effect - tendency to align with airflow
      if (airspeed > this.minSpeed) {
        // Calculate sideslip angle
        const sideComponent = airspeedVector.clone().normalize().dot(right);
        
        // Apply weathervane correction
        const weathervaneCorrection = -sideComponent * this.yawStability * controlEffectiveness;
        yawCommand += weathervaneCorrection;
      }
      
      if (yawCommand !== 0) {
        const yawTorque = up.clone().multiplyScalar(
          yawCommand * this.yawSensitivity * 
      ...
      // When rolling, add appropriate yaw to make turns feel more natural
      if (Math.abs(this.controlSurfaces.aileron) > 0.1 && airspeed > this.takeoffSpeed) {
        const coordinatedYaw = this.controlSurfaces.aileron * 0.3 * controlEffectiveness;
        const coordinatedTorque = up.clone().multiplyScalar(
          coordinatedYaw * this.yawSensitivity * controlForce
        );
        this.body.addTorque({
          x: coordinatedTorque.x,
          y: coordinatedTorque.y,
          z: coordinatedTorque.z
        });
      }
    }
    
    // Enhanced angular damping for stability
    const angVel = this.body.angvel();
    
    // Different damping for each axis
    const pitchDamping = 2.0;
    const rollDamping = 3.0;  // Higher damping for roll
    const yawDamping = 4.0;   // Highest damping for yaw
    
    // Calculate damping in local space
    const localAngVel = new THREE.Vector3(angVel.x, angVel.y, angVel.z);
    const worldToLocal = new THREE.Matrix4().makeRotationFromQuaternion(quaternion).invert();
    localAngVel.applyMatrix4(worldToLocal);
    
    // Apply different damping per axis
    const dampingTorque = new THREE.Vector3(
      -localAngVel.x * rollDamping,
      -localAngVel.y * yawDamping,
      -localAngVel.z * pitchDamping
    );
    
    // Convert back to world space
    dampingTorque.applyQuaternion(quaternion);
