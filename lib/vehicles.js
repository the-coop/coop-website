...
export const WeaponHelpers = {
  ...
  createMissile(options) {
    ...
    const missileData = {
      ...
      motorIgnited: false,
      guidanceActive: true, // Make sure this is true
      hasExploded: false,
      ...
      trackingStrength: 1.0, // Increased from 0.5 - How aggressively missile tracks
      previousPos: launchPos.clone(), // Initialize previous position
      launchTime: Date.now(), // Track launch time
      initialRotation: rotation, // Store initial rotation for lerping
      preIgnitionOriented: false, // Track if we've oriented towards target
      rotationLocked: false // Track if rotation axes are locked for guided missiles
    };
    
    console.log('Created missile with guidance active:', missileData.guidanceActive);
  ...
  updateMissile(missile, physics, pyrotechnics, scene, deltaTime = 0.016, launcherCollider = null) {
    ...
    // Pre-ignition orientation towards locked target
    if (!missile.motorIgnited && missile.lockedTarget && missile.lockedTarget.getPosition) {
      const missilePos = new THREE.Vector3(pos.x, pos.y, pos.z);
      const targetPos = missile.lockedTarget.getPosition();
      
      // Calculate desired orientation
      const toTarget = targetPos.clone().sub(missilePos);
      toTarget.normalize();
      
      // Create desired quaternion
      const desiredQuat = new THREE.Quaternion();
      const up = new THREE.Vector3(0, 1, 0);
      const lookMatrix = new THREE.Matrix4();
      lookMatrix.lookAt(missilePos, targetPos, up);
      desiredQuat.setFromRotationMatrix(lookMatrix);
      
      // Lerp rotation towards target - INCREASED SPEED
      const currentQuat = new THREE.Quaternion(rot.x, rot.y, rot.z, rot.w);
      const lerpSpeed = 8.0 * deltaTime; // Increased from 3.0 for faster orientation
      currentQuat.slerp(desiredQuat, lerpSpeed);
      ...
      missile.preIgnitionOriented = true;
    }
    ...
    if (!missile.motorIgnited && missile.age >= 1.0) {
      ...
      // === CRITICAL: SWITCH TO KINEMATIC CONTROL FOR GUIDED MISSILES ===
      if (missile.guidanceActive && missile.lockedTarget) {
        console.log('=== SWITCHING MISSILE TO KINEMATIC CONTROL FOR GUIDANCE ===');
        
        // Store current velocity for kinematic movement
        const currentVel = missile.body.linvel();
        missile.kinematicVelocity = new THREE.Vector3(currentVel.x, currentVel.y, currentVel.z);
        
        // Convert to kinematic body
        missile.body.setBodyType(physics.RAPIER.RigidBodyType.KinematicVelocityBased);
        
        // Set initial kinematic velocity
        missile.body.setLinvel({
          x: missile.kinematicVelocity.x,
          y: missile.kinematicVelocity.y,
          z: missile.kinematicVelocity.z
        }, true);
        
        console.log('Missile converted to kinematic control with velocity:', missile.kinematicVelocity.toArray());
      } else {
        // For unguided missiles, keep dynamic behavior
        console.log('Unguided missile - keeping dynamic physics');
      }
      
      // Start afterburner effects
      missile.afterburnerGroup.visible = true;
      missile.thrust = missile.maxThrust;
      
      // Apply initial ignition impulse for dynamic missiles only
      if (!missile.guidanceActive || !missile.lockedTarget) {
        const forward = new THREE.Vector3(0, 0, 1);
        forward.applyQuaternion(missile.mesh.quaternion);
        
        const impulse = forward.multiplyScalar(missile.ignitionImpulse);
      ...
      console.log('Missile motor ignited!');
    }
    
    // === NEW KINEMATIC GUIDANCE SYSTEM ===
    if (missile.guidanceActive && missile.motorIgnited && !missile.hasExploded && missile.lockedTarget) {
      // Check if missile is kinematic
      const bodyType = missile.body.bodyType();
      const isKinematic = bodyType === physics.RAPIER.RigidBodyType.KinematicVelocityBased;
      
      if (isKinematic && missile.lockedTarget.getPosition) {
        const missilePos = new THREE.Vector3(pos.x, pos.y, pos.z);
        const targetPos = missile.lockedTarget.getPosition();
        ...
        console.log(`KINEMATIC MISSILE GUIDANCE: distance=${distanceToTarget.toFixed(1)}m`);
        
        if (distanceToTarget < 500 && distanceToTarget > 2) {
          toTarget.normalize();
          
          // === KINEMATIC VELOCITY CONTROL ===
          // Get current kinematic velocity
          const currentVel = missile.body.linvel();
          const currentVelVec = new THREE.Vector3(currentVel.x, currentVel.y, currentVel.z);
          const currentSpeed = currentVelVec.length();
          
          // Calculate desired velocity direction (towards target)
          const desiredDirection = toTarget.clone();
          
          // Gradually steer velocity towards target
          const steerStrength = Math.min(deltaTime * 5.0, 1.0); // How aggressively to steer
          const currentDirection = currentVelVec.clone().normalize();
          
          // Blend current direction with desired direction
          const newDirection = currentDirection.lerp(desiredDirection, steerStrength);
          
          // Maintain or increase speed
          const targetSpeed = Math.max(currentSpeed, missile.maxThrust * 2.0);
          const newVelocity = newDirection.multiplyScalar(targetSpeed);
          ...
          // === KINEMATIC ROTATION CONTROL ===
          // Smoothly rotate missile to face target
          const missileForward = new THREE.Vector3(0, 0, 1);
          missileForward.applyQuaternion(missile.mesh.quaternion);
          
          // Calculate desired rotation
          const angleToTarget = missileForward.angleTo(toTarget);
          
          if (angleToTarget > 0.01) {
            // Calculate rotation axis
            const rotationAxis = new THREE.Vector3().crossVectors(missileForward, toTarget);
            
            if (rotationAxis.length() > 0.001) {
              rotationAxis.normalize();
              
              // Aggressive turning for kinematic control
              const maxTurnRate = 10.0 * deltaTime; // Much faster than before
              const actualTurnRate = Math.min(angleToTarget, maxTurnRate);
              
              // Create rotation quaternion
              const rotationQuat = new THREE.Quaternion();
              rotationQuat.setFromAxisAngle(rotationAxis, actualTurnRate);
              
              // Apply rotation
              const currentQuat = new THREE.Quaternion(rot.x, rot.y, rot.z, rot.w);
              currentQuat.multiplyQuaternions(rotationQuat, currentQuat);
              currentQuat.normalize();
              
              // Set kinematic rotation
              missile.body.setRotation({
                x: currentQuat.x,
                y: currentQuat.y,
                z: currentQuat.z,
          ...
          console.log(`KINEMATIC GUIDANCE: speed=${targetSpeed.toFixed(1)}, angle=${(angleToTarget * 180/Math.PI).toFixed(1)}°`);
          
        } else if (distanceToTarget > 500) {
          // Target too far, disable tracking
          missile.guidanceActive = false;
          console.log('Target out of range, guidance disabled');
        }
      } else if (!isKinematic) {
        console.warn('Guided missile is not kinematic - this may cause physics conflicts!');
      }
    }
    
    // === DYNAMIC MISSILE PHYSICS (for unguided missiles only) ===
    else if (missile.motorIgnited && missile.thrust > 0) {
      const bodyType = missile.body.bodyType();
      const isDynamic = bodyType === physics.RAPIER.RigidBodyType.Dynamic;
      
      if (isDynamic) {
        // Apply thrust force for dynamic missiles
        const forward = new THREE.Vector3(0, 0, 1);
        forward.applyQuaternion(missile.mesh.quaternion);
        
        const thrustForce = forward.multiplyScalar(missile.thrust);
        ...
        // Apply gravity to dynamic missiles
        physics.applyGravityToBody(missile.body, deltaTime);
      }
    }
    
    // Update exhaust particles for both kinematic and dynamic missiles
    if (missile.motorIgnited && missile.thrust > 0) {
      // ...existing exhaust particle code...
    }
    ...
    if ((missile.age >= missile.maxAge || missile.hasExploded)) {
      ...
      pyrotechnics.createExplosion(explosionPos, {
        size: missile.vehicleType === 'helicopter' ? 8 : 6,
        debris: 20,
        shockwave: true
      });
      ...
      console.log(`${missile.vehicleType} missile exploded after ${missile.age.toFixed(1)}s`);
      return true;
    }
