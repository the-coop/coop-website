...
export const WeaponHelpers = {
  ...
  updateMissile(missile, physics, pyrotechnics, scene, deltaTime = 0.016, launcherCollider = null) {
    ...
    // Check for collisions using multiple methods
    if (!missile.hasExploded) {
      const currentPos = new THREE.Vector3(pos.x, pos.y, pos.z);
      
      // Method 1: Check collision events from physics engine
      // Remove the incorrect contactPair call - it needs two valid colliders
      
      // Method 2: Raycast from previous to current position
      if (missile.previousPos && !missile.hasExploded) {
        const direction = currentPos.clone().sub(missile.previousPos);
        const distance = direction.length();
        
        if (distance > 0.01) { // Only check if missile has moved
          direction.normalize();
          ...
          const hit = physics.castRay(
            missile.previousPos,
            direction,
            distance + 0.5, // Add a bit extra to ensure we catch the collision
            missile.collider.handle // Exclude self
          );
          
          if (hit && hit.toi < distance + 0.5) {
            // Check if we hit the launcher (skip for first 0.5 seconds)
            const timeSinceLaunch = Date.now() - missile.launchTime;
            if (timeSinceLaunch > 500 || (launcherCollider && hit.colliderHandle !== launcherCollider)) {
              missile.hasExploded = true;
              console.log('Missile collision detected via raycast at distance:', hit.toi);
            }
          }
        }
      }
      
      // Method 3: Check intersections with all colliders in physics world
      if (!missile.hasExploded) {
        // Use a short ray in the direction of movement to detect imminent collisions
        const velocity = missile.body.linvel();
        const speed = Math.sqrt(velocity.x * velocity.x + velocity.y * velocity.y + velocity.z * velocity.z);
        
        if (speed > 0.1) {
          const moveDir = new THREE.Vector3(velocity.x, velocity.y, velocity.z).normalize();
          const checkDistance = Math.min(speed * deltaTime * 2, 1.0); // Check ahead based on speed
          ...
          if (hit) {
            // Skip launcher for first 0.5 seconds
            const timeSinceLaunch = Date.now() - missile.launchTime;
            if (timeSinceLaunch > 500 || (launcherCollider && hit.colliderHandle !== launcherCollider)) {
              missile.hasExploded = true;
              console.log('Missile collision imminent, detonating!');
            }
          }
        }
      }
    ...
    if (!missile.hasExploded) {
      ...
      // Also cast a short ray downward to check for immediate ground
      ...
      const groundHit = physics.castRay(
        missilePos,
        downDir,
        2.0, // Check 2 units down
        missile.collider.handle
      );
      
      if (distanceToCenter <= groundRadius + 0.5 || (groundHit && groundHit.toi < 1.0)) {
        missile.hasExploded = true;
