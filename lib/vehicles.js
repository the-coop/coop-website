...
export const WeaponHelpers = {
  ...
  createMissile(options) {
    ...
    physics.world.createCollider(missileCollider, missileBody);
    ...
    // Missile parameters - REDUCED VALUES
    const missileData = {
      body: missileBody,
      mesh: missileGroup,
      ...
      exhaustParticles: [],
      // Tuned thrust values for each vehicle type
      maxThrust: vehicleType === 'helicopter' ? 50 : 60, // Reduced from 70/80
      ignitionImpulse: vehicleType === 'helicopter' ? 8 : 10, // Reduced from 12/15
  ...
  updateMissile(missile, physics, pyrotechnics, scene, deltaTime = 0.016) {
    if (!missile.body || missile.hasExploded) return false;
