...
export class RemotePlayer {
  ...
  alignToSurface(gravityDirection) {
    ...
    // Use physics manager's alignment calculation with slower lerp for remote players
    const alignedQuat = this.physics.calculateSurfaceAlignment(
      currentQuat,
      this.lastGroundNormal,
      0.1 // Slower lerp for remote players to avoid jitter
    );
  ...
  update(deltaTime) {
    ...
    // For rotation, only interpolate when not grounded (when grounded, alignToSurface handles it)
    if (!this.isGrounded) {
      const currentQuat = new THREE.Quaternion(
        this.body.rotation().x,
        this.body.rotation().y,
