...
export class RemotePlayer {
  ...
  update(deltaTime) {
    ...
    // Get gravity direction - reuse playerPos from above
...
export class PlayerManager {
  ...
  updateAllNetworkPlayers(deltaTime) {
    this.players.forEach(player => {
      // Apply gravity and forces to the body (but not if swimming)
      if (!player.isSwimming) {
        ...
        const buoyancyStrength = this.physics.gravity.strength * 0.2;
        ...
        player.body.applyImpulse({
          x: -velocity.x * 2.0 * deltaTime,
          y: -velocity.y * 2.0 * deltaTime,
          z: -velocity.z * 2.0 * deltaTime
        });
      }
