...
export class RemotePlayer {
  ...
  alignToSurface(gravityDirection) {
    ...
    const lerpFactor = 0.1; // Consistent lerp for remote players
  ...
  update(deltaTime) {
    ...
    // Set body rotation (even though it has locked rotations, we can still set it)
    this.body.setRotation({
      x: currentQuat.x,
      y: currentQuat.y,
      z: currentQuat.z,
    ...
    // If grounded, apply additional surface alignment on top
    if (this.isGrounded) {
      // Get the current rotation after interpolation
      const alignedQuat = this.physics.calculateSurfaceAlignment(
        currentQuat,
        this.lastGroundNormal,
        0.2 // Stronger alignment for remote players
      );
