...
export class RemotePlayer {
  ...
  updateWeaponDisplay(weaponType) {
    ...
    let weaponGeometry;
    let weaponMaterial = new THREE.MeshStandardMaterial({
      color: 0x333333,
      roughness: 0.3,
      metalness: 0.7
    });
    ...
    switch (weaponType) {
      case 'hands':
        ...
        weaponGeometry = new THREE.BoxGeometry(0.1, 0.2, 0.1);
        weaponMaterial.color.set(0xffdbac);
        break;
      case 'pistol':
        weaponGeometry = new THREE.BoxGeometry(0.05, 0.15, 0.2);
        break;
      case 'rifle':
        weaponGeometry = new THREE.BoxGeometry(0.08, 0.12, 0.6);
        break;
      case 'shotgun':
        weaponGeometry = new THREE.BoxGeometry(0.08, 0.1, 0.5);
        break;
      default:
        return;
    }
    ...
    this.weaponMesh.position.set(0.2, 0.5, -0.2);
  ...
  updateState(state) {
    ...
    if (state.weaponState && state.weaponState.slots) {
      const currentSlot = state.weaponState.currentSlot || 0;
      const weapon = state.weaponState.slots[currentSlot];
      if (weapon) {
        this.updateWeaponDisplay(weapon.type);
      }
    }
  }
  ...
  update(deltaTime) {
    ...
    // Apply gravity or buoyancy based on swimming state
    if (!this.isSwimming) {
    ...
    let predictedTarget = this.targetPosition.clone();
    if (this.enablePrediction && this.targetVelocity.lengthSq() > 0.1) {
      // Add velocity-based prediction
      const predictionOffset = this.targetVelocity.clone().multiplyScalar(this.predictionTime);
      predictedTarget.add(predictionOffset);
    }
    
    // Calculate position error
    const positionError = predictedTarget.clone().sub(currentPosition);
    const errorDistance = positionError.length();
    
    // If error is large, teleport (prevents drift)
    if (errorDistance > 5.0) {
    ...
    } else if (errorDistance > 0.01) {
      // Use velocity-based correction instead of position setting
      const correction = positionError.multiplyScalar(this.interpolationFactor / deltaTime);
      
      // Blend correction with target velocity
      const blendedVelocity = this.targetVelocity.clone().lerp(correction, 0.5);
      ...
      // Small error - just use target velocity
      this.body.setLinvel({
        x: this.targetVelocity.x,
        y: this.targetVelocity.y,
        z: this.targetVelocity.z
    ...
    // Use faster rotation interpolation for dynamic states
