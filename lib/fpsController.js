...
export class FPSController {
  ...
  handleMovement(deltaTime) {
    ...
    // Only apply gravity if not swimming
    let gravityForce;
    if (!this.isSwimming) {
      gravityForce = gravityDir.clone().multiplyScalar(gravityStrength * deltaTime);
    } else {
      // When swimming, no gravity force
      gravityForce = new THREE.Vector3(0, 0, 0);
      
      // Reset forces to prevent any gravity accumulation
      this.body.resetForces(true);
      
      // Apply more realistic buoyancy force
      const mass = this.body.mass();
      const buoyancyStrength = gravityStrength * 0.2; // Much weaker - only 20% of gravity
      const buoyancyForce = gravityDir.clone().multiplyScalar(-buoyancyStrength * mass); // Upward force
      
      // Apply as an impulse scaled by deltaTime
    ...
    // Only add gravity if not swimming
    if (!this.isSwimming) {
  ...
  checkForNearbyVehicle() {
    if (!this.body || this.isInVehicle) return null;
    ...
    if (this.scene.vehicles) {
      this.scene.vehicles.forEach((car, id) => {
        const carPos = car.getPosition();
        const distance = playerPos.distanceTo(carPos);
        
        if (distance < closestDistance && !car.isOccupied) {
          closestVehicle = car;
  ...
  update(deltaTime) {
    if (this.isInVehicle && this.currentVehicle) {
      // Don't update player physics when in vehicle
  ...
  // Add method to get full player state including grounded
  getNetworkState() {
    ...
    return {
      ...
      isSwimming: this.isSwimming // Add swimming state to network
    };
  }
