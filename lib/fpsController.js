...
export class FPSController {
  ...
  handleMovement(deltaTime) {
    // Handle vehicle interaction with U key - moved before the in-vehicle check
    ...
    // Don't process movement if in vehicle - moved after interaction handling
    if (this.isInVehicle) {
      return;
    }
  ...
  toggleCamera() {
    if (this.tpController) {
      ...
      // Notify weapon system about camera change
      if (window.weaponSystem) {
        window.weaponSystem.updateWeaponVisibility();
      }
    }
  }
  ...
  checkForNearbyVehicle() {
    ...
    this.scene.vehicles.forEach((vehicle, id) => {
      if (!vehicle.chassisBody && !vehicle.body) return; // Check for both car and helicopter body
    ...
    this.scene.dynamicObjects.forEach((obj, id) => {
      if ((obj.type === 'vehicle' || obj.type === 'plane' || obj.type === 'helicopter') && obj.controller && !obj.controller.isOccupied) {
  ...
  update(deltaTime) {
    ...
    // Skip regular update if in vehicle but still handle interaction
    if (this.isInVehicle) {
      // Still check for exit input
