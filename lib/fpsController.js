...
export class FPSController {
  ...
  getUpDirection() {
    // Calculate up direction based on gravity center
    ...
    const up = new THREE.Vector3()
      .subVectors(playerPos, gravityCenter)
      .normalize();
  ...
  checkGrounded() {
    ...
    const rayDir = up.clone().multiplyScalar(-1); // Down is opposite of up
    
    const hit = this.physics.castRay(
      rayOrigin,
      rayDir,
      this.height * 0.6 + 0.1, // Slightly more than half height + small margin
      this.collider.handle
    );
    
    this.isGrounded = hit !== null;
    
    // Also check recent collision events
    const currentTime = performance.now();
    const recentContact = (currentTime - this.physics.lastGroundContact) < 100;
    
    if (recentContact && this.physics.groundCollisions.size > 0) {
  ...
  update(deltaTime) {
    ...
    // Apply movement forces through physics manager
    this.physics.applyMovementForces(
      this.body, 
      movement, 
      this.isGrounded, 
      deltaTime
