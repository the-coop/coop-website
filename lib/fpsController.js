...
export class FPSController {
  ...
  enterVehicle(vehicle) {
    ...
    // Remember camera state before entering
    ...
    if (success) {
      ...
      // Hide weapon view model when entering vehicle
      if (window.weaponSystem && window.weaponSystem.viewModelGroup) {
        window.weaponSystem.viewModelGroup.visible = false;
      }
      
      // Deactivate third-person camera if active
      if (this.tpController && this.tpController.isActive) {
  ...
  exitVehicle(exitPosition) {
    ...
    // Get exit info from vehicle
    let exitInfo = null;
    if (this.currentVehicle.exitCar) {
      exitInfo = this.currentVehicle.exitCar();
    } else if (this.currentVehicle.exitHelicopter) {
      exitInfo = this.currentVehicle.exitHelicopter();
    } else if (this.currentVehicle.exitPlane) {
      exitInfo = this.currentVehicle.exitPlane();
    }
    
    if (!exitInfo) {
      console.error('Failed to get exit info from vehicle');
      return;
    }
    
    // Use exit position from vehicle info
    if (exitInfo && exitInfo.exitPosition) {
      exitPosition = exitInfo.exitPosition;
    }
    
    if (!exitPosition) {
      // Default exit position above vehicle
      exitPosition = this.currentVehicle.getPosition().clone();
      exitPosition.y += 3;
    }
    
    console.log('Exiting vehicle at position:', exitPosition);
    
    // Clear vehicle state FIRST
    this.isInVehicle = false;
    this.currentVehicle = null;
    
    // Re-enable physics body BEFORE setting position
    if (this.body) {
      this.body.setEnabled(true);
      
      // Set position on physics body
    ...
    // Show player mesh and update its position
    if (this.mesh) {
      this.mesh.visible = true;
      this.mesh.position.copy(exitPosition);
    }
    
    // Ensure camera is removed from any parent first
    if (this.scene.camera.parent) {
      this.scene.camera.parent.remove(this.scene.camera);
    }
    
    // Re-attach camera to player mesh
    if (this.mesh) {
      this.mesh.add(this.scene.camera);
      this.scene.camera.position.set(0, this.height * 0.8, 0);
      this.scene.camera.rotation.set(0, 0, 0);
    ...
    // Restore camera mode based on previous state
    if (this.cameraStateBeforeVehicle.isThirdPerson && this.tpController) {
      console.log('Restoring third-person camera');
      // Delay activation to ensure everything is set up
      setTimeout(() => {
    ...
    // Show weapon view model when exiting vehicle (only if in first person)
    if (window.weaponSystem) {
      window.weaponSystem.updateWeaponVisibility();
    }
    
    // Reset grounded state to force a ground check
    this.isGrounded = false;
    this.wasGrounded = false;
    
    // Set exit cooldown to prevent immediate re-entry
    this.vehicleExitCooldown = Date.now() + 500; // 500ms cooldown
