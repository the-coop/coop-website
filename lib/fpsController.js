...
export class FPSController {
  ...
  handleMovement(deltaTime) {
    ...
    // Don't process movement if in vehicle
    if (this.isInVehicle) {
      return;
    }
  ...
  exitVehicle(exitPosition) {
    ...
    // Show player mesh FIRST before doing anything with camera
    if (this.mesh) {
    ...
    // Always re-attach camera to player mesh first
    this.mesh.add(this.scene.camera);
    this.scene.camera.position.set(0, this.height * 0.8, 0);
    this.scene.camera.rotation.set(0, 0, 0);
    
    // Then restore camera mode based on previous state
    if (this.cameraStateBeforeVehicle.isThirdPerson && this.tpController) {
      console.log('Restoring third-person camera');
      // Activate third-person after camera is attached
      this.tpController.activate();
    } else {
      console.log('Restored first-person camera');
      // Camera is already in first-person position
    }
