...
export class FPSController {
  constructor(scene, physics) {
    ...
    this.keys = {
      ...
      toggleCamera: false // Add toggle camera key (O)
    };
  ...
  create(spawnPosition = new THREE.Vector3(0, 35, 0)) {
    ...
    this.scene.camera.position.set(0, this.height * 0.4, 0); // Lowered from 0.8 to center weapons better
  ...
  checkForNearbyVehicle() {
    // Don't check for vehicles during exit cooldown
    ...
    if (this.scene.dynamicObjects) {
      for (const [id, obj] of this.scene.dynamicObjects) {
        if ((obj.type === 'vehicle' || obj.type === 'plane' || obj.type === 'helicopter') && obj.controller) {
    ...
    // Check for vehicles stored on mesh userData
    let nearestVehicle = null;
    let nearestDistance = interactionDistance;
    
    this.scene.scene.traverse((child) => {
      if (child.isMesh && child.userData.interactable && child.userData.interactionType === 'vehicle') {
        const vehicleController = child.userData.carController || 
                                child.userData.planeController || 
                                child.userData.helicopterController;
        
        if (vehicleController) {
          const distance = playerPos.distanceTo(child.position);
          if (distance < nearestDistance) {
            nearestDistance = distance;
            nearestVehicle = vehicleController;
          }
        }
      }
    });
  ...
  enterVehicle(vehicle) {
    if (this.isInVehicle || !vehicle) return false;
    
    // Store camera state before entering
    this.cameraStateBeforeVehicle.isThirdPerson = this.tpController && this.tpController.isActive;
    
    // Deactivate third-person if active
    if (this.tpController && this.tpController.isActive) {
    ...
    // Call the appropriate enter method based on vehicle type
    if (vehicle.enterCar) {
      success = vehicle.enterCar(this);
    } else if (vehicle.enterPlane) {
      success = vehicle.enterPlane(this);
    } else if (vehicle.enterHelicopter) {
      success = vehicle.enterHelicopter(this);
    }
    ...
    if (success) {
      ...
      // Disable player physics body while in vehicle
      if (this.body) {
  ...
  exitVehicle(exitPosition) {
    if (!this.isInVehicle || !this.currentVehicle) return null;
    
    console.log('FPSController exitVehicle called');
    
    let exitResult = null;
    
    // Call the vehicle's exit method
    if (this.currentVehicle.exitCar) {
      exitResult = this.currentVehicle.exitCar();
    } else if (this.currentVehicle.exitPlane) {
      exitResult = this.currentVehicle.exitPlane();
    } else if (this.currentVehicle.exitHelicopter) {
      exitResult = this.currentVehicle.exitHelicopter();
    }
    
    if (!exitResult) return null;
    
    // Set a longer exit cooldown to prevent immediate re-entry
    this.vehicleExitCooldown = 2.0;
    
    // Clear vehicle state first
    this.isInVehicle = false;
    this.currentVehicle = null;
    
    const finalExitPosition = exitPosition || exitResult.exitPosition;
    
    console.log('Exiting vehicle at position:', finalExitPosition);
    
    // Re-enable player physics body BEFORE setting position
    if (this.body) {
    ...
    this.body.setTranslation({
      x: finalExitPosition.x,
      y: finalExitPosition.y,
      z: finalExitPosition.z
    });
    
    // Clear velocity to prevent momentum carrying into vehicle
    this.body.setLinvel({ x: 0, y: 0, z: 0 });
    
    // Ensure player mesh follows body position
    if (this.mesh) {
      this.mesh.position.set(finalExitPosition.x, finalExitPosition.y, finalExitPosition.z);
    }
    
    // Restore camera state based on what it was before entering vehicle
    if (this.cameraStateBeforeVehicle.isThirdPerson) {
      // Switch back to third-person if that's what we had before
      if (this.tpController) {
        this.tpController.activate();
        console.log('Restored third-person camera');
      }
    } else {
      ...
      // Ensure camera is properly attached to player mesh
      if (this.mesh && !this.mesh.children.includes(this.scene.camera)) {
        // Remove camera from wherever it might be
        if (this.scene.camera.parent) {
          this.scene.camera.parent.remove(this.scene.camera);
        } else if (this.scene.scene.children.includes(this.scene.camera)) {
          this.scene.scene.remove(this.scene.camera);
        }
        
        // Add to player mesh with corrected position
        this.mesh.add(this.scene.camera);
        this.scene.camera.position.set(0, this.height * 0.4, 0); // Consistent with create()
        this.scene.camera.rotation.set(0, 0, 0);
        
        console.log('Restored first-person camera');
      }
    }
    
    // Reset any rotation that might have been applied during vehicle control
    if (this.body) {
      // Keep the yaw rotation but reset pitch and roll for grounded state
      ...
      // Extract just the Y rotation (yaw) and reset pitch/roll
    ...
    // Force a physics update to ensure everything is synced
    if (this.body) {
      this.body.wakeUp();
    }
  ...
  update(deltaTime) {
    ...
    if (this.vehicleExitCooldown > 0) {
      this.vehicleExitCooldown = Math.max(0, this.vehicleExitCooldown - deltaTime);
    }
    ...
    // Vehicle interaction (only when not in vehicle and cooldown expired)
    if (!this.isInVehicle && this.vehicleExitCooldown <= 0) {
      // Check for interaction with vehicles
    ...
    // Handle vehicle exit
    if (this.isInVehicle) {
      // Exit vehicle with interact key
      if (this.keys.interact && !this.wasInteracting) {
        this.wasInteracting = true;
        this.exitVehicle();
      } else if (!this.keys.interact) {
      ...
      // Don't process normal movement when in vehicle
      return;
    }
    
    // Regular player update when not in vehicle
