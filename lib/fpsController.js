...
export class FPSController {
  ...
  create(spawnPosition = new THREE.Vector3(0, 35, 0)) {
    ...
    // Create weapon system after mesh is created
  ...
  exitVehicle(exitPosition) {
    if (!this.isInVehicle || !this.body) return;
  ...
  // Add weapon interaction methods
  tryPickupWeapon() {
    if (!this.weaponSystem) return false;
    
    const nearbyPickup = this.checkNearbyWeaponPickup();
    if (nearbyPickup) {
      return this.weaponSystem.pickupWeapon(nearbyPickup.id, this);
    }
    return false;
  }
  ...
  checkNearbyWeaponPickup() {
    ...
    this.weaponSystem.weaponPickups.forEach((pickup) => {
  ...
  switchWeapon(slot) {
    if (!this.weaponSystem) return false;
    return this.weaponSystem.switchToSlot(slot);
  }
