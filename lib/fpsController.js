...
export class FPSController {
  ...
  alignToSurface(gravityDirection) {
    ...
    // Use physics manager's alignment calculation
    const lerpFactor = this.wasGrounded ? 0.15 : 0.05; // Slower initial alignment when just landed
  ...
  handleMouseMove(event, lookSensitivity = 0.001, yawSensitivity = 0.002) {
    ...
    if (this.isGrounded) {
      ...
      // Player yaw - use last ground normal for better surface alignment
      let upVector = this.lastGroundNormal.clone();
