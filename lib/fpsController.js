...
export class FPSController {
  constructor(scene, physics) {
    ...
    this.keys = {
      ...
      interact: false,
      toggleCamera: false,
      crouch: false,        // Make sure this exists
      landingGear: false,   // Add L key
      fireGun: false,       // Add G key
      fireMissile: false,   // Add M key
      toggleLights: false   // Add N key for aircraft lights
    };
  ...
  checkForNearbyVehicle() {
    ...
    if (this.scene.dynamicObjects) {
      for (const [id, obj] of this.scene.dynamicObjects) {
        if ((obj.type === 'vehicle' || obj.type === 'plane' || obj.type === 'helicopter') && obj.controller) {
          if (obj.controller.isOccupied) continue;
          
          // Use the controller's getPosition method
          const vehiclePos = obj.controller.getPosition();
          const distance = playerPos.distanceTo(vehiclePos);
          
          if (distance < interactionDistance) {
            return obj.controller; // Return the controller, not the obj
          }
        }
      }
    }
    ...
    this.scene.scene.traverse((child) => {
      if (child.isMesh && child.userData.interactable && child.userData.interactionType === 'vehicle') {
        const controller = child.userData.carController || child.userData.planeController || child.userData.helicopterController;
        
        if (controller && !controller.isOccupied) {
          const vehiclePos = controller.getPosition();
          const distance = playerPos.distanceTo(vehiclePos);
          
          if (distance < nearestDistance) {
            nearestVehicle = controller;
  ...
  enterVehicle(vehicle) {
    ...
    // Ensure camera is properly detached from player before vehicle takes control
    ...
    if (success) {
      ...
      // Connect player's keys to vehicle's controls
      // This creates a reference so key states are shared
      vehicle.keys = this.keys;
      
      // IMPORTANT: Mark that we need to wait for interact key release
      vehicle.wasInteracting = true;  // Force the vehicle to wait for key release
      ...
      // Reset camera rotation to avoid conflicts
      this.cameraRotation.x = 0;
      this.cameraRotation.y = 0;
      this.cameraRotation.z = 0;
  ...
  exitVehicle() {
    ...
    // Call the appropriate exit method based on vehicle type
    if (this.currentVehicle.exitCar) {
      exitData = this.currentVehicle.exitCar();
    } else if (this.currentVehicle.exitPlane) {
      exitData = this.currentVehicle.exitPlane();
    } else if (this.currentVehicle.exitHelicopter) {
      exitData = this.currentVehicle.exitHelicopter();
    }
    
    if (exitData && exitData.exitPosition) {
      ...
      if (this.body) {
        ...
        this.body.setTranslation({
          x: exitData.exitPosition.x,
          y: exitData.exitPosition.y,
          z: exitData.exitPosition.z
        });
      }
      ...
      if (this.scene.camera.parent !== this.mesh) {
        ...
        // Add back to player mesh
        this.mesh.add(this.scene.camera);
        
        // Reset camera to FPS position
        this.scene.camera.position.set(0, this.height * 0.4, 0);
        this.scene.camera.rotation.set(0, 0, 0);
      ...
      this.vehicleExitCooldown = 1.0; // 1 second cooldown
      
      // Restore camera state
      if (this.cameraStateBeforeVehicle.isThirdPerson && this.tpController) {
        // Reactivate third person if it was active before
  ...
  update(deltaTime) {
    ...
    if (!this.isInVehicle && this.vehicleExitCooldown <= 0) {
      ...
      if (this.keys.interact && !this.wasInteracting) {
        ...
        if (nearbyVehicle) {
          const success = this.enterVehicle(nearbyVehicle);
          if (success) {
            // Set interaction state to prevent immediate exit
            this.wasInteracting = true;
          }
        }
      }
    } else if (this.isInVehicle) {
      // Handle vehicle exit - wait for key release
      if (this.keys.interact && !this.wasInteracting) {
        this.exitVehicle();
      }
    }
