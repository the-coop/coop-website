export class WebSocketManager {
  ...
  handleMessage(data) {
    switch(data.type) {
      ...
      case 'weapon_pickup_respawned':
        if (this.onWeaponRespawned) {
          this.onWeaponRespawned(data.pickup_id);
        }
        break;
        
      case 'vehicle_spawned':
        if (this.onVehicleSpawned) {
          this.onVehicleSpawned(data.vehicle_id, data.vehicle_type, data.position);
        }
        break;
        
      case 'vehicle_occupied':
        if (this.onVehicleOccupied) {
          this.onVehicleOccupied(data.vehicle_id, data.player_id);
        }
        break;
        
      case 'vehicle_vacated':
        if (this.onVehicleVacated) {
          this.onVehicleVacated(data.vehicle_id);
        }
        break;
  ...
  // Add method to send weapon pickup request
  sendWeaponPickup(weaponId) {
    this.sendPlayerAction('pickup_weapon', { weapon_id: weaponId });
  }
