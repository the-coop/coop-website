...
export class PhysicsManager {
  ...
  createBallCollider(radius, options = {}) {
    ...
    // For dynamic objects and kinematic objects, ensure they collide with everything
    if (!options.collisionGroups) {
      desc.setCollisionGroups(
    ...
    // Ensure solver groups allow interaction with all objects
    if (!options.solverGroups) {
      desc.setSolverGroups(this.COLLISION_GROUPS.ALL);
    } else {
      desc.setSolverGroups(options.solverGroups);
    }
    
    // Enable collision events for kinematic bodies if specified
    if (options.activeEvents) {
      desc.setActiveEvents(options.activeEvents);
    } else {
      // Default to enabling collision events
      desc.setActiveEvents(RAPIER.ActiveEvents.COLLISION_EVENTS);
    }
    
    // Set as sensor if specified (for ghost objects)
    if (options.isSensor) {
      desc.setSensor(true);
    }
