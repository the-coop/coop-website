...
export class WeaponSystem {
  ...
  pickupWeapon(pickupId) {
    ...
    if (!pickup) {
      console.warn(`Weapon pickup ${pickupId} not found`);
      return false;
    }
    
    // Check if this is a multiplayer pickup
    if (pickup.mesh.userData.isMultiplayerPickup) {
      // Send pickup request to server
      if (this.networkManager) {
        this.networkManager.sendPickupWeapon(pickup.mesh.userData.multiplayerPickupId);
      }
      
      // Don't remove the pickup immediately - wait for server confirmation
      // But do add the weapon to inventory optimistically
      console.log(`Attempting to pickup multiplayer weapon: ${pickup.type}`);
    }
