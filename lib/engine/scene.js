...
export class SceneManager {
  ...
  spawnMultiplayerRock(objectId, position, scale = 1.0) {
    ...
    // Use the physics manager's RAPIER reference
    ...
    this.dynamicObjects.set(objectId, {
      mesh: mesh,
      body: body, // Dynamic body for proper collision
      type: 'rock',
      scale: scale,
      collider: collider,
  ...
  updateDynamicObjects() {
    ...
    if (this.pyrotechnics) {
      this.pyrotechnics.update(0.016); // Approximate 60fps delta
    }
    
    // Update weapon pickups
    if (this.gameMode === 'sandbox') {
      this.updateWeaponPickups(0.016); // Approximate 60fps delta
    }
  }
  ...
  async setupNetwork() {
    ...
    this.network.onPlayerJoin = (playerId, position) => {
      ...
      // Add player with world position
      this.playerManager.addPlayer(playerId, pos);
      
      // If we have a local origin, update the remote player's position
      if (this.localOrigin) {
        const remotePlayer = this.playerManager.players.get(playerId);
