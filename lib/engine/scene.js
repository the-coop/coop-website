...
export class SceneManager {
  constructor(physics) {
    ...
    // Track multiplayer weapon pickups
    this.multiplayerWeaponPickups = new Map();
  }
  ...
  buildLevelFromData(levelObjects) {
    ...
    // Store weapon and vehicle spawn data for later processing
    const weaponSpawns = [];
    const vehicleSpawns = [];
    ...
    levelObjects.forEach(obj => {
      ...
      switch(obj.object_type || obj.type) {
        ...
        case 'weapon_spawn':
          // Store weapon spawn data for processing after player is initialized
          if (obj.properties && obj.properties.weapon_type) {
            weaponSpawns.push({
              type: obj.properties.weapon_type,
        ...
        case 'vehicle_spawn':
          // Store vehicle spawn data
          if (obj.properties && obj.properties.vehicle_type) {
            vehicleSpawns.push({
              id: obj.id,
              type: obj.properties.vehicle_type,
              position: new THREE.Vector3(obj.position.x, obj.position.y, obj.position.z)
            });
          }
          break;
    ...
    // Store spawn data for later use
    this.pendingWeaponSpawns = weaponSpawns;
    this.pendingVehicleSpawns = vehicleSpawns;
    ...
    console.log(`Pending weapon spawns: ${weaponSpawns.length}`);
    console.log(`Pending vehicle spawns: ${vehicleSpawns.length}`);
  }

  // Add method to process pending spawns after player is initialized
  processPendingSpawns(weaponSystem) {
    // Process weapon spawns
    if (this.pendingWeaponSpawns && weaponSystem) {
      console.log(`Processing ${this.pendingWeaponSpawns.length} weapon spawns for multiplayer`);
      this.pendingWeaponSpawns.forEach(spawn => {
        console.log(`Spawning ${spawn.type} at`, spawn.position);
        weaponSystem.spawnWeaponPickup(spawn.type, spawn.position);
      });
      this.pendingWeaponSpawns = null;
    }
    
    // Process vehicle spawns
    if (this.pendingVehicleSpawns) {
      console.log(`Processing ${this.pendingVehicleSpawns.length} vehicle spawns for multiplayer`);
      this.pendingVehicleSpawns.forEach(spawn => {
        switch(spawn.type) {
          case 'car':
            this.spawnTestCar(spawn.position);
            break;
          case 'plane':
            this.spawnPlane(spawn.position);
            break;
          case 'helicopter':
            this.spawnHelicopter(spawn.position);
            break;
          case 'spaceship':
            this.spawnSpaceship(spawn.position);
            break;
        }
      });
      this.pendingVehicleSpawns = null;
    }
  }
  ...
  async setupNetwork() {
    ...
    // Add weapon pickup handlers
    this.network.onWeaponPickupSpawned = (pickupId, weaponType, position) => {
      console.log(`Weapon pickup spawned: ${weaponType} at`, position);
      this.spawnMultiplayerWeaponPickup(pickupId, weaponType, position);
    };
    
    this.network.onWeaponPickedUp = (pickupId, playerId, weaponType) => {
      console.log(`Weapon picked up: ${weaponType} by player ${playerId}`);
      this.handleMultiplayerWeaponPickup(pickupId, playerId);
    };
    
    this.network.onWeaponPickupRespawned = (pickupId) => {
      console.log(`Weapon pickup respawned: ${pickupId}`);
      this.respawnMultiplayerWeaponPickup(pickupId);
    };
    
    // ...existing code...
  }
  
  spawnMultiplayerWeaponPickup(pickupId, weaponType, position) {
    // Check if player has weapon system
    if (!this.player || !this.player.weaponSystem) {
      console.warn('Cannot spawn weapon pickup - no weapon system available');
      return;
    }
    
    const pickupPos = new THREE.Vector3(position.x, position.y, position.z);
    
    // Use the weapon system's spawn method
    const pickupData = this.player.weaponSystem.spawnWeaponPickup(weaponType, pickupPos);
    
    if (pickupData) {
      // Store the multiplayer pickup mapping
      this.multiplayerWeaponPickups.set(pickupId, {
        localId: pickupData.id,
        weaponType: weaponType,
        pickupData: pickupData
      });
      
      // Override the pickup behavior for multiplayer
      pickupData.mesh.userData.multiplayerPickupId = pickupId;
      pickupData.mesh.userData.isMultiplayerPickup = true;
    }
  }
  
  handleMultiplayerWeaponPickup(pickupId, playerId) {
    const pickupInfo = this.multiplayerWeaponPickups.get(pickupId);
    if (!pickupInfo) return;
    
    // Remove the pickup visually
    if (this.player && this.player.weaponSystem) {
      this.player.weaponSystem.removePickup(pickupInfo.localId);
    }
    
    // If it was us who picked it up, the weapon is already added locally
    // If it was another player, just remove the visual
    
    // Remove from tracking
    this.multiplayerWeaponPickups.delete(pickupId);
  }
  
  respawnMultiplayerWeaponPickup(pickupId) {
    const pickupInfo = this.multiplayerWeaponPickups.get(pickupId);
    if (!pickupInfo || !pickupInfo.pickupData) return;
    
    // Re-enable the pickup visually
    if (pickupInfo.pickupData.mesh) {
      pickupInfo.pickupData.mesh.visible = true;
      
      // Re-enable collision
      if (pickupInfo.pickupData.body && this.physics.world) {
        // The pickup should already exist, just make it visible again
        this.scene.add(pickupInfo.pickupData.mesh);
      }
    }
  }
