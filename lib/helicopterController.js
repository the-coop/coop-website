...
export class HelicopterController {
  ...
  update(deltaTime) {
    ...
    if (this.rotorSpeed > 0.1) {
      // Increased lift multiplier for better performance
      const liftMagnitude = this.rotorSpeed * this.maxThrust * 0.8; // Increased from 0.5
  ...
  enterHelicopter(player) {
    ...
    // Position camera much further back behind the helicopter
    // Helicopter faces -Z, so positive Z is behind it
    this.scene.camera.position.set(0, 12, 35);  // Higher and much further back
    // Look forward at the helicopter
    this.scene.camera.rotation.set(-0.2, 0, 0);  // Look down slightly, no Y rotation needed
