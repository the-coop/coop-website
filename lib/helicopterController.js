...
export class HelicopterController {
  constructor(scene, physics, position) {
    ...
    // Helicopter properties - INCREASED THRUST
    this.maxLiftForce = 50.0;  // Increased from 30.0
    this.maxTorque = 10.0;
  ...
  create(position) {
    ...
    // Create angular fuselage
    const fuselageShape = new THREE.Shape();
    fuselageShape.moveTo(-0.8, 0);
    fuselageShape.lineTo(-0.8, 0.6);
    fuselageShape.lineTo(-0.4, 1);
    fuselageShape.lineTo(0.4, 1);
    fuselageShape.lineTo(0.8, 0.6);
    fuselageShape.lineTo(0.8, 0);
    fuselageShape.lineTo(0.4, -0.6);
    fuselageShape.lineTo(-0.4, -0.6);
    ...
    // Create tail boom (thinner)
    const tailGeometry = new THREE.CylinderGeometry(0.2, 0.4, 6);
  ...
  createWeaponPylons() {
    ...
    for (let side of [-1, 1]) {
      ...
      for (let i = 0; i < 19; i++) {
        ...
        rocket.position.y = Math.sin(angle) * radius;
  ...
  update(deltaTime) {
    ...
    if (this.isOccupied && this.keys) {
      ...
      if (this.keys.fireMissile && !this.wasFiringRocket) {
        this.fireRocket();
      }
      this.wasFiringRocket = this.keys.fireMissile;
    }
  ...
  updateCamera() {
    if (this.scene.camera.parent === this.mesh) {
      ...
      // Dynamic camera distance based on speed - INCREASED DISTANCES
      const baseDist = 30;  // Increased from 20
      const speedFactor = Math.min(speed / 30, 1);
      const extraDist = speedFactor * 10;  // Increased from 5
      ...
      this.scene.camera.position.y = 15 + verticalFactor * 5;  // Increased from 10 + 3
  ...
  createPhysics(position) {
    ...
    const fuselageCollider = this.physics.createBoxCollider(
      ...
      {
        density: 0.3,  // Reduced from 0.5 for lighter helicopter
        friction: 0.5,
        restitution: 0.3
      }
    );
  ...
  getFlightData() {
    ...
    // Calculate vertical speed - FIX: gravityDir not defined
    const playerPos = new THREE.Vector3(position.x, position.y, position.z);
    const gravityDir = new THREE.Vector3()
      .subVectors(this.physics.gravity.center, playerPos)
      .normalize();
