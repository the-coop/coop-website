...
export class HelicopterController {
  constructor(scene, physics, position) {
    ...
    this.lockOnSystem = {
      ...
      lockProgress: 0,
      maxLockRange: 300,
      minLockTime: 1.5, // seconds to achieve lock
  ...
  fireMissile() {
    ...
    // Add lock-on target if available
    if (this.lockOnSystem.isLocked && this.lockOnSystem.targetVehicle) {
      missile.lockedTarget = this.lockOnSystem.targetVehicle;
      missile.trackingStrength = 0.8; // Strong tracking for locked missiles
      console.log('Firing locked missile at target!');
    }
  ...
  updateLockOnSystem(deltaTime) {
    ...
    if (this.scene.vehicles && this.scene.vehicles instanceof Map) {
      ...
      if (this.scene.helicopterController && this.scene.helicopterController !== this && 
          this.scene.helicopterController.body && this.scene.helicopterController.mesh) {
        potentialTargets.push({
          type: 'helicopter',
