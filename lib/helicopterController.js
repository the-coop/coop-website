...
export class HelicopterController {
  ...
  createWeaponPylons() {
    ...
    for (let side of [-1, 1]) {
      ...
      // Create 19 rockets per pod (hexagonal pattern) - FIXED positions
      for (let i = 0; i < 19; i++) {
  ...
  enterHelicopter(player) {
    ...
    // Position camera closer to the helicopter for a better view
    this.scene.camera.position.set(0, 8, -25);  // Reduced from y:20, z:-70 to be closer
    ...
    this.scene.camera.rotation.x = -0.15; // Slight downward angle (reduced from -0.2)
  ...
  createMissilePods() {
    const podPositions = [
      { x: 0, y: -0.5, z: -1.5 }, // Left pod
      { x: 0, y: -0.5, z: 1.5 }   // Right pod
    ];
    
    podPositions.forEach((pos, index) => {
      const podGroup = new THREE.Group();
      
      // Pod body - rotated to face forward like the helicopter
      const podGeometry = new THREE.CylinderGeometry(0.15, 0.15, 1.2, 8);
      const podMaterial = new THREE.MeshStandardMaterial({
        color: 0x333333,
        metalness: 0.8
      });
      const pod = new THREE.Mesh(podGeometry, podMaterial);
      pod.rotation.z = Math.PI / 2; // Rotate to face forward instead of x-axis
      podGroup.add(pod);
      
      // Create 4 missiles per pod
      for (let i = 0; i < 4; i++) {
        const missileGeometry = new THREE.CylinderGeometry(0.04, 0.04, 0.6, 6);
        ...
        // Position missiles around the pod
        const angle = (i / 4) * Math.PI * 2;
        missile.position.x = 0; // Missiles along the length now
        missile.position.y = Math.cos(angle) * 0.12;
        missile.position.z = Math.sin(angle) * 0.12;
        missile.rotation.z = Math.PI / 2; // Align with pod
        
        // Add fins
        const finGeometry = new THREE.BoxGeometry(0.02, 0.08, 0.04);
        for (let j = 0; j < 4; j++) {
          const fin = new THREE.Mesh(finGeometry, missileMaterial);
          const finAngle = (j / 4) * Math.PI * 2;
          fin.position.x = -0.2; // Fins at back of missile
          fin.position.y = Math.cos(finAngle) * 0.03;
          fin.position.z = Math.sin(finAngle) * 0.03;
          missile.add(fin);
        }
        
        podGroup.add(missile);
      }
      
      podGroup.position.set(pos.x, pos.y, pos.z);
      podGroup.userData.podIndex = index;
      this.mesh.add(podGroup);
      this.missilePods.push(podGroup);
    });
  }
  ...
  fireMissile() {
    ...
    const missile = {
      ...
      age: 0,
      maxAge: 2.0, // Changed from 12 to 2 seconds
      thrust: 40,
      guidanceActive: true,
      hasExploded: false
    };
  ...
  getFlightData() {
    ...
    return {
      ...
      flares: this.currentFlares, // Add flare count
      rockets: this.currentRockets // Add rocket count for consistency
    };
  }
