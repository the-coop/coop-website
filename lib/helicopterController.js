...
export class HelicopterController {
  ...
  updateLockOnSystem(deltaTime) {
    ...
    if (this.scene.vehicles && this.scene.vehicles instanceof Map) {
      ...
      console.log('Vehicle registry size:', this.scene.vehicles.size);
      
      this.scene.vehicles.forEach((vehicle, id) => {
        console.log(`Vehicle ${id}:`, {
          type: vehicle.constructor.name,
          hasGetPosition: typeof vehicle.getPosition === 'function',
          position: vehicle.getPosition ? vehicle.getPosition().toArray() : 'N/A'
        });
      ...
      // Fallback to individual controller references
      ...
      if (this.scene.carController && this.scene.carController !== this) {
        potentialTargets.push({
    ...
    console.log('Found potential targets:', potentialTargets.length);
    ...
    potentialTargets.forEach(target => {
      ...
      if (inCone && distance < closestDistance) {
        closestTarget = {
    ...
    console.log('Closest target:', closestTarget ? {
      ...
      distance: closestTarget.distance.toFixed(1),
      hasGetPosition: typeof closestTarget.vehicle.getPosition === 'function'
    } : 'none');
    
    // SIMPLIFIED INSTANT LOCK LOGIC - FIX THE TARGET ASSIGNMENT
    if (closestTarget) {
      // LOCK ONTO CLOSEST TARGET IMMEDIATELY
      this.lockOnSystem.target = closestTarget;
      this.lockOnSystem.targetVehicle = closestTarget.vehicle; // ⭐ THIS WAS MISSING!
