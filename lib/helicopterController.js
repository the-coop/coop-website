...
export class HelicopterController {
  ...
  updateCamera() {
    ...
    if (this.scene.camera.parent === this.mesh) {
      ...
      const baseDist = 70; // Increased base distance
      const speedFactor = Math.min(speed / 30, 1); // Normalize to helicopter speeds
      const extraDist = speedFactor * 15; // Extra distance at speed
      
      this.scene.camera.position.z = -(baseDist + extraDist);  // Negative Z for behind
      
      // Adjust height based on collective pitch and speed
      const baseHeight = 20;
      const pitchFactor = this.collectivePitch * 3; // Higher when climbing
      const speedHeight = speedFactor * 2;
      this.scene.camera.position.y = baseHeight + pitchFactor + speedHeight;
      
      // Keep camera rotation consistent
      this.scene.camera.rotation.y = Math.PI; // Face forward (180 degrees)
      this.scene.camera.rotation.x = -0.2; // Consistent downward angle
  ...
  handleControls(deltaTime, upDir) {
    ...
    if (this.controls.throttleUp) {
      this.collectivePitch = Math.min(this.collectivePitch + deltaTime * 1.5, 1.0); // Faster response
    } else if (this.controls.throttleDown) {
      this.collectivePitch = Math.max(this.collectivePitch - deltaTime * 1.5, 0);
    }
    ...
    if (this.controls.yawLeft) {
      this.tailRotorPitch = Math.max(this.tailRotorPitch - deltaTime * 2, -this.maxYawRate);
    } else if (this.controls.yawRight) {
      this.tailRotorPitch = Math.min(this.tailRotorPitch + deltaTime * 2, this.maxYawRate);
    } else {
