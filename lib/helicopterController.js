...
export class HelicopterController {
  ...
  updateLockOnSystem(deltaTime) {
    ...
    if (closestTarget) {
      // Instant lock on any target in cone
      ...
      this.lockOnSystem.lockProgress = 1.0; // Always fully locked
      this.lockOnSystem.isLocked = true;
      this.lockOnSystem.lastLockTime = Date.now();
      
      console.log(`LOCKED ON: ${closestTarget.type} at ${Math.round(closestDistance)}m`);
      
    } else {
      // No target - clear lock
      if (this.lockOnSystem.isLocked) {
        console.log('Lock lost - no target in cone');
      }
