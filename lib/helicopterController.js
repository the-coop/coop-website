...
export class HelicopterController {
  ...
  enterHelicopter(player) {
    ...
    // Position camera for helicopter view
    this.scene.camera.position.set(0, 5, 15);  // Behind and above
    
    // Reset camera rotation first
    this.scene.camera.rotation.set(0, 0, 0);
    
    // Apply slight downward look
    this.scene.camera.rotation.x = -0.1;
  ...
  updateCamera() {
    ...
    // Only adjust position for shake effect, keep rotation stable
    if (this.scene.camera.parent === this.mesh) {
      const shake = this.rotorSpeed * 0.01;
      this.scene.camera.position.x = Math.sin(Date.now() * 0.01) * shake;
      this.scene.camera.position.y = 5 + Math.sin(Date.now() * 0.015) * shake;
      this.scene.camera.position.z = 15; // Keep Z position stable
      
      // Maintain fixed camera rotation
      this.scene.camera.rotation.x = -0.1;
