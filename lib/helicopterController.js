...
export class HelicopterController {
  constructor(scene, physics, position) {
    ...
    this.maxThrust = 30.0;
    this.maxPitch = 0.3; // Reduced from 0.5
    this.maxRoll = 0.3; // Reduced from 0.5
    this.maxYawRate = 0.6; // Reduced from 1.0
    this.rotorSpeed = 0;
    ...
    this.minLiftoffRotorSpeed = 0.7; // 70% rotor speed needed to lift off
  ...
  create(position) {
    ...
    // Add tail boom
    const tailGeometry = new THREE.CylinderGeometry(0.2, 0.5, 4);  // Swapped radius parameters for correct taper
    const tail = new THREE.Mesh(tailGeometry, fuselageMaterial);
    tail.rotation.x = Math.PI / 2;  // Rotate around X axis instead of Z to point backward
  ...
  createPhysics(position) {
    ...
    this.body = this.physics.createDynamicBody(position, {
      linearDamping: 1.5, // Increased from 1.0
      angularDamping: 3.0  // Increased from 2.0
    });
    ...
    const rotorAxis = { x: 0, y: 1, z: 0 };      // Rotation axis (Y axis only)
    
    // Create params object with limits to constrain to single axis
    const rotorParams = this.physics.RAPIER.JointData.revolute(
      rotorAnchor1,
      rotorAnchor2,
      rotorAxis
    );
    ...
    // Remove limits to allow continuous rotation
    this.rotorJoint.setContactsEnabled(false);
    ...
    // Create revolute joint for tail rotor - rotate around X axis
    const tailAnchor1 = { x: 0.6, y: 0.8, z: -5 }; // On fuselage
    const tailAnchor2 = { x: 0, y: 0, z: 0 };      // On tail rotor
    const tailAxis = { x: 1, y: 0, z: 0 };         // Rotation axis (X axis)
    
    const tailParams = this.physics.RAPIER.JointData.revolute(
      tailAnchor1,
      tailAnchor2,
      tailAxis
    );
    ...
    this.tailRotorJoint.setContactsEnabled(false);
  }
  ...
  update(deltaTime) {
    ...
    // Update rotor visual rotations - simplified
    if (this.mainRotor && this.rotorBody) {
      // Main rotor just rotates around Y axis
      const rotorRotation = this.rotorBody.rotation();
      // Extract Y rotation from quaternion
      const euler = new THREE.Euler().setFromQuaternion(
        new THREE.Quaternion(rotorRotation.x, rotorRotation.y, rotorRotation.z, rotorRotation.w)
      );
      this.mainRotor.rotation.y = euler.y;
    }
    
    if (this.tailRotor && this.tailRotorBody) {
      // Tail rotor rotates around its local X axis (which is world Z after parent rotation)
      const tailRotation = this.tailRotorBody.rotation();
      const euler = new THREE.Euler().setFromQuaternion(
        new THREE.Quaternion(tailRotation.x, tailRotation.y, tailRotation.z, tailRotation.w)
      );
      // Since tail rotor group is rotated 90 degrees, we apply rotation to Z
      this.tailRotor.rotation.z = euler.x;
    }
  ...
  handleControls(deltaTime) {
    ...
    // Update pitch/roll/yaw - reduced rates and only when airborne or high rotor speed
    const canControl = !this.isGrounded || this.enginePower > this.minLiftoffRotorSpeed;
    const controlRate = this.isGrounded ? 0.5 : 1.5;
    const returnRate = 0.85; // Faster return to center for stability
    ...
    if (canControl) {
      ...
      if (this.controls.rollLeft) {
        ...
        this.roll *= returnRate;
      }
      
      // Yaw control only when lifted off
      if (!this.isGrounded) {
    ...
    // Apply torques - reduced when grounded
    const torqueMultiplier = this.isGrounded ? 0.2 : 1.0;
    const torque = new THREE.Vector3(
      this.pitch * 8 * torqueMultiplier, // Reduced from 10
      this.yaw * 3 * torqueMultiplier, // Reduced from 5
      -this.roll * 8 * torqueMultiplier // Reduced from 10
    ...
    // Add stronger stabilization
    const angVel = this.body.angvel();
    const stabilization = this.isGrounded ? 5 : 3;
    this.body.addTorque({
