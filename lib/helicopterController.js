...
export class HelicopterController {
  ...
  handleControls(deltaTime) {
    // Collective (up/down)
    if (this.controls.collectiveUp) {
      this.collectivePitch = Math.min(this.collectivePitch + deltaTime * 0.5, 1.0);
    } else if (this.controls.collectiveDown) {
      this.collectivePitch = Math.max(this.collectivePitch - deltaTime * 0.5, 0);
    }
    
    // Cyclic pitch (forward/backward tilt)
    if (this.controls.pitchForward) {
      this.cyclicPitch = Math.max(this.cyclicPitch - deltaTime * 2, -1);
    } else if (this.controls.pitchBackward) {
      this.cyclicPitch = Math.min(this.cyclicPitch + deltaTime * 2, 1);
    } else {
      this.cyclicPitch *= 0.9;
    }
    
    // Cyclic roll (left/right tilt)
    if (this.controls.rollLeft) {
      this.cyclicRoll = Math.max(this.cyclicRoll - deltaTime * 2, -1);
    } else if (this.controls.rollRight) {
      this.cyclicRoll = Math.min(this.cyclicRoll + deltaTime * 2, 1);
    } else {
      this.cyclicRoll *= 0.9;
    }
    
    // Tail rotor (yaw) - FIX: Remove inversion
    if (this.controls.yawLeft) {
      this.tailRotorPitch = Math.max(this.tailRotorPitch - deltaTime * 2, -1);
    } else if (this.controls.yawRight) {
      this.tailRotorPitch = Math.min(this.tailRotorPitch + deltaTime * 2, 1);
    } else {
      this.tailRotorPitch *= 0.9;
    }
  }
  ...
  enterHelicopter(player) {
    ...
    // Position camera further behind the helicopter
  ...
  updateCamera() {
    ...
    if (this.scene.camera.parent === this.mesh) {
      ...
      // Also adjust height based on speed
      this.scene.camera.position.y = 10 + speedFactor * 3;  // Increased from 8 + speedFactor * 2
      
      // Keep camera rotation fixed
      this.scene.camera.rotation.y = Math.PI;  // Face backward
      this.scene.camera.rotation.x = -0.2;  // Increased downward angle
  ...
  enterHelicopter(player) {
    ...
    // Position camera behind and above the helicopter - INCREASED DISTANCE
    this.scene.camera.position.set(0, 15, 30);  // Increased from 10, 20
