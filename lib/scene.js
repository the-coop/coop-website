...
export class SceneManager {
  ...
  updateOtherPlayer(playerId, playerData, worldOriginOffset) {
    ...
    if (velocity.length() > 0.1) {
      console.log(`Player ${playerId} coordinate conversion:
        Server world pos: [${serverWorldPos.x.toFixed(1)}, ${serverWorldPos.y.toFixed(1)}, ${serverWorldPos.z.toFixed(1)}]
        Server world origin: [${serverWorldOrigin.x.toFixed(1)}, ${serverWorldOrigin.y.toFixed(1)}, ${serverWorldOrigin.z.toFixed(1)}]
        Our world origin: [${worldOriginOffset.x.toFixed(1)}, ${worldOriginOffset.y.toFixed(1)}, ${worldOriginOffset.z.toFixed(1)}]
        Calculated local pos: [${localPosition.x.toFixed(1)}, ${localPosition.y.toFixed(1)}, ${localPosition.z.toFixed(1)}]`);
    }
  ...
  updateOtherPlayersInterpolation() {
    ...
    Object.entries(this.otherPlayerMeshes).forEach(([playerId, mesh]) => {
      ...
      // Sync physics body with visual position for collision detection
      if (mesh.userData.physicsBody) {
        const body = mesh.userData.physicsBody;
        
        // Set physics body to match interpolated visual position
        body.setTranslation({
          x: mesh.position.x,
          y: mesh.position.y,
          z: mesh.position.z
  ...
  updateDynamicObjects(movingPlatformBody) {
    ...
    // Update other physics bodies (excluding server-controlled players)
