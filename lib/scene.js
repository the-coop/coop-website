...
export const createPlanet = (scene, physicsWorld, debugInfo, gravity) => {
  ...
  try {
    ...
    addPlanetFeatures(scene, physicsWorld);
...
const addPlanetFeatures = (scene, physicsWorld) => {
...
export const createPushableRock = (scene, physicsWorld, position, scale = 1.0) => {
...
export const createPlatform = (scene, physicsWorld, debugInfo) => {
  if (!scene || !physicsWorld) {
    console.error("Scene or physics world not initialized");
    return null;
  }
  ...
  try {
    ...
    createWall(scene, physicsWorld, debugInfo);
    
    // Add a ramp on the platform
    const movingPlatform = createRamp(scene, physicsWorld, debugInfo);
    ...
    return { platform, movingPlatform };
  } catch (e) {
...
const createWall = (scene, physicsWorld, debugInfo) => {
  const wallWidth = 20;
  const wallHeight = 8;
  const wallDepth = 1;
...
// Ramp and moving platform creation function
const createRamp = (scene, physicsWorld, debugInfo) => {
  const rampWidth = 10;
  const rampHeight = 5;
  const rampDepth = 15;
  ...
  return createMovingPlatform(scene, physicsWorld, -15, rampTopHeight, rampTopZ);
};

// Moving platform creation function
const createMovingPlatform = (scene, physicsWorld, x, y, z) => {
  ...
  movingPlatform.position.set(
    x, // Start at ramp X position
    y + movingPlatformHeight/2, // Position at ramp top
    z + movingPlatformDepth/2 + 1 // Past the high edge
  );
  ...
  movingPlatform.userData = {
    ...
    moveSpeed: 0.2, // Slower movement
    physicsBody: movingPlatformBody
  };
...
// Function to update dynamic objects
export const updateDynamicObjects = (scene) => {
  if (!scene) return;
  ...
  scene.traverse((child) => {
    if (child.isMesh && child.userData.physicsBody) {
      ...
      // Handle moving platform specifically
      if (child.userData.moveSpeed !== undefined) {
        ...
        userData.physicsBody.setNextKinematicTranslation({
          x: newX,
          y: child.position.y,
          z: child.position.z
        });
      } else {
        // Update mesh position from physics body for other objects
