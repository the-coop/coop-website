...
export class SceneManager {
  ...
  updateOtherPlayer(playerId, playerData, worldOriginOffset) {
    ...
    const worldPosition = new THREE.Vector3(...parsedData.position);
    ...
    console.log(`Player ${playerId} update - world pos: [${worldPosition.x.toFixed(1)}, ${worldPosition.y.toFixed(1)}, ${worldPosition.z.toFixed(1)}], local pos: [${localPosition.x.toFixed(1)}, ${localPosition.y.toFixed(1)}, ${localPosition.z.toFixed(1)}], vel: [${velocity.x.toFixed(1)}, ${velocity.y.toFixed(1)}, ${velocity.z.toFixed(1)}], moving: ${velocity.length() > 0.1}, grounded: ${parsedData.isGrounded}`);
    ...
    if (!this.otherPlayerMeshes[playerId]) {
      ...
      // Set initial position IMMEDIATELY - use the server's world position converted to local
      playerMesh.position.set(localPosition.x, localPosition.y, localPosition.z);
      playerMesh.quaternion.set(rotation.x, rotation.y, rotation.z, rotation.w);
      
      this.scene.add(playerMesh);
      
      // Create physics body for other player at the CORRECT position with proper collider
      if (this.physicsManager) {
        ...
        // IMMEDIATELY set the correct velocity and rotation
        ...
        playerMesh.userData.serverAuthorityStrength = 1.0;
      }
      
      this.otherPlayerMeshes[playerId] = playerMesh;
      
      console.log('Created other player:', playerId, 'at world position:', worldPosition, 'local position:', localPosition);
    }
    
    const playerMesh = this.otherPlayerMeshes[playerId];
    
    // Store server data for physics correction
    ...
    // IMMEDIATELY apply server state to physics body - this ensures collisions work properly
    if (playerMesh.userData.physicsBody) {
      const body = playerMesh.userData.physicsBody;
      
      // FORCE the physics body to the server's position and rotation
      body.setTranslation({
        x: localPosition.x,
        y: localPosition.y,
        z: localPosition.z
      ...
      // IMPORTANT: Apply server rotation immediately so collider orientation matches visual
      body.setRotation({
        x: rotation.x,
        y: rotation.y,
        z: rotation.z,
      ...
      // IMMEDIATELY update visual position and rotation to match physics
      const physicsPos = body.translation();
      const physicsRot = body.rotation();
      
      playerMesh.position.set(physicsPos.x, physicsPos.y, physicsPos.z);
      playerMesh.quaternion.set(physicsRot.x, physicsRot.y, physicsRot.z, physicsRot.w);
      
      console.log(`Updated player ${playerId} physics body to position: [${physicsPos.x.toFixed(1)}, ${physicsPos.y.toFixed(1)}, ${physicsPos.z.toFixed(1)}] rotation: [${physicsRot.x.toFixed(2)}, ${physicsRot.y.toFixed(2)}, ${physicsRot.z.toFixed(2)}, ${physicsRot.w.toFixed(2)}]`);
    } else {
      // Fallback - direct visual update
      playerMesh.position.copy(localPosition);
      playerMesh.quaternion.copy(rotation);
      
      console.log(`Set player ${playerId} visual mesh to position: [${localPosition.x.toFixed(1)}, ${localPosition.y.toFixed(1)}, ${localPosition.z.toFixed(1)}]`);
    }
  }
  ...
  updateOtherPlayersInterpolation() {
    ...
    Object.entries(this.otherPlayerMeshes).forEach(([playerId, mesh]) => {
      ...
      // Keep physics body synchronized with server data with full authority
      if (mesh.userData.serverPosition && timeSinceUpdate < 500) { // Longer timeout
        const serverPos = mesh.userData.serverPosition;
        const serverVel = mesh.userData.serverVelocity;
        const serverRot = mesh.userData.serverRotation;
        const serverGrounded = mesh.userData.serverGrounded;
        
        // Apply server state with FULL authority - including rotation for proper collisions
        body.setTranslation({
          x: serverPos.x,
          y: serverPos.y,
          z: serverPos.z
        ...
        // CRITICAL: Always apply server rotation so collider matches visual orientation
        if (serverRot) {
          body.setRotation({
            x: serverRot.x,
            y: serverRot.y,
        ...
        // Handle grounded state - if server says grounded, prevent bouncing
        if (serverGrounded) {
          const currentVel = body.linvel();
          const planetCenter = new THREE.Vector3(0, -250, 0);
          const bodyPos = new THREE.Vector3(serverPos.x, serverPos.y, serverPos.z);
          const gravityDir = planetCenter.clone().sub(bodyPos).normalize();
          
          // If they're moving against gravity while grounded, reduce that velocity
          const downwardVel = currentVel.x * gravityDir.x + currentVel.y * gravityDir.y + currentVel.z * gravityDir.z;
          if (downwardVel > 2.0) {
            const adjustedVel = {
              x: currentVel.x - gravityDir.x * (downwardVel - 1.0),
              y: currentVel.y - gravityDir.y * (downwardVel - 1.0),
              z: currentVel.z - gravityDir.z * (downwardVel - 1.0)
            };
            
            body.setLinvel(adjustedVel);
          }
        }
      }
      
      // Update visual mesh to match physics body exactly - no delays
      const physicsPos = body.translation();
      const physicsRot = body.rotation();
      
      // Ensure visual mesh exactly matches physics body position and rotation
      mesh.position.set(physicsPos.x, physicsPos.y, physicsPos.z);
      mesh.quaternion.set(physicsRot.x, physicsRot.y, physicsRot.z, physicsRot.w);
    });
  }
  ...
  updateDynamicObjects(movingPlatformBody) {
    ...
    // Update local physics bodies (not server-controlled ones)
