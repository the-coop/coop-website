...
export class SceneManager {
  ...
  updateOtherPlayer(playerId, playerData, worldOriginOffset) {
    ...
    // Add more detailed logging to see if updates are coming through
    if (Math.random() < 0.1) { // Log 10% of updates to avoid spam
      console.log(`Player ${playerId} update - world pos: [${worldPosition.x.toFixed(1)}, ${worldPosition.y.toFixed(1)}, ${worldPosition.z.toFixed(1)}], vel: [${velocity.x.toFixed(1)}, ${velocity.y.toFixed(1)}, ${velocity.z.toFixed(1)}], grounded: ${parsedData.isGrounded}`);
    }
    ...
    if (!this.otherPlayerMeshes[playerId]) {
      ...
      // Create physics body for other player - now dynamic for interactions
      if (this.physicsManager) {
        ...
        playerMesh.userData.serverAuthorityStrength = 0.8; // Strong server authority
      }
    ...
    // Store server data for physics correction with better tracking
    ...
    // Track if player is actually moving for debugging
    const wasMoving = playerMesh.userData.wasMoving || false;
    const isMoving = velocity.length() > 0.1;
    playerMesh.userData.wasMoving = isMoving;
    
    if (isMoving !== wasMoving) {
      console.log(`Player ${playerId} movement changed - now ${isMoving ? 'moving' : 'stopped'} with velocity [${velocity.x.toFixed(1)}, ${velocity.y.toFixed(1)}, ${velocity.z.toFixed(1)}]`);
    }
    
    // Immediately apply server state to physics body for authoritative updates
    if (playerMesh.userData.physicsBody) {
      const body = playerMesh.userData.physicsBody;
      
      // Directly set server position and velocity
      body.setTranslation({
        x: localPosition.x,
        y: localPosition.y,
        z: localPosition.z
      ...
      // If server says they're grounded, reduce their downward velocity
      if (parsedData.isGrounded) {
        const currentVel = body.linvel();
        if (currentVel.y < -1.0) { // They're falling fast but server says grounded
          body.setLinvel({
            x: currentVel.x,
            y: Math.max(currentVel.y, -1.0), // Clamp downward velocity
            z: currentVel.z
          });
        }
      }
    ...
    const physicsPos = playerMesh.userData.physicsBody.translation();
    const physicsRot = playerMesh.userData.physicsBody.rotation();
  ...
  updateOtherPlayersInterpolation() {
    ...
    Object.entries(this.otherPlayerMeshes).forEach(([playerId, mesh]) => {
      ...
      // Keep physics body synchronized with server data
      if (mesh.userData.serverPosition && timeSinceUpdate < 200) {
        ...
        // Apply server state with strong authority for recent updates
        body.setTranslation({
          x: serverPos.x,
          y: serverPos.y,
          z: serverPos.z
      ...
      // Update visual mesh to match physics body exactly
