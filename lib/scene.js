...
export class SceneManager {
  ...
  addPlanetFeatures() {
    ...
    if (this.gameMode === 'multiplayer') {
      // Spawn car on the platform
      this.spawnTestCar(new THREE.Vector3(10, 35, 10));
    }
  }
  ...
  updateDynamicObject(objectId, state) {
    ...
    // For multiplayer rocks, directly set position/rotation to match server
    if (this.gameMode === 'multiplayer' && obj.isMultiplayer && obj.body && obj.type === 'rock') {
  ...
  removeDynamicObject(objectId) {
    ...
    // Remove physics body
    if (object.collider && this.physics.world) {
      this.physics.world.removeCollider(object.collider, true);
    }
    if (object.body && this.physics.world) {
 ...
 this.scene.add(ramp);
  ...
  createWaterVolumeFromData(data) {
    ...
    const material = new THREE.MeshStandardMaterial({
      color: data.properties?.color || 0x4488ff,
      transparent: true,
