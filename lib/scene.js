...
export class SceneManager {
  constructor() {
    ...
    this.physicsManager = null; // Add this property
  ...
  updateOtherPlayer(playerId, playerData, worldOriginOffset) {
    ...
    if (!this.otherPlayerMeshes[playerId]) {
      ...
      // Create physics body for other player with locked rotations
      ...
      console.log('Created other player:', playerId);
    }
    
    const playerMesh = this.otherPlayerMeshes[playerId];
    
    // Update physics body if it exists - set authoritative server state
    if (playerMesh.userData.physicsBody && this.physicsManager) {
      const body = playerMesh.userData.physicsBody;
      
      // Set position and velocity from server (authoritative)
      body.setTranslation({
        x: localPosition.x,
        y: localPosition.y,
        z: localPosition.z
    ...
    // Update interpolation targets with server data
    const interp = playerMesh.userData.interpolation;
    interp.startPos.copy(playerMesh.position);
    interp.targetPos.copy(localPosition);
    interp.startRot.copy(playerMesh.quaternion);
    interp.targetRot.copy(rotation);
    interp.startTime = Date.now();
    
    // Store velocity for visual effects if needed
    playerMesh.userData.velocity = velocity;
  }
  ...
  updateOtherPlayersInterpolation() {
    ...
    Object.entries(this.otherPlayerMeshes).forEach(([playerId, mesh]) => {
      ...
      // Keep physics body in sync with visual for smooth movement
