...
export class SceneManager {
  ...
  spawnMultiplayerRock(objectId, position = null, scale = 1.0) {
    ...
    // In multiplayer, create rocks as visual-only - no physics bodies
    const geometry = new THREE.DodecahedronGeometry(2 * scale, 0);
    const material = new THREE.MeshStandardMaterial({
      color: 0x666666,
      roughness: 1,
    ...
    // Track dynamic object without physics body
    this.dynamicObjects.set(objectId, {
      mesh: rock,
      body: null, // No physics body in multiplayer
      type: 'rock',
      scale: scale
    });
  ...
  updateDynamicObject(objectId, state) {
    ...
    // In multiplayer, just update visual position/rotation
    if (state.position && obj.mesh) {
      obj.mesh.position.set(state.position.x, state.position.y, state.position.z);
      
      // Log positions for debugging
      if (Math.random() < 0.01) { // 1% chance to log
        console.log(`Rock ${objectId} at y=${state.position.y.toFixed(2)}, platform at y=${this.objects.platform?.position.y || 'N/A'}`);
      }
    }
    ...
    // Don't update physics bodies in multiplayer - visual only
  }
  ...
  updateDynamicObjects() {
    ...
    // Only update physics-driven objects in single-player modes
    if (this.gameMode !== 'multiplayer') {
      // Update all tracked dynamic objects from physics
      this.dynamicObjects.forEach((obj, id) => {
