...
export class SceneManager {
  constructor(physicsManager) {
    ...
    this.ladders = new Map(); // Add ladders tracking
  }
  ...
  spawnMultiplayerRock(objectId, position = null, scale = 1.0) {
    ...
    // In multiplayer, create rocks with dynamic physics bodies but control them via server updates
    const geometry = new THREE.DodecahedronGeometry(2 * scale, 0);
    const material = new THREE.MeshStandardMaterial({
      color: 0x666666,
      roughness: 1,
 ...
 this.scene.add(rock);
  ...
  spawnTestCar(position = null) {
    ...
    import('./carController.js').then(({ CarController }) => {
  ...
  createPlatform() {
    ...
    if (this.gameMode === 'sandbox') {
      ...
      const carSpawnHeight = 30 + platformHeight/2 + 2; // Just 2 units above platform
    ...
    // Add a test ladder in sandbox mode
    if (this.gameMode === 'sandbox') {
      const ladderPos = new THREE.Vector3(-10, 30 + platformHeight/2, -10);
      this.createLadder(ladderPos, 12); // 12 unit tall ladder
    }
  ...
  clearLevel() {
    ...
    // Clear ladders
    this.ladders.forEach((ladder, id) => {
      this.scene.remove(ladder.mesh);
      if (this.physics.world && ladder.body) {
        this.physics.world.removeRigidBody(ladder.body);
      }
      if (ladder.mesh.geometry) ladder.mesh.geometry.dispose();
      if (ladder.mesh.material) ladder.mesh.material.dispose();
    });
    this.ladders.clear();
  ...
  createPlanetFromData(data) {
    ...
    // Check if server provided terrain data
    if (data.terrain_data && data.terrain_data.vertices && data.terrain_data.indices) {
      ...
      // Create geometry from server data
      geometry = new THREE.BufferGeometry();
      
      // Set vertices from server
      const vertices = new Float32Array(data.terrain_data.vertices);
      geometry.setAttribute('position', new THREE.BufferAttribute(vertices, 3));
      
      // Set indices from server
      ...
      // Use server vertices for collision mesh
      const colliderVertices = data.terrain_data.vertices;
      const colliderIndices = data.terrain_data.indices;
      
      // Create trimesh collider from server data
      const trimeshDesc = this.physics.createTrimeshCollider(
        colliderVertices,
        colliderIndices,
        { friction: 0.8, restitution: 0.1 }
      ...
      // Fallback to local generation (for backwards compatibility)
      console.log('Generating terrain locally (no server data)');
      
      const subdivisions = 5;
      geometry = new THREE.IcosahedronGeometry(planetRadius, subdivisions);
 ...
 this.scene.add(ramp);
  ...
  createWaterVolumeFromData(data) {
    ...
    const material = new THREE.MeshStandardMaterial({
      color: data.properties?.color || '#4488ff',
      transparent: true,
      opacity: data.properties?.opacity || 0.5,
      roughness: 0.1,
      metalness: 0.8,
  ...
  createDynamicPlatformFromData(data) {
    ...
    const material = new THREE.MeshStandardMaterial({
      color: data.properties?.color || '#ff8844',
      roughness: 0.6,
      metalness: 0.3,
      emissive: data.properties?.color || '#ff8844',
      emissiveIntensity: 0.1
    });
  ...
  spawnHelicopter(position = null) {
    const heliPos = position || new THREE.Vector3(-20, 40, 20);
    const helicopter = new HelicopterController(this, this.physics, heliPos);
    
    // Generate unique ID for the helicopter
    const heliId = 'helicopter_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    
    // Store in vehicles map
    this.vehicles.set(heliId, helicopter);
    
    // Also track as dynamic object
    this.dynamicObjects.set(heliId, {
      type: 'helicopter',
    ...
    console.log(`Spawned helicopter ${heliId} at`, heliPos);
    return helicopter;
  }

  // Add method to create a ladder
  createLadder(position, height = 10) {
    ...
    // Create rungs
    const rungGeometry = new THREE.BoxGeometry(width, 0.1, 0.1);
    const numRungs = Math.floor(height / rungSpacing);
    
    for (let i = 0; i < numRungs; i++) {
      const rung = new THREE.Mesh(rungGeometry, railMaterial);
      rung.position.set(0, -height / 2 + (i + 1) * rungSpacing, 0);
    ...
    // Create physics body (kinematic for ladder)
    const body = this.physics.createKinematicBody(position);
    
    // Create trigger zone for ladder detection
    const triggerDesc = this.physics.createBoxCollider(
      new THREE.Vector3(width * 0.8, height / 2, 0.5),
      {
        friction: 0.0,
        restitution: 0.0,
        isSensor: true
    ...
    triggerDesc.setCollisionGroups(0x00080000); // Ladder layer
    
    const collider = this.physics.world.createCollider(triggerDesc, body);
    
    // Generate unique ID for the ladder
    const ladderId = 'ladder_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    ...
    this.ladders.set(ladderId, ladderData);
    
    // Store ladder reference on mesh
    group.userData.ladder = ladderData;
    group.userData.interactable = true;
    group.userData.interactionType = 'ladder';
    
    console.log(`Created ladder ${ladderId} at`, position);
    return ladderData;
  }

  // Check if a position is near any ladder
  getNearbyLadder(position, range = 2.0) {
    ...
    for (const [id, ladder] of this.ladders) {
      ...
      // Check if position is within ladder bounds
      if (position.x >= bounds.min.x && position.x <= bounds.max.x &&
          position.y >= bounds.min.y - 1 && position.y <= bounds.max.y + 1 &&
          position.z >= bounds.min.z && position.z <= bounds.max.z) {
        
        const distance = position.distanceTo(ladder.position);
  ...
  // Get clamped position on ladder
  getClampedLadderPosition(ladder, playerPosition) {
    const bounds = ladder.bounds;
    
    return new THREE.Vector3(
      ladder.position.x,
      THREE.MathUtils.clamp(playerPosition.y, bounds.min.y, bounds.max.y),
      ladder.position.z
    );
  }

  createLadderFromData(data) {
    console.log('Creating ladder at position:', data.position, 'with height:', data.scale?.y || 10);
    ...
    // Override ID if provided in data
    if (data.id) {
      this.ladders.delete(ladder.id);
      ladder.id = data.id;
      this.ladders.set(data.id, ladder);
    }
