...
export class SceneManager {
  ...
  createRayVisualizations() {
    if (!this.scene || !this.player) {
      console.warn("Scene or player not initialized - cannot create ray visualizations yet");
      return;
    }
  ...
  updateOtherPlayer(playerId, playerData, worldOriginOffset) {
    ...
    // Apply server state to physics body with interpolation
    if (playerMesh.userData.physicsBody) {
      ...
      // Apply server state to physics body (not visual mesh)
      // This allows gravity and physics to work while server provides corrections
      body.setTranslation({
        x: localPosition.x,
        y: localPosition.y,
        z: localPosition.z
      ...
      console.log(`Applied server state to player ${playerId} physics body: pos [${localPosition.x.toFixed(1)}, ${localPosition.y.toFixed(1)}, ${localPosition.z.toFixed(1)}], vel [${velocity.x.toFixed(1)}, ${velocity.y.toFixed(1)}, ${velocity.z.toFixed(1)}]`);
    }
  }
  ...
  updateOtherPlayersInterpolation() {
    ...
    Object.entries(this.otherPlayerMeshes).forEach(([playerId, mesh]) => {
      ...
      // Update physics body to match visual position (not the other way around)
  ...
  updateDynamicObjects(movingPlatformBody) {
    ...
    // Update ALL physics bodies to their visual representation
    // This includes other players - their physics bodies will move and visuals follow
    this.scene.traverse((child) => {
      if (child.isMesh && child.userData.physicsBody) {
        const body = child.userData.physicsBody;
        
        // For ALL objects (including other players), sync visual to physics
        ...
        // Log other player movement for debugging
        if (child.userData.playerId) {
          const velocity = body.linvel();
          if (velocity.x !== 0 || velocity.y !== 0 || velocity.z !== 0) {
            console.log(`Physics moved player ${child.userData.playerId} to [${position.x.toFixed(1)}, ${position.y.toFixed(1)}, ${position.z.toFixed(1)}] with velocity [${velocity.x.toFixed(1)}, ${velocity.y.toFixed(1)}, ${velocity.z.toFixed(1)}]`);
          }
        }
      }
    });
