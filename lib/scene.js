...
export class SceneManager {
  ...
  updateDynamicObject(objectId, state) {
    ...
    if (this.gameMode === 'multiplayer' && obj.body && obj.type === 'rock') {
      ...
      // Store server state with velocity prediction
      if (state.position) {
    ...
    this.dynamicObjects.forEach((obj, id) => {
      if (this.gameMode === 'multiplayer' && obj.type === 'rock') {
        ...
        if (obj.lastServerUpdate && obj.mesh && obj.body) {
          const now = performance.now();
          const timeSinceUpdate = (now - obj.lastServerUpdate.timestamp) / 1000; // Convert to seconds
          
          // Predict position based on velocity
          const predictedPosition = obj.lastServerUpdate.position.clone();
          if (obj.lastServerUpdate.velocity && timeSinceUpdate < 0.2) { // Only predict for recent updates
            const velocityOffset = obj.lastServerUpdate.velocity.clone().multiplyScalar(timeSinceUpdate);
            predictedPosition.add(velocityOffset);
          }
          
          // Faster interpolation for more responsive movement
          const interpolationSpeed = 10.0; // Higher = faster convergence
          const interpolationFactor = 1.0 - Math.exp(-interpolationSpeed * timeSinceUpdate);
          
          // Interpolate position
          obj.mesh.position.lerp(predictedPosition, Math.min(interpolationFactor, 0.9));
          
          // Interpolate rotation (visual only - physics body has locked rotation)
          obj.mesh.quaternion.slerp(obj.lastServerUpdate.rotation, Math.min(interpolationFactor * 0.5, 0.5));
