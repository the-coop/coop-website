...
export class SceneManager {
  ...
  updateOtherPlayer(playerId, playerData, worldOriginOffset) {
    ...
    if (!this.otherPlayerMeshes[playerId]) {
      ...
      // Create physics body for other player at the CORRECT position
    ...
    // IMMEDIATELY apply server state to physics body - no delays or interpolation
    if (playerMesh.userData.physicsBody) {
      const body = playerMesh.userData.physicsBody;
      
      // FORCE the physics body to the server's position
      body.setTranslation({
        x: localPosition.x,
        y: localPosition.y,
        z: localPosition.z
      ...
      // IMMEDIATELY update visual position to match physics
      ...
      console.log(`Set player ${playerId} physics body to position: [${physicsPos.x.toFixed(1)}, ${physicsPos.y.toFixed(1)}, ${physicsPos.z.toFixed(1)}]`);
    } else {
  ...
  updateOtherPlayersInterpolation() {
    ...
    Object.entries(this.otherPlayerMeshes).forEach(([playerId, mesh]) => {
      ...
      if (mesh.userData.serverPosition && timeSinceUpdate < 500) { // Longer timeout
        ...
        // Apply server state with FULL authority - no smoothing for movement
        body.setTranslation({
          x: serverPos.x,
          y: serverPos.y,
          z: serverPos.z
