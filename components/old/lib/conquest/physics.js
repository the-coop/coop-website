...
import { PLAYER_SPEED, THRUST_AMOUNT, JUMP_AMOUNT } from './config';

import ComputerInput from './gameplay/inputs/computer';
...
export default class Physics {
    ...
    static translatePlayerInput(player, playerHeight, surfaceHeight) {
        ...
        //Caculate Effective Thrust
        let TotalThrust = THRUST_AMOUNT;
      if (player.onGround) {
          TotalThrust = TotalThrust + JUMP_AMOUNT;
        }
        ...
        if (ComputerInput.inputs.space) {
              player.velocity.addScaledVector(thrustDirection, TotalThrust);
        }
    ...
    static captureSOI(player, playerHeight, worldPos, isSelf) {
        ...
        if (newPlanet) { 
            ...
            // Send soi change.
            if (isSelf) {
                console.log("Own SOI changed");
                player.soiChangeCount++;
                WORLD.me.player.emitPosition();

                // Let controls re-orient to new SOI/pivot/rotation.
                window.WORLD.controls.onChangeSOI();
            }
        }
